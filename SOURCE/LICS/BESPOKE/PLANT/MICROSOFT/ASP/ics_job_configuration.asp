...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobJob").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobDescription").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobResGroup").Value) & "',"
   strStatement = strStatement & objForm.Fields("DTA_JobExeHistory").Value & ","
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobOprAlert").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobEmaGroup").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobType").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobIntGroup").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobProcedure").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobNext").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_JobInterval").Value) & "',"
   strStatement = strStatement & "'" & objForm.Fields("DTA_JobStatus").Value & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " t01.job_status"
   strQuery = strQuery & " from lics_job t01"
   strQuery = strQuery & " where t01.job_job = '" & objForm.Fields("DTA_JobJob").Value & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strMode = "SELECT"
      strReturn = FormatError(strReturn)
