...
sub ProcessSelect()
   ...
   select case objForm.Fields("Mode").Value
      case "SEARCH"
         ...
         strOrder = "asc"
      case "PREVIOUS"
         strWhere = " and t01.code < '" & objForm.Fields("STR_Source").Value & "'"
         strOrder = "desc"
      case "NEXT"
         strWhere = " and t01.code > '" & objForm.Fields("END_Source").Value & "'"
         strOrder = "asc"
   end select
   ...
   strQuery = strQuery & " from care_bw_xref t01"
   ...
   if objForm.Fields("QRY_Source").Value <> "" then
      if objForm.Fields("QRY_Test").Value = "EQ" then
         strQuery = strQuery & " and t01.code = '" & objForm.Fields("QRY_Code").Value & "'"
      end if
   ...
   strQuery = strQuery & " order by t01.xref_type " & strOrder
...
sub ProcessInsertAccept()
   ...
   '//
   strStatement = "care_xref.insert_data("
...
sub ProcessUpdateAccept()
   ...
   '//
   strStatement = "care_xref.update_data("
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_Code").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_XrefType").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_XrefDesc").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_BWCode").Value) & "')"
...
sub ProcessDeleteAccept()
   ...
   '//
   strStatement = "care_xref.delete_data("
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_Code").Value) & "')"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
