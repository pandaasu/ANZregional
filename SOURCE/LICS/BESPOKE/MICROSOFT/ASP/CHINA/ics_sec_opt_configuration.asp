...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SeoOption").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SeoDescription").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SeoScript").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SeoStatus").Value) & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " from lics_sec_option t01"
   strQuery = strQuery & " where t01.seo_option = '" & objForm.Fields("DTA_SeoOption").Value & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
      strMode = "SELECT"
