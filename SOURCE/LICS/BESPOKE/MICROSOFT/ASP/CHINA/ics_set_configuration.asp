...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SetGroup").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SetCode").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_SetValue").Value) & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " t01.set_value"
   strQuery = strQuery & " from lics_setting t01"
   strQuery = strQuery & " where t01.set_group = '" & objForm.Fields("DTA_SetGroup").Value & "'"
   strQuery = strQuery & " and t01.set_code = '" & objForm.Fields("DTA_SetCode").Value & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strMode = "SELECT"
      strReturn = FormatError(strReturn)
