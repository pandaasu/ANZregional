...
sub ProcessSelect()
   ...
   if strMode = "SELECT" then
      ...
      call objForm.AddField("DTA_ForType", "PRDBR2")
...
sub ProcessCreate()
   ...
   if objForm.Fields("DTA_ForSource").Value = "*TXQ" or objForm.Fields("DTA_ForSource").Value = "*TXV" then
      lngIndex = 1
      do while lngIndex <= len(objForm.Fields("DTA_ForStream").Value)
         call objProcedure.Execute("lics_form.set_value('FOR_STREAM','" & objSecurity.FixString(mid(objForm.Fields("DTA_ForStream").Value,lngIndex,2000)) & "')")
   ...
   if objForm.Fields("DTA_ForType").Value = "PRDBR1" then
      ...
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForDescription").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForReplace").Value)  & "',"
      strStatement = strStatement & "'*BR1',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,1,5))  & "',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,6,5))  & "',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,11,5))  & "',"
      strStatement = strStatement & "'" & objForm.Fields("DTA_ForSource").Value  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForMaterial").Value)  & "',"
      strStatement = strStatement & "'" & strUser  & "')"
   end if
   if objForm.Fields("DTA_ForType").Value = "PRDBR2" then
      strStatement = strStatement & "create_period_load("
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForIdentifier").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForDescription").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForReplace").Value)  & "',"
      strStatement = strStatement & "'*BR2',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,1,5))  & "',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,6,5))  & "',"
      strStatement = strStatement & "'" & trim(mid(objForm.Fields("DTA_ForSplit").Value,11,5))  & "',"
      strStatement = strStatement & "'" & objForm.Fields("DTA_ForSource").Value  & "',"
   ...
   if objForm.Fields("DTA_ForType").Value = "PRDOP1" then
      strStatement = strStatement & "create_period_load("
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForIdentifier").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForDescription").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForReplace").Value)  & "',"
   ...
   if objForm.Fields("DTA_ForType").Value = "PRDOP2" then
      strStatement = strStatement & "create_period_load("
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForIdentifier").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForDescription").Value)  & "',"
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_ForReplace").Value)  & "',"
...
sub ProcessDelete()
   ...
   '// Execute the forecast load creation
   '//
   strStatement = "dw_forecast_loading.delete_load('" & objSecurity.FixString(objForm.Fields("DTA_ForIdentifier").Value)  & "')"
