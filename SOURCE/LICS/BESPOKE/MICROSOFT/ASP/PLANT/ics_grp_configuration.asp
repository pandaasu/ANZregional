...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_GroGroup").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_GroDescription").Value) & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " t01.gro_description"
   strQuery = strQuery & " from lics_group t01"
   strQuery = strQuery & " where t01.gro_group = '" & objForm.Fields("DTA_GroGroup").Value & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
      strMode = "SELECT"
   ...
   strQuery = strQuery & " where t01.gri_group = '" & objForm.Fields("DTA_GroGroup").Value & "') t02"
...
sub ProcessUpdateAccept()
   ...
   strStatement = strStatement & "'" & objForm.Fields("DTA_GroGroup").Value & "'"
   strStatement = strStatement & ")"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
