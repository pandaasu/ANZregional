...
sub ProcessForm()
   ...
   strQuery = strQuery & " union all"
   strQuery = strQuery & " select"
   strQuery = strQuery & " '(*STREAM) '||t01.sth_str_code as backlog_code,"
   strQuery = strQuery & " t01.sth_str_code||' - Stream instances' as backlog_desc,"
   strQuery = strQuery & " '*STREAMED' as backlog_type,"
   strQuery = strQuery & " t01.sth_str_code as backlog_group,"
   strQuery = strQuery & " count(*) as backlog_count"
   strQuery = strQuery & " from lics_str_exe_header t01"
   strQuery = strQuery & " where t01.sth_exe_status in ('*PENDING','*OPENED','*OPNPAUSED')"
   strQuery = strQuery & " group by t01.sth_str_code"
   strQuery = strQuery & " union all"
   strQuery = strQuery & " select"
