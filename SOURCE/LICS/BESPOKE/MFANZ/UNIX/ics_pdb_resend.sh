...
#
#  This script is used to resend files from LADS to the Plant Databases.  To 
#  use this script you need to add the filenames which need to be resent to 
#  a file.  This will only send for a single interface.
#  Use the following command to get a list of filenames from the archive dir:
#
#   ls -al LADPDB02.1* | awk '{print $9}'
#
# ---------------------------------------------------------------------------
...
{
    FILE_INT="$1"
    CMD="${AMI_PATH}/bin/mqft/mqftssnd -source ${AMI_QMGR},${FILE_INT} -target ${TAR_QMGR},${TAR_PATH}/${PDB_FILE} -compress"
    ...
    if [[ $rc -ne 0 ]] ; then
        error_exit "ERROR: [send_file] MQFT send failed to PDB.  Return code [${rc}]."            
    fi
...
check_params()
{
    ...
    elif [[ -z $TAR_QMGR ]] ; then
        error_exit "ERROR: [check_params] Target queue manager not specified"
    elif [[ -z $TAR_PATH ]] ; then
        error_exit "ERROR: [check_params] Target path not specified"        
    else
...
TAR_QMGR=${3}               # Variable: Target queue manager
TAR_PATH=${4}               # Variable: Target path
...
# ------- ----------- ------------- -----------------------------
# 1.0     24-NOV-2008 T. Keon       Original
# ---------------------------------------------------------------------------
