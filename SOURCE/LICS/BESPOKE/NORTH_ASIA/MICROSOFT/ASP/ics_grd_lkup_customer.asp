...
sub ProcessForm()
   ...
   if objForm.Fields("Mode").Value = "SEARCH" then
		...
		strQuery = strQuery & "      bds_addr_customer_zh b,"
		...
		if objForm.Fields("QRY_DESC").Value <> "" then
		   strWhere = strWhere & " and upper(b.name) like upper('%" & objForm.Fields("QRY_DESC").Value & "%')"   
		end if
		if objForm.Fields("QRY_ACC_GRP").Value <> "" then
		   strWhere = strWhere & " and c.account_group_code = '" & objForm.Fields("QRY_ACC_GRP").Value & "'"   
		end if
