...
/**
 System  : lics
 Package : lics_poller_processor
 Owner   : lics_app
 Author  : Steve Gregan - November 2005

 DESCRIPTION
 -----------
 Local Interface Control System - Poller Processor

 The package implements the poller processor functionality.
...
/* Package Header */
/******************/
create or replace package lics_poller_processor as
...
end lics_poller_processor;
/

/****************/
/* Package Body */
/****************/
create or replace package body lics_poller_processor as
   ...
   begin
      ...
      var_interval := par_interval;

      /*-*/
      /* Set the controls
      /*-*/
      var_ctl_pipe := lics_constant.queue_poller || var_group;
      var_ctl_wake := false;
      ...
      /*-*/
      /* Process the poller procedure until stop requested
      /*-*/
      loop
         ...
         /*-*/
         /* Wait for pipe message (blocked - var_interval seconds)
         /* **note** this is the sleep code
         /*-*/
         var_message := lics_pipe.receive_timed(var_ctl_pipe, var_interval);
      ...
      when others then
         ...
         begin
            lics_notification.log_fatal(var_job,
                                        var_execution,
                                        lics_constant.type_poller,
                                        var_group,
                                        par_procedure,
                                        null,
                                        null,
                                        null,
                                        'POLLER PROCESSOR FAILED - ' || substr(SQLERRM, 1, 1024));
         exception
            when others then
               raise_application_error(-20000, 'FATAL ERROR - Interface Control System - Poller Processor - ' || substr(SQLERRM, 1, 1024));
         end;
...
/**************************/
create or replace public synonym lics_poller_processor for lics_app.lics_poller_processor;
grant execute on lics_poller_processor to public;