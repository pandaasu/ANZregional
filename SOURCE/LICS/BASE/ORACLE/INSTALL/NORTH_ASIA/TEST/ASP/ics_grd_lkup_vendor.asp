...
sub ProcessForm()
   ...
   if objForm.Fields("Mode").Value = "SEARCH" then
		...
		if objForm.Fields("INP_CODE_TYPE").Value = "1" and objForm.Fields("QRY_CODE").Value <> "" then 'GRD
		   strWhere = strWhere & " and ltrim(a.xrf_target,'0') like '%" & objForm.Fields("QRY_CODE").Value & "%'"
		end if
		if objForm.Fields("INP_CODE_TYPE").Value = "0" and objForm.Fields("QRY_CODE").Value <> "" then
		   strWhere = strWhere & " and ltrim(a.xrf_source,'0') like '%" & objForm.Fields("QRY_CODE").Value & "%'"    
		end if
		
		if objForm.Fields("QRY_DESC").Value <> "" then
		   strWhere = strWhere & " and upper(b.vendor_name_01) like upper('%" & objForm.Fields("QRY_DESC").Value & "%')"   
		end if
		if objForm.Fields("QRY_ZH_DESC").Value <> "" then
		   strWhere = strWhere & " and upper(c.name) like upper('%" & objForm.Fields("QRY_ZH_DESC").Value & "%')"   
		end if
		if objForm.Fields("QRY_ACC_GRP").Value <> "" then
		   strWhere = strWhere & " and b.account_group_code = '" & objForm.Fields("QRY_ACC_GRP").Value & "'"   
		end if
