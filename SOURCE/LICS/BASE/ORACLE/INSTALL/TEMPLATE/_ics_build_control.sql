...
1. Create a WO for the DBA with the following...
   ...
   1.2. The following scripts from the repository directory (...local path to repository...\SOURCE\LICS\BASE\ORACLE\LICS_APP\CODE)
...
4. Load the ICS java classes into the LICS_APP schema using the following commands...
      ...
      loadjava -f -v -r -u lics_app/xxxxxx@db9999x.ap.mars ...local path to repository...\SOURCE\LICS\BASE\JAVA\ICS_General\build\classes\com\isi\ics\cExternalProcess.class
      loadjava -f -v -r -u lics_app/xxxxxx@db9999x.ap.mars ...local path to repository...\SOURCE\LICS\BASE\JAVA\ICS_General\build\classes\com\isi\ics\cFileSystem.class

5. Connect as LICS_APP using SQL+ and compile the file system package as follows...

      @...local path to repository...\SOURCE\LICS\BASE\ORACLE\LICS_APP\CODE\lics_filesystem.sql;
...
8. Compile the LICS schema objects using the script ==> @...local path to repository...\SOURCE\LICS\BASE\ORACLE\INSTALL\...Installation....\...Environment...(eg. NORTH_ASIA\TEST)\_lics_build.sql

9. Compile the LICS_APP schema objects using the script ==> @...local path to repository...\SOURCE\LICS\BASE\ORACLE\INSTALL\...Installation....\...Environment...(eg. NORTH_ASIA\TEST)\_lics_app_build.sql
