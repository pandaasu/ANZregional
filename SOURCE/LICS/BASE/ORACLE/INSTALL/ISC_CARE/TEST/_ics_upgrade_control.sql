...
1. Create a WO for the DBA with the following...
   ...
   1.2. The DBA needs to connect as SYS when executing the following commands.
        ...
        1.2.1. Add the following to the WO...
               ...
               commit;
               create directory ics_bin as '/ics/cuc/test/bin';
               create directory ics_config as '/ics/cuc/test/config';
               create directory ics_archive as '/ics/cuc/test/archive';
...
4. Check that the database has been updated as requested...
   ...
   4.3. Check the oracle directories have been created using the statement ==> select * from all_directories;
	   ...
	   ICS_ARCHIVE   /ics/cuc/test/archive
	   ICS_CONFIG    /ics/cuc/test/config
	   ICS_BIN       /ics/cuc/test/bin

5. Load the ICS java classes into the LICS_APP schema using the following commands...

      loadjava -f -v -r -u lics_app/culics@DB1314T.AP.MARS D:\Vivian\LADS\SourceRepository\SOURCE\LICS\BASE\JAVA\ICS_General\build\classes\com\isi\ics\cExternalProcess.class
      loadjava -f -v -r -u lics_app/culics@DB1314T.AP.MARS D:\Vivian\LADS\SourceRepository\SOURCE\LICS\BASE\JAVA\ICS_General\build\classes\com\isi\ics\cFileSystem.class
...
10. Recompile the LICS_APP schema objects using the script ==> @D:\Vivian\LADS\SourceRepository\SOURCE\LICS\BASE\ORACLE\INSTALL\ISC_CARE\TEST\_lics_app_build.sql
