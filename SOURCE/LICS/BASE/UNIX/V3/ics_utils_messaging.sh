...
# --------------------------------------------------------------------------
#
# check_sftp_log Description:
# Checks the status message of the sftp log
#
# Parameters:
# 1 - The file to extract the status message from
#
...
check_sftp_log()
{
    ...
    if [[ $? -ne 0 ]] ; then
        ...
        error_exit "ERROR: [check_log] An unknown error has occured during sftp"
    fi
    
    log_file "INFO: [check_log] Verified sftp transaction successful" "HARMLESS"
}
...
# --------------------------------------------------------------------------
#
# process_outbound_sftp Description:
# Process the outbound SFTP interface.
#
# Parameters: <none>
#
# --------------------------------------------------------------------------
process_outbound_sftp()
{    
    if [[ ! -z ${DEST_PATH##*/} || ! -z ${DEST_PATH%%/*} ]] ; then
        error_exit "ERROR: [process_outbound_sftp] Destination path must start and end with /"
    fi

    log_file "INFO: [process_outbound_sftp] Verified destination path valid [${DEST_PATH}]" "HARMLESS"
    ...
    if [ $CURRENT_OS = $HP_UNIX_OS ] ; then    
        ...
        error_exit "ERROR: [process_outbound_sftp] Specified O/S [${CURRENT_OS}] is not supported"
    fi
    
    if [[ $? -ne 0 ]] ; then
        error_exit "ERROR: [process_outbound_sftp] Destination Server [${DEST_SERVER}] unreachable"
    fi

    log_file "INFO: [process_outbound_sftp] Verified destination server [${DEST_SERVER}] reachable" "HARMLESS"
    log_file "INFO: [process_outbound_sftp] Executing command [${BIN_PATH}/ics_outbound_sftp_runfile.sh] " "HARMLESS"

    eval "${BIN_PATH}/ics_outbound_sftp_runfile.sh" >> $TMP_OUT 2>&1
    if [[ $? -ne 0 ]] ; then
        error_exit "ERROR: [process_outbound_sftp] SFTP job failed."
    fi

    log_file "INFO: [process_outbound_sftp] SFTP job ran successfully " "HARMLESS"
    
    check_sftp_log "${TMP_OUT}"
    archive_file "${FILE}"
}
