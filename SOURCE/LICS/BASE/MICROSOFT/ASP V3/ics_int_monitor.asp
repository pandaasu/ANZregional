...
sub ProcessForm()
   ...
   select case objForm.Fields().Item("Mode")
      case "SEARCH"
         ...
         strOrder = "desc"
      case "PREVIOUS"
         strWhere = " where (t01.het_header > " & objForm.Fields().Item("STR_Header")
         strWhere = strWhere & " or (t01.het_header = " & objForm.Fields().Item("STR_Header") & " and t01.het_hdr_trace > " & objForm.Fields().Item("STR_Trace") & "))"
         strTest = " and "
         strOrder = "asc"
      case "NEXT"
         strWhere = " where (t01.het_header < " & objForm.Fields().Item("END_Header")
         strWhere = strWhere & " or (t01.het_header = " & objForm.Fields().Item("END_Header") & " and t01.het_hdr_trace < " & objForm.Fields().Item("END_Trace") & "))"
         strTest = " and "
         strOrder = "desc"
   end select
   ...
   '//
   if objForm.Fields().Item("QRY_Advanced") <> "1" then
      ...
      if objForm.Fields().Item("QRY_Interface") <> "" then
         strQuery = strQuery & " and t01.hea_interface = '" & objForm.Fields().Item("QRY_Interface") & "'"
      else
         if objForm.Fields().Item("QRY_Grouping") <> "" then
            strQuery = strQuery & " and t01.hea_interface in (select gri_interface from lics_grp_interface where gri_group = '" & objForm.Fields().Item("QRY_Grouping") & "')"
         end if
      end if
   ...
   '//
      ...
      strQuery = strQuery & " t02.int_group"
      strQuery = strQuery & " from lics_header t01, lics_interface t02"
      if objForm.Fields().Item("QRY_Reference") <> "" then
         intRefCount = 0
         strReference = ""
         aryReference = split(objForm.Fields().Item("QRY_Reference") , chr(10))
         ...
         if strReference <> "" then
            ...
            if objForm.Fields().Item("QRY_Condition") = "AND" then
      ...
      strQuery = strQuery & " where t01.hea_interface = t02.int_interface(+)"
      if objForm.Fields().Item("QRY_Reference") <> "" and strReference <> "" then
         strQuery = strQuery & " and t01.hea_header = t03.hes_header"
      end if
      if objForm.Fields().Item("QRY_Header") <> "" then
         if objForm.Fields().Item("QRY_Test") = "EQ" then
            strQuery = strQuery & " and t01.hea_header = " & objForm.Fields().Item("QRY_Header")
         end if
      ...
      if objForm.Fields().Item("QRY_Type") <> "" then
         strQuery = strQuery & " and t02.int_type = '" & objForm.Fields().Item("QRY_Type") & "'"
         strTest = " and "
      end if
      if objForm.Fields().Item("QRY_Process") <> "" then
         strQuery = strQuery & " and t02.int_group = '" & objForm.Fields().Item("QRY_Process") & "'"
         strTest = " and "
      end if
      strQuery = strQuery & " ) t01, (select t01.* from lics_hdr_trace t01"
      if strWhere <> "" then
      ...
      if objForm.Fields().Item("QRY_Execution") <> "" then
         strQuery = strQuery & strTest & "t01.het_execution = " & objForm.Fields().Item("QRY_Execution")
         strTest = " and "
      end if
      if objForm.Fields().Item("QRY_StrTime") <> "" then
         strQuery = strQuery & strTest & "to_char(t01.het_str_time,'YYYYMMDDHH24MISS') >= '" & objForm.Fields().Item("QRY_StrTime") & "'"
         strTest = " and "
      end if
      if objForm.Fields().Item("QRY_EndTime") <> "" then
         strQuery = strQuery & strTest & "to_char(t01.het_end_time,'YYYYMMDDHH24MISS') <= '" & objForm.Fields().Item("QRY_EndTime") & "'"
         strTest = " and "
      end if
      if objForm.Fields().Item("QRY_Status") <> "" then
         strQuery = strQuery & strTest & "t01.het_status = '" & objForm.Fields().Item("QRY_Status") & "'"
         strTest = " and "
      end if
      strQuery = strQuery & " ) t02"
      strQuery = strQuery & " where t01.hea_header = t02.het_header"
   ...
   if objForm.Fields().Item("QRY_Grouping") <> "" then
      strQuery = strQuery & " where t01.int_interface in (select gri_interface from lics_grp_interface where gri_group = '" & objForm.Fields().Item("QRY_Grouping") & "')"
   end if
