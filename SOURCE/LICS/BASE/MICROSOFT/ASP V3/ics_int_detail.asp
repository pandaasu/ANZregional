...
sub ProcessForm()
   ...
   '//
   strStatement = "lics_security.check_intfc_hdr_security(" & objForm.Fields().Item("QRY_Header") & ",'" & GetUser() & "')"
   ...
   strQuery = strQuery & " where t05.hem_header = " & objForm.Fields().Item("QRY_Header")
   strQuery = strQuery & " and t05.hem_hdr_trace = " & objForm.Fields().Item("QRY_Trace")
   ...
   strQuery = strQuery & " and t01.het_header = " & objForm.Fields().Item("QRY_Header")
   strQuery = strQuery & " and t01.het_hdr_trace = " & objForm.Fields().Item("QRY_Trace")
   ...
   if objSelection.ListValue04("DETAIL",0) <> "*INBOUND" then
      ...
      '//
      strStatement = "lics_interface_view.retrieve(" & objForm.Fields().Item("QRY_Header") & ")"
      strReturn = objFunction.Execute(strStatement)
      if strReturn <> "*OK" then
         strReturn = FormatError(strReturn)
         exit sub
   ...
   '//
   if objSelection.ListValue04("DETAIL",0) = "*INBOUND" then
      if objForm.Fields().Item("QRY_Error") <> "Y" then
         ...
         strQuery = strQuery & " where t03.dam_header = " & objForm.Fields().Item("QRY_Header")
         strQuery = strQuery & " and t03.dam_hdr_trace = " & objForm.Fields().Item("QRY_Trace")
         strQuery = strQuery & " group by t03.dam_dta_seq) t02"
         strQuery = strQuery & " where t01.dat_dta_seq = t02.dam_dta_seq(+)"
         strQuery = strQuery & " and t01.dat_header = " & objForm.Fields().Item("QRY_Header")
   ...
   if objSelection.ListValue04("DETAIL",0) = "*INBOUND" then
      ...
      strQuery = strQuery & " from lics_data t01"
      strQuery = strQuery & " where t01.dat_header = " & objForm.Fields().Item("QRY_Header")
