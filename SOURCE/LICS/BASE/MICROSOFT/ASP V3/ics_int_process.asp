...
sub ProcessSelect()
   ...
   if objForm.Fields().Item("QRY_Header") <> "" then
      if objForm.Fields().Item("QRY_Test") = "EQ" then
         strQuery = strQuery & strTest & "t01.het_header = " & objForm.Fields().Item("QRY_Header")
         strTest = " and "
      end if
   ...
   if objForm.Fields().Item("QRY_Interface") <> "" then
      strQuery = strQuery & strTest & "t02.hea_interface = '" & objForm.Fields().Item("QRY_Interface") & "'"
      strTest = " and "
   end if
   if objForm.Fields().Item("QRY_Type") <> "" then
      strQuery = strQuery & strTest & "t03.int_type = '" & objForm.Fields().Item("QRY_Type") & "'"
      strTest = " and "
   end if
   if objForm.Fields().Item("QRY_Group") <> "" then
      strQuery = strQuery & strTest & "t03.int_group = '" & objForm.Fields().Item("QRY_Group") & "'"
      strTest = " and "
   end if
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " and t01.hea_header = " & objForm.Fields().Item("DTA_HeaHeader")
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strMode = "SELECT"
...
sub ProcessUpdateAccept()
   ...
   strStatement = strStatement & "'" & objForm.Fields().Item("DTA_HeaHeader") & "'"
