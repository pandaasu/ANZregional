...
sub ProcessForm()
   ...
   select case objForm.Fields().Item("Mode")
      case "SEARCH"
         ...
         strOrder = "desc"
      case "PREVIOUS"
         strWhere = " where t01.eve_sequence > " & objForm.Fields().Item("STR_Sequence")
         strTest = " and "
         strOrder = "asc"
      case "NEXT"
         strWhere = " where t01.eve_sequence < " & objForm.Fields().Item("END_Sequence")
         strTest = " and "
         strOrder = "desc"
   end select
   ...
   if objForm.Fields().Item("QRY_Time") <> "" then
      strQuery = strQuery & strTest & "to_char(t01.eve_time,'YYYYMMDDHH24MISS') <= " & objForm.Fields().Item("QRY_Time")
      strTest = " and "
   end if
   if objForm.Fields().Item("QRY_Result") <> "" then
      strQuery = strQuery & strTest & "t01.eve_result = '" & objForm.Fields().Item("QRY_Result") & "'"
      strTest = " and "
   end if
   if objForm.Fields().Item("QRY_Job") <> "" then
      strQuery = strQuery & strTest & "t01.eve_job = '" & objForm.Fields().Item("QRY_Job") & "'"
      strTest = " and "
   end if
   if objForm.Fields().Item("QRY_Interface") <> "" then
      strQuery = strQuery & strTest & "t01.eve_interface = '" & objForm.Fields().Item("QRY_Interface") & "'"
      strTest = " and "
   end if
