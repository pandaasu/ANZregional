...
sub ProcessGet()
   ...
   '//
   set objServer = Server.CreateObject("ics_xlserver2.ICS_XLSERVER")
   objServer.Security(objSecurity)
   strReturn = objServer.GetSpreadsheetV1(objForm.Fields().Item("DTA_Procedure"))
   if strReturn = "*OK" then
...
sub ProcessRead()
   ...
   '//
   strStream = ""
   lngCount = clng(objForm.Fields().Item("DTA_Count"))
   for i = 1 to lngCount
      strStream = strStream & objForm.Fields().Item("StreamPart" & i)
   next
   ...
   strReturn = objServer.SetSpreadsheet(objForm.Fields().Item("DTA_Procedure"))
...
<!--
   ...
   function doSave() {
      ...
      strReturn = objClient.SetXMLStream('<%=objForm.Fields().Item("DTA_Name")%>');
      objClient = null;
      if (strReturn == '*OK') {
         doCancel();
      } else {
...
<!--
   function doUpload() {
      ...
      objClient = new ActiveXObject('ICS_XLCLIENT.Object');
      strReturn = objClient.GetXMLStream('<%=objForm.Fields().Item("DTA_File")%>');
...
<form name="main" action="<%=strBase%><%=strTarget%>" method="post">
   ...
   <input type="hidden" name="DTA_Procedure" value="<%=objForm.Fields().Item("DTA_Procedure")%>">
