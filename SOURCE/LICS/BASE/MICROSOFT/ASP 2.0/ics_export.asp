...
<!--
   ...
   function doUrl() {
      ...
      window.open('<%=objForm.Fields().Item("DTA_ActionUrl")%>',strName,'"' + strFeatures + '"',true);
   }
   function doExecute() {
   ...
   <table class="clsPopup" align=center cols=2 height=100% width=100% cellpadding="1" cellspacing="0">
      ...
      <tr>
         <td class="clsLabelBB" align=center colspan=2 nowrap><nobr>
            <table class="clsTable01" align=center cols=<%if objForm.Fields().Item("DTA_ActionText") = "" then%>1<%else%>3<%end if%> cellpadding="0" cellspacing="0">
               <tr>
                  <td align=center colspan=1 nowrap><nobr><a class="clsButton" href="javascript:doClose();">&nbsp;Close&nbsp;</a></nobr></td><%if objForm.Fields().Item("DTA_ActionText") <> "" then%>
                  <td align=center colspan=1 nowrap><nobr>&nbsp;</nobr></td>
                  <td align=center colspan=1 nowrap><nobr><a class="clsButton" href="javascript:doUrl();">&nbsp;<%=objForm.Fields().Item("DTA_ActionText")%>&nbsp;</a></nobr></td><%end if%>
               </tr>
            </table>
         </nobr></td>
      </tr>
...
sub ProcessExportFile()
   ...
   '//
   strReturn = objSelection.Execute("EXPORT", "select * from table(" & objForm.Fields().Item("DTA_Query") & ")", 0)
   if strReturn = "*OK" then
      Response.Buffer = true
      Response.ContentType = "application/octet-stream"
      Response.AddHeader "content-disposition", "attachment; filename=" & objForm.Fields().Item("DTA_Name") & ".TXT"
      for i = clng(objSelection.ListLower("EXPORT")) to clng(objSelection.ListUpper("EXPORT"))
         if i > clng(objSelection.ListLower("EXPORT")) then
...
sub ProcessExportInterface()
   ...
   '//
   strReturn = objProcedure.Execute(objForm.Fields().Item("DTA_Query"))%>
<html>
<script language="javascript">
<!--
   function document.onmouseover() {
