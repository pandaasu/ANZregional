...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_SrchTxt")) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_MsgTxt")) & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " t01.ale_msg_txt"
   strQuery = strQuery & " from lics_alert t01"
   strQuery = strQuery & " where t01.ale_srch_txt = '" & objForm.Fields().Item("DTA_SrchTxt") & "'"
   strQuery = strQuery & " and t01.ale_msg_txt = '" & objForm.Fields().Item("DTA_MsgTxt") & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strMode = "SELECT"
      strReturn = FormatError(strReturn)
