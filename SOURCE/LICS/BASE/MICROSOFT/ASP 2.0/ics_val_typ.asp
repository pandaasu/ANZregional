...
sub RetrieveRule(strGroup)
   ...
   strQuery = strQuery & " where t01.vtr_type = '" & objForm.Fields().Item("DTA_VatType") & "') t02"
...
sub ProcessInsertLoad()
   ...
   '// Retrieve the rules
   '//
   call RetrieveRule(objForm.Fields().Item("DTA_VatGroup"))
   if strMode = "FATAL" then
      exit sub
   end if
...
sub ProcessInsertAccept()
   ...
   call objProcedure.Execute("lics_form.set_value('VAT_TYPE','" & objSecurity.FixString(objForm.Fields().Item("DTA_VatType")) & "')")
   call objProcedure.Execute("lics_form.set_value('VAT_DESCRIPTION','" & objSecurity.FixString(objForm.Fields().Item("DTA_VatDescription")) & "')")
   call objProcedure.Execute("lics_form.set_value('VAT_GROUP','" & objSecurity.FixString(objForm.Fields().Item("DTA_VatGroup")) & "')")
   ...
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VTR_RULE','" & objSecurity.FixString(objForm.Fields().Item("DET_Rule" & i)) & "')")
   next
