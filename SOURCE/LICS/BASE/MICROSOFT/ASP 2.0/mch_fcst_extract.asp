...
sub ProcessSelect()
   ...
   strQuery = strQuery & " t02.extract_action_text,"
   strQuery = strQuery & " t02.extract_action_url"
...
sub ProcessCreate()
   ...
   '//
   strLoadIdentifier = ""
   lngCount = clng(objForm.Fields().Item("DET_ExtractLinkCount"))
   for i = 1 to lngCount
      if strLoadIdentifier <> "" then
         strLoadIdentifier = strLoadIdentifier & ","
      end if
      strLoadIdentifier = strLoadIdentifier & objSecurity.FixString(objForm.Fields().Item("DET_ExtractLink" & i))
   next
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_ExtractType")) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_ExtractIdentifier")) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_ExtractDescription")) & "',"
   strStatement = strStatement & objSecurity.FixString(objForm.Fields().Item("DTA_ExtractVersion"))  & ","
...
sub ProcessDelete()
   ...
   '//
   strStatement = "dw_fcst_maintenance.delete_extract('" & objSecurity.FixString(objForm.Fields().Item("DTA_ExtractIdentifier")) & "')"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
   end if
