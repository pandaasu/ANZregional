...
sub ProcessRequest
   ...
   '// Classification
   '//
   if objForm.Fields().Item("DTA_Data") = "*CLASS" then
      ...
      strQuery = strQuery & " from sap_val_mes"
      strQuery = strQuery & " where vam_class = '" & objForm.Fields().Item("DTA_Select") & "'"
   ...
   '// Type
   '//
   if objForm.Fields().Item("DTA_Data") = "*TYPE" then
      ...
      strQuery = "select vam_code, vam_sequence, vam_group, vam_class, vam_type, vam_filter, vam_rule, vam_text"
      strQuery = strQuery & " from sap_val_mes"
      strQuery = strQuery & " where vam_type = '" & objForm.Fields().Item("DTA_Select") & "'"
      strQuery = strQuery & " and vam_version = 0"
      strQuery = strQuery & " order by vam_code asc, vam_sequence asc"
      strReturn = objSelection.Execute("DATA", strQuery, 0)
      if strReturn <> "*OK" then
   ...
   '// Filter
   '//
   if objForm.Fields().Item("DTA_Data") = "*FILTER" then
      ...
      strQuery = "select vam_code, vam_sequence, vam_group, vam_class, vam_type, vam_filter, vam_rule, vam_text"
      strQuery = strQuery & " from sap_val_mes"
      strQuery = strQuery & " where vam_filter = '" & objForm.Fields().Item("DTA_Select") & "'"
      strQuery = strQuery & " and vam_version = 0"
      strQuery = strQuery & " order by vam_code asc, vam_sequence asc"
      strReturn = objSelection.Execute("DATA", strQuery, 0)
      if strReturn <> "*OK" then
   ...
   '// Rule
   '//
   if objForm.Fields().Item("DTA_Data") = "*RULE" then
      ...
      strQuery = "select vam_code, vam_sequence, vam_group, vam_class, vam_type, vam_filter, vam_rule, vam_text"
      strQuery = strQuery & " from sap_val_mes"
      strQuery = strQuery & " where vam_rule = '" & objForm.Fields().Item("DTA_Select") & "'"
      strQuery = strQuery & " and vam_version = 0"
      strQuery = strQuery & " order by vam_code asc, vam_sequence asc"
      strReturn = objSelection.Execute("DATA", strQuery, 0)
      if strReturn <> "*OK" then
   ...
   Response.AddHeader "content-disposition", "attachment; filename=" & objForm.Fields().Item("DTA_Select") & ".xls;FileFormat=56"
