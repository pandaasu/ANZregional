...
sub ProcessConfirm()
   ...
   '//
   if objForm.Fields().Item("DTA_Transaction") = "*ORD" then
      strStatement = "lads_atllad13_deletion.execute('*CON',"
   end if
   if objForm.Fields().Item("DTA_Transaction") = "*DLV" then
      strStatement = "lads_atllad16_deletion.execute('*CON',"
   end if
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_Identifier")) & "')"
