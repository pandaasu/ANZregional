...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_SemMenu")) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_SemDescription")) & "'"
   ...
   '//
   lngLink = 0
   lngCount = clng(objForm.Fields().Item("DET_LinkMenuCount"))
   for i = 1 to lngCount
      lngLink = lngLink + 1
      ...
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DET_LinkMenu" & i)) & "'"
      strStatement = strStatement & ")"
      strReturn = objFunction.Execute(strStatement)
      if strReturn <> "*OK" then
         strError = FormatError(strReturn)
   ...
   next
   lngCount = clng(objForm.Fields().Item("DET_LinkOptionCount"))
   for i = 1 to lngCount
      lngLink = lngLink + 1
      strStatement = "lics_security_configuration.insert_menu_link("
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DTA_SemMenu")) & "',"
      ...
      strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields().Item("DET_LinkOption" & i)) & "'"
      strStatement = strStatement & ")"
      strReturn = objFunction.Execute(strStatement)
      if strReturn <> "*OK" then
         strError = FormatError(strReturn)
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " t01.sem_description"
   strQuery = strQuery & " from lics_sec_menu t01"
   strQuery = strQuery & " where t01.sem_menu = '" & objForm.Fields().Item("DTA_SemMenu") & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
      strMode = "SELECT"
   ...
   strQuery = strQuery & " from lics_sec_menu t01"
   strQuery = strQuery & " where t01.sem_menu != '" & objForm.Fields().Item("DTA_SemMenu") & "'"
   strQuery = strQuery & " and t01.sem_menu not in (select sel_link from lics_sec_link where sel_menu = '" & objForm.Fields().Item("DTA_SemMenu") & "' and sel_type = '*MNU')"
   strQuery = strQuery & " order by t01.sem_menu asc"
   strReturn = objSelection.Execute("MENULINK", strQuery, 0)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
   ...
   strQuery = strQuery & " t01.seo_description"
   strQuery = strQuery & " from lics_sec_option t01"
   strQuery = strQuery & " where t01.seo_option not in (select sel_link from lics_sec_link where sel_menu = '" & objForm.Fields().Item("DTA_SemMenu") & "' and sel_type = '*OPT')"
   strQuery = strQuery & " order by t01.seo_option asc"
   strReturn = objSelection.Execute("OPTIONLINK", strQuery, 0)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
   ...
   strQuery = strQuery & " and t01.sel_menu = '" & objForm.Fields().Item("DTA_SemMenu") & "'"
...
sub ProcessUpdateAccept()
   ...
   strStatement = strStatement & "'" & objForm.Fields().Item("DTA_SemMenu") & "'"
   strStatement = strStatement & ")"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
