...
sub ProcessInsertAccept()
   ...
   call objProcedure.Execute("lics_form.set_value('VAR_RULE','" & objSecurity.FixString(objForm.Fields().Item("DTA_VarRule")) & "')")
   call objProcedure.Execute("lics_form.set_value('VAR_DESCRIPTION','" & objSecurity.FixString(objForm.Fields().Item("DTA_VarDescription")) & "')")
   call objProcedure.Execute("lics_form.set_value('VAR_GROUP','" & objSecurity.FixString(objForm.Fields().Item("DTA_VarGroup")) & "')")
   lngIndex = 1
   do while lngIndex <= len(objForm.Fields().Item("DTA_VarQuery"))
      call objProcedure.Execute("lics_form.set_value('VAR_QUERY','" & objSecurity.FixString(mid(objForm.Fields().Item("DTA_VarQuery"),lngIndex,2000)) & "')")
      lngIndex = lngIndex + 2000
   loop
   call objProcedure.Execute("lics_form.set_value('VAR_TEST','" & objSecurity.FixString(objForm.Fields().Item("DTA_VarTest")) & "')")
   call objProcedure.Execute("lics_form.set_value('VAR_MESSAGE','" & objSecurity.FixString(objForm.Fields().Item("DTA_VarMessage")) & "')")

   '//
   '// Insert the rule
   '//
   strStatement = "lads_val_configuration.insert_rule"
...
sub ProcessUpdateLoad()
   ...
   '// Retrieve the rule
   '//
   strStatement = "lads_val_configuration.retrieve_rule"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "FATAL"
      exit sub
...
sub ProcessUpdateAccept()
   ...
   '// Update the rule
   '//
   strStatement = "lads_val_configuration.update_rule"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "UPDATE"
      strError = FormatError(strReturn)
...
sub ProcessDeleteAccept()
   ...
   '// Update the rule
   '//
   strStatement = "lads_val_configuration.delete_rule"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
      strError = FormatError(strReturn)
