CREATE OR REPLACE PACKAGE pds_ap_claims_01_rep IS

/*******************************************************************************
  NAME:      run_pds_ap_claims_01_rep
...
/*******************************************************************************
  ...
  Pos  Type   Format   Description                          Example
  ---- ------ -------- ------------------------------------ --------------------
  1    IN     VARCHAR2 Data Type                            AP Claims
  2    IN     VARCHAR2 Sort Field                           AP Claims Code
...
END pds_ap_claims_01_rep;
/


CREATE OR REPLACE PACKAGE BODY pds_ap_claims_01_rep IS
  ...
  pc_job_type_ap_claims_01_rep CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('ap_claims_01_rep','JOB_TYPE');
  ...
  pc_job_name_ap_claims_01_prc CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('ap_claims_01_prc','JOB_NAME');
...
PROCEDURE run_pds_ap_claims_01_rep IS
...
BEGIN
  ...
  -- Start run_pds_ap_claims_01_rep procedure.
  pds_utils.create_log;
  write_log(pc_job_type_ap_claims_01_rep,'N/A',pv_log_level,'run_pds_ap_claims_01_rep - START.');

  -- Read through each of the Company/Division records to be reported.
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level,'Open csr_cmpny_div_list cursor.');
  OPEN csr_cmpny_div_list;
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level,'Looping through the csr_cmpny_div_list cursor.');
  LOOP
  ...
  END LOOP;
   write_log(pc_data_type_ap_claims, 'N/A', pv_log_level, 'End of csr_cmpny_div_list cursor loop.');

  -- Close the cursor.
  write_log(pc_data_type_ap_claims, 'N/A', pv_log_level, 'Close csr_cmpny_div_list cursor.');
  CLOSE csr_cmpny_div_list;

  -- End run_pds_ap_claims_01_rep procedure.
  write_log(pc_job_type_ap_claims_01_rep,'N/A',pv_log_level,'run_pds_ap_claims_01_rep - END.');
  pds_utils.end_log;
...
PROCEDURE validate_error_report (
  ...
  CURSOR csr_valdtn_rec IS
    ...
    WHERE
      t1.valdtn_type_code = pc_valdtn_type_ap_claims
  ...
  CURSOR csr_email_addresses IS
    ...
    FROM
      pds_email_list t1
    WHERE
      t1.job_type_code = pc_job_type_ap_claims_01_rep
...
BEGIN
  ...
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'validate_error_report - START.');
  ...
  -- Read through each of the invalid AP Claims records to be reported.
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'Open csr_valdtn_rec cursor.');
  OPEN csr_valdtn_rec;
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'Looping through the csr_valdtn_rec cursor.');
  LOOP
    ...
    IF v_count = 0 THEN
      ...
      write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 2,'PROMAX: '|| pc_system_tier||' - Invalid AP Claims records found for '||i_cmpny_desc||' '||i_div_desc||'.');

      -- Insert header record into the array.
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
      tbl_error(v_count).code := 'Interface Type:    '||pc_job_name_ap_claims_01_prc;
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
      tbl_error(v_count).code := ' ';
      v_count := v_count + 1;
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
      tbl_error(v_count).code := 'Invalid AP Claims record(s).';
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
      tbl_error(v_count).code := 'Batch |'|| 'Cmpny|' || 'Div|' || 'Claims Seq|' || 'Severity|' || 'Message';
    ...
    tbl_error(v_count).rec_type := pc_valdtn_type_ap_claims;
    tbl_error(v_count).code := RPAD(rv_valdtn_rec.intfc_batch_code,6) || ' ' || -- Interface Batch Code.
    RPAD(rv_valdtn_rec.cmpny_code, 5) || ' ' || -- Company Code.
    RPAD(rv_valdtn_rec.div_code,3) || ' ' || -- Division Code.
    RPAD(rv_valdtn_rec.ap_claims_seq, 10)     || ' ' || -- AP Claims Seq.
    RPAD(rv_valdtn_rec.severity, 8)     || ' ' ||
    TRIM(rv_valdtn_rec.message);

  END LOOP;
  write_log(pc_data_type_ap_claims, 'N/A', pv_log_level + 1, 'End of csr_valdtn_rec cursor loop.');

  -- Close the cursor.
  write_log(pc_data_type_ap_claims, 'N/A', pv_log_level + 1, 'Close csr_valdtn_rec cursor.');
  ...
  IF v_count > 0 THEN
    ...
    write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'Open csr_email_addresses cursor.');
    OPEN csr_email_addresses;
    write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'Looping through the csr_email_addresses cursor.');
    LOOP
      ...
      pds_utils.start_long_email(rv_email_addresses.email_address,
        'PROMAX: '|| pc_system_tier||' - Invalid AP Claims ('||i_cmpny_desc||' '||i_div_desc||').',
        pv_log_level + 2);
    ...
    END LOOP;
    write_log(pc_data_type_ap_claims, 'N/A', pv_log_level + 1, 'End of csr_email_addresses cursor loop.');

    -- Close the cursor.
    write_log(pc_data_type_ap_claims, 'N/A', pv_log_level + 1, 'Close csr_email_addresses cursor.');
    CLOSE csr_email_addresses;

  END IF;

  -- End validate_error_report procedure.
  write_log(pc_data_type_ap_claims,'N/A',pv_log_level + 1,'validate_error_report - END.');

EXCEPTION
  -- Send warning message via e-mail and pds_log.
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_failure_msg('PDS_AP_CLAIMS_01_REP.VALIDATE_ERROR_REPORT:',
    ...
    pds_utils.send_email_to_group(pc_job_type_ap_claims_01_rep,'MFANZ Promax AP Claims 01 Report',
      pv_result_msg);
...
BEGIN
  ...
  pds_utils.log(pc_job_type_ap_claims_01_rep,
                i_data_type,
                i_sort_field,
                i_log_level,
                i_log_text);
...
EXCEPTION
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_error_msg('PDS_AP_CLAIMS_01_REP.WRITE_LOG:',
        'Unable to write to the PDS_LOG table.') ||
      utils.create_sql_err_msg();
    pds_utils.log(pc_job_type_ap_claims_01_rep,pc_data_type_not_applicable,'N/A',i_log_level,
      pv_result_msg);
END write_log;
