CREATE OR REPLACE PACKAGE pmx_cdw_cust_rep IS

/*******************************************************************************
  NAME:      run_pmx_cdw_cust_rep
  PURPOSE:   This procedure calls the validate_error_report procedure for each
             ...
             PMX_CDW_CUST_PRC procedure.
             .
  REVISIONS:
  Ver   Date       Author               Description
  ----- ---------- -------------------- ----------------------------------------
  1.0   16/10/2009 Steve Gregan        Created this procedure.
  ...
  NOTES:
********************************************************************************/
PROCEDURE run_pmx_cdw_cust_rep;
...
END pmx_cdw_cust_rep;
/

CREATE OR REPLACE PACKAGE BODY pmx_cdw_cust_rep IS
  ...
  pc_job_type_cdw_cust_rep     CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('cdw_cust_rep','JOB_TYPE');
  ...
  pc_job_name_cdw_cust_prc     CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('cdw_cust_prc','JOB_NAME');
...
PROCEDURE run_pmx_cdw_cust_rep IS
...
BEGIN
  ...
  -- Start run_pmx_cdw_cust_rep procedure.
  write_log(pc_job_type_cdw_cust_rep,'N/A',pv_log_level,'run_pmx_cdw_cust_rep - START.');
  ...
  -- End run_pmx_cdw_cust_rep procedure.
  write_log(pc_job_type_cdw_cust_rep,'N/A',pv_log_level,'run_pmx_cdw_cust_rep - END.');
...
PROCEDURE validate_error_report (
  ...
  CURSOR csr_email_addresses IS
    ...
    FROM
      pds_email_list t1
    WHERE
      t1.job_type_code = pc_job_type_cdw_cust_rep
...
BEGIN
  ...
  LOOP
    ...
    IF v_count = 0 THEN
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_cust;
      tbl_error(v_count).code := 'Interface Type:    '||pc_job_name_cdw_cust_prc;
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_cust;
      tbl_error(v_count).code := ' ';
      v_count := v_count + 1;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_failure_msg('PDS_CDW_CUST_REP.VALIDATE_ERROR_REPORT:',
    ...
    pds_utils.send_email_to_group(pc_job_type_pmx_cdw_cust_rep,'MFANZ CDW Customer Report',
      pv_result_msg);
...
BEGIN
  ...
  pds_utils.log(pc_job_type_cdw_cust_rep,
                i_data_type,
                i_sort_field,
                i_log_level,
                i_log_text);
...
EXCEPTION
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_error_msg('PDS_CDW_CUST_REP.WRITE_LOG:',
        'Unable to write to the PDS_LOG table.') ||
      utils.create_sql_err_msg();
    pds_utils.log(pc_job_type_pmx_cdw_cust_rep,pc_data_type_not_applicable,'N/A',i_log_level,pv_result_msg);
END write_log;
