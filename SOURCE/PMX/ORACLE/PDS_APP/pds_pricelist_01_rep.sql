CREATE OR REPLACE PACKAGE pds_pricelist_01_rep IS

/*******************************************************************************
  NAME:      run_pds_pricelist_01_rep
  PURPOSE:   This procedure calls the validate_error_report procedure for each
             ...
             PDS_PRICELIST_01_PRC procedure.
             .
  REVISIONS:
...
/*******************************************************************************
  ...
  Pos  Type   Format   Description                          Example
  ---- ------ -------- ------------------------------------ --------------------
  1    IN     VARCHAR2 Data Type                            Pricelist
  2    IN     VARCHAR2 Sort Field                           Pricelist Code
...
END pds_pricelist_01_rep;
/


CREATE OR REPLACE PACKAGE BODY pds_pricelist_01_rep IS
  ...
  pc_job_type_pricelist_01_rep CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('pricelist_01_rep','JOB_TYPE');
  ...
  pc_job_name_pricelist_01_prc CONSTANT pds_constants.const_value%TYPE := pds_lookup.lookup_constant('pricelist_01_prc','JOB_NAME');
...
PROCEDURE run_pds_pricelist_01_rep IS
...
BEGIN
  ...
  END LOOP;
   write_log(pc_data_type_pricelist, 'N/A', pv_log_level, 'End of csr_cmpny_div_list cursor loop.');

  -- Close the cursor.
  write_log(pc_data_type_pricelist, 'N/A', pv_log_level, 'Close csr_cmpny_div_list cursor.');
...
PROCEDURE validate_error_report (
  ...
  -- CURSOR DECLARATIONS.
  -- Select all invalid Pricelist records.
  CURSOR csr_valdtn_rec IS
    SELECT
      t1.item_code_1 AS cmpny_code,
      t1.item_code_2 AS div_code,
      t1.item_code_3 AS distbn_chnl_code,
      t1.item_code_4 AS matl_code,
      t1.item_code_5 AS effdate,
    ...
    WHERE
      t1.valdtn_type_code = pc_valdtn_type_pricelist
      AND t1.valdtn_reasn_hdr_code = t2.valdtn_reasn_hdr_code
      AND t1.item_code_1 = v_valdtn_cmpny_code
      AND t1.item_code_2 = v_valdtn_div_code
    ORDER BY
      t1.item_code_1,
      t1.item_code_2,
      t1.item_code_3,
  ...
  CURSOR csr_email_addresses IS
    ...
    FROM
      pds_email_list t1
    WHERE
      t1.job_type_code = pc_job_type_pricelist_01_rep
...
BEGIN
  ...
  write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'validate_error_report - START.');
  ...
  -- Read through each of the invalid Pricelist records to be reported.
  write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'Open csr_valdtn_rec cursor.');
  OPEN csr_valdtn_rec;
  write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'Looping through the csr_valdtn_rec cursor.');
  LOOP
    ...
    IF v_count = 0 THEN
      ...
      write_log(pc_data_type_pricelist,'N/A',pv_log_level + 2,'PROMAX: '|| pc_system_tier||' - Invalid Pricelist records found for '||i_cmpny_desc||' '||i_div_desc||'.');

      -- Insert header record into the array.
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
      tbl_error(v_count).code := 'Interface Type:    '||pc_job_name_pricelist_01_prc;
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
      tbl_error(v_count).code := ' ';
      v_count := v_count + 1;
      ...
      tbl_error(v_count).code := ' ';
       v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
      tbl_error(v_count).code := 'Invalid Pricelist record(s).';
      ...
      v_count := v_count + 1;
      tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
      tbl_error(v_count).code := 'Cmpny|' || 'Div|' || 'Distbn Chnl|' || 'Material|' || 'Eff Date|' || 'Severity|'|| 'Message';
    ...
    tbl_error(v_count).rec_type := pc_valdtn_type_pricelist;
    tbl_error(v_count).code := RPAD(rv_valdtn_rec.cmpny_code,5) || ' '|| -- Company Code.
    RPAD(rv_valdtn_rec.div_code, 3) || ' ' || -- Division Code.
    RPAD(rv_valdtn_rec.distbn_chnl_code,11) || ' '|| -- Distribution Channel Code.
    RPAD(rv_valdtn_rec.matl_code,8) || ' ' || -- Material Code.
    RPAD(rv_valdtn_rec.effdate, 8)     || ' ' || -- Effective Date.
    RPAD(rv_valdtn_rec.severity, 8)     || ' ' ||
    TRIM(rv_valdtn_rec.message);

  END LOOP;
  write_log(pc_data_type_pricelist, 'N/A', pv_log_level + 1, 'End of csr_valdtn_rec cursor loop.');

  -- Close the cursor.
  write_log(pc_data_type_pricelist, 'N/A', pv_log_level + 1, 'Close csr_valdtn_rec cursor.');
  ...
  IF v_count > 0 THEN
    ...
    write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'Open csr_email_addresses cursor.');
    OPEN csr_email_addresses;
    write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'Looping through the csr_email_addresses cursor.');
    LOOP
      ...
      EXIT WHEN csr_email_addresses%NOTFOUND;
       ...
       pds_utils.start_long_email(rv_email_addresses.email_address,
        'PROMAX: '|| pc_system_tier||' - Invalid Pricelist ('||i_cmpny_desc||' '||i_div_desc||').',
        pv_log_level + 2);
    ...
    END LOOP;
    write_log(pc_data_type_pricelist, 'N/A', pv_log_level + 1, 'End of csr_email_addresses cursor loop.');

    -- Close the cursor.
    write_log(pc_data_type_pricelist, 'N/A', pv_log_level + 1, 'Close csr_email_addresses cursor.');
    CLOSE csr_email_addresses;

  END IF;

  -- End validate_error_report procedure.
  write_log(pc_data_type_pricelist,'N/A',pv_log_level + 1,'validate_error_report - END.');

EXCEPTION
  -- Send warning message via e-mail and pds_log.
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_failure_msg('PDS_PRICELIST_01_REP.VALIDATE_ERROR_REPORT:',
    ...
    pds_utils.send_email_to_group(pc_job_type_pricelist_01_rep,'MFANZ Promax Pricelist 01 Report',
      pv_result_msg);
...
BEGIN
  ...
  pds_utils.log(pc_job_type_pricelist_01_rep,
                i_data_type,
                i_sort_field,
                i_log_level,
                i_log_text);
...
EXCEPTION
  WHEN OTHERS THEN
    pv_result_msg :=
      utils.create_error_msg('PDS_PRICELIST_01_REP.WRITE_LOG:',
        'Unable to write to the PDS_LOG table.') ||
      utils.create_sql_err_msg();
    pds_utils.log(pc_job_type_pricelist_01_rep,pc_data_type_not_applicable,'N/A',i_log_level,
      pv_result_msg);
END write_log;
