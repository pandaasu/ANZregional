...
--  NOTES:
--********************************************************************************
select t01.cocode,t01.divcode,t01.channel,t01.prodcode,t01.startdate,t01.price1,t01.mnfcost,t01.rrp
  from (select t01.*,
               rank() over (partition by t01.cocode, t01.divcode, t01.channel, t01.prodcode
                                order by t01.startdate asc) as rnkseq
          from (select x.cocode,x.divcode,x.channel,x.prodcode,x.startdate,x.price1,x.mnfcost,y.rrp
                  from
(
select
  ltrim(a.vkorg,1) as cocode,
  f.dvsn as divcode,
...
where a.vakey  = e.vakey
  ...
  and f.matl_type = 'ZREP'
  and f.trdd_unit = 'X'
  and (a.vkorg,h.dstrbtn_chnl) in (('147','11'),('147','12'),('149','10'))
  and a.matnr = h.matl_code
  and a.matnr = f.matl_code
  and a.datbi >= to_char(sysdate, 'yyyymmdd')
) y
where x.cocode=y.cocode(+)
  and x.divcode=y.divcode(+)
  and x.channel=y.channel(+)
  and x.prodcode=y.prodcode(+)
) t01) t01 where t01.rnkseq = 1;