...
'// Script  : sms_sys_control_select.asp                         //
'// Author  : Steve Gregan                                       //
'// Date    : July 2009                                          //
...
sub ProcessRequest()
   ...
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('SMS_STREAM','" & objSecurity.FixString(objForm.Fields("StreamPart" & i).Value) & "')")
   next
   ...
   '//
   '// Retrieve the system control selection
   '//
   strStatement = "select xml_text from table(sms_app.sms_gen_function.retrieve_system_control)"
