...
/* Package Header */
/******************/
create or replace package dw_flag_base_creation as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : dw_flag_base_creation
    Owner   : dw_app
    ...
    This package contain the data warehouse flag base creation. The package exposes one
    procedure EXECUTE that performs the flag base creation based on the following parameters:

    1. PAR_COMPANY (Company Code) (MANDATORY)

       The company code for the flag base.
    ...
    **notes**
    1. A web log is produced under the search value DW_FLAG_BASE_CREATION where all errors are logged.

    2. All errors will raise an exception to the calling application so that an alert can be raised.

    YYYY/MM   Author         Description
    -------   ------         -----------
    2011/04   Steve Gregan   Created
...
end dw_flag_base_creation;
/

/****************/
/* Package Body */
/****************/
create or replace package body dw_flag_base_creation as
   ...
   procedure execute(par_company in varchar2) is
      ...
      /* Local constants
      /*-*/
      con_function constant varchar2(128) := 'DW Flag Base Creation';
   ...
   begin
      ...
      /*-*/
      var_log_prefix := 'DW - FLAG_BASE_CREATION';
      var_log_search := 'DW_FLAG_BASE_CREATION' || '_' || lics_stream_processor.callback_event;
      ...
      if upper(par_company) != 'CON' then
         var_process_code := 'FLAGBASE_'||var_company_code;
      else
         var_process_code := 'FLAGBASE_CON';
      end if;
      if upper(par_company) != 'CON' then
      ...
      /*-*/
      if upper(par_company) = 'CON' then
         var_filename := 'base4749.txt';
      else
         var_filename := 'base'||par_company||'.txt';
      end if;
      ...
      /* Begin procedure
      /*-*/
      lics_logging.write_log('Begin - Flag Base Creation - Parameters(' || par_company || ' + ' || to_char(to_date(var_process_date,'yyyymmdd'),'yyyy/mm/dd') || ')');
      ...
      /* End procedure
      /*-*/
      lics_logging.write_log('End - Flag Base Creation');
      ...
      if var_errors = true then
         ...
         if not(trim(var_email) is null) and trim(upper(var_email)) != '*NONE' then
            lics_notification.send_email(dw_parameter.system_code,
                                         ...
                                         con_function,
                                         'DW_FLAG_BASE_CREATION',
                                         var_email,
                                         'One or more errors occurred during the Flag Base Creation execution - refer to web log - ' || lics_logging.callback_identifier);
         end if;
      ...
      /*-*/
         ...
         /*-*/
         /* Set the flag base creation process trace
         /*-*/
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FLAG_BASE_CREATION - ' || var_exception);
...
/**************************/
create or replace public synonym dw_flag_base_creation for dw_app.dw_flag_base_creation;
grant execute on dw_flag_base_creation to public;
