...
/* Package Header */
/******************/
create or replace package data_mart_01_extract as
   ...
   /**
    System  : cdw
    Package : data_mart_01_extract
    Owner   : ods_app
    Author  : Steve Gregan

    DESCRIPTION
    -----------
    Corporate Data Warehouse - data_mart_01_extract

    The package builds the data mart for the Venus MFANZ303_01 report.
   ...
   *******************************************************************************/
   ...
   /* Public declarations
   /*-*/
   procedure build(par_company_code in varchar2);

end data_mart_01_extract;
/

/****************/
/* Package Body */
/****************/
create or replace package body data_mart_01_extract as
   ...
   /*********************************************/
   /* This procedure performs the build routine */
   /*********************************************/
   procedure build(par_company_code in varchar2) is
   ...
   begin
      ...
      /*-*/
      delete from data_mart_01_hdr where company_code = par_company_code;
      delete from data_mart_01_det where company_code = par_company_code;
      commit;
      ...
      /*-*/
      /* Update the data mart header status
      /*-*/
      update data_mart_01_hdr
         set extract_status = '*COMPLETE',
             extract_end_time = sysdate
       where company_code = par_company_code;
      commit;
   ...
   exception
      ...
      when others then
         ...
         begin
            update data_mart_01_hdr
               set extract_status = '*FAILED - ' || var_exception,
                   extract_end_time = sysdate
             where company_code = par_company_code;
            commit;
         exception
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DATA_MART_01_EXTRACT - BUILD - ' || var_exception);
   ...
   procedure extract_sale(par_company_code in varchar2) is
      ...
      var_str_yyyyppw number(7,0);
      var_end_yyyyppw number(7,0);
      ...
      cursor csr_sales_extract_01 is
         select t01.company_code,
                ...
                nvl(sum(case when t01.billing_eff_yyyypp >= var_ytd_str_yyyypp and t01.billing_eff_yyyypp <= var_ytd_end_yyyypp then t01.base_uom_billed_qty end),0) as ytd_qty,
                nvl(sum(case when t01.billing_eff_yyyypp >= var_ytd_str_yyyypp and t01.billing_eff_yyyypp <= var_ytd_end_yyyypp then t01.gsv end),0) as ytd_gsv,
                ...
                nvl(sum(case when t01.billing_eff_yyyypp >= var_mat_str_yyyypp and t01.billing_eff_yyyypp <= var_mat_end_yyyypp then t01.base_uom_billed_qty end),0) as mat_qty,
                nvl(sum(case when t01.billing_eff_yyyypp >= var_mat_str_yyyypp and t01.billing_eff_yyyypp <= var_mat_end_yyyypp then t01.gsv end),0) as mat_gsv,
                ...
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p01 then t01.base_uom_billed_qty end),0) as p01_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p02 then t01.base_uom_billed_qty end),0) as p02_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p03 then t01.base_uom_billed_qty end),0) as p03_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p04 then t01.base_uom_billed_qty end),0) as p04_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p05 then t01.base_uom_billed_qty end),0) as p05_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p06 then t01.base_uom_billed_qty end),0) as p06_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p07 then t01.base_uom_billed_qty end),0) as p07_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p08 then t01.base_uom_billed_qty end),0) as p08_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p09 then t01.base_uom_billed_qty end),0) as p09_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p10 then t01.base_uom_billed_qty end),0) as p10_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p11 then t01.base_uom_billed_qty end),0) as p11_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p12 then t01.base_uom_billed_qty end),0) as p12_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p13 then t01.base_uom_billed_qty end),0) as p13_qty,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p01 then t01.gsv end),0) as p01_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p02 then t01.gsv end),0) as p02_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p03 then t01.gsv end),0) as p03_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p04 then t01.gsv end),0) as p04_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p05 then t01.gsv end),0) as p05_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p06 then t01.gsv end),0) as p06_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p07 then t01.gsv end),0) as p07_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p08 then t01.gsv end),0) as p08_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p09 then t01.gsv end),0) as p09_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p10 then t01.gsv end),0) as p10_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p11 then t01.gsv end),0) as p11_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p12 then t01.gsv end),0) as p12_gsv,
                nvl(sum(case when t01.billing_eff_yyyypp = var_cyr_p13 then t01.gsv end),0) as p13_gsv,
   ...
   begin
      ...
      var_str_yyyyppw := (var_current_yyyypp * 10) + 1;
      var_end_yyyyppw := var_current_yyyyppw - 1;
      ...
      /*-*/
      /* Extract the weekly sales values
      /*-*/
   ...
   begin
      ...
      /*-*/
      open csr_data_mart_01_det;
      fetch csr_data_mart_01_det into var_work;
      if csr_data_mart_01_det%notfound then

         /*-*/
         /* Data type QTY
         /*-*/
         insert into data_mart_01_det
            (company_code,
             matl_code,
...
/**************************/
create or replace public synonym data_mart_01_extract for ods_app.data_mart_01_extract;
grant execute on data_mart_01_extract to public;