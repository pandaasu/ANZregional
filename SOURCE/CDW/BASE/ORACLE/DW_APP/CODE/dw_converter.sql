...
create or replace package body dw_converter as
   ...
   begin
      ...
      loop
         ...
         if var_exit = false then
            ...
            tab_dw_purch_base(var_work).purch_order_line_status := '*OPEN';
   ...
   begin
      ...
      loop
         ...
         if var_exit = false then
            ...
            tab_dw_order_base(var_work).order_line_status := '*OPEN';
   ...
   begin
      ...
      loop
         ...
         if var_exit = false then
            ...
            tab_dw_dlvry_base(var_work).dlvry_line_status := '*OPEN';
   ...
   /* This procedure performs the execute delivery fact routine */
   /*************************************************************/
   procedure update_dlvry_fact is
   ...
   begin
      ...
      loop
         ...
         if var_exit = false then
            var_work := var_work + 1;
            tbl_doc(var_work) := rcd_source.dlvry_doc_num;
            tbl_lin(var_work) := rcd_source.dlvry_doc_line_num;
         end if;

         if (var_exit = false and var_work = var_size) or
            (var_exit = true and var_work > 0) then
            forall idx in 1..var_work
               update dw_dlvry_base set dlvry_doc_line_num = to_char(to_number(dlvry_doc_line_num),'fm000000')
                where dlvry_doc_num = tbl_doc(idx)
                  and dlvry_doc_line_num = tbl_lin(idx);
            commit;
            var_work := 0;
            tbl_doc.delete;
            tbl_lin.delete;
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - CDW - CONVERTER - UPDATE - DLVRY_FACT Procedure -' || substr(SQLERRM, 1, 1024));
