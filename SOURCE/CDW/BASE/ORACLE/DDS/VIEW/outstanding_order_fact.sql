CREATE OR REPLACE FORCE VIEW dds.outstanding_order_fact (company_code,
                                                         doc_xactn_type_code,
                                                         doc_num,
                                                         doc_line_num,
                                                         doc_type_code,
...
AS
   SELECT t1.company_code AS company_code, 1 AS doc_xactn_type_code,
                                                         -- 1 = Purchase Order
          ...
          t1.purch_order_reasn_code AS reasn_code, NULL AS sold_to_cust_code,
          NULL AS bill_to_cust_code, NULL AS payer_cust_code,
          t1.out_qty AS qty, t1.out_qty_base_uom AS base_uom_qty,
          t1.out_qty_gross_tonnes AS qty_gross_tonnes,
          t1.out_qty_net_tonnes AS qty_net_tonnes,
          t1.cust_code AS ship_to_cust_code,
          t1.cust_code AS hier_link_cust_code, t1.matl_code AS matl_code,
          t1.purch_order_uom_code AS qty_uom_code,
          t1.purch_order_base_uom_code AS qty_base_uom_code,
          t1.plant_code AS plant_code,
          t1.storage_locn_code AS storage_locn_code, t1.out_gsv AS gsv,
          t1.out_gsv_xactn AS gsv_xactn, t1.out_gsv_aud AS gsv_aud,
          t1.out_gsv_usd AS gsv_usd, t1.out_gsv_eur AS gsv_eur, 0 AS niv,
          0 AS niv_xactn, 0 AS niv_aud, 0 AS niv_usd, 0 AS niv_eur, 0 AS ngv,
          0 AS ngv_xactn, 0 AS ngv_aud, 0 AS ngv_usd, 0 AS ngv_eur,
          t1.mfanz_icb_flag AS mfanz_icb_flag,
          t1.demand_plng_grp_division_code AS demand_plng_grp_division_code
     FROM dds.dw_purch_base t1, dds.matl_dim t2
    WHERE                                                      --'OUTSTANDING'
   ...
   SELECT t1.company_code AS company_code, 2 AS doc_xactn_type_code,
                                                            -- 2 = Sales Order
          t1.order_doc_num AS doc_num, t1.order_doc_line_num AS doc_line_num,
          t1.order_type_code AS doc_type_code, t1.creatn_date AS creatn_date,
          ...
          t1.mfanz_icb_flag AS mfanz_icb_flag,
          t1.demand_plng_grp_division_code AS demand_plng_grp_division_code
     FROM dds.dw_order_base t1
    WHERE                                                      --'OUTSTANDING'
          t1.order_line_status = '*OPEN'
   UNION ALL
   SELECT t1.company_code AS company_code, 3 AS doc_xactn_type_code,
                                                               -- 3 = Delivery
          t1.dlvry_doc_num AS doc_num,
          TO_CHAR (t1.dlvry_doc_line_num) AS doc_line_num,
          t1.dlvry_type_code AS doc_type_code, t1.creatn_date AS creatn_date,
          ...
          t1.company_currcy_code AS company_currcy_code,
          t1.exch_rate AS exch_rate, NULL AS purchg_company_code,
          NULL AS reasn_code, t1.sold_to_cust_code AS sold_to_cust_code,
          t1.bill_to_cust_code AS bill_to_cust_code,
          t1.payer_cust_code AS payer_cust_code, t1.del_qty AS qty,
          t1.del_qty_base_uom AS base_uom_qty,
          t1.del_qty_gross_tonnes AS qty_gross_tonnes,
          t1.del_qty_net_tonnes AS qty_net_tonnes,
          t1.ship_to_cust_code AS ship_to_cust_code,
          t1.sold_to_cust_code AS hier_link_cust_code,
          t1.matl_code AS matl_code, t1.dlvry_uom_code AS qty_uom_code,
          t1.dlvry_base_uom_code AS qty_base_uom_code,
          t1.plant_code AS plant_code,
          t1.storage_locn_code AS storage_locn_code, t1.del_gsv AS gsv,
          t1.del_gsv_xactn AS gsv_xactn, t1.del_gsv_aud AS gsv_aud,
          t1.del_gsv_usd AS gsv_usd, t1.del_gsv_eur AS gsv_eur, 0 AS niv,
          0 AS niv_xactn, 0 AS niv_aud, 0 AS niv_usd, 0 AS niv_eur, 0 AS ngv,
          0 AS ngv_xactn, 0 AS ngv_aud, 0 AS ngv_usd, 0 AS ngv_eur,
          t1.mfanz_icb_flag AS mfanz_icb_flag,
          t1.demand_plng_grp_division_code AS demand_plng_grp_division_code
     FROM dds.dw_dlvry_base t1
    WHERE                                                        --OUTSTANDING
