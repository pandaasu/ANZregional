create or replace package bi_app.qv_csvqvs06_loader as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : qv_csvqvs06_loader
    Owner   : bi_app

    Description
    -----------
    CSV File to Qlikview - CSVQVS05 - Shipment Freight to Customer OP
...
end qv_csvqvs06_loader;

create or replace package body bi_app.qv_csvqvs06_loader as
   ...
   rcd_shp_ftc_op shp_ftc_op%rowtype;  
   ...
   begin
      ...
      /* Set sequence number
      /*-*/
      select qv.shp_ftc_op_seq.nextval
      into var_sequence
      from dual;
   ...
   begin
      ...
      if var_trn_count <= con_heading_count then
         ...
         rcd_shp_ftc_op.sfo_version := var_sequence;
         rcd_shp_ftc_op.sfo_accnt_assignmnt := lics_inbound_utility.get_variable('ACCNT_ASSIGNMNT_CODE');
         rcd_shp_ftc_op.sfo_plant := lics_inbound_utility.get_variable('PLANT_CODE');  
             
         for i in 1..con_mars_periods loop          
            var_period_text := 'PERIOD_' || to_char(i);
            rcd_shp_ftc_op.sfo_forecast := to_number(lics_inbound_utility.get_variable(var_period_text));
            rcd_shp_ftc_op.sfo_period := tbl_period(i); 
                     
            /*-*/
            /* Insert the row when required
            /*-*/            
            insert into qv.shp_ftc_op values rcd_shp_ftc_op;
         end loop;
      end if; 
...
/* Authority 
/**/
grant execute on qv_csvqvs06_loader to lics_app, fflu_app;

/**/
/* Synonym 
/**/
create or replace public synonym qv_csvqvs06_loader for bi_app.qv_csvqvs06_loader;