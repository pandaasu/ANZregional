...
/* Package Header */
/******************/
create or replace package asn_dcs_edi_default as
   ...
   /**
    System  : asn
    Package : asn_dcs_edi_default
    Owner   : ics_app
    Author  : Steve Gregan

    Description
    -----------
    Advance Shipping Notice - ASN Distribution Centre Shipment EDI Message - DEFAULT

    This package contain the processing logic for distribution centre advance shipping notices
    DEFAULT EDI message. The package exposes the following procedure:
...
end asn_dcs_edi_default;
/

/****************/
/* Package Body */
/****************/
create or replace package body asn_dcs_edi_default as
   ...
   begin
      ...
      /*-*/
      /* Retrieve the customer interchange identifier conversion
      /*-*/
      var_cust_iid := asn_parameter.retrieve_value('CUST_IID_XRF', rcd_asn_dcs_hdr.dch_trn_cust_iid);
      if var_cust_iid is null or upper(var_cust_iid) = '*NONE' then
         var_cust_iid := rcd_asn_dcs_hdr.dch_trn_cust_iid;
      end if;
...
/**************************/
create or replace public synonym asn_dcs_edi_default for ics_app.asn_dcs_edi_default;
grant execute on asn_dcs_edi_default to public;
