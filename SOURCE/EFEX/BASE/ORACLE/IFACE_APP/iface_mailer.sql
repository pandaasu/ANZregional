...
/* Package Header */
/******************/
create or replace package iface_mailer as

/******************************************************************************/
/* Package Definition                                                         */
/******************************************************************************/
/**
 Package : iface_mailer
 Owner   : iface_app
 ...
 The package implements the mailer functionality.
  ...
  EXAMPLE USAGE :
     ...
     begin
        ...
        iface_mailer.create_email('RECORD_LOADER@AP0001P.AP.MARS','linden.glen','Data Loading System',null,null);
        iface_mailer.create_part(null);
        iface_mailer.append_data('100 records were just loaded.');
        iface_mailer.append_data('See attachment for details :');

        iface_mailer.create_part('file_load.txt');
        < open loop >
           iface_mailer.append_data(< cursor record >);
        < finish loop >

        iface_mailer.create_part(null);
        iface_mailer.append_data('End of load report');
        
        iface_mailer.finalise_email;

     exception
        when others then
           if (iface_mailer.is_created) then
              iface_mailer.append_data('** FATAL ERROR DURING LOAD ** - Records listed were loaded successfully');
              iface_mailer.finalise_email;
           end if;
     end;
  ...
  SHORT EMAIL EXAMPLE USAGE :
     ...
     begin
        ...
        iface_mailer.send_short_email('LOADER@AP0001P.AP.MARS',
                                     'linden.glen', 
...
/* Package Body */
/****************/
create or replace package body iface_mailer as
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - IFACE_MAILER - SEND_SHORT_EMAIL - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - IFACE_MAILER - CREATE_EMAIL - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - IFACE_MAILER - CREATE_PART - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - IFACE_MAILER - APPEND_DATA - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - IFACE_MAILER - FINALISE_EMAIL - ' || substr(SQLERRM, 1, 1024));
...
/**************************/
create or replace public synonym iface_mailer for iface_app.iface_mailer;
grant execute on iface_mailer to public;
