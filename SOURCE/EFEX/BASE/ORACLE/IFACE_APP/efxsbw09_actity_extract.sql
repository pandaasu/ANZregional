...
/* Package Header */
/******************/
create or replace package efxsbw09_actity_extract as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : efxsbw09_actity_extract
    Owner   : iface_app

    Description
    -----------
    Activity Extract - EFEX to SAP BW

    This package extracts the Efex activities that have been modified within the last
    history number of days and sends the extract file to the SAP BW environment.
    The ICS interface EFXSBW09 has been created for this purpose.
...
end efxsbw09_actity_extract;
/

/****************/
/* Package Body */
/****************/
create or replace package body efxsbw09_actity_extract as
   ...
   procedure execute(par_sales_org_code in varchar2,
      ...
      cursor csr_extract is
         select t01.customer_id as customer_id,
                t01.activity_item_id as activity_item_id,
   ...
   begin
      ...
      loop
         ...
         /*-*/
         if (var_start) then
            var_instance := lics_outbound_loader.create_interface('EFXSBW09',null,'EFEX_ACTITY_EXTRACT.DAT.'||to_char(sysdate,'yyyymmddhh24miss'));
            var_start := false;
         end if;
         ...
         lics_outbound_loader.append_data('"'||replace(par_sales_org_code,'"','""')||'";'||
                                          ...
                                          '"'||replace(to_char(rcd_extract.activity_item_id),'"','""')||'";'||
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - EFXSBW09 EFEX_ACTITY_EXTRACT - ' || var_exception);
...
/**************************/
create or replace public synonym efxsbw09_actity_extract for iface_app.efxsbw09_actity_extract;
grant execute on efxsbw09_actity_extract to public;
