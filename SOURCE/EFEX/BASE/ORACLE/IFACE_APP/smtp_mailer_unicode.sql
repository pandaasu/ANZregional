CREATE OR REPLACE PACKAGE smtp_mailer_unicode IS
...
CREATE OR REPLACE PACKAGE BODY smtp_mailer_unicode IS
  ...
  BEGIN
    ...
    v_raw_data := utl_raw.cast_to_raw(subject);
    v_length := utl_raw.length(v_raw_data);
    v_index := 1;
    while (v_index < v_length) loop
       if (v_index + 20 < v_length) THEN
          utl_smtp.write_raw_data(conn, utl_encode.quoted_printable_encode(utl_raw.substr(v_raw_data, v_index, 20)));
       else
          utl_smtp.write_raw_data(conn, utl_encode.quoted_printable_encode(utl_raw.substr(v_raw_data, v_index)));
       end if;
       v_index := v_index + 20;
    end loop;
    utl_smtp.write_data(conn, '?='||utl_tcp.CRLF);
