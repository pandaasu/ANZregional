   ...
   begin
      ...
      loop
         ...
         if var_count = con_group then
            ...
            lics_outbound_loader.append_data('CTL'||'EFXCDW21'||rpad(' ',32-length('EFXCDW21'),' ')||nvl(par_market,'0')||rpad(' ',10-length(nvl(par_market,'0')),' ')||nvl(par_timestamp,' ')||rpad(' ',14-length(nvl(par_timestamp,' ')),' '));
            var_count := 0;
         end if;
