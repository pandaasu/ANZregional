...
/**
 * This class implements the Efex application distributor object model layer. The
 ...
 */
public final class cDistributorStore extends cDataStore {
   
   /**
    * Constructs a new instance
    */
   public cDistributorStore() {
      super.cstrDataStore = "EfexDistributor";
      super.cchrRecord = cMailbox.EFEX_DIS;
   }
   ...
   public java.util.Vector getDataList() throws Exception {
      ...
      chrListFields = new char[] {cMailbox.EFEX_DIS_ID, cMailbox.EFEX_DIS_NAME};
      objDataValues = super.getListing(cMailbox.EFEX_DIS, cMailbox.EFEX_DIS_NAME, chrListFields);
      if(objDataValues != null) {
         for (int i=0; i<objDataValues.size(); i++) {
            ...
            switch (objDataValue.getDataCode()) {
               case cMailbox.EFEX_DIS: {
                  objObject = new cDistributorList(Integer.parseInt(objDataValue.getDataValue()));
                  objList.addElement(objObject);
                  break;
               }
               case cMailbox.EFEX_DIS_ID: {
                  ((cDistributorList)objObject).setId(objDataValue.getDataValue());
                  break;
               }
               case cMailbox.EFEX_DIS_NAME: {
                  ((cDistributorList)objObject).setName(objDataValue.getDataValue());
                  break;
               }
               default: {
                  break;
