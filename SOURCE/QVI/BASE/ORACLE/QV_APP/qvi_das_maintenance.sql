...
create or replace package qv_app.qvi_das_maintenance as
   ...
   *******************************************************************************/
   ...
   function retrieve_time_data return qvi_xml_type pipelined;
   procedure update_time_data(par_user in varchar2);
...
create or replace package body qv_app.qvi_das_maintenance as
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         qvi_gen_function.add_mesg_data('FATAL ERROR - QVI_DAS_MAINTENANCE - SELECT_TIME_LIST - ' || substr(SQLERRM, 1, 1536));
   ...
   /**********************************************************/
   /* This procedure performs the retrieve time data routine */
   /**********************************************************/
   function retrieve_time_data return qvi_xml_type pipelined is
           ...
           from qvi_fac_part t01,
                (select t11.qft_par_code
                  ...
                  where t11.qft_das_code = var_das_code
                    ...
                    and t11.qft_tim_code = var_tim_code) t02,
   ...
   begin
      ...
      /*-*/
      /* Retrieve the existing fact time when required
      /*-*/
      if var_action = '*UPDDEF' then
         var_found := false;
         open csr_retrieve;
         ...
         close csr_retrieve;
         if var_found = false then
            qvi_gen_function.add_mesg_data('Dashboard/Fact/Time ('||var_das_code||'/'||var_fac_code||'/'||var_tim_code||') does not exist');
         end if;
         if qvi_gen_function.get_mesg_count != 0 then
            return;
         end if;
      ...
      /*-*/
      /* Pipe the fact time parts
      /*-*/
      open csr_time_part;
      loop
         fetch csr_time_part into rcd_time_part;
         if csr_time_part%notfound then
            exit;
         end if;
         ...
         pipe row(qvi_xml_object('<TIMPAR PARCDE="'||qvi_to_xml(rcd_time_part.qfp_par_code)||
                                       ...
                                       '" LODEND="'||qvi_to_xml(rcd_time_part.qsh_end_date)||'"/>'));
      end loop;
      close csr_time_part;
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         qvi_gen_function.add_mesg_data('FATAL ERROR - QVI_DAS_MAINTENANCE - RETRIEVE_TIME_DATA - ' || substr(SQLERRM, 1, 1536));

   /*-------------*/
   /* End routine */
   /*-------------*/
   end retrieve_time_data;

   /********************************************************/
   /* This procedure performs the update time data routine */
   /********************************************************/
   procedure update_time_data(par_user in varchar2) is
      ...
      obj_xml_document xmlDom.domDocument;
      obj_qvi_request xmlDom.domNode;
      obj_par_list xmlDom.domNodeList;
      obj_par_node xmlDom.domNode;
      var_action varchar2(32);
      var_confirm varchar2(32);
      var_found boolean;
      var_par_code varchar2(32);
      ...
      cursor csr_retrieve is
         select t01.*
           from qvi_fac_time t01
          where t01.qft_das_code = rcd_qvi_fac_time.qft_das_code
            and t01.qft_fac_code = rcd_qvi_fac_time.qft_fac_code
            and t01.qft_tim_code = rcd_qvi_fac_time.qft_tim_code
            for update nowait;
      rcd_retrieve csr_retrieve%rowtype;
      ...
      cursor csr_part is
         select t01.*
           from qvi_fac_part t01
          where t01.qfp_das_code = rcd_qvi_fac_time.qft_das_code
            and t01.qfp_fac_code = rcd_qvi_fac_time.qft_fac_code
            and t01.qfp_par_code = var_par_code;
      rcd_part csr_part%rowtype;
   ...
   begin
      ...
      rcd_qvi_fac_time.qft_das_code := upper(qvi_from_xml(xslProcessor.valueOf(obj_qvi_request,'@DASCDE')));
      rcd_qvi_fac_time.qft_fac_code := upper(qvi_from_xml(xslProcessor.valueOf(obj_qvi_request,'@FACCDE')));
      rcd_qvi_fac_time.qft_tim_code := upper(qvi_from_xml(xslProcessor.valueOf(obj_qvi_request,'@TIMCDE')));
      ...
      /* Validate the input
      /*-*/
      if rcd_qvi_fac_time.qft_das_code is null then
         qvi_gen_function.add_mesg_data('Dashboard code must be supplied');
      end if;
      if rcd_qvi_fac_time.qft_fac_code is null then
         qvi_gen_function.add_mesg_data('Fact code must be supplied');
      end if;
      if rcd_qvi_fac_time.qft_tim_code is null then
         qvi_gen_function.add_mesg_data('Time code must be supplied');
      end if;
      if qvi_gen_function.get_mesg_count != 0 then
         return;
      end if;
      ...
      for idx in 0..xmlDom.getLength(obj_par_list)-1 loop
         ...
         if var_add_flag = '1' then
            ...
            if csr_part%notfound then
               qvi_gen_function.add_mesg_data('Part code ('||var_par_code||') does not exist');
            end if;
            close csr_part;
         end if;
      end loop;
      ...
      exception
         when others then
            var_found := true;
            qvi_gen_function.add_mesg_data('Dashboard/Fact/Time code ('||rcd_qvi_fac_time.qft_das_code||'/'||rcd_qvi_fac_time.qft_fac_code||'/'||rcd_qvi_fac_time.qft_tim_code||') is currently locked');
      end;
      if var_found = false then
      ...
      /*-*/
      /* Update the existing fact time data (the status is updated to 1=opened)
      /*-*/
      update qvi_fac_time
         set qft_tim_status = rcd_qvi_fac_time.qft_tim_status,
             qft_upd_user = rcd_qvi_fac_time.qft_upd_user,
             qft_upd_date = rcd_qvi_fac_time.qft_upd_date
       where qft_das_code = rcd_qvi_fac_time.qft_das_code
         and qft_fac_code = rcd_qvi_fac_time.qft_fac_code
      ...
      rcd_qvi_fac_tpar.qft_tim_code := rcd_qvi_fac_time.qft_tim_code;
      obj_par_list := xslProcessor.selectNodes(xmlDom.makeNode(obj_xml_document),'/QVI_REQUEST/PARLST');
      for idx in 0..xmlDom.getLength(obj_par_list)-1 loop
         obj_par_node := xmlDom.item(obj_par_list,idx);
         var_add_flag := qvi_from_xml(xslProcessor.valueOf(obj_par_node,'@ADDFLG'));
         ...
         if var_rem_flag = '1' then
            delete from qvi_src_data
             where qsd_das_code = rcd_qvi_fac_tpar.qft_das_code
               ...
               and qsd_tim_code = rcd_qvi_fac_tpar.qft_tim_code
               and qsd_par_code = rcd_qvi_fac_tpar.qft_par_code;
            delete from qvi_src_hedr
             where qsh_das_code = rcd_qvi_fac_tpar.qft_das_code
               and qsh_fac_code = rcd_qvi_fac_tpar.qft_fac_code
               and qsh_tim_code = rcd_qvi_fac_tpar.qft_tim_code
               and qsh_par_code = rcd_qvi_fac_tpar.qft_par_code;
      ...
      /* Send the confirm message
      /*-*/
      qvi_gen_function.set_cfrm_data('Dashboard/Fact/Time code ('||rcd_qvi_fac_time.qft_das_code||'/'||rcd_qvi_fac_time.qft_fac_code||'/'||rcd_qvi_fac_time.qft_tim_code||') successfully '||var_confirm);
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         qvi_gen_function.add_mesg_data('FATAL ERROR - QVI_DAS_MAINTENANCE - UPDATE_TIME_DATA - ' || substr(SQLERRM, 1, 1536));

   /*-------------*/
   /* End routine */
   /*-------------*/
   end update_time_data;
