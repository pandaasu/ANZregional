...
/* Package Header                                                             */
/******************************************************************************/
create or replace package qv_app.qvi_fppqvi01_fact_pkg as

   /***************************************************************************/
   /* Package Definition                                                      */
   /***************************************************************************/
   /**
   Package : qvi_fppqvi01_fact_pkg
   Owner   : qv_app

   Description
   -----------
   QlikView Interfacing - Package for FPPQVI01 Interface, FPPS Actuals
   ...
   5. Any untrapped exceptions will cause the load processing to abort in the 
    ...
    5. QlikView is passed the function string, for example, qvi_app.qvi_fppqvi01_fact_retriever.get_table('DAS_CODE','FAC_CODE','TIME_CODE')
   ...
   ****************************************************************************/
   ...
   /*-*/
   function get_table(par_das_code in varchar2, par_fac_code in varchar2, par_tim_code in varchar2) return qvi_fppqvi01_fact_tab pipelined;
   
end qvi_fppqvi01_fact_pkg;
/

/******************************************************************************/
/* Package Body                                                               */
/******************************************************************************/
create or replace package body qv_app.qvi_fppqvi01_fact_pkg as
   /*-*/
   /* Private exceptions
   /*-*/
   ...
   con_package_name constant varchar2(30) := 'QVI_FPPQVI01_FACT_PKG';
   ...
   var_current_rec qvi_fppqvi01_fact_obj;
   ...
   begin
      ...
      var_current_rec := qvi_fppqvi01_fact_obj(null,null,null,null,null,null,null,null,null,null);
      ...
      lics_inbound_utility.set_csv_definition('Source Code',4);
      lics_inbound_utility.set_csv_definition('Dest Code',5);
      lics_inbound_utility.set_csv_definition('Value',6);
   ...
   begin
      ...
      if substr(var_row_data_prefix_removed,1,3) = ',,,' then
         ...
         /*-*/
         /* Parse Period header record for Period
         /*-*/
         var_period := to_number(qvi_util.get_validated_value(
            'Period',substr(replace(var_row_data_prefix_removed,','),2,2),'Number of 1 to 2 digits','^[[:digit:]]{1,2}$',var_src_error));
         /*-*/
   ...
   procedure build_fact(par_das_code in varchar2, par_fac_code in varchar2, par_tim_code in varchar2) is
      ...
      /* Local definitions
      /*-*/
      var_data qvi_fppqvi01_fact_obj;
   ...
   /*********************************************************************/
   function get_table(par_das_code in varchar2, par_fac_code in varchar2, par_tim_code in varchar2) return qvi_fppqvi01_fact_tab pipelined is
      /*-*/
      /* Local definitions
      /*-*/
