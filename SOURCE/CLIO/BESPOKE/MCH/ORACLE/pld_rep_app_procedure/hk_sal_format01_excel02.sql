...
/* Package : hk_sal_format01_excel02                            */
...
/* Package Header */
/******************/
create or replace package hk_sal_format01_excel02 as

/**DESCRIPTION**
 Slow Shipment Report - Invoice date aggregations.
...
end hk_sal_format01_excel02;
/

/****************/
/* Package Body */
/****************/
create or replace package body hk_sal_format01_excel02 as
   ...
   function main(par_sap_company_code in varchar2,
      ...
      cursor pld_sal_format0100_c01 is 
         select pld_sal_format0100.extract_date,
                pld_sal_format0100.logical_date,
                pld_sal_format0100.current_YYYYPP,
                pld_sal_format0100.current_YYYYMM,
                pld_sal_format0100.extract_status,
                pld_sal_format0100.sales_date,
                pld_sal_format0100.sales_status,
                pld_sal_format0100.prd_asofdays,
                pld_sal_format0100.prd_percent,
                pld_sal_format0100.mth_asofdays,
                pld_sal_format0100.mth_percent
         from pld_sal_format0100
         where pld_sal_format0100.sap_company_code = var_sap_company_code;
   ...
   begin
      ...
      /*-*/
      var_found := true;
      open pld_sal_format0100_c01;
      fetch pld_sal_format0100_c01 into var_extract_date,
                                        var_logical_date,
                                        var_current_YYYYPP,
                                        var_current_YYYYMM,
                                        var_extract_status,
                                        ...
                                        var_mth_asofdays,
                                        var_mth_percent;
      if pld_sal_format0100_c01%notfound then
         var_found := false;
      end if;
      close pld_sal_format0100_c01;
      if var_found = false then
         raise_application_error(-20000, 'Format control row PLD_SAL_FORMAT0100 not found');
      end if;
      ...
      /*-*/
      if var_for_type = 'PRD_BR' then
         var_wrk_string := 'Slow Shipment Report - Period - BR Forecast (HK$ Thousands) - Invoice Date - Tracking < ' || to_char(var_rpt_percent,'FM990.00') || '%';
      elsif var_for_type = 'PRD_RB' then
         var_wrk_string := 'Slow Shipment Report - Period - ROB Forecast (HK$ Thousands) - Invoice Date - Tracking < ' || to_char(var_rpt_percent,'FM990.00') || '%';
      elsif var_for_type = 'MTH_BR' then
         var_wrk_string := 'Slow Shipment Report - Month - BR Forecast (HK$ Thousands) - Invoice Date - Tracking < ' || to_char(var_rpt_percent,'FM990.00') || '%';
      elsif var_for_type = 'MTH_RB' then
         var_wrk_string := 'Slow Shipment Report - Month - ROB Forecast (HK$ Thousands) - Invoice Date - Tracking < ' || to_char(var_rpt_percent,'FM990.00') || '%';
      end if;
   ...
   procedure doDetail is
      ...
      var_day_billed_qty number;
      var_day_billed_gsv number;
      var_wrk_billed_qty number;
      var_wrk_billed_gsv number;
      var_wrk_ly_qty number;
      var_wrk_ly_gsv number;
      var_wrk_op_qty number;
      var_wrk_op_gsv number;
      var_wrk_fc_qty number;
      var_wrk_fc_gsv number;
      ...
      pld_sal_format01_c01 typCursor;
   ...
   begin
      ...
      var_dynamic_sql := 'select sum(t01.day_billed_qty),
                        ...
                        group by t02.' || var_grp_literal || '
                          having decode(sum(t01.' || var_typ_literal || '_' || var_for_literal || '_qty),0,100,
                                        round((sum(t01.' || var_typ_literal || '_billed_qty)/sum(t01.' || var_typ_literal || '_' || var_for_literal || '_qty))*100,2)) < :C
                        order by tracking_percent asc, ' || var_ord_literal;

      /*-*/
      /* Retrieve the detail rows */
      /*-*/
      open pld_sal_format01_c01 for var_dynamic_sql using var_sap_company_code, var_sap_bus_sgmnt_code, var_wrk_percent;
      loop
         fetch pld_sal_format01_c01 into var_day_billed_qty,
                                         var_day_billed_gsv,
                                         var_wrk_billed_qty,
                                         var_wrk_billed_gsv,
                                         var_wrk_ly_qty,
         ...
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_day_billed_gsv/1000,'FM9999999999999990.00000');
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_wrk_billed_qty,'FM9999999999999999999990');
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_wrk_billed_gsv/1000,'FM9999999999999990.00000');
         ...
         /* Detail plan value */
         /*-*/
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_wrk_op_gsv/1000,'FM9999999999999990.00000');
         ...
         /* Detail BR/RB value */
         /*-*/
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_wrk_fc_gsv/1000,'FM9999999999999990.00000');
         ...
         /*-*/
         var_wrk_array := var_wrk_array || chr(9) || to_char(var_wrk_ly_gsv/1000,'FM9999999999999990.00000');
...
/**************************/
create or replace public synonym hk_sal_format01_excel02 for pld_rep_app.hk_sal_format01_excel02;
grant execute on hk_sal_format01_excel02 to public;