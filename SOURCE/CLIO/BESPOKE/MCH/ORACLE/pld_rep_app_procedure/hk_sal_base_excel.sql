...
create or replace package body hk_sal_base_excel as
   ...
   type rcd_wrkp is record(par_name varchar2(30 char),
                           par_valu varchar2(256 char));
   type typ_wrkp is table of rcd_wrkp index by binary_integer;
   tbl_wrkp typ_wrkp;
   ...
   begin
      ...
      /*-*/
      var_found := false;
      for idx in 1..tbl_wrkp.count loop
         if tbl_wrkp(idx).par_name = upper(par_parameter) then
            tbl_wrkp(idx).par_valu := par_value;
            var_found := true;
         end if;
      end loop;
      if var_found = false then
         var_index := tbl_wrkp.count + 1;
         tbl_wrkp(var_index).par_name := upper(par_parameter);
         tbl_wrkp(var_index).par_valu := par_value;
      end if;
   ...
   begin
      ...
      for idx in 1..tbl_wrkp.count loop
         if tbl_wrkp(idx).par_name = upper(par_parameter) then
            tbl_wrkp(idx).par_valu := to_char(par_value);
            var_found := true;
         end if;
      end loop;
      if var_found = false then
         var_index := tbl_wrkp.count + 1;
         tbl_wrkp(var_index).par_name := upper(par_parameter);
         tbl_wrkp(var_index).par_valu := to_char(par_value);
      end if;
   ...
   begin
      ...
      for idx in 1..tbl_wrkp.count loop
         if tbl_wrkp(idx).par_name = upper(par_parameter) then
            tbl_wrkp(idx).par_valu := to_char(par_value,'yyyymmdd');
            var_found := true;
         end if;
      end loop;
      if var_found = false then
         var_index := tbl_wrkp.count + 1;
         tbl_wrkp(var_index).par_name := upper(par_parameter);
         tbl_wrkp(var_index).par_valu := to_char(par_value,'yyyymmdd');
      end if;
   ...
   begin
      ...
      /*-*/
      /* Initialise the parameter literal
      /*-*/
      var_par_literal := null;
      for idx in 1..tbl_wrkp.count loop
         var_par_literal := var_par_literal || ' and ' || replace(tbl_parameter(tbl_wrkp(idx).par_name).par_psql,':VAL',tbl_wrkp(idx).par_valu);
      end loop;
   ...
   begin
      ...
      if not(var_print_xml is null) then
