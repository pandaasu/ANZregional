...
/* Package : hk_sal_format01_extract                            */
...
/* Package Header */
/******************/
create or replace package hk_sal_format01_extract as

/**DESCRIPTION**
 Sales Extract Format 01 - Invoice date aggregations.
...
end hk_sal_format01_extract;
/

/****************/
/* Package Body */
/****************/
create or replace package body hk_sal_format01_extract as
   ...
   begin
      ...
      /**/
      delete from pld_sal_format0100 where sap_company_code = par_sap_company_code;
      delete from pld_sal_format0101 where sap_company_code = par_sap_company_code;
      commit;
   ...
   begin
      ...
      /*-*/
      insert into pld_sal_format0100
         (sap_company_code,
          extract_date,
          logical_date,
          current_YYYYPP,
   ...
   procedure exe_sales_data(par_sap_company_code in varchar2) is
      ...
      cursor sales_fact_c01 is 
         ...
         where sales_fact.sap_company_code = par_sap_company_code
           and sales_fact.billing_date >= var_logical_date
           and sales_fact.billing_date < var_logical_date + 1
   ...
   begin
      ...
      /*-*/
      select trunc(logical_date) into var_logical_date from pld_sal_format0100 where sap_company_code = par_sap_company_code;
      select current_YYYYPP into var_current_YYYYPP from pld_sal_format0100 where sap_company_code = par_sap_company_code;
      select current_YYYYMM into var_current_YYYYMM from pld_sal_format0100 where sap_company_code = par_sap_company_code;
   ...
   procedure exe_create_material(par_sap_company_code in varchar2,
      ...
      /* Cursor definitions */
      /*-*/
      cursor pld_sal_format0101_c01 is 
         select 'x'
         from pld_sal_format0101
         where pld_sal_format0101.sap_company_code = par_sap_company_code
           and pld_sal_format0101.sap_material_code = par_sap_material_code;
   ...
   begin    
      ...
      /*-*/
      open pld_sal_format0101_c01;
      fetch pld_sal_format0101_c01 into var_work;
      if pld_sal_format0101_c01%notfound then
         insert into pld_sal_format0101
            (sap_company_code,
             sap_material_code,
             day_billed_qty,
             day_billed_gsv,
...
/**************************/
create or replace public synonym hk_sal_format01_extract for pld_rep_app.hk_sal_format01_extract;
grant execute on hk_sal_format01_extract to public;