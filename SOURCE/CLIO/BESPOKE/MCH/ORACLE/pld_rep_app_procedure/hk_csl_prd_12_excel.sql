...
/* Package Header */
/******************/
create or replace package hk_csl_prd_12_excel as
...
end hk_csl_prd_12_excel;
/

/****************/
/* Package Body */
/****************/
create or replace package body hk_csl_prd_12_excel as

   /*-*/
   /* Private package variables
   /*-*/
   rcd_pld_csl_prd_1200 pld_csl_prd_1200%rowtype;
   ...
   procedure start_report(par_company_code in varchar2, par_material_type in varchar2) is
      ...
      /* Cursor definitions
      /*-*/
      cursor csr_pld_csl_prd_1200 is 
         select *
         from pld_csl_prd_1200 t01
   ...
   begin
      ...
      /*-*/
      var_found := true;
      open csr_pld_csl_prd_1200;
      fetch csr_pld_csl_prd_1200 into rcd_pld_csl_prd_1200;
      if csr_pld_csl_prd_1200%notfound then
         var_found := false;
      end if;
      close csr_pld_csl_prd_1200;
      if var_found = false then
         raise_application_error(-20000, 'Extract control row PLD_CSL_PRD_1200 not found');
      end if;
   ...
   begin
      ...
      hk_sal_base_excel.start_sheet(par_htxt1,
                                    ...
                                    'Period End: ' || substr(to_char(rcd_pld_csl_prd_1200.current_yyyypp,'fm000000'),1,4) || '/' || substr(to_char(rcd_pld_csl_prd_1200.current_yyyypp,'fm000000'),5,6));
   ...
   begin
      ...
      /*-*/
      hk_sal_base_excel.tbl_main_name := 'pld_csl_prd_1201 t01,material_dim t02,std_hier t03';
   ...
   begin
      ...
      /* Initialise the customer tables variables
      /*-*/
      hk_sal_base_excel.tbl_main_name := 'pld_csl_prd_1202 t01,std_hier t03';
...
/**************************/
create or replace public synonym hk_csl_prd_12_excel for pld_rep_app.hk_csl_prd_12_excel;
grant execute on hk_csl_prd_12_excel to public;