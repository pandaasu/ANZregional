...
/* Package : hk_inv_format02_excel05                            */
...
/* Package Header */
/******************/
create or replace package hk_inv_format02_excel05 as
...
end hk_inv_format02_excel05;
/

/****************/
/* Package Body */
/****************/
create or replace package body hk_inv_format02_excel05 as
   ...
   begin
      ...
      if var_val_type = 'QTY' then
         ...
         var_wrk_string := 'Finished Goods Ageing Inventory Report - (HK$ Thousands)';
      end if;
               ...
               for idx in 1..tblWeek.count loop
                  ...
                  if var_val_type = 'QTY' then
                     ...
                     if tblWeek(idx).inv_val <> 0 then
                        var_wrk_string := var_wrk_string || to_char(tblWeek(idx).inv_val/1000,'FM999999990.00000');
                     end if;
                  end if;
               end loop;
...
/**************************/
create or replace public synonym hk_inv_format02_excel05 for pld_rep_app.hk_inv_format02_excel05;
grant execute on hk_inv_format02_excel05 to public;