...
/* Package Header */
/******************/
create or replace package dw_fcst_extract09 as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : dw_fcst_extract09
    Owner   : dw_app

    Description
    -----------
    Dimensional Data Store - Forecast Extract - BW Forecast Cluster Extract

    This package contains the BW forecast cluster extract procedure.
...
end dw_fcst_extract09;
/

/****************/
/* Package Body */
/****************/
create or replace package body dw_fcst_extract09 as
   ...
   begin
      ...
      loop
         ...
         /*-*/
         /* Retrieve the forecast load detail - Domestic load
         /*-*/
         ...
         loop
            ...
            if rcd_fcst_load_detail_01.fcst_yyyypp >= rcd_fcst_extract_header.extract_version then
               ...
               var_output := var_output || chr(9) || rcd_fcst_load_detail_01.dmnd_group;
               var_output := var_output || chr(9) || null;
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT09 - EXPORT - ' || substr(SQLERRM, 1, 1024));
   ...
   begin
      ...
      pipe row('<td align=center style="FONT-FAMILY:Arial,Verdana,Tahoma,sans-serif;FONT-SIZE:9pt;FONT-WEIGHT:bold;BACKGROUND-COLOR:#CCFFCC;COLOR:#000000;">Cluster Code</td>');
      ...
      loop
         ...
         /*-*/
         /* Retrieve the forecast load rows - Domestic load
         /*-*/
         var_dmnd_group := null;
         ...
         loop
            ...
            if var_dmnd_group is null or
               var_dmnd_group != rcd_fcst_load_01.dmnd_group or
               ...
               /* Initialise the row
               /*-*/
               var_dmnd_group := rcd_fcst_load_01.dmnd_group;
               var_material_code := rcd_fcst_load_01.material_code;
               var_plant_code := rcd_fcst_load_01.plant_code;
               var_dmnd_group_desc := '('||rcd_fcst_load_01.dmnd_group||')';
               if not(rcd_fcst_load_01.customer_desc_zh is null) then
                  var_dmnd_group_desc := var_dmnd_group_desc||' '||rcd_fcst_load_01.customer_desc_zh;
               elsif not(rcd_fcst_load_01.customer_desc_en is null) then
                  var_dmnd_group_desc := var_dmnd_group_desc||' '||rcd_fcst_load_01.customer_desc_en;
               else
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT09 - REPORT - ' || substr(SQLERRM, 1, 1024));
...
/**************************/
create or replace public synonym dw_fcst_extract09 for dw_app.dw_fcst_extract09;
grant execute on dw_fcst_extract09 to public;
