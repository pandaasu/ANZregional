...
/* Package Header */
/******************/
create or replace package dw_fcst_extract04 as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : dw_fcst_extract04
    Owner   : dw_app

    Description
    -----------
    Dimensional Data Store - Forecast Extract - BW Replan Extract

    This package contains the BW replan extract procedure.
...
end dw_fcst_extract04;
/

/****************/
/* Package Body */
/****************/
create or replace package body dw_fcst_extract04 as
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT04 - REPORT - ' || substr(SQLERRM, 1, 1024));
...
/**************************/
create or replace public synonym dw_fcst_extract04 for dw_app.dw_fcst_extract04;
grant execute on dw_fcst_extract04 to public;
