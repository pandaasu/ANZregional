...
/* Package Header */
/******************/
create or replace package dw_fcst_extract03 as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : dw_fcst_extract03
    Owner   : dw_app

    Description
    -----------
    Dimensional Data Store - Forecast Extract - BW Plan Extract

    This package contains the BW plan extract procedure.
...
end dw_fcst_extract03;
/

/****************/
/* Package Body */
/****************/
create or replace package body dw_fcst_extract03 as
      ...
      loop
         ...
         if upper(rcd_fcst_load_header.load_type_channel) != '*AFFILIATE' then
            ...
            loop
               ...
               /*-*/
               var_type := 'CN_YEE_HST';
               var_version := 'OP_'||substr(to_char(rcd_fcst_extract_header.extract_version,'fm0000'),1,4);
               var_planner := rcd_fcst_extract_header.extract_planner;
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT03 - EXPORT - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT03 - REPORT - ' || substr(SQLERRM, 1, 1024));
...
/**************************/
create or replace public synonym dw_fcst_extract03 for dw_app.dw_fcst_extract03;
grant execute on dw_fcst_extract03 to public;
