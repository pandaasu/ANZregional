...
/* Package Header */
/******************/
create or replace package dw_fcst_extract02 as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : dw_fcst_extract02
    Owner   : dw_app

    Description
    -----------
    Dimensional Data Store - Forecast Extract - BW Draft Extract

    This package contains the BW draft extract procedure.

    YYYY/MM   Author         Description
    -------   ------         -----------
    2008/03   Steve Gregan   Created
    2008/05   Steve Gregan   Changed customer description logic
    2008/08   Steve Gregan   Changed draft period selection to include all periods
...
end dw_fcst_extract02;
/

/****************/
/* Package Body */
/****************/
create or replace package body dw_fcst_extract02 as
   ...
   begin
      ...
      /*-*/
      /* Retrieve the forecast extract loads (CN_FCS_DRF output)
      /*-*/
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT02 - EXPORT - ' || substr(SQLERRM, 1, 1024));
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - DW_FCST_EXTRACT02 - REPORT - ' || substr(SQLERRM, 1, 1024));
...
/**************************/
create or replace public synonym dw_fcst_extract02 for dw_app.dw_fcst_extract02;
grant execute on dw_fcst_extract02 to public;
