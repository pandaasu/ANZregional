...
sub ProcessInsertAccept()
   ...
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_CustBank").Value) & "',"
   strStatement = strStatement & "'" & objSecurity.FixString(objForm.Fields("DTA_TaxCode").Value) & "'"
...
sub ProcessUpdateLoad()
   ...
   strQuery = strQuery & " from china_tax_customer t01"
   strQuery = strQuery & " where t01.cust_code = '" & objForm.Fields("DTA_CustCode").Value & "'"
   strReturn = objSelection.Execute("LIST", strQuery, lngSize)
   if strReturn <> "*OK" then
      strError = FormatError(strReturn)
      strMode = "SELECT"
