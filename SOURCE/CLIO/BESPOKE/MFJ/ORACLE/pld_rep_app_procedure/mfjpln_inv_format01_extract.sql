...
/* Package : mfjpln_inv_format01_extract                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_inv_format01_extract as
...
end mfjpln_inv_format01_extract;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_inv_format01_extract as
   ...
   procedure exe_inventory_data;
   procedure exe_calculate_cover;
   ...
   begin
      ...
      /**/
      mfjpln_truncate.truncate_table('pld_inv_format0100');
      mfjpln_truncate.truncate_table('pld_inv_format0101');
      mfjpln_truncate.truncate_table('pld_inv_format0102');
      mfjpln_truncate.truncate_table('pld_inv_format0103');
      mfjpln_truncate.truncate_table('pld_inv_format0104');
      commit;
   ...
   procedure exe_sales_data is
      ...
      cursor sales_period_01_fact_c01 is 
         select max(sales_period_01_fact.sap_company_code),
                max(sales_period_01_fact.sap_material_code),
                sum(nvl(sales_period_01_fact.base_uom_billed_qty, 0))
         from sales_period_01_fact
         where sales_period_01_fact.billing_YYYYPP = var_current_YYYYPP
      ...
      cursor sales_month_01_fact_c01 is 
         select max(sales_month_01_fact.sap_company_code),
                max(sales_month_01_fact.sap_material_code),
                sum(nvl(sales_month_01_fact.base_uom_billed_qty, 0))
         from sales_month_01_fact
         where sales_month_01_fact.billing_YYYYMM = var_current_YYYYMM
   ...
   begin
      ...
      /*-*/
      select current_YYYYPP into var_current_YYYYPP from pld_inv_format0100;
      select current_YYYYMM into var_current_YYYYMM from pld_inv_format0100;
   ...
   procedure exe_forecast_data is
      ...
      var_br_qty number(15,0);
      var_le_qty number(15,0);
   ...
   /******************************************************/
   procedure exe_inventory_data is
      ...
      var_inv_level number(15,3);
      var_sap_stock_type_code varchar2(2 char);
      var_material_batch_desc varchar2(8 char);
      var_sap_bus_sgmnt_code varchar2(10 char);
      var_price_unit number(5,0);
      var_std_price number(10,2);
      var_inv_tot_value number(22,0);
      var_inv_tot_qty number(22,3);
      var_inv_int_qty number(22,3);
      var_inv_war_value number(22,0);
      var_inv_war_qty number(22,3);
      var_inv_hld_qty number(22,3);
      var_inv_unk_qty number(22,3);
      var_inv_age_qty number(22,3);
      var_inv_sal_qty number(22,3);
      var_found boolean;
      ...
      cursor inv_blnc_hdr_c01 is 
         ...
         where inv_blnc_hdr.sap_plant_code = plant_dim.sap_plant_code(+)
           ...
           and plant_dim.sap_plant_code <> 'JP01'
   ...
   begin
      ...
      /*-*/
      select inventory_date into var_blnc_date from pld_inv_format0100;
      select prd_percent into var_prd_percent from pld_inv_format0100;
      select mth_percent into var_mth_percent from pld_inv_format0100;
      ...
      /* **note** this does not include warehouse JP01 (intransit) */
      /*-*/
      tblWarehouse.Delete;
      ...
      loop
         ...
         /* **note** this is hard-coded to JP01 */
         /*-*/
         if var_sap_plant_code = 'JP01' then
   ...
   /*******************************************************/
   procedure exe_calculate_cover is
      ...
      var_inv_tot_qty number(22,3);
      var_inv_sal_qty number(22,3);
      var_prd_fut_br_qty number(22,0);
      var_prd_fut_le_qty number(22,0);
      var_mth_fut_br_qty number(22,0);
      var_mth_fut_le_qty number(22,0);
      var_inv_prd_br_ons_cover number(9,2);
      var_inv_prd_le_ons_cover number(9,2);
      var_inv_mth_br_ons_cover number(9,2);
      var_inv_mth_le_ons_cover number(9,2);
      var_inv_prd_br_sal_cover number(9,2);
      var_inv_prd_le_sal_cover number(9,2);
      var_inv_mth_br_sal_cover number(9,2);
      var_inv_mth_le_sal_cover number(9,2);
      var_br_ons_qty number(22,3);
      var_br_sal_qty number(22,3);
      var_le_ons_qty number(22,3);
      var_le_sal_qty number(22,3);
      var_br_ons_start boolean;
      var_br_sal_start boolean;
      var_le_ons_start boolean;
      var_le_sal_start boolean;
      ...
      cursor pld_inv_format0101_c01 is 
         ...
         where pld_inv_format0101.inv_tot_qty <> 0 or
   ...
   begin
      ...
      loop
         ...
         var_le_ons_start := false;
         var_le_sal_start := false;
         var_br_ons_qty := var_inv_tot_qty;
         var_br_sal_qty := var_inv_sal_qty;
         var_le_ons_qty := var_inv_tot_qty;
         var_le_sal_qty := var_inv_sal_qty;
         ...
         loop
            ...
            and var_le_ons_qty <= 0
            and var_le_sal_qty <= 0 then
               exit;
            end if;
            ...
            if var_YYYYPP > var_current_YYYYPP then
               ...
               var_prd_fut_le_qty := var_prd_fut_le_qty + var_le_qty;
            end if;
            ...
            /*-*/
            if var_le_ons_qty > 0 then
               if var_le_qty <> 0 then
                  var_for_pct := 1;
                  if var_YYYYPP = var_current_YYYYPP then
                     var_for_pct := 1 - (var_prd_percent / 100);
                  end if;
                  var_for_qty := var_le_qty * var_for_pct;
                  var_wrk_pct := 1;
                  if var_le_ons_qty - var_for_qty < 0 then
                     var_wrk_pct := var_le_ons_qty / var_for_qty;
                  end if;
                  var_le_ons_qty := var_le_ons_qty - var_for_qty;
                  var_inv_prd_le_ons_cover := var_inv_prd_le_ons_cover + round((4 * var_for_pct) * var_wrk_pct, 2);
                  var_le_ons_start := true;
               else
                  if var_le_ons_start = true then
                     var_inv_prd_le_ons_cover := var_inv_prd_le_ons_cover + 4;
                  end if;
               end if;
            end if;
            ...
            if var_le_sal_qty > 0 then
               if var_le_qty <> 0 then
                  ...
                  var_for_qty := var_le_qty * var_for_pct;
                  var_wrk_pct := 1;
                  if var_le_sal_qty - var_for_qty < 0 then
                     var_wrk_pct := var_le_sal_qty / var_for_qty;
                  end if;
                  var_le_sal_qty := var_le_sal_qty - var_for_qty;
                  var_inv_prd_le_sal_cover := var_inv_prd_le_sal_cover + round((4 * var_for_pct) * var_wrk_pct, 2);
                  var_le_sal_start := true;
               else
                  if var_le_sal_start = true then
                     var_inv_prd_le_sal_cover := var_inv_prd_le_sal_cover + 4;
                  end if;
               end if;
            end if;
         ...
         loop
            ...
            if var_YYYYMM > var_current_YYYYMM then
               ...
               var_mth_fut_le_qty := var_mth_fut_le_qty + var_le_qty;
            end if;
            ...
            if var_le_ons_qty > 0 then
               if var_le_qty <> 0 then
                  ...
                  var_le_ons_qty := var_le_ons_qty - var_for_qty;
                  var_inv_mth_le_ons_cover := var_inv_mth_le_ons_cover + round((4 * var_for_pct) * var_wrk_pct, 2);
                  var_le_ons_start := true;
               else
                  if var_le_ons_start = true then
                     var_inv_mth_le_ons_cover := var_inv_mth_le_ons_cover + 4;
                  end if;
               end if;
            end if;
            ...
            if var_le_sal_qty > 0 then
               if var_le_qty <> 0 then
                  ...
                  var_le_sal_qty := var_le_sal_qty - var_for_qty;
                  var_inv_mth_le_sal_cover := var_inv_mth_le_sal_cover + round((4 * var_for_pct) * var_wrk_pct, 2);
                  var_le_sal_start := true;
               else
                  if var_le_sal_start = true then
                     var_inv_mth_le_sal_cover := var_inv_mth_le_sal_cover + 4;
                  end if;
               end if;
            end if;
         ...
         update pld_inv_format0101
            set prd_fut_br_qty = var_prd_fut_br_qty,
                prd_fut_le_qty = var_prd_fut_le_qty,
                mth_fut_br_qty = var_mth_fut_br_qty,
                mth_fut_le_qty = var_mth_fut_le_qty,
                inv_prd_br_ons_cover = var_inv_prd_br_ons_cover,
                inv_prd_le_ons_cover = var_inv_prd_le_ons_cover,
                inv_mth_br_ons_cover = var_inv_mth_br_ons_cover,
                inv_mth_le_ons_cover = var_inv_mth_le_ons_cover,
                inv_prd_br_sal_cover = var_inv_prd_br_sal_cover,
                inv_prd_le_sal_cover = var_inv_prd_le_sal_cover,
                inv_mth_br_sal_cover = var_inv_mth_br_sal_cover,
                inv_mth_le_sal_cover = var_inv_mth_le_sal_cover
            where sap_company_code = var_sap_company_code
              and sap_material_code = var_sap_material_code;
      ...
      if pld_inv_format0101_c01%notfound then
         insert into pld_inv_format0101
            (sap_company_code,
             ...
             inv_prd_le_ons_cover,
             inv_mth_br_ons_cover,
             inv_mth_le_ons_cover,
             inv_prd_br_sal_cover,
             inv_prd_le_sal_cover,
             inv_mth_br_sal_cover,
             inv_mth_le_sal_cover)
...
/**************************/
create or replace public synonym mfjpln_inv_format01_extract for pld_rep_app.mfjpln_inv_format01_extract;
grant execute on mfjpln_inv_format01_extract to public;