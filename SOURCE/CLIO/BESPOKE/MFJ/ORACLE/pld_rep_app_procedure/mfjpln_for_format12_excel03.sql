...
/* Package : mfjpln_for_format12_excel03                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_for_format12_excel03 as

/**DESCRIPTION**
 Forecast Accuracy Product Category Total Report - SAP billing date aggregations.
...
end mfjpln_for_format12_excel03;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_for_format12_excel03 as
   ...
   begin
      ...
      if mars_date_c01%notfound then
         raise_application_error(-20000, 'No mars date found for extract date from pld_for_format1200');
      end if;
      close mars_date_c01;
      ...
      /*-*/
      var_wrk_string := 'Forecast Accuracy Category Summary Report - Delivery Date';
            ...
            if tblForecast(idx).for_category = '0' then
               ...
               if var_wrk_accuracy is not null and var_wrk_category <> 'X' then
                  if var_wrk_accuracy > var_acc_range01 then
                     if var_for_accuracy_output = 'Y' then
                        doAccumulate(var_wrk_category, tblForecast(idx).for_yyyynn, 1, 0, 0);
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 1, 0, 0);
                     else
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 1, 0, 0);
                     end if;
                  elsif var_wrk_accuracy < var_acc_range03 then
                     if var_for_accuracy_output = 'Y' then
                        doAccumulate(var_wrk_category, tblForecast(idx).for_yyyynn, 0, 0, 1);
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 0, 0, 1);
                     else
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 0, 0, 1);
                     end if;
                  else
                     if var_for_accuracy_output = 'Y' then
                        doAccumulate(var_wrk_category, tblForecast(idx).for_yyyynn, 0, 1, 0);
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 0, 1, 0);
                     else
                        doAccumulate(var_for_accuracy_output, tblForecast(idx).for_yyyynn, 0, 1, 0);
                     end if;
                  end if;
...
/**************************/
create or replace public synonym mfjpln_for_format12_excel03 for pld_rep_app.mfjpln_for_format12_excel03;
grant execute on mfjpln_for_format12_excel03 to public;