...
/* Package : mfjpln_for_format13_excel04                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_for_format13_excel04 as
 ...
 **NOTES**
 ...
 200603 - removed above line
 200711 - changed forecast accuracy algorithm - sales becomes the denominator
...
end mfjpln_for_format13_excel04;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_for_format13_excel04 as
   ...
   begin
      ...
      loop
         ...
         for idx in 1..tblForecast.count loop
            ...
            if var_wrk_count = 1 then
               if var_w201_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w101_val-var_w201_val)/var_w201_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 2 then
               if var_w202_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w102_val-var_w202_val)/var_w202_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 3 then
               if var_w203_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w103_val-var_w203_val)/var_w203_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 4 then
               if var_w204_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w104_val-var_w204_val)/var_w204_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 5 then
               if var_w205_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w105_val-var_w205_val)/var_w205_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 6 then
               if var_w206_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w106_val-var_w206_val)/var_w206_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 7 then
               if var_w207_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w107_val-var_w207_val)/var_w207_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 8 then
               if var_w208_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w108_val-var_w208_val)/var_w208_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 9 then
               if var_w209_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w109_val-var_w209_val)/var_w209_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 10 then
               if var_w210_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w110_val-var_w210_val)/var_w210_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 11 then
               if var_w211_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w111_val-var_w211_val)/var_w211_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 12 then
               if var_w212_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w112_val-var_w212_val)/var_w212_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 13 then
               if var_w213_val <> 0 then
                  var_wrk_accuracy := round((1-abs(var_w113_val-var_w213_val)/var_w213_val)*100,2);
               else
                  var_wrk_accuracy := null;
               end if;
            end if;
            if var_wrk_accuracy is not null then
               if var_wrk_accuracy > var_acc_range01 then
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, var_wrk_accuracy, 1, 0, 0);
               elsif var_wrk_accuracy < var_acc_range03 then
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, var_wrk_accuracy, 0, 0, 1);
               else
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, var_wrk_accuracy, 0, 1, 0);
               end if;
            end if;
         end loop;
      ...
      loop
         ...
         var_wrk_array := var_wrk_array || chr(9) || 'Within Range (item)';
         var_wrk_count := 0;
         for idx in 1..tblForecast.count loop
            var_wrk_count := var_wrk_count + 1;
            if var_wrk_count = 1 then
...
/**************************/
create or replace public synonym mfjpln_for_format13_excel04 for pld_rep_app.mfjpln_for_format13_excel04;
grant execute on mfjpln_for_format13_excel04 to public;