...
/* Package : mfjpln_for_format11_extract                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_for_format11_extract as
...
end mfjpln_for_format11_extract;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_for_format11_extract as
   ...
   begin
      ...
      /**/
      mfjpln_truncate.truncate_table('pld_for_format1100');
      mfjpln_truncate.truncate_table('pld_for_format1101');
      mfjpln_truncate.truncate_table('pld_for_format1102');
      mfjpln_truncate.truncate_table('pld_for_format1103');
      mfjpln_truncate.truncate_table('pld_for_format1104');
      commit;
   ...
   begin
      ...
      /*-*/
      insert into pld_for_format1100
         (extract_date)
         values(sysdate);
   ...
   begin
      ...
      loop
         ...
         insert into pld_for_format1101
            (sap_material_code,
             planning_status,
             planning_type,
             planning_cat_old,
      ...
      loop
         ...
         insert into pld_for_format1102
            (sap_material_code,
             channel_id,
             ld_time)
            values(var_sap_material_code,
   ...
   begin
      ...
      loop
         ...
         if var_asof_yyyynn = var_wrk_yyyynn then
            insert into pld_for_format1103
               (sap_material_code,
                asof_yyyypp,
                fcst_yyyypp,
                case_qty)
      ...
      loop
         ...
         /*-*/
         if var_asof_yyyynn = var_wrk_yyyynn then
            insert into pld_for_format1104
               (sap_material_code,
                asof_yyyymm,
                fcst_yyyymm,
                case_qty)
   ...
   procedure extractSales is
      ...
      cursor sales_period_03_fact_c01 is 
         select t01.sap_material_code,
                t02.sap_billing_yyyypp,
                sum(nvl(t02.base_uom_billed_qty, 0))
           from pld_for_format1101 t01,
                sales_period_03_fact t02
          where t01.sap_material_code = t02.sap_material_code
            and t02.sap_billing_yyyypp >= var_str_yyyynn
            and t02.sap_billing_yyyypp <= var_end_yyyynn
      ...
      cursor sales_month_04_fact_c01 is 
         select t01.sap_material_code,
                t02.sap_billing_yyyymm,
                sum(nvl(t02.base_uom_billed_qty, 0))
           from pld_for_format1101 t01,
                sales_month_04_fact t02
          where t01.sap_material_code = t02.sap_material_code
            and t02.sap_billing_yyyymm >= var_str_yyyynn
            and t02.sap_billing_yyyymm <= var_end_yyyynn
            and t02.base_uom_billed_qty is not null
            and t02.base_uom_billed_qty <> 0
          group by t01.sap_material_code,
                   t02.sap_billing_yyyymm;
   ...
   begin
      ...
      /*-*/
      select min(fcst_yyyypp) into var_str_yyyynn from pld_for_format1103;
      select max(fcst_yyyypp) into var_end_yyyynn from pld_for_format1103;
      ...
      /*-*/
      select min(fcst_yyyymm) into var_str_yyyynn from pld_for_format1104;
      select max(fcst_yyyymm) into var_end_yyyynn from pld_for_format1104;
...
/**************************/
create or replace public synonym mfjpln_for_format11_extract for pld_rep_app.mfjpln_for_format11_extract;
grant execute on mfjpln_for_format11_extract to public;