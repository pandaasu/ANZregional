...
/* Package : mfjpln_sal_format13_excel02                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_sal_format13_excel02 as

/**DESCRIPTION**
 Business Segment Full Year - SAP billing date aggregations.
 ...
 **NOTES**
 ...
 3. A different sort is used for each business segment (hard-coded)
...
end mfjpln_sal_format13_excel02;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_sal_format13_excel02 as
   ...
   function main(par_sap_company_code in varchar2,
      ...
      cursor pld_sal_format1300_c01 is 
         select pld_sal_format1300.extract_date,
                pld_sal_format1300.logical_date,
                pld_sal_format1300.current_YYYYPP,
                pld_sal_format1300.current_YYYYMM,
                pld_sal_format1300.extract_status,
                pld_sal_format1300.sales_date,
                pld_sal_format1300.sales_status
         from pld_sal_format1300;
   ...
   begin
      ...
      /*-*/
      var_found := true;
      open pld_sal_format1300_c01;
      fetch pld_sal_format1300_c01 into var_extract_date,
                                        var_logical_date,
                                        var_current_YYYYPP,
                                        var_current_YYYYMM,
                                        var_extract_status,
                                        var_sales_date,
                                        var_sales_status;
      if pld_sal_format1300_c01%notfound then
         var_found := false;
      end if;
      close pld_sal_format1300_c01;
      if var_found = false then
         raise_application_error(-20000, 'Format control row PLD_SAL_FORMAT1300 not found');
      end if;
      ...
      /*-*/
      var_wrk_string := 'Business Segment Full Year';
   ...
   procedure doDetail is
      ...
      pld_sal_format13_c01 typCursor;
   ...
   begin
      ...
      /*-*/
      if var_for_type = 'PRD' then
         var_dynamic_sql := 'select t01.tot_ty_val,
                                    t01.tot_ly_val,
                                    t01.tot_op_val,
                                    t01.w01_val,
                                    t01.w02_val,
                               ...
                               from (select t11.sap_material_code sap_material_code,
                                       ...
                                       from pld_sal_format1301 t11, (select t22.sap_company_code sap_company_code,
         ...
         var_dynamic_sql := 'select t01.tot_ty_val,
                               ...
                               from (select t11.sap_material_code sap_material_code,
                                            ...
                                            nvl(t12.w11_val,0) w11_val,
                                            nvl(t12.w12_val,0) w12_val
                                       from pld_sal_format1302 t11, (select t22.sap_company_code sap_company_code,
                                                                            t22.sap_material_code sap_material_code,
      ...
      /*-*/
      open pld_sal_format13_c01 for var_dynamic_sql using var_sap_company_code, var_sap_company_code, var_sap_bus_sgmnt_code;
      loop
         if var_for_type = 'PRD' then
            fetch pld_sal_format13_c01 into var_tot_ty_val,
                                            var_tot_ly_val,
                                            var_tot_op_val,
                                            var_w01_val,
                                            var_w02_val,
         ...
         if pld_sal_format13_c01%notfound then
            exit;
         end if;
...
/**************************/
create or replace public synonym mfjpln_sal_format13_excel02 for pld_rep_app.mfjpln_sal_format13_excel02;
grant execute on mfjpln_sal_format13_excel02 to public;