...
/* Package : mfjpln_for_format12_excel04                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_for_format12_excel04 as

/**DESCRIPTION**
 Forecast Accuracy Source Unit Report - SAP billing date aggregations.
...
end mfjpln_for_format12_excel04;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_for_format12_excel04 as
   ...
   type rcdForecast is record(for_yyyynn varchar2(6 char), aso_yyyynn varchar2(6 char), column_id varchar2(2 char));
   ...
   begin
      ...
      /*-*/
      var_wrk_string := 'Forecast Accuracy Source Unit Report - Delivery Date';
      ...
      /*-*/
      var_row_head := var_row_head + 1;
      xlxml_object.SetRange('A' || to_char(var_row_head,'FM999999990') || ':A' || to_char(var_row_head,'FM999999990'), null, xlxml_object.GetHeadingType(7), -1, 0, false, 'Source Unit');
      xlxml_object.SetRange('B' || to_char(var_row_head,'FM999999990') || ':B' || to_char(var_row_head,'FM999999990'), null, xlxml_object.GetHeadingType(7), -1, 0, false, null);
   ...
   begin
         ...
         for idx in 1..tblForecast.count loop
            ...
            if var_wrk_count = 1 then
               if var_w101_val <> 0 then
                  var_wrk_accuracy := round((var_w201_val / var_w101_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 2 then
               if var_w102_val <> 0 then
                  var_wrk_accuracy := round((var_w202_val / var_w102_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 3 then
               if var_w103_val <> 0 then
                  var_wrk_accuracy := round((var_w203_val / var_w103_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 4 then
               if var_w104_val <> 0 then
                  var_wrk_accuracy := round((var_w204_val / var_w104_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 5 then
               if var_w105_val <> 0 then
                  var_wrk_accuracy := round((var_w205_val / var_w105_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 6 then
               if var_w106_val <> 0 then
                  var_wrk_accuracy := round((var_w206_val / var_w106_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 7 then
               if var_w107_val <> 0 then
                  var_wrk_accuracy := round((var_w207_val / var_w107_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 8 then
               if var_w108_val <> 0 then
                  var_wrk_accuracy := round((var_w208_val / var_w108_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 9 then
               if var_w109_val <> 0 then
                  var_wrk_accuracy := round((var_w209_val / var_w109_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 10 then
               if var_w110_val <> 0 then
                  var_wrk_accuracy := round((var_w210_val / var_w110_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 11 then
               if var_w111_val <> 0 then
                  var_wrk_accuracy := round((var_w211_val / var_w111_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 12 then
               if var_w112_val <> 0 then
                  var_wrk_accuracy := round((var_w212_val / var_w112_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            elsif var_wrk_count = 13 then
               if var_w113_val <> 0 then
                  var_wrk_accuracy := round((var_w213_val / var_w113_val) * 100, 2);
               else
                  var_wrk_accuracy := null;
               end if;
            end if;
            if var_wrk_accuracy is not null then
               if var_wrk_accuracy > var_acc_range01 then
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, 1, 0, 0);
               elsif var_wrk_accuracy < var_acc_range03 then
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, 0, 0, 1);
               else
                  doAccumulate(var_wrk_src_unit, tblForecast(idx).for_yyyynn, 0, 1, 0);
               end if;
            end if;
         end loop;
      ...
      if var_for_type = 'PRD' then
         ...
         /*-*/
         var_dynamic_sql := 'select t01.summary' || var_for_literal01 || var_for_literal02 || var_for_literal03 || '
                               from pld_for_work0201 t01
         ...
         for idx in 1..tblForecast.count loop
            ...
            if var_wrk_count = 1 then
               if (var_w101_val + var_w201_val + var_w301_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w201_val / (var_w101_val + var_w201_val + var_w301_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w201_val / (var_w101_val + var_w201_val + var_w301_val)) * 100, 0),'FM9999990.00');
               else
               ...
               if (var_w102_val + var_w202_val + var_w302_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w202_val / (var_w102_val + var_w202_val + var_w302_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w202_val / (var_w102_val + var_w202_val + var_w302_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w103_val + var_w203_val + var_w303_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w203_val / (var_w103_val + var_w203_val + var_w303_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w203_val / (var_w103_val + var_w203_val + var_w303_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w104_val + var_w204_val + var_w304_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w204_val / (var_w104_val + var_w204_val + var_w304_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w204_val / (var_w104_val + var_w204_val + var_w304_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w105_val + var_w205_val + var_w305_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w205_val / (var_w105_val + var_w205_val + var_w305_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w205_val / (var_w105_val + var_w205_val + var_w305_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w106_val + var_w206_val + var_w306_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w206_val / (var_w106_val + var_w206_val + var_w306_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w206_val / (var_w106_val + var_w206_val + var_w306_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w107_val + var_w207_val + var_w307_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w207_val / (var_w107_val + var_w207_val + var_w307_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w207_val / (var_w107_val + var_w207_val + var_w307_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w108_val + var_w208_val + var_w308_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w208_val / (var_w108_val + var_w208_val + var_w308_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w208_val / (var_w108_val + var_w208_val + var_w308_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w109_val + var_w209_val + var_w309_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w209_val / (var_w109_val + var_w209_val + var_w309_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w209_val / (var_w109_val + var_w209_val + var_w309_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w110_val + var_w210_val + var_w310_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w210_val / (var_w110_val + var_w210_val + var_w310_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w210_val / (var_w110_val + var_w210_val + var_w310_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w111_val + var_w211_val + var_w311_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w211_val / (var_w111_val + var_w211_val + var_w311_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w211_val / (var_w111_val + var_w211_val + var_w311_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w112_val + var_w212_val + var_w312_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w212_val / (var_w112_val + var_w212_val + var_w312_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w212_val / (var_w112_val + var_w212_val + var_w312_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
               ...
               if (var_w113_val + var_w213_val + var_w313_val) <> 0 then
                  var_wrk_array := var_wrk_array || chr(9) || to_char(round((var_w213_val / (var_w113_val + var_w213_val + var_w313_val)) * 100, 0),'FM9999990.00');
                  var_wrk_string := var_wrk_string || to_char(round((var_w213_val / (var_w113_val + var_w213_val + var_w313_val)) * 100, 0),'FM9999990.00');
               else
                  var_wrk_array := var_wrk_array || chr(9) || 'NF';
                  var_wrk_string := var_wrk_string || '0';
               end if;
   ...
   begin
      ...
      /*-*/
      update pld_for_work0201
         set fcst_count01 = fcst_count01 + par_count01,
      ...
      if sql%notfound then
         insert into pld_for_work0201
            (summary, fcst_yyyynn, fcst_count01, fcst_count02, fcst_count03)
            values(par_summary, par_fcst_yyyynn, par_count01, par_count02, par_count03);
      end if;
...
/**************************/
create or replace public synonym mfjpln_for_format12_excel04 for pld_rep_app.mfjpln_for_format12_excel04;
grant execute on mfjpln_for_format12_excel04 to public;