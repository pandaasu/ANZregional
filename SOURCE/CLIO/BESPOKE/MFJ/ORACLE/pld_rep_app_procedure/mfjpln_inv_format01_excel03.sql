...
/* Package : mfjpln_inv_format01_excel03                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_inv_format01_excel03 as

/**DESCRIPTION**
 Finished Goods Inventory Excess Exception Report - Invoice date aggregations.
...
end mfjpln_inv_format01_excel03;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_inv_format01_excel03 as

   /*-*/
   /* Private global declarations */
   /*-*/
   procedure doDetail(par_type in varchar2);
   ...
   var_stock_excess_cover number(2,0);
   ...
   function main(par_sap_company_code in varchar2,
      ...
      cursor pld_rep_parameter_c01 is 
         select to_number(par_value)
          ...
          where par_group = 'STOCK_EXCESS_COVER'
   ...
   begin
      ...
      /*-*/
      /* Retrieve the stock excess cover parameter */
      /*-*/
      open pld_rep_parameter_c01;
      fetch pld_rep_parameter_c01 into var_stock_excess_cover;
      if pld_rep_parameter_c01%notfound then
         open pld_rep_parameter_c02;
         fetch pld_rep_parameter_c02 into var_stock_excess_cover;
         if pld_rep_parameter_c02%notfound then
            var_stock_excess_cover := 6;
         end if;
      ...
      if var_for_type = 'PRD_BR' then
         var_wrk_string := 'Finished Goods Inventory Excess Exceptions Report (greater than ' || to_char(var_stock_excess_cover,'FM90') || ' weeks cover) - Period - BR Forecast (Yen Millions)';
      elsif var_for_type = 'PRD_LE' then
         var_wrk_string := 'Finished Goods Inventory Excess Exceptions Report (greater than ' || to_char(var_stock_excess_cover,'FM90') || ' weeks cover) - Period - LE Forecast (Yen Millions)';
      elsif var_for_type = 'MTH_BR' then
         var_wrk_string := 'Finished Goods Inventory Excess Exceptions Report (greater than ' || to_char(var_stock_excess_cover,'FM90') || ' weeks cover) - Month - BR Forecast (Yen Millions)';
      elsif var_for_type = 'MTH_LE' then
         var_wrk_string := 'Finished Goods Inventory Excess Exceptions Report (greater than ' || to_char(var_stock_excess_cover,'FM90') || ' weeks cover) - Month - LE Forecast (Yen Millions)';
      end if;
      ...
      xlxml_object.SetRangeType('G6:G7', xlxml_object.TYPE_HEADING_HI);
      ...
      /*-*/
      /* Process the no forecast detail */
      /*-*/
      doDetail('1');

      /*-*/
      /* Process the excess detail */
      /*-*/
      doDetail('2');
   ...
   /**********************************************/
   procedure doDetail(par_type in varchar2) is
   ...
   begin
      ...
      /*-*/
      /* No forecast saleable inventory */
      /*-*/
      if par_type = '1' then
         var_dynamic_sql := 'select t01.' || var_typ_literal || '_billed_qty,
                              ...
                              where t01.sap_material_code = t02.sap_material_code(+)
                                ...
                                and t01.inv_tot_qty <> 0
                                and t01.' || var_typ_literal || '_' || var_for_literal || '_qty = 0 
                                and t01.' || var_typ_literal || '_fut_' || var_for_literal || '_qty = 0
                                and t01.inv_' || var_typ_literal || '_' || var_for_literal || '_sal_cover = 0
                           order by sort_desc asc';
      end if;
      ...
      if par_type = '2' then
         var_dynamic_sql := 'select t01.' || var_typ_literal || '_billed_qty,
                              ...
                              where t01.sap_material_code = t02.sap_material_code(+)
                                ...
                                and t01.inv_' || var_typ_literal || '_' || var_for_literal || '_ons_cover > :C
                           order by t01.inv_' || var_typ_literal || '_' || var_for_literal || '_ons_cover desc,
                                    sort_desc asc';
      end if;
      ...
      if par_type = '1' then
         ...
         open pld_inv_format0101_c01 for var_dynamic_sql using var_sap_company_code, var_sap_bus_sgmnt_code, var_stock_excess_cover;
      end if;
      loop
...
/**************************/
create or replace public synonym mfjpln_inv_format01_excel03 for pld_rep_app.mfjpln_inv_format01_excel03;
grant execute on mfjpln_inv_format01_excel03 to public;