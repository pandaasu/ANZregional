...
/* Package : mfjpln_for_format12_excel06                        */
...
/* Package Header */
/******************/
create or replace package mfjpln_for_format12_excel06 as

/**DESCRIPTION**
 Forecast Variant Source Unit Report - SAP billing date aggregations.
...
end mfjpln_for_format12_excel06;
/

/****************/
/* Package Body */
/****************/
create or replace package body mfjpln_for_format12_excel06 as
   ...
   var_planning_src_unit_count number(15,0);
   ...
   function main return varchar2 is
      ...
      cursor planning_src_unit_c01 is 
         select t01.var_code
         from pld_variable t01
         where t01.var_type = 'PLANNING_SRC_UNIT'
         order by t01.var_code asc;
   ...
   begin
      ...
      /*-*/
      var_planning_src_unit_count := 0;
      select count(*) into var_planning_src_unit_count from pld_variable where var_type = 'PLANNING_SRC_UNIT';
      ...
      /*-*/
      var_wrk_string := 'Forecast Variant Source Unit Report - Delivery Date';
      ...
      /*-*/
      /* Report heading line - source unit selections */
      /*-*/
      if var_planning_src_unit_count = 0 then
         var_planning_src_unit_desc := 'ALL';
         var_row_head := var_row_head + 1;
         var_wrk_string := 'Source Units: ' || var_planning_src_unit_desc;
         ...
         if var_planning_src_unit_count > 1 then
            var_row_head := var_row_head + 1;
            var_wrk_string := 'Source Units';
            xlxml_object.SetRange('A' || to_char(var_row_head,'FM999999990') || ':A' || to_char(var_row_head,'FM999999990'), 'A' || to_char(var_row_head,'FM999999990') || ':' || var_lst_column || to_char(var_row_head,'FM999999990'), xlxml_object.GetHeadingType(7), -2, 0, false, var_wrk_string);
         end if;
         open planning_src_unit_c01;
         loop
            fetch planning_src_unit_c01 into var_planning_src_unit_desc;
            if planning_src_unit_c01%notfound then
               exit;
            end if;
            var_row_head := var_row_head + 1;
            if var_planning_src_unit_count = 1 then
               ...
               var_wrk_string := var_planning_src_unit_desc;
            end if;
            xlxml_object.SetRange('A' || to_char(var_row_head,'FM999999990') || ':A' || to_char(var_row_head,'FM999999990'), 'A' || to_char(var_row_head,'FM999999990') || ':' || var_lst_column || to_char(var_row_head,'FM999999990'), xlxml_object.GetHeadingType(7), -2, 0, false, var_wrk_string);
         end loop;
         close planning_src_unit_c01;
      end if;
   ...
   begin
      ...
      if var_planning_src_unit_count <> 0 then
         ...
         var_slt_literal := var_slt_literal || ' and (t81.var_type = ''PLANNING_SRC_UNIT'' and t81.var_code = t02.planning_src_unit)';
      end if;
      if var_brand_flag_count <> 0 then
         var_fil_literal := var_fil_literal || ', pld_variable t82';
         var_slt_literal := var_slt_literal || ' and (t82.var_type = ''BRAND_FLAG'' and t82.var_code = t01.sap_brand_flag_code)';
      end if;
      if var_brand_sub_flag_count <> 0 then
         var_fil_literal := var_fil_literal || ', pld_variable t83';
         var_slt_literal := var_slt_literal || ' and (t83.var_type = ''BRAND_SUB_FLAG'' and t83.var_code = t01.sap_brand_sub_flag_code)';
      end if;
      if var_prdct_pack_size_count <> 0 then
         var_fil_literal := var_fil_literal || ', pld_variable t84';
         var_slt_literal := var_slt_literal || ' and (t84.var_type = ''PRDCT_PACK_SIZE'' and t84.var_code = t01.sap_prdct_pack_size_code)';
      end if;
      ...
      if var_for_type = 'PRD' then
         ...
         /*-*/
         var_dynamic_sql := 'select t03.planning_src_unit,
                                    nvl(case when max(t02.sap_brand_flag_code) = ''000'' then
      ...
      open dynamic_c01 for var_dynamic_sql;
      loop
         fetch dynamic_c01 into var_planning_src_unit_desc,
                                var_brand_flag_desc,
         ...
         /*-*/
         tblSummary(1).current_value := var_planning_src_unit_desc;
         tblSummary(2).current_value := var_brand_flag_desc;
         tblSummary(3).current_value := var_brand_sub_flag_desc;
         tblSummary(4).current_value := var_prdct_pack_size_desc;
         ...
         /*-*/
         tblSummary(1).description := var_planning_src_unit_desc;
         tblSummary(2).description := var_brand_flag_desc;
         tblSummary(3).description := var_brand_sub_flag_desc;
         tblSummary(4).description := var_prdct_pack_size_desc;
...
/**************************/
create or replace public synonym mfjpln_for_format12_excel06 for pld_rep_app.mfjpln_for_format12_excel06;
grant execute on mfjpln_for_format12_excel06 to public;