...
/* View creation
/*-*/
create or replace force view bo_user.fp_rpt_ptd_all
...
FROM
(
  SELECT * FROM FP_RPT_PTD_FD
  UNION ALL
  SELECT * FROM
  (
    SELECT * FROM FP_RPT_PTD_PS
    UNION ALL
    SELECT * FROM 
    (
      SELECT * FROM FP_RPT_PTD_PG2
      UNION ALL
      SELECT * FROM FP_RPT_PTD_PG1
    ) T1
  ) TT1
) TTT1
ORDER BY
