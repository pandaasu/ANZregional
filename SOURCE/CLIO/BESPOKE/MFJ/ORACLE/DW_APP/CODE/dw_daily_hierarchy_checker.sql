...
create or replace package body dw_daily_hierarchy_checker as
   ...
   procedure execute is
      ...
      cursor csr_hierarchy is
         select count(*) as leaf_count
          ...
          where t01.hdrdat = to_char(sysdate,'yyyymmdd');
   ...
   begin
      ...
      if rcd_hierarchy.leaf_count = 0 then
         var_status := '*ERROR';
         lics_logging.write_log('Daily Hierarchy Checker - Hierarchy NOT RECEIVED for - ' || to_char(sysdate,'yyyymmdd'));
      else
         var_status := '*OK';
         lics_logging.write_log('Daily Hierarchy Checker - Hierarchy received for - ' || to_char(sysdate,'yyyymmdd'));
      end if;
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - CLIO - DW_DAILY_HIERARCHY_CHECKER - ' || substr(SQLERRM, 1, 1024));
