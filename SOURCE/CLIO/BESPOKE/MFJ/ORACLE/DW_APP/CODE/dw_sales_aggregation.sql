...
/******************************************************************************/
/**
 Package : dw_sales_aggregation
 Owner   : dw_app

 Description
 -----------
 Dimensional Data Store - Sales Aggregation
 ...
 **notes**
 ...
 4. Sales aggregations based on SAP_BILLING_DATE are hard-coded to BILLING_DATE for 2004.
    (specific MFJ business requirement)
...
create or replace package body dw_sales_aggregation as
   ...
   procedure cust_partner_funcn2_build(par_company in varchar2);
   ...
   begin
      ...
      if upper(par_table) != '*ALL' and
         ...
         upper(par_table) != 'SALES_PERIOD_04_FACT' and
         upper(par_table) != 'CUST_PARTNER_FUNCN2' then
      ...
      if var_locked = true then
         ...
         if var_errors = false then
            ...
            /*-*/
            /* CUST_PARTNER_FUNCN2 aggregation
            /*-*/
            if upper(par_table) = '*ALL' or upper(par_table) = 'CUST_PARTNER_FUNCN2' then
               begin
                  cust_partner_funcn2_build(par_company);
               exception
                  when others then
                     var_errors := true;
               end;
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - CLIO - DW_SALES_AGGREGATION - ' || substr(SQLERRM, 1, 1024));
	    ...
	    var_invoice_type_factor := -1;
         ...
         loop
            ...
            if not(rcd_sales_fact.sap_material_code is null) then
               ...
               rcd_sales_fact.base_uom_billed_qty := rcd_lads_inv_gen.fklmg * var_invoice_type_factor;
               ...
               loop
                  ...
                  elsif rcd_lads_inv_irf.qualf =  '016' then
                     rcd_sales_fact.ref_doc_num := rcd_lads_inv_irf.refnr;
                     rcd_sales_fact.ref_doc_line_num := rcd_lads_inv_irf.zeile;
                  end if;
               end loop;
               ...
               loop
                  ...
                  case rcd_lads_inv_icn.kschl
                     when 'ZRSP' then rcd_sales_fact.sales_dtl_price_value_1 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'PR00' then rcd_sales_fact.sales_dtl_price_value_2 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK30' then rcd_sales_fact.sales_dtl_price_value_3 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK21' then rcd_sales_fact.sales_dtl_price_value_4 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK31' then rcd_sales_fact.sales_dtl_price_value_5 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK29' then rcd_sales_fact.sales_dtl_price_value_6 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK28' then rcd_sales_fact.sales_dtl_price_value_7 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK22' then rcd_sales_fact.sales_dtl_price_value_8 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK23' then rcd_sales_fact.sales_dtl_price_value_9 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK24' then rcd_sales_fact.sales_dtl_price_value_10 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZCRP' then rcd_sales_fact.sales_dtl_price_value_12 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK25' then rcd_sales_fact.sales_dtl_price_value_14 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK26' then rcd_sales_fact.sales_dtl_price_value_15 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'MWST' then rcd_sales_fact.sales_dtl_price_value_18 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK20' then rcd_sales_fact.sales_dtl_price_value_19 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZK27' then rcd_sales_fact.sales_dtl_price_value_20 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     when 'ZPS1' then rcd_sales_fact.sales_dtl_price_value_21 := round(rcd_lads_inv_icn.betrg*rcd_sales_fact.exch_rate,0);
                     else null;
                  end case;
               end loop;
               ...
               loop
                  ...
                  case rcd_lads_inv_ias.qualf
                     when '901' then rcd_sales_fact.sales_dtl_price_value_11 := round(rcd_lads_inv_ias.betrg*rcd_sales_fact.exch_rate,0);
                     when '902' then rcd_sales_fact.sales_dtl_price_value_13 := round(rcd_lads_inv_ias.betrg*rcd_sales_fact.exch_rate,0);
                     when '011' then rcd_sales_fact.sales_dtl_price_value_16 := round(rcd_lads_inv_ias.betrg*rcd_sales_fact.exch_rate,0);
                     when '003' then rcd_sales_fact.sales_dtl_price_value_17 := round(rcd_lads_inv_ias.betrg*rcd_sales_fact.exch_rate,0);
                     else null;
                  end case;
               end loop;
   ...
   /*****************************************************************/
   /* This procedure performs the customer partner function routine */
   /*****************************************************************/
   procedure cust_partner_funcn2_build(par_company in varchar2) is
   ...
   begin
      ...
      /* Begin procedure
      /*-*/
      lics_logging.write_log('Begin - CUST_PARTNER_FUNCN2 Build - Parameters(' || par_company || ')');

      /*-*/
      /* Delete the customer partner function rows
      /*-*/
      lics_logging.write_log('CUST_PARTNER_FUNCN2 Build - Deleting the table data');
      delete from cust_partner_funcn2
         where sap_sales_org_code = par_company;

      /*-*/
      /* Insert the customer partner function rows
      /*-*/
      lics_logging.write_log('CUST_PARTNER_FUNCN2 Build - Building the table data');
      insert into cust_partner_funcn2
        (select t1.sap_ship_to_cust_code as sap_ship_to_cust_code,
                t1.sap_sold_to_cust_code as sap_sold_to_cust_code,
                ...
                decode(t2.sap_cust_partner_code, null, decode(t3.sap_cust_partner_code, null, null, t3.sap_cust_partner_code), t2.sap_cust_partner_code) as sap_hier_cust_code
           from sales_fact t1,
                cust_partner_funcn t2,
                cust_partner_funcn t3
          where t1.sap_ship_to_cust_code = t2.sap_cust_code(+)
            and t1.sap_sales_dtl_sales_org_code = t2.sap_sales_org_code(+)
            and t1.sap_sales_dtl_distbn_chnl_code = t2.sap_distbn_chnl_code(+)
            and t1.sap_sales_dtl_division_code = t2.sap_division_code(+)
            and t2.sap_partner_funcn_code(+) = 'ZA'
            and t1.sap_sold_to_cust_code = t3.sap_cust_code(+)
            and t1.sap_sales_dtl_sales_org_code = t3.sap_sales_org_code(+)
            and t1.sap_sales_dtl_distbn_chnl_code = t3.sap_distbn_chnl_code(+)
            and t1.sap_sales_dtl_division_code = t3.sap_division_code(+)
            and t3.sap_partner_funcn_code(+) = 'ZA'
            and decode(t2.sap_cust_partner_code, null, decode(t3.sap_cust_partner_code, null, 0, 1), 1) = 1
            and t1.sap_sales_dtl_sales_org_code = par_company
          group by t1.sap_ship_to_cust_code,
                   t1.sap_sold_to_cust_code,
                   t1.sap_sales_dtl_distbn_chnl_code,
                   t1.sap_sales_dtl_division_code,
                   t1.sap_sales_dtl_sales_org_code,
                   decode(t2.sap_cust_partner_code, null, decode(t3.sap_cust_partner_code, null, null, t3.sap_cust_partner_code), t2.sap_cust_partner_code));
      ...
      /* End procedure
      /*-*/
      lics_logging.write_log('End - CUST_PARTNER_FUNCN2 Build');
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         if lics_logging.is_created = true then
            lics_logging.write_log('**ERROR** - CUST_PARTNER_FUNCN2 Build - ' || substr(SQLERRM, 1, 1024));
            lics_logging.write_log('End - CUST_PARTNER_FUNCN2 Build');
         end if;
