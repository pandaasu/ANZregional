...
*******************************************************************************/
   ...
   procedure sales_status(par_date out date, par_message out varchar2);
   procedure inventory_status(par_date out date, par_message out varchar2);
...
create or replace package body dw_reconciliation as
   ...
   /****************************************************/
   procedure sales_status(par_date out date, par_message out varchar2) is
      ...
      /*-*/
      cursor csr_date is
         select max(t1.fkdat) as fkdat
           from lads_inv_sum_hdr t1;
      rcd_date csr_date%rowtype;

      cursor csr_company is
         select t1.bukrs as bukrs
           from lads_inv_sum_hdr t1
          where t1.fkdat = rcd_date.fkdat
   ...
   begin
      ...
      if csr_date%found and
         ...
         /*-*/
         /* Retrieve the variance companies
         /*-*/
         open csr_company;
         loop
            fetch csr_company into rcd_company;
            ...
            var_message := var_message || rcd_company.bukrs;
         end loop;
         close csr_company;
      ...
      if var_found = false then
         ...
         if var_message is null then
            ...
            par_message := 'Sales Date: ' || to_char(to_date(rcd_date.fkdat,'yyyymmdd'),'yyyy/mm/dd') || ' Status: ERRORS - Companies(' || var_message || ')';
         end if;
      end if;
   ...
   /********************************************************/
   procedure inventory_status(par_date out date, par_message out varchar2) is
