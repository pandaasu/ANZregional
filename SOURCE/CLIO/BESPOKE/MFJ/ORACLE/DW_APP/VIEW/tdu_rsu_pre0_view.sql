...
/* View creation
/*-*/
create or replace force view tdu_rsu_pre0_view
   (TDU_SAP_CODE,
    TDU_DESC,
    TDU_JAN,
    TDU_NET_WGT,
    ...
    RSU_WGT_UNIT,
    TDU_TO_RSU_QTY) AS 
   SELECT D.SAP_MATERIAL_CODE AS TDU_SAP_CODE, 
    ...
    FROM (SELECT T3.SAP_MATERIAL_CODE, 
                 T3.MATERIAL_DESC_JA, 
                 T3.EAN_UPC, 
                 T3.NET_WGT, 
                 T3.SAP_WGT_UNIT_CODE 
            FROM DW_APP.TDU_PRE0_VIEW T3) D, 
         (SELECT T4.SAP_MATERIAL_CODE, 
                 T4.MATERIAL_DESC_JA, 
                 T4.EAN_UPC, 
                 T4.NET_WGT, 
   ...
   WHERE D.SAP_MATERIAL_CODE = F.SAP_MATERIAL_CODE AND 
         F.CMPNT_MATERIAL_CODE = E.SAP_MATERIAL_CODE
...
/* Authority
/*-*/
grant select on dw_app.tdu_rsu_pre0_view to bo_user;

/*-*/
/* Synonym
/*-*/
create or replace public synonym tdu_rsu_pre0_view for dw_app.tdu_rsu_pre0_view;
