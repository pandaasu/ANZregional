...
/******************************************************************************/
/**
 Object : kpi_invoice_summary_view
 Owner  : dw_app
...
/* View creation
/*-*/
create or replace force view dw_app.kpi_invoice_summary_view
   (BILLING_YYYYPPDD,
    BILLING_DATE,
    SAP_SALES_ORG_CODE,
    ...
    INV_BPS,
    INV_SHIP_TOS) AS 
   SELECT MAIN.BILLING_YYYYPPDD, 
          MD.CALENDAR_DATE AS BILLING_DATE, 
          MAIN.SAP_SALES_DTL_SALES_ORG_CODE AS SALES_ORG_CODE, 
          MAIN.SAP_SALES_DTL_DISTBN_CHNL_CODE AS DISTBN_CHNL_CODE, 
          MAIN.SAP_SALES_DTL_DIVISION_CODE AS DIVISION_CODE, 
          MAIN.SAP_PLANT_CODE, 
     ...
     FROM PLANT_DIM PT, 
          ...
          (SELECT BILLING_YYYYPPDD, 
             ...
             FROM (SELECT SF.BILLING_YYYYPPDD, 
                     ...
                     FROM (SELECT * FROM SALES_FACT WHERE BILLING_YYYYPPDD >= 20040101) SF, 
                          MARS_DATE MD, 
                          CUST_DIM CU 
                    WHERE SF.BILLING_YYYYPPDD = MD.MARS_YYYYPPDD (+) AND 
                          --SF.BILLING_YYYYPPDD >= 20040101 AND 
                          SF.SAP_SHIP_TO_CUST_CODE = CU.SAP_CUST_CODE (+) 
                    GROUP BY SF.BILLING_YYYYPPDD, 
                             SF.SAP_SALES_DTL_SALES_ORG_CODE, 
                             SF.SAP_SALES_DTL_DISTBN_CHNL_CODE, 
                             SF.SAP_SALES_DTL_DIVISION_CODE, 
                             SF.SAP_PLANT_CODE, 
    ...
    WHERE MAIN.BILLING_YYYYPPDD = MD.MARS_YYYYPPDD (+) AND 
          MAIN.SAP_PLANT_CODE = PT.SAP_PLANT_CODE (+) AND 
...
/* Authority
/*-*/
grant select on dw_app.kpi_invoice_summary_view to public;

/*-*/
/* Synonym
/*-*/
create or replace public synonym kpi_invoice_summary_view for dw_app.kpi_invoice_summary_view;