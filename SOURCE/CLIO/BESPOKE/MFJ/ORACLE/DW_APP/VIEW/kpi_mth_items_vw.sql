...
/* View creation
/*-*/
create or replace force view dw_app.kpi_mth_items_vw
   (SALES_AREA,
    BILLING_YEAR,
    BILLING_MONTH,
    DIST_INV_ITEMS) AS 
   SELECT NVL(SO.SALES_ORG_DESC, 'UNKNOWN') || ' - ' || NVL(DC.DISTBN_CHNL_DESC, 'UNKNOWN') || ' - ' || NVL(DV.DIVISION_DESC, 'UNKNOWN') AS SALES_AREA, 
     ...
     FROM SALES_ORG_DIM SO, 
          ...
          DIVISION_DIM DV, 
          (SELECT DISTINCT 
                  SF.SAP_SALES_DTL_SALES_ORG_CODE AS SAP_SALES_ORG_CODE, 
...
/* Authority
/*-*/
grant select on dw_app.kpi_mth_items_vw to public;

/*-*/
/* Synonym
/*-*/
create or replace public synonym kpi_mth_items_vw for dw_app.kpi_mth_items_vw;
