...
create or replace package ods_app.qu5_qu5cdw14 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw14
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw14] Entity [CustomerContactTraining] Table [qu5_cust_contact_training][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw14 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw14';
  g_entity_name constant varchar2(64 char) := 'CustomerContactTraining';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('cust_contact_id',2);
    lics_inbound_utility.set_csv_definition('training_id',3);
    lics_inbound_utility.set_csv_definition('training_desc',4);
    lics_inbound_utility.set_csv_definition('created_date',5);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,CustomerContact_Id,TrainingId,Training_Description,CreatedDate';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_cust_contact_training_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.cust_contact_id := qu5_util.get_number('cust_contact_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.training_id := qu5_util.get_number('training_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.training_desc := qu5_util.get_string('training_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_cust_contact_training_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_cust_contact_training_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_cust_contact_training_load
    ...
    update ods.qu5_cust_contact_training_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_cust_contact_training_hist
    select *
    from ods.qu5_cust_contact_training_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_cust_contact_training_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_cust_contact_training/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw14 for ods_app.qu5_qu5cdw14;

-- Grants
grant execute on ods_app.qu5_qu5cdw14 to lics_app;
