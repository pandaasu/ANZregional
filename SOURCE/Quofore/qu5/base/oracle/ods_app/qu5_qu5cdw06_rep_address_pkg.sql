...
create or replace package ods_app.qu5_qu5cdw06 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw06
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw06] Entity [RepAddress] Table [qu5_rep_address][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw06 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw06';
  g_entity_name constant varchar2(64 char) := 'RepAddress';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('address_type_id',4);
    lics_inbound_utility.set_csv_definition('address_type_id_desc',5);
    lics_inbound_utility.set_csv_definition('street_1',6);
    lics_inbound_utility.set_csv_definition('street_2',7);
    lics_inbound_utility.set_csv_definition('town',8);
    lics_inbound_utility.set_csv_definition('city',9);
    lics_inbound_utility.set_csv_definition('postcode',10);
    lics_inbound_utility.set_csv_definition('state_id',11);
    lics_inbound_utility.set_csv_definition('state_id_desc',12);
    lics_inbound_utility.set_csv_definition('country_id',13);
    lics_inbound_utility.set_csv_definition('country_id_desc',14);
    lics_inbound_utility.set_csv_definition('latitude',15);
    lics_inbound_utility.set_csv_definition('longitude',16);
    lics_inbound_utility.set_csv_definition('full_name',17);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,Rep_Id,AddressTypeId,AddressTypeId_Description,Street1,Street2,Town,City,PostCode,StateId,StateId_Description,CountryId,CountryId_Description,Latitude,Longitude,FullName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_rep_address_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.qu5_rep_address_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_rep_address_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_rep_address_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_rep_address_hist
    select *
    from ods.qu5_rep_address_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_rep_address_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_rep_address/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw06 for ods_app.qu5_qu5cdw06;

-- Grants
grant execute on ods_app.qu5_qu5cdw06 to lics_app;
