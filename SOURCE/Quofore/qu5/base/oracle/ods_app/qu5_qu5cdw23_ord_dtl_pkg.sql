...
create or replace package ods_app.qu5_qu5cdw23 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw23
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw23] Entity [OrderDetail] Table [qu5_ord_dtl][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw23 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw23';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('bonuse_qty',2);
    lics_inbound_utility.set_csv_definition('discount_amt',3);
    ...
    lics_inbound_utility.set_csv_definition('total_line_amt',10);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_ord_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.bonuse_qty := qu5_util.get_number('bonuse_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.discount_amt := qu5_util.get_number('discount_amt',18,4,true,g_source_file_error_flag);
      l_entity_load_row.ord_hdr_id := qu5_util.get_number('ord_hdr_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.ord_qty := qu5_util.get_number('ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.original_ord_qty := qu5_util.get_number('original_ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.per_unit_price := qu5_util.get_number('per_unit_price',18,4,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu5_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.tax_percent := qu5_util.get_number('tax_percent',9,6,true,g_source_file_error_flag);
      l_entity_load_row.total_line_amt := qu5_util.get_number('total_line_amt',18,4,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_ord_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_ord_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_ord_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_ord_dtl_hist
    select *
    from ods.qu5_ord_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_ord_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_ord_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw23 for ods_app.qu5_qu5cdw23;

-- Grants
grant execute on ods_app.qu5_qu5cdw23 to lics_app;
