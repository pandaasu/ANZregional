...
create or replace package ods_app.qu5_qu5cdw00 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw00
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw00] Entity [Digest] Table [qu5_digest][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw00 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw00';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_digest_load%rowtype;
  ...
  begin
    ...
    begin
      l_entity_load_row.file_name := qu5_util.get_string('file_name',64,false,g_source_file_error_flag);
      l_entity_load_row.row_count := qu5_util.get_number('row_count',10,0,false,g_source_file_error_flag);
      l_entity_load_row.entity_name := upper(qu5_util.get_file_entity_name(l_entity_load_row.file_name)); -- Special Case for Digest
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_digest_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_digest_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_digest_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_digest_hist
    select *
    from ods.qu5_digest_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_digest_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_digest/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw00 for ods_app.qu5_qu5cdw00;

-- Grants
grant execute on ods_app.qu5_qu5cdw00 to lics_app;
