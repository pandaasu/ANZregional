...
create or replace package ods_app.qu5_qu5cdw27 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw27
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw27] Entity [Survey] Table [qu5_survey][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw27 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw27';
  g_entity_name constant varchar2(64 char) := 'Survey';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_type_id',4);
    lics_inbound_utility.set_csv_definition('survey_type_id_desc',5);
    lics_inbound_utility.set_csv_definition('name',6);
    lics_inbound_utility.set_csv_definition('notes',7);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,CreatedDate,SurveyTypeId,SurveyTypeId_Description,Name,Notes';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_survey_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_type_id := qu5_util.get_number('survey_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_type_id_desc := qu5_util.get_string('survey_type_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_survey_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_survey_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_survey_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_survey_hist
    select *
    from ods.qu5_survey_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_survey_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_survey/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw27 for ods_app.qu5_qu5cdw27;

-- Grants
grant execute on ods_app.qu5_qu5cdw27 to lics_app;
