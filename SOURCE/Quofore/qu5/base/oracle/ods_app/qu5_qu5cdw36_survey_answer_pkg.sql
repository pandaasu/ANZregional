...
create or replace package ods_app.qu5_qu5cdw36 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw36
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw36] Entity [SurveyAnswer] Table [qu5_survey_answer][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw36 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw36';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('response_opt_id',5);
    lics_inbound_utility.set_csv_definition('freeform_response',6);
    lics_inbound_utility.set_csv_definition('rep_id',7);
    lics_inbound_utility.set_csv_definition('callcard_id',8);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_survey_answer_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_response_id := qu5_util.get_number('survey_response_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_id := qu5_util.get_number('survey_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.survey_question_id := qu5_util.get_number('survey_question_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.response_opt_id := qu5_util.get_number('response_opt_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.freeform_response := qu5_util.get_string('freeform_response',4000,true,g_source_file_error_flag);
      ...
      l_entity_load_row.survey_response_created_date := qu5_util.get_datetime('survey_response_created_date',true,g_source_file_error_flag);
      l_entity_load_row.task_created_date := qu5_util.get_datetime('task_created_date',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_survey_answer_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_survey_answer_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_survey_answer_load
    ...
    update ods.qu5_survey_answer_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_survey_answer_hist
    select *
    from ods.qu5_survey_answer_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_survey_answer_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_survey_answer/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw36 for ods_app.qu5_qu5cdw36;

-- Grants
grant execute on ods_app.qu5_qu5cdw36 to lics_app;
