...
create or replace package ods_app.qu5_qu5cdw30 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw30
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw30] Entity [Task] Table [qu5_task][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw30 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw30';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('status_id',4);
    lics_inbound_utility.set_csv_definition('status_id_desc',5);
    lics_inbound_utility.set_csv_definition('is_recurring',6);
    lics_inbound_utility.set_csv_definition('is_all_day',7);
    lics_inbound_utility.set_csv_definition('name',8);
    lics_inbound_utility.set_csv_definition('start_date',9);
    lics_inbound_utility.set_csv_definition('end_date',10);
    lics_inbound_utility.set_csv_definition('priority_id',11);
    lics_inbound_utility.set_csv_definition('priority_id_desc',12);
    lics_inbound_utility.set_csv_definition('due_date',13);
    lics_inbound_utility.set_csv_definition('time',14);
    lics_inbound_utility.set_csv_definition('duration',15);
    lics_inbound_utility.set_csv_definition('notes',16);
    lics_inbound_utility.set_csv_definition('task_type_id',17);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,CreatedDate,StatusId,StatusId_Description,IsRecurring,IsAllDay,Name,StartDate,EndDate,PriorityId,PriorityId_Description,DueDate,Time,Duration,Notes,TaskTypeId';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_task_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.status_id := qu5_util.get_number('status_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.status_id_desc := qu5_util.get_string('status_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.is_recurring := qu5_util.get_boolean_as_number('is_recurring',true,g_source_file_error_flag);
      l_entity_load_row.is_all_day := qu5_util.get_boolean_as_number('is_all_day',true,g_source_file_error_flag);
      l_entity_load_row.name := qu5_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.start_date := qu5_util.get_date('start_date',true,g_source_file_error_flag);
      l_entity_load_row.end_date := qu5_util.get_date('end_date',true,g_source_file_error_flag);
      l_entity_load_row.priority_id := qu5_util.get_number('priority_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.priority_id_desc := qu5_util.get_string('priority_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.due_date := qu5_util.get_date('due_date',true,g_source_file_error_flag);
      l_entity_load_row.time := qu5_util.get_time('time',true,g_source_file_error_flag);
      l_entity_load_row.duration := qu5_util.get_time('duration',true,g_source_file_error_flag);
      l_entity_load_row.notes := qu5_util.get_string('notes',200,true,g_source_file_error_flag);
      l_entity_load_row.task_type_id := qu5_util.get_number('task_type_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_task_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_task_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_task_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_task_hist
    select *
    from ods.qu5_task_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_task_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_task/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw30 for ods_app.qu5_qu5cdw30;

-- Grants
grant execute on ods_app.qu5_qu5cdw30 to lics_app;
