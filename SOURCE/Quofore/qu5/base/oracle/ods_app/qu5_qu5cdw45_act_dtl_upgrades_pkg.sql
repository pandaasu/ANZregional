...
create or replace package ods_app.qu5_qu5cdw45 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw45
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw45] Entity [ActivityDetailUpgrades] Table [qu5_act_dtl_upgrades][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw45 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw45';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailUpgrades';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('upgrades_at_visit_qty',4);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,Activity_Id,HierarchyNode_Id,UPG_UpgradesAtVisit';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_act_dtl_upgrades_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.upgrades_at_visit_qty := qu5_util.get_number('upgrades_at_visit_qty',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_act_dtl_upgrades_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_act_dtl_upgrades_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_act_dtl_upgrades_load
    ...
    update ods.qu5_act_dtl_upgrades_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_act_dtl_upgrades_hist
    select *
    from ods.qu5_act_dtl_upgrades_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_act_dtl_upgrades_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_act_dtl_upgrades/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw45 for ods_app.qu5_qu5cdw45;

-- Grants
grant execute on ods_app.qu5_qu5cdw45 to lics_app;
