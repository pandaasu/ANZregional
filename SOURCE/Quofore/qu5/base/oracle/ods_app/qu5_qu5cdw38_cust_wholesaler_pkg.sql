...
create or replace package ods_app.qu5_qu5cdw38 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw38
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw38] Entity [CustomerWholesaler] Table [qu5_cust_wholesaler][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw38 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw38';
  g_entity_name constant varchar2(64 char) := 'CustomerWholesaler';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('unique_id',1);
    lics_inbound_utility.set_csv_definition('cust_id',2);
    lics_inbound_utility.set_csv_definition('wholesales_id',3);
    lics_inbound_utility.set_csv_definition('created_date',4);
    lics_inbound_utility.set_csv_definition('account_no',5);
    lics_inbound_utility.set_csv_definition('is_default_wholesales',6);
    lics_inbound_utility.set_csv_definition('id',7);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'UniqueId,Customer_Id,Wholesaler_Id,CreatedDate,AccountNumber,IsDefaultWholesaler,Id';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_cust_wholesaler_load%rowtype;
  ...
  begin
    ...
    begin
      l_entity_load_row.unique_id := qu5_util.get_string('unique_id',50,true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu5_util.get_number('cust_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.wholesales_id := qu5_util.get_number('wholesales_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.created_date := qu5_util.get_datetime('created_date',true,g_source_file_error_flag);
      l_entity_load_row.account_no := qu5_util.get_string('account_no',50,true,g_source_file_error_flag);
      l_entity_load_row.is_default_wholesales := qu5_util.get_boolean_as_number('is_default_wholesales',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_cust_wholesaler_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_cust_wholesaler_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_cust_wholesaler_load
    ...
    update ods.qu5_cust_wholesaler_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_cust_wholesaler_hist
    select *
    from ods.qu5_cust_wholesaler_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_cust_wholesaler_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_cust_wholesaler/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw38 for ods_app.qu5_qu5cdw38;

-- Grants
grant execute on ods_app.qu5_qu5cdw38 to lics_app;
