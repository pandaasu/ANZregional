...
create or replace package ods_app.qu5_qu5cdw17 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw17
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw17] Entity [ProductAssortmentDetail] Table [qu5_prod_assort_dtl][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw17 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw17';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('assort_dtl_id',3);
    lics_inbound_utility.set_csv_definition('prod_id',4);
    lics_inbound_utility.set_csv_definition('prod_hier_node_id',5);
    lics_inbound_utility.set_csv_definition('priority_assort_dtl_id',6);
    lics_inbound_utility.set_csv_definition('effective_from',7);
    lics_inbound_utility.set_csv_definition('effective_to',8);
    lics_inbound_utility.set_csv_definition('priority_from',9);
    lics_inbound_utility.set_csv_definition('priotity_to',10);
    lics_inbound_utility.set_csv_definition('assort_dtl_id_lookup',11);
    lics_inbound_utility.set_csv_definition('id_lookup',12);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,CreatedDate,AssortmentDetail_Id,Product_Id,Product_HierarchyNode_Id,PriorityAssortmentDetail_Id,EffectiveFrom,EffectiveTo,PriorityFrom,PriorityTo,AssortmentDetail_Id_Lookup,Id_Lookup,Product_Id_Lookup,Product_Id_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_prod_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_dtl_id := qu5_util.get_number('assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu5_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_hier_node_id := qu5_util.get_number('prod_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.priority_assort_dtl_id := qu5_util.get_number('priority_assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.effective_from := qu5_util.get_datetime('effective_from',true,g_source_file_error_flag);
      l_entity_load_row.effective_to := qu5_util.get_datetime('effective_to',true,g_source_file_error_flag);
      l_entity_load_row.priority_from := qu5_util.get_datetime('priority_from',true,g_source_file_error_flag);
      l_entity_load_row.priotity_to := qu5_util.get_datetime('priotity_to',true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_id_lookup := qu5_util.get_string('assort_dtl_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.id_lookup := qu5_util.get_string('id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_lookup := qu5_util.get_string('prod_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_desc := qu5_util.get_string('prod_id_desc',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_prod_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_prod_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_prod_assort_dtl_load
    ...
    update ods.qu5_prod_assort_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_prod_assort_dtl_hist
    select *
    from ods.qu5_prod_assort_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_prod_assort_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_prod_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw17 for ods_app.qu5_qu5cdw17;

-- Grants
grant execute on ods_app.qu5_qu5cdw17 to lics_app;
