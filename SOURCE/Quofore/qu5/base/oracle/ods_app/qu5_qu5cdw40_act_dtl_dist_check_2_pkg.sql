...
create or replace package ods_app.qu5_qu5cdw40 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw40
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw40] Entity [ActivityDetailDistCheck2] Table [qu5_act_dtl_dist_check_2][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw40 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw40';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailDistCheck2';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('prod_id',3);
    lics_inbound_utility.set_csv_definition('is_prod_in_distribution',4);
    lics_inbound_utility.set_csv_definition('is_prime_loc_on_shelf',5);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,Activity_Id,Product_ID,DC2_InDist,DC2_PrimeLocOnShelf';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_act_dtl_dist_check_2_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.act_id := qu5_util.get_number('act_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu5_util.get_number('prod_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.is_prod_in_distribution := qu5_util.get_boolean_as_number('is_prod_in_distribution',true,g_source_file_error_flag);
      l_entity_load_row.is_prime_loc_on_shelf := qu5_util.get_boolean_as_number('is_prime_loc_on_shelf',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_act_dtl_dist_check_2_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_act_dtl_dist_check_2_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_act_dtl_dist_check_2_load
    ...
    update ods.qu5_act_dtl_dist_check_2_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_act_dtl_dist_check_2_hist
    select *
    from ods.qu5_act_dtl_dist_check_2_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_act_dtl_dist_check_2_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_act_dtl_dist_check_2/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw40 for ods_app.qu5_qu5cdw40;

-- Grants
grant execute on ods_app.qu5_qu5cdw40 to lics_app;
