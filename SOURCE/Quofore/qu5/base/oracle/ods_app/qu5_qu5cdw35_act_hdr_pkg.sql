...
create or replace package ods_app.qu5_qu5cdw35 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw35
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw35] Entity [ActivityHeader] Table [qu5_act_hdr][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw35 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw35';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('is_display_in_store',15);
    lics_inbound_utility.set_csv_definition('is_display_to_gold_std',16);
    lics_inbound_utility.set_csv_definition('displays_qty',17);
    lics_inbound_utility.set_csv_definition('is_gwp_allocation_received',18);
    lics_inbound_utility.set_csv_definition('have_demo_in_store',19);
    lics_inbound_utility.set_csv_definition('have_display_in_store',20);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,Task_Id,Rep_Id,StartDate,IsComplete,EndDate,Callcard_Id,IncompleteReasonId,IncompleteReasonId_Description,Notes,DueDate,FullName,Task_Id_Description,Comments,DIS_DispInStore,DIS_DispToGoldStd,DIS_NoOfDisplays,GWP_AllocationReceived,SA_Demo,SA_Display';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_act_hdr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.start_date := qu5_util.get_datetime('start_date',true,g_source_file_error_flag);
      l_entity_load_row.is_complete := qu5_util.get_boolean_as_number('is_complete',true,g_source_file_error_flag);
      l_entity_load_row.end_date := qu5_util.get_datetime('end_date',true,g_source_file_error_flag);
      l_entity_load_row.callcard_id := qu5_util.get_number('callcard_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.incomplete_reason_id := qu5_util.get_number('incomplete_reason_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.incomplete_reason_id_desc := qu5_util.get_string('incomplete_reason_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.notes := qu5_util.get_string('notes',200,true,g_source_file_error_flag);
      l_entity_load_row.due_date := qu5_util.get_datetime('due_date',true,g_source_file_error_flag);
      l_entity_load_row.full_name := qu5_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.task_id_desc := qu5_util.get_string('task_id_desc',200,true,g_source_file_error_flag);
      l_entity_load_row.comments := qu5_util.get_string('comments',100,true,g_source_file_error_flag);
      l_entity_load_row.is_display_in_store := qu5_util.get_number('is_display_in_store',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_display_to_gold_std := qu5_util.get_number('is_display_to_gold_std',10,0,true,g_source_file_error_flag);
      l_entity_load_row.displays_qty := qu5_util.get_number('displays_qty',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_gwp_allocation_received := qu5_util.get_number('is_gwp_allocation_received',10,0,true,g_source_file_error_flag);
      l_entity_load_row.have_demo_in_store := qu5_util.get_number('have_demo_in_store',10,0,true,g_source_file_error_flag);
      l_entity_load_row.have_display_in_store := qu5_util.get_number('have_display_in_store',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_act_hdr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_act_hdr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_act_hdr_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_act_hdr_hist
    select *
    from ods.qu5_act_hdr_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_act_hdr_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_act_hdr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw35 for ods_app.qu5_qu5cdw35;

-- Grants
grant execute on ods_app.qu5_qu5cdw35 to lics_app;
