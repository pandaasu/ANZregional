...
create or replace package ods_app.qu5_qu5cdw18 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw18
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw18] Entity [AuthorisedListProduct] Table [qu5_auth_list_prod][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw18 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw18';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_auth_list_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.authorised_list_id := qu5_util.get_number('authorised_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu5_util.get_number('cust_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id_lookup := qu5_util.get_number('cust_id_lookup',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_include := qu5_util.get_boolean_as_number('is_include',true,g_source_file_error_flag);
      l_entity_load_row.effective_from := qu5_util.get_date('effective_from',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_auth_list_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_auth_list_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_auth_list_prod_load
    ...
    update ods.qu5_auth_list_prod_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_auth_list_prod_hist
    select *
    from ods.qu5_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_auth_list_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw18 for ods_app.qu5_qu5cdw18;

-- Grants
grant execute on ods_app.qu5_qu5cdw18 to lics_app;
