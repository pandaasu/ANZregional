...
create or replace package ods_app.qu5_qu5cdw15 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw15
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw15] Entity [AssortmentDetail] Table [qu5_assort_dtl][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw15 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw15';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_id := qu5_util.get_number('assort_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_created_date := qu5_util.get_datetime('assort_created_date',true,g_source_file_error_flag);
      l_entity_load_row.name := qu5_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id := qu5_util.get_number('assort_dtl_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_desc := qu5_util.get_string('assort_dtl_type_id_desc',100,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_name := qu5_util.get_string('assort_dtl_name',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_level := qu5_util.get_number('hier_level',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id := qu5_util.get_number('cust_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.parent_id := qu5_util.get_number('parent_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id_lookup := qu5_util.get_string('cust_hier_node_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_lookup := qu5_util.get_string('parent_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_lookup := qu5_util.get_string('assort_dtl_type_id_lookup',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_assort_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_assort_dtl_hist
    select *
    from ods.qu5_assort_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_assort_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw15 for ods_app.qu5_qu5cdw15;

-- Grants
grant execute on ods_app.qu5_qu5cdw15 to lics_app;
