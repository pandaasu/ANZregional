...
create or replace package ods_app.qu5_qu5cdw02 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw02
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw02] Entity [GeneralList] Table [qu5_general_list][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw02 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw02';
  g_entity_name constant varchar2(64 char) := 'GeneralList';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('lookup_list_item_is_active',2);
    lics_inbound_utility.set_csv_definition('lookup_list_item_created_date',3);
    lics_inbound_utility.set_csv_definition('lookup_list_item_name',4);
    lics_inbound_utility.set_csv_definition('lookup_list_is_active',5);
    lics_inbound_utility.set_csv_definition('lookup_list_id',6);
    lics_inbound_utility.set_csv_definition('lookup_list_name',7);
    lics_inbound_utility.set_csv_definition('lookup_list_created_date',8);
    lics_inbound_utility.set_csv_definition('lookup_list_sort_order',9);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,LookupListItemIsActive,LookupListItemCreatedDate,LookupListItemName,LookupListIsActive,LookupList_Id,LookupListName,LookupListCreatedDate,LookupListSortOrder';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_general_list_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.lookup_list_item_is_active := qu5_util.get_boolean_as_number('lookup_list_item_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_created_date := qu5_util.get_datetime('lookup_list_item_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_name := qu5_util.get_string('lookup_list_item_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_is_active := qu5_util.get_boolean_as_number('lookup_list_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_id := qu5_util.get_number('lookup_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_name := qu5_util.get_string('lookup_list_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_created_date := qu5_util.get_datetime('lookup_list_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_sort_order := qu5_util.get_number('lookup_list_sort_order',5,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_general_list_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_general_list_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_general_list_load
    ...
    update ods.qu5_general_list_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_general_list_hist
    select *
    from ods.qu5_general_list_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_general_list_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_general_list/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw02 for ods_app.qu5_qu5cdw02;

-- Grants
grant execute on ods_app.qu5_qu5cdw02 to lics_app;
