...
create or replace package ods_app.qu5_qu5cdw01 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw01
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw01] Entity [Hierarchy] Table [qu5_hier][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw01 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw01';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_hier_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.id_desc := qu5_util.get_string('id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.is_active := qu5_util.get_boolean_as_number('is_active',true,g_source_file_error_flag);
      l_entity_load_row.node := qu5_util.get_string('node',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_full_path := qu5_util.get_string('hier_full_path',4000,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_is_active := qu5_util.get_boolean_as_number('hier_root_is_active',true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id := qu5_util.get_number('hier_root_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id_desc := qu5_util.get_string('hier_root_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id := qu5_util.get_number('parent_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_desc := qu5_util.get_string('parent_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_node := qu5_util.get_string('parent_node',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_is_active := qu5_util.get_boolean_as_number('parent_is_active',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_hier_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_hier_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_hier_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_hier_hist
    select *
    from ods.qu5_hier_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_hier_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_hier/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw01 for ods_app.qu5_qu5cdw01;

-- Grants
grant execute on ods_app.qu5_qu5cdw01 to lics_app;
