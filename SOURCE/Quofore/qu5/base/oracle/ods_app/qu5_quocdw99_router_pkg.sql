...
create or replace package ods_app.qu5_qu5cdw99 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
...
create or replace package body ods_app.qu5_qu5cdw99 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw99';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    g_prefixed_file_name := 'qu5_'||lics_inbound_processor.callback_file_name;
    ...
    g_log_search_text := qu5_constants.setting_group||'_ROUTER_'||g_source_interface_name;
    --
    ...
    begin
      g_entity_name := qu5_util.get_file_entity_name(g_original_file_name);
    exception
      when others then
    ...
    begin
      g_target_interface_name := qu5_util.get_entity_interface_name(g_entity_name);
    exception
      when others then
...
-- Synonyms
create or replace public synonym qu5_qu5cdw99 for ods_app.qu5_qu5cdw99;

-- Grants
grant execute on ods_app.qu5_qu5cdw99 to lics_app;
