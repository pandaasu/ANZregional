...
create or replace package ods_app.qu5_qu5cdw10 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw10
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw10] Entity [CustomerAddress] Table [qu5_cust_address][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw10 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw10';
  g_entity_name constant varchar2(64 char) := 'CustomerAddress';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('address_type_id',2);
    lics_inbound_utility.set_csv_definition('address_type_id_desc',3);
    ...
    lics_inbound_utility.set_csv_definition('cust_id',7);
    lics_inbound_utility.set_csv_definition('is_active',8);
    lics_inbound_utility.set_csv_definition('latitude',9);
    lics_inbound_utility.set_csv_definition('longitude',10);
    lics_inbound_utility.set_csv_definition('postcode',11);
    lics_inbound_utility.set_csv_definition('state_id',12);
    lics_inbound_utility.set_csv_definition('state_id_desc',13);
    lics_inbound_utility.set_csv_definition('street_1',14);
    lics_inbound_utility.set_csv_definition('street_2',15);
    lics_inbound_utility.set_csv_definition('town',16);
    lics_inbound_utility.set_csv_definition('cust_id_desc',17);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,AddressTypeId,AddressTypeId_Description,City,CountryId,CountryId_Description,Customer_Id,IsActive,Latitude,Longitude,Postcode,StateId,StateId_Description,Street1,Street2,Town,Customer_Id_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_cust_address_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.address_type_id := qu5_util.get_number('address_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.address_type_id_desc := qu5_util.get_string('address_type_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu5_cust_address_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_cust_address_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_cust_address_load
    ...
    update ods.qu5_cust_address_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_cust_address_hist
    select *
    from ods.qu5_cust_address_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_cust_address_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_cust_address/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw10 for ods_app.qu5_qu5cdw10;

-- Grants
grant execute on ods_app.qu5_qu5cdw10 to lics_app;
