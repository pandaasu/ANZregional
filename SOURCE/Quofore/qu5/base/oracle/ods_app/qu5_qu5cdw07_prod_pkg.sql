...
create or replace package ods_app.qu5_qu5cdw07 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw07
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw07] Entity [Product] Table [qu5_prod][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw07 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw07';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('segment_hier_id',17);
    ...
    lics_inbound_utility.set_csv_definition('priority_start',21);
    lics_inbound_utility.set_csv_definition('priority_end',22);
    lics_inbound_utility.set_csv_definition('is_direct_orderable',23);
    lics_inbound_utility.set_csv_definition('is_indirect_orderable',24);
    lics_inbound_utility.set_csv_definition('do_first_do_next_do_last_id',25);
    lics_inbound_utility.set_csv_definition('layers_per_pallet',26);
    lics_inbound_utility.set_csv_definition('case_per_layer',27);
    lics_inbound_utility.set_csv_definition('pallet_case_count',28);
    lics_inbound_utility.set_csv_definition('unit_name',29);
    lics_inbound_utility.set_csv_definition('inner_name',30);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,CreatedDate,InnersPerCase,IsNew,IsPriority,ListPrice,UnitOfMeasureId,UnitOfMeasureId_Description,PackSize,PackDescription,UnitSize,SKUCode,Name,UnitsPerCase,UnitsPerInner,Segment_Hierarchy_Id,Brand_Hierarchy_Id,NewStart,NewEnd,PriorityStart,PriorityEnd,IsDirectOrderable,IsIndirectOrderable,DFDNDLId,LayersPerPallet,CasesPerLayer,PalletCaseCount,UnitName,InnerName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.is_active := qu5_util.get_boolean_as_number('is_active',true,g_source_file_error_flag);
      l_entity_load_row.created_date := qu5_util.get_datetime('created_date',true,g_source_file_error_flag);
      l_entity_load_row.inners_per_case := qu5_util.get_number('inners_per_case',5,0,true,g_source_file_error_flag);
      l_entity_load_row.is_new := qu5_util.get_boolean_as_number('is_new',true,g_source_file_error_flag);
      l_entity_load_row.is_priority := qu5_util.get_boolean_as_number('is_priority',true,g_source_file_error_flag);
      l_entity_load_row.list_price := qu5_util.get_number('list_price',18,4,true,g_source_file_error_flag);
      l_entity_load_row.unit_of_measure_id := qu5_util.get_number('unit_of_measure_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.unit_of_measure_id_desc := qu5_util.get_string('unit_of_measure_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.pack_size := qu5_util.get_number('pack_size',5,0,true,g_source_file_error_flag);
      l_entity_load_row.pack_desc := qu5_util.get_string('pack_desc',200,true,g_source_file_error_flag);
      l_entity_load_row.unit_size := qu5_util.get_number('unit_size',5,0,true,g_source_file_error_flag);
      l_entity_load_row.sku_code := qu5_util.get_string('sku_code',50,true,g_source_file_error_flag);
      l_entity_load_row.name := qu5_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.units_per_case := qu5_util.get_number('units_per_case',5,0,true,g_source_file_error_flag);
      l_entity_load_row.units_per_inner := qu5_util.get_number('units_per_inner',5,0,true,g_source_file_error_flag);
      l_entity_load_row.segment_hier_id := qu5_util.get_number('segment_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.brand_hier_id := qu5_util.get_number('brand_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.new_start := qu5_util.get_datetime('new_start',true,g_source_file_error_flag);
      l_entity_load_row.new_end := qu5_util.get_datetime('new_end',true,g_source_file_error_flag);
      l_entity_load_row.priority_start := qu5_util.get_datetime('priority_start',true,g_source_file_error_flag);
      l_entity_load_row.priority_end := qu5_util.get_datetime('priority_end',true,g_source_file_error_flag);
      l_entity_load_row.is_direct_orderable := qu5_util.get_boolean_as_number('is_direct_orderable',true,g_source_file_error_flag);
      l_entity_load_row.is_indirect_orderable := qu5_util.get_boolean_as_number('is_indirect_orderable',true,g_source_file_error_flag);
      l_entity_load_row.do_first_do_next_do_last_id := qu5_util.get_number('do_first_do_next_do_last_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.layers_per_pallet := qu5_util.get_number('layers_per_pallet',10,0,true,g_source_file_error_flag);
      l_entity_load_row.case_per_layer := qu5_util.get_number('case_per_layer',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pallet_case_count := qu5_util.get_number('pallet_case_count',10,0,true,g_source_file_error_flag);
      l_entity_load_row.unit_name := qu5_util.get_string('unit_name',50,true,g_source_file_error_flag);
      l_entity_load_row.inner_name := qu5_util.get_string('inner_name',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_prod_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_prod_hist
    select *
    from ods.qu5_prod_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_prod_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw07 for ods_app.qu5_qu5cdw07;

-- Grants
grant execute on ods_app.qu5_qu5cdw07 to lics_app;
