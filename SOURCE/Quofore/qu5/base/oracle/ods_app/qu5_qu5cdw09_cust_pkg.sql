...
create or replace package ods_app.qu5_qu5cdw09 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw09
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw09] Entity [Customer] Table [qu5_cust][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw09 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw09';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('outlet_ref_id',8);
    lics_inbound_utility.set_csv_definition('phone_no',9);
    lics_inbound_utility.set_csv_definition('web_address',10);
    lics_inbound_utility.set_csv_definition('channel_hier_id',11);
    lics_inbound_utility.set_csv_definition('banner_hier_id',12);
    lics_inbound_utility.set_csv_definition('grade_hier_id',13);
    lics_inbound_utility.set_csv_definition('region_hier_id',14);
    lics_inbound_utility.set_csv_definition('email_2',15);
    lics_inbound_utility.set_csv_definition('sap_no',16);
    lics_inbound_utility.set_csv_definition('is_influenceable',17);
    lics_inbound_utility.set_csv_definition('is_wholesaler',18);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,CreatedDate,CustomerName,CustomerRefId,Email,FaxNumber,OutletRefId,PhoneNumber,WebAddress,Channel_Hierarchy_Id,Banner_Hierarchy_Id,Grade_Hierarchy_Id,Region_Hierarchy_Id,Email2,SAPNumber,Influenceable,IsWholesaler';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_cust_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.cust_name := qu5_util.get_string('cust_name',50,true,g_source_file_error_flag);
      l_entity_load_row.cust_ref_id := qu5_util.get_string('cust_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.email := qu5_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.fax_no := qu5_util.get_string('fax_no',30,true,g_source_file_error_flag);
      l_entity_load_row.outlet_ref_id := qu5_util.get_string('outlet_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := qu5_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.web_address := qu5_util.get_string('web_address',60,true,g_source_file_error_flag);
      l_entity_load_row.channel_hier_id := qu5_util.get_number('channel_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.banner_hier_id := qu5_util.get_number('banner_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.grade_hier_id := qu5_util.get_number('grade_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.region_hier_id := qu5_util.get_number('region_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.email_2 := qu5_util.get_string('email_2',50,true,g_source_file_error_flag);
      l_entity_load_row.sap_no := qu5_util.get_string('sap_no',50,true,g_source_file_error_flag);
      l_entity_load_row.is_influenceable := qu5_util.get_boolean_as_number('is_influenceable',true,g_source_file_error_flag);
      l_entity_load_row.is_wholesaler := qu5_util.get_boolean_as_number('is_wholesaler',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_cust_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_cust_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu5_cust_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_cust_hist
    select *
    from ods.qu5_cust_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_cust_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_cust/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw09 for ods_app.qu5_qu5cdw09;

-- Grants
grant execute on ods_app.qu5_qu5cdw09 to lics_app;
