...
create or replace package ods_app.qu5_qu5cdw47 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw47
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw47] Entity [ActivityDetailShareOfShelf] Table [qu5_act_dtl_shelf_share][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw47 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw47';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailShareOfShelf';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('is_share_of_shelf_photo_sent',4);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,Activity_Id,HierarchyNode_Id,SOS_ShareOfShelfPhotos';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_act_dtl_shelf_share_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.is_share_of_shelf_photo_sent := qu5_util.get_number('is_share_of_shelf_photo_sent',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_act_dtl_shelf_share_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_act_dtl_shelf_share_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_act_dtl_shelf_share_load
    ...
    update ods.qu5_act_dtl_shelf_share_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_act_dtl_shelf_share_hist
    select *
    from ods.qu5_act_dtl_shelf_share_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_act_dtl_shelf_share_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_act_dtl_shelf_share/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw47 for ods_app.qu5_qu5cdw47;

-- Grants
grant execute on ods_app.qu5_qu5cdw47 to lics_app;
