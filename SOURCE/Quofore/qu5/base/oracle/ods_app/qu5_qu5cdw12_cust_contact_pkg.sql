...
create or replace package ods_app.qu5_qu5cdw12 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu5
    Owner    : ods_app
    Package  : qu5_qu5cdw12
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    Loader Package, Interface [qu5cdw12] Entity [CustomerContact] Table [qu5_cust_contact][_load/_hist]
...
create or replace package body ods_app.qu5_qu5cdw12 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu5_qu5cdw12';
  g_entity_name constant varchar2(64 char) := 'CustomerContact';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('has_authority_to_sign',4);
    lics_inbound_utility.set_csv_definition('cust_id',5);
    lics_inbound_utility.set_csv_definition('dept_id',6);
    lics_inbound_utility.set_csv_definition('dept_id_desc',7);
    lics_inbound_utility.set_csv_definition('email',8);
    lics_inbound_utility.set_csv_definition('first_name',9);
    lics_inbound_utility.set_csv_definition('full_name',10);
    lics_inbound_utility.set_csv_definition('last_name',11);
    lics_inbound_utility.set_csv_definition('middle_name',12);
    lics_inbound_utility.set_csv_definition('mobile_no',13);
    lics_inbound_utility.set_csv_definition('phone_no',14);
    lics_inbound_utility.set_csv_definition('photo_file_name',15);
    lics_inbound_utility.set_csv_definition('pos_id',16);
    lics_inbound_utility.set_csv_definition('pos_id_desc',17);
    lics_inbound_utility.set_csv_definition('pref_contact_method_id',18);
    lics_inbound_utility.set_csv_definition('pref_contact_method_id_desc',19);
    lics_inbound_utility.set_csv_definition('cust_id_desc',20);
    lics_inbound_utility.set_csv_definition('notes',21);
    lics_inbound_utility.set_csv_definition('is_primary',22);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,IsActive,CreatedDate,CanSign,Customer_Id,DepartmentId,DepartmentId_Description,Email,FirstName,FullName,LastName,MiddleName,MobileNumber,PhoneNumber,PhotoFileName,PositionId,PositionId_Description,PreferredContactMethodId,PreferredContactMethodId_Description,Customer_Id_Description,Notes,IsPrimary';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu5_cust_contact_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.has_authority_to_sign := qu5_util.get_boolean_as_number('has_authority_to_sign',true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu5_util.get_number('cust_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.dept_id := qu5_util.get_number('dept_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.dept_id_desc := qu5_util.get_string('dept_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.email := qu5_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.first_name := qu5_util.get_string('first_name',50,true,g_source_file_error_flag);
      l_entity_load_row.full_name := qu5_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.last_name := qu5_util.get_string('last_name',50,true,g_source_file_error_flag);
      l_entity_load_row.middle_name := qu5_util.get_string('middle_name',50,true,g_source_file_error_flag);
      l_entity_load_row.mobile_no := qu5_util.get_string('mobile_no',30,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := qu5_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.photo_file_name := qu5_util.get_string('photo_file_name',50,true,g_source_file_error_flag);
      l_entity_load_row.pos_id := qu5_util.get_number('pos_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pos_id_desc := qu5_util.get_string('pos_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.pref_contact_method_id := qu5_util.get_number('pref_contact_method_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pref_contact_method_id_desc := qu5_util.get_string('pref_contact_method_id_desc',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.is_primary := qu5_util.get_boolean_as_number('is_primary',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu5_cust_contact_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu5_cust_contact_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu5_cust_contact_load
    ...
    update ods.qu5_cust_contact_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu5_cust_contact_hist
    select *
    from ods.qu5_cust_contact_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu5_cust_contact_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu5_cust_contact/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu5_qu5cdw12 for ods_app.qu5_qu5cdw12;

-- Grants
grant execute on ods_app.qu5_qu5cdw12 to lics_app;
