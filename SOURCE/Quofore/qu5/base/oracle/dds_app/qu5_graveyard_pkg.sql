...
create or replace package dds_app.qu5_graveyard_pkg as
  /***************************************************************-****************
  ** Package Definition
  ********************************************************************************
    ...
    System   : qu5
    Owner    : dds_app
    Package  : qu5_graveyard_pkg
    Author   : [Auto-Generate]

    Description
    ------------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    View Package, Entity [Graveyard] Table [qu5_graveyard][_load/_hist]
  ...
  *******************************************************************************/
  ...
  -- Public : Type
  type qu5_graveyard_type is table of ods.qu5_graveyard_hist%rowtype;

  -- Public : Functions
  function view_at_date(p_date in date) return qu5_graveyard_type pipelined;
  function view_history return qu5_graveyard_type pipelined;

end qu5_graveyard_pkg;
/

create or replace package body dds_app.qu5_graveyard_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu5_graveyard_pkg';
  g_entity_name constant varchar2(64 char) := 'Graveyard';

  /*****************************************************************************
  ** Function : Return Entity : As at Date
  *****************************************************************************/
  function view_at_date(p_date in date) return qu5_graveyard_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      where (id,entity,q4x_batch_id) in (
        select id,entity,
  ...
  *****************************************************************************/
  function view_history return qu5_graveyard_type pipelined is
...
-- Synonyms
create or replace public synonym qu5_graveyard_pkg for dds_app.qu5_graveyard_pkg;

-- Grants
grant execute on dds_app.qu5_graveyard_pkg to qv_user, bo_user;
