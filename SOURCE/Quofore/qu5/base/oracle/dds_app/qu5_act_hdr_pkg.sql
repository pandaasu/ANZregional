...
create or replace package dds_app.qu5_act_hdr_pkg as
  /***************************************************************-****************
  ** Package Definition
  ********************************************************************************
    ...
    System   : qu5
    Owner    : dds_app
    Package  : qu5_act_hdr_pkg
    Author   : [Auto-Generate]

    Description
    ------------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    View Package, Entity [ActivityHeader] Table [qu5_act_hdr][_load/_hist]
  ...
  *******************************************************************************/
  ...
  -- Public : Type
  type qu5_act_hdr_type is table of ods.qu5_act_hdr_hist%rowtype;

  -- Public : Functions
  function view_at_date(p_date in date) return qu5_act_hdr_type pipelined;
  function view_at_start_date(p_date in date) return qu5_act_hdr_type pipelined;
  function view_history return qu5_act_hdr_type pipelined;

end qu5_act_hdr_pkg;
/

create or replace package body dds_app.qu5_act_hdr_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu5_act_hdr_pkg';
  g_entity_name constant varchar2(64 char) := 'ActivityHeader';

  /*****************************************************************************
  ** Function : Return Entity : As at Date
  *****************************************************************************/
  function view_at_date(p_date in date) return qu5_act_hdr_type pipelined is

  begin

    for l_entity in (
      select *
      from qu5_act_hdr_hist
  ...
  *****************************************************************************/
  function view_at_start_date(p_date in date) return qu5_act_hdr_type pipelined is
  ...
  *****************************************************************************/
  function view_history return qu5_act_hdr_type pipelined is
...
-- Synonyms
create or replace public synonym qu5_act_hdr_pkg for dds_app.qu5_act_hdr_pkg;

-- Grants
grant execute on dds_app.qu5_act_hdr_pkg to qv_user, bo_user;
