...
create or replace package dds_app.qu5_pos_terr_pkg as
  /***************************************************************-****************
  ** Package Definition
  ********************************************************************************
    ...
    System   : qu5
    Owner    : dds_app
    Package  : qu5_pos_terr_pkg
    Author   : [Auto-Generate]

    Description
    ------------------------------------------------------------------------------
    [qu5] Quofore - Mars New Zealand
    View Package, Entity [PositionTerritory] Table [qu5_pos_terr][_load/_hist]
  ...
  *******************************************************************************/
  ...
  -- Public : Type
  type qu5_pos_terr_type is table of ods.qu5_pos_terr_hist%rowtype;

  -- Public : Functions
  function view_at_date(p_date in date) return qu5_pos_terr_type pipelined;
  function view_history return qu5_pos_terr_type pipelined;

end qu5_pos_terr_pkg;
/

create or replace package body dds_app.qu5_pos_terr_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu5_pos_terr_pkg';
  g_entity_name constant varchar2(64 char) := 'PositionTerritory';

  /*****************************************************************************
  ** Function : Return Entity : As at Date
  *****************************************************************************/
  function view_at_date(p_date in date) return qu5_pos_terr_type pipelined is

  begin

    for l_entity in (
      select *
      from qu5_pos_terr_hist
  ...
  *****************************************************************************/
  function view_history return qu5_pos_terr_type pipelined is
...
-- Synonyms
create or replace public synonym qu5_pos_terr_pkg for dds_app.qu5_pos_terr_pkg;

-- Grants
grant execute on dds_app.qu5_pos_terr_pkg to qv_user, bo_user;
