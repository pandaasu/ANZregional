...
create or replace package ods_app.qu4_qu4cdw28 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw28
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw28] Entity [ResponseOption] Table [qu4_response_opt][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw28 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw28';
  g_entity_name constant varchar2(64 char) := 'ResponseOption';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_question_id',2);
    lics_inbound_utility.set_csv_definition('sort_ord',3);
    lics_inbound_utility.set_csv_definition('enable_set',4);
    lics_inbound_utility.set_csv_definition('response_label',5);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,SurveyQuestion_ID,SortOrder,EnableSet,ResponseLabel';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_response_opt_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.sort_ord := qu4_util.get_number('sort_ord',3,0,true,g_source_file_error_flag);
      l_entity_load_row.enable_set := qu4_util.get_number('enable_set',3,0,true,g_source_file_error_flag);
      l_entity_load_row.response_label := qu4_util.get_string('response_label',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_response_opt_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_response_opt_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_response_opt_load
    ...
    update ods.qu4_response_opt_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_response_opt_hist
    select *
    from ods.qu4_response_opt_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_response_opt_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_response_opt/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw28 for ods_app.qu4_qu4cdw28;

-- Grants
grant execute on ods_app.qu4_qu4cdw28 to lics_app;
