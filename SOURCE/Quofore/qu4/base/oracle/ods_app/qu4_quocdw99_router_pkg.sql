...
create or replace package ods_app.qu4_qu4cdw99 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
...
create or replace package body ods_app.qu4_qu4cdw99 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw99';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    g_prefixed_file_name := 'qu4_'||lics_inbound_processor.callback_file_name;
    ...
    begin
      g_entity_name := qu4_util.get_file_entity_name(g_original_file_name);
    exception
      when others then
    ...
    begin
      g_source_id := qu4_util.get_interface_source_id(g_source_interface_name);
    exception
      when others then
    ...
    begin
      g_target_interface_name := qu4_util.get_entity_interface_name(g_entity_name);
    exception
      when others then
        lics_logging.write_log(substr('Cannot Extract Source Id : '||SQLERRM, 1, 4000));
        lics_logging.end_log;
...
-- Synonyms
create or replace public synonym qu4_qu4cdw99 for ods_app.qu4_qu4cdw99;

-- Grants
grant execute on ods_app.qu4_qu4cdw99 to lics_app;
