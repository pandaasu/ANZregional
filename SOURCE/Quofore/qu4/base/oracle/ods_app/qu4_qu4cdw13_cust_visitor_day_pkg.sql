...
create or replace package ods_app.qu4_qu4cdw13 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw13
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw13] Entity [CustomerVisitorDay] Table [qu4_cust_visitor_day][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw13 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw13';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('day_of_week',2);
    lics_inbound_utility.set_csv_definition('is_do_not_call',3);
    lics_inbound_utility.set_csv_definition('is_appointment_reqd',4);
    lics_inbound_utility.set_csv_definition('visit_from',5);
    lics_inbound_utility.set_csv_definition('visit_to',6);
    lics_inbound_utility.set_csv_definition('cust_visitor_id',7);
    lics_inbound_utility.set_csv_definition('cust_id',8);
    lics_inbound_utility.set_csv_definition('days_between_visit',9);
    lics_inbound_utility.set_csv_definition('visit_duration',10);
    lics_inbound_utility.set_csv_definition('notes',11);
    lics_inbound_utility.set_csv_definition('role_id',12);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,DayOfWeek,IsDonotCall,IsAppointmentReqd,VisitFrom,VisitTo,CustomerVisitor_ID,Customer_Id,DaysBetweenVisit,VisitDuration,Notes,Role_Id';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_cust_visitor_day_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.day_of_week := qu4_util.get_number('day_of_week',3,0,true,g_source_file_error_flag);
      l_entity_load_row.is_do_not_call := qu4_util.get_boolean_as_number('is_do_not_call',true,g_source_file_error_flag);
      l_entity_load_row.is_appointment_reqd := qu4_util.get_boolean_as_number('is_appointment_reqd',true,g_source_file_error_flag);
      l_entity_load_row.visit_from := qu4_util.get_time('visit_from',true,g_source_file_error_flag);
      l_entity_load_row.visit_to := qu4_util.get_time('visit_to',true,g_source_file_error_flag);
      l_entity_load_row.cust_visitor_id := qu4_util.get_number('cust_visitor_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu4_util.get_number('cust_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.days_between_visit := qu4_util.get_number('days_between_visit',3,0,true,g_source_file_error_flag);
      l_entity_load_row.visit_duration := qu4_util.get_time('visit_duration',true,g_source_file_error_flag);
      l_entity_load_row.notes := qu4_util.get_string('notes',200,true,g_source_file_error_flag);
      l_entity_load_row.role_id := qu4_util.get_number('role_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_cust_visitor_day_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_cust_visitor_day_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_cust_visitor_day_load
    ...
    update ods.qu4_cust_visitor_day_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_cust_visitor_day_hist
    select *
    from ods.qu4_cust_visitor_day_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_cust_visitor_day_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_cust_visitor_day/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw13 for ods_app.qu4_qu4cdw13;

-- Grants
grant execute on ods_app.qu4_qu4cdw13 to lics_app;
