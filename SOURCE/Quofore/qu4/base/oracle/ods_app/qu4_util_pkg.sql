...
create or replace package ods_app.qu4_util as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
  ...
  *****************************************************************************/
  ...
  function get_interface_source_id(p_interface_name in varchar2) return number;
  function get_source_desc(p_source_id in number) return varchar2;
...
create or replace package body ods_app.qu4_util as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_util';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    select qu4_load_seq.nextval into l_next_seq from dual;
    return l_next_seq; -- Normal Return

  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_next_load_seq] : Failed to Get Next Sequence [ods.qu4_load_seq.nextval] : '||SQLERRM, 1, 4000));
  ...
  *****************************************************************************/
  ...
  begin
    ...
    l_set_value := trim(lics_setting_configuration.retrieve_setting(qu4_constants.setting_group,l_set_code));
    if l_set_value is null then
      raise_application_error(-20000, 'Email Default : Group ['||qu4_constants.setting_group||'] Code ['||l_set_code||'] : Not Configured in Table [lics.lics_setting]');
    end if;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_entity_interface_name] : Entity ['||upper(p_entity_name)||'] : Table [ods.qu4_interface_list] : '||SQLERRM, 1, 4000));
  ...
  *****************************************************************************/
  ...
  begin
    ...
    if l_entity_interface_name is null then
      raise_application_error(-20000, 'Entity ['||upper(p_entity_name)||'] Interface Not Configured in Table [ods.qu4_interface_list]');
    end if;

    -- Return Error for Not Configured Source
    if get_interface_source_id(p_interface_name) is null then
      raise_application_error(-20000, 'Source for Interface ['||p_interface_name||'] : Group ['||qu4_constants.setting_group||'] : Not Configured in Table [lics.lics_setting]');
    end if;
    ...
    if upper(substr(p_interface_name,1,length(l_entity_interface_name))) != upper(l_entity_interface_name) then
      raise_application_error(-20000, 'Invalid Interface for Entity, Value ['||upper(p_interface_name)||'] : Expected ['||upper(l_entity_interface_name)||'.{Source Id}]');
    end if;
  ...
  /*****************************************************************************
  ** PUBLIC Function : Get Interface Source Id
  *****************************************************************************/
  function get_interface_source_id(p_interface_name in varchar2) return number as
  ...
  begin
    ...
    l_source_id := null;
    begin
      l_source_id := to_number(substr(p_interface_name,3,1));
    exception
      when others then
        raise_application_error(-20000, 'Unable to Extract Source Id ['||p_interface_name||'] : Expected Format [{Interface}.{Source Id:9999}]');
    end;

    -- Return Error for Not Configured Source Id
    if get_source_desc(l_source_id) is null then
      raise_application_error(-20000, 'Source ['||qu4_constants.setting_group||']['||l_source_id||'] : Not Configured in Table [lics.lics_setting]');
    end if;

    return l_source_id; -- Normal Return

  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_interface_source_id] : '||SQLERRM, 1, 4000));

  end get_interface_source_id;

  /*****************************************************************************
  ** PUBLIC Function : Get Source Description
  *****************************************************************************/
  function get_source_desc(p_source_id in number) return varchar2 as
  ...
  begin
    ...
    -- Retrieve Source Desc
    l_set_code := 'SOURCE:'||p_source_id;
    l_set_value := trim(lics_setting_configuration.retrieve_setting(qu4_constants.setting_group,l_set_code));
    if l_set_value is null then
      raise_application_error(-20000, 'Source ['||qu4_constants.setting_group||']['||l_set_code||'] Not Configured in Table [lics.lics_setting]');
    end if;
  ...
  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_source_desc] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_util for ods_app.qu4_util;

-- Grants
grant execute on ods_app.qu4_util to lics_app, qv_user, bo_user;
