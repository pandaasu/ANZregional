...
create or replace package ods_app.qu4_qu4cdw22 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw22
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw22] Entity [OrderDetail] Table [qu4_ord_dtl][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw22 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw22';
  g_entity_name constant varchar2(64 char) := 'OrderDetail';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('bonus_qty',2);
    lics_inbound_utility.set_csv_definition('discount_amount',3);
    lics_inbound_utility.set_csv_definition('ord_header_id',4);
    lics_inbound_utility.set_csv_definition('ord_qty',5);
    lics_inbound_utility.set_csv_definition('original_ord_qty',6);
    lics_inbound_utility.set_csv_definition('per_unit_price',7);
    lics_inbound_utility.set_csv_definition('prod_id',8);
    lics_inbound_utility.set_csv_definition('tax_percent',9);
    lics_inbound_utility.set_csv_definition('total_line_amount',10);
    lics_inbound_utility.set_csv_definition('prod_id_desc',11);
    lics_inbound_utility.set_csv_definition('line_discount_percent',12);
    lics_inbound_utility.set_csv_definition('credit_reason',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,BonusQuantity,DiscountAmount,OrderHeader_Id,OrderQuantity,OriginalOrderQuantity,PerUnitPrice,Product_Id,TaxPercent,TotalLineAmount,Product_ID_Description,LineDiscountPercent,CreditReason';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_ord_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.bonus_qty := qu4_util.get_number('bonus_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.discount_amount := qu4_util.get_number('discount_amount',18,4,true,g_source_file_error_flag);
      l_entity_load_row.ord_header_id := qu4_util.get_number('ord_header_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.ord_qty := qu4_util.get_number('ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.original_ord_qty := qu4_util.get_number('original_ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.per_unit_price := qu4_util.get_number('per_unit_price',18,4,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu4_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.tax_percent := qu4_util.get_number('tax_percent',9,6,true,g_source_file_error_flag);
      l_entity_load_row.total_line_amount := qu4_util.get_number('total_line_amount',18,4,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_desc := qu4_util.get_string('prod_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.line_discount_percent := qu4_util.get_number('line_discount_percent',18,4,true,g_source_file_error_flag);
      l_entity_load_row.credit_reason := qu4_util.get_string('credit_reason',200,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_ord_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_ord_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu4_ord_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_ord_dtl_hist
    select *
    from ods.qu4_ord_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_ord_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_ord_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw22 for ods_app.qu4_qu4cdw22;

-- Grants
grant execute on ods_app.qu4_qu4cdw22 to lics_app;
