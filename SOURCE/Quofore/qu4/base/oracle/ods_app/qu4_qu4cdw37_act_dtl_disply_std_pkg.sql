...
create or replace package ods_app.qu4_qu4cdw37 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw37
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw37] Entity [ActivityDetailDisplayTradeStd] Table [qu4_act_dtl_disply_std][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw37 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw37';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailDisplayTradeStd';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('hier_node_id',3);
    lics_inbound_utility.set_csv_definition('display_trade_is_to_standard',4);
    lics_inbound_utility.set_csv_definition('display_trade_no_of_displays',5);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,HierarchyNode_Id,DisplayTradeStd_IsToStandard,DisplayTradeStd_NoOfDisplays,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_act_dtl_disply_std_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.act_id := qu4_util.get_number('act_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.hier_node_id := qu4_util.get_number('hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.display_trade_is_to_standard := qu4_util.get_number('display_trade_is_to_standard',10,0,true,g_source_file_error_flag);
      l_entity_load_row.display_trade_no_of_displays := qu4_util.get_number('display_trade_no_of_displays',10,0,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_act_dtl_disply_std_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_act_dtl_disply_std_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_act_dtl_disply_std_load
    ...
    update ods.qu4_act_dtl_disply_std_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_act_dtl_disply_std_hist
    select *
    from ods.qu4_act_dtl_disply_std_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_act_dtl_disply_std_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_act_dtl_disply_std/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw37 for ods_app.qu4_qu4cdw37;

-- Grants
grant execute on ods_app.qu4_qu4cdw37 to lics_app;
