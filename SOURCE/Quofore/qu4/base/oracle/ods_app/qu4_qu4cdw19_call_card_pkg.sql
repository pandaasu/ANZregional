...
create or replace package ods_app.qu4_qu4cdw19 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw19
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw19] Entity [CallCard] Table [qu4_call_card][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw19 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw19';
  g_entity_name constant varchar2(64 char) := 'CallCard';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('appointment_id',3);
    lics_inbound_utility.set_csv_definition('call_type_id',4);
    lics_inbound_utility.set_csv_definition('call_type_id_desc',5);
    lics_inbound_utility.set_csv_definition('cust_id',6);
    lics_inbound_utility.set_csv_definition('duration',7);
    lics_inbound_utility.set_csv_definition('pos_id',8);
    lics_inbound_utility.set_csv_definition('pos_id_lookup',9);
    lics_inbound_utility.set_csv_definition('rep_id',10);
    lics_inbound_utility.set_csv_definition('start_date',11);
    lics_inbound_utility.set_csv_definition('cust_id_desc',12);
    lics_inbound_utility.set_csv_definition('full_name',13);
    lics_inbound_utility.set_csv_definition('terr_id',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,CreatedDate,Appointment_Id,CallTypeId,CallTypeId_Description,Customer_Id,Duration,Position_Id,Position_Id_Lookup,Rep_Id,StartDate,Customer_ID_Description,FullName,Territory_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_call_card_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.appointment_id := qu4_util.get_number('appointment_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id := qu4_util.get_number('call_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id_desc := qu4_util.get_string('call_type_id_desc',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.pos_id := qu4_util.get_number('pos_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.pos_id_lookup := qu4_util.get_string('pos_id_lookup',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.terr_id := qu4_util.get_number('terr_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_call_card_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_call_card_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu4_call_card_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_call_card_hist
    select *
    from ods.qu4_call_card_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_call_card_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_call_card/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw19 for ods_app.qu4_qu4cdw19;

-- Grants
grant execute on ods_app.qu4_qu4cdw19 to lics_app;
