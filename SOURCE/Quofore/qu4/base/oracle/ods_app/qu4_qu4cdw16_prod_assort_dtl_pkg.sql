...
create or replace package ods_app.qu4_qu4cdw16 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw16
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw16] Entity [ProductAssortmentDetail] Table [qu4_prod_assort_dtl][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw16 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw16';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_prod_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_dtl_id := qu4_util.get_number('assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu4_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_hier_node_id := qu4_util.get_number('prod_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.priority_assort_dtl_id := qu4_util.get_number('priority_assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.effective_from := qu4_util.get_datetime('effective_from',true,g_source_file_error_flag);
      l_entity_load_row.effective_to := qu4_util.get_datetime('effective_to',true,g_source_file_error_flag);
      l_entity_load_row.priority_from := qu4_util.get_datetime('priority_from',true,g_source_file_error_flag);
      l_entity_load_row.priority_to := qu4_util.get_datetime('priority_to',true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_id_lookup := qu4_util.get_number('assort_dtl_id_lookup',10,0,true,g_source_file_error_flag);
      l_entity_load_row.id_lookup := qu4_util.get_number('id_lookup',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_lookup := qu4_util.get_number('prod_id_lookup',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_prod_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_prod_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_prod_assort_dtl_load
    ...
    update ods.qu4_prod_assort_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_prod_assort_dtl_hist
    select *
    from ods.qu4_prod_assort_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_prod_assort_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_prod_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw16 for ods_app.qu4_qu4cdw16;

-- Grants
grant execute on ods_app.qu4_qu4cdw16 to lics_app;
