...
create or replace package ods_app.qu4_qu4cdw17 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw17
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw17] Entity [AuthorisedListProduct] Table [qu4_auth_list_prod][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw17 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw17';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('authorised_list_id',2);
    lics_inbound_utility.set_csv_definition('cust_id',3);
    lics_inbound_utility.set_csv_definition('cust_id_lookup',4);
    lics_inbound_utility.set_csv_definition('is_include',5);
    lics_inbound_utility.set_csv_definition('effective_from',6);
    lics_inbound_utility.set_csv_definition('prod_id',7);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',8);
    lics_inbound_utility.set_csv_definition('prod_id_desc',9);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,AuthorisedList_ID,Customer_ID,customer_id_lookup,IsInclude,EffectiveFrom,Product_ID,product_id_lookup,product_id_description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_auth_list_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.authorised_list_id := qu4_util.get_number('authorised_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu4_util.get_number('cust_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id_lookup := qu4_util.get_number('cust_id_lookup',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_include := qu4_util.get_boolean_as_number('is_include',true,g_source_file_error_flag);
      l_entity_load_row.effective_from := qu4_util.get_date('effective_from',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_auth_list_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_auth_list_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_auth_list_prod_load
    ...
    update ods.qu4_auth_list_prod_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_auth_list_prod_hist
    select *
    from ods.qu4_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_auth_list_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw17 for ods_app.qu4_qu4cdw17;

-- Grants
grant execute on ods_app.qu4_qu4cdw17 to lics_app;
