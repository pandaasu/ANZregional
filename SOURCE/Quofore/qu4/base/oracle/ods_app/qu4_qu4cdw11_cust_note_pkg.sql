...
create or replace package ods_app.qu4_qu4cdw11 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw11
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw11] Entity [CustomerNote] Table [qu4_cust_note][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw11 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw11';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_cust_note_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.note_text := qu4_util.get_string('note_text',200,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_cust_note_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_cust_note_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu4_cust_note_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_cust_note_hist
    select *
    from ods.qu4_cust_note_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_cust_note_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_cust_note/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw11 for ods_app.qu4_qu4cdw11;

-- Grants
grant execute on ods_app.qu4_qu4cdw11 to lics_app;
