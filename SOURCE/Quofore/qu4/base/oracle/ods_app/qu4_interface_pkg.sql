...
create or replace package ods_app.qu4_interface as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System  : qu4
    Owner   : ods_app
    Package : qu4_interface
    Author  : Mal Chambeyron

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Interface Control Package .. Maintains the [qu4_interface_hdr] table

    YYYY-MM-DD  Author                Description
    ----------  --------------------  ------------------------------------------
    2013-02-19  Mal Chambeyron        Created
    2013-03-05  Mal Chambeyron        Add Check Batches
    2014-05-15  Mal Chambeyron        Remove source_id from tables / calls
...
create or replace package body ods_app.qu4_interface as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_interface';
  g_package_desc constant varchar2(64 char) := 'Interface Header [ods.qu4_interface_hdr]';
  ...
  *****************************************************************************/
  ...
  begin
    -- Set Key Description
    l_key_desc := 'Source/Batch/Entity ['||qu4_constants.source_id||']['||p_batch_id||']['||upper(p_entity_name)||']';
    ...
    l_load_seq := qu4_util.get_next_load_seq;

    -- Create Interface Header
    begin

      insert into qu4_interface_hdr (
        q4x_load_seq,
        q4x_create_user,
        q4x_create_time,
        q4x_modify_user,
        ...
        qu4_constants.status_started, -- q4x_status
  ...
  *****************************************************************************/
  ...
  begin
    -- Set Key Description
    l_key_desc := 'Load/Source/Batch/Entity ['||p_load_seq||']['||qu4_constants.source_id||']['||p_batch_id||']['||upper(p_entity_name)||']';
    ...
    if l_status <> qu4_constants.status_started then
        raise_application_error(-20000, g_package_desc||' for '||l_key_desc||' Invalid Status ['||l_status||'] Expected ['||qu4_constants.status_started||']');
    end if;

    -- Update Interface Header
    begin

      update qu4_interface_hdr
      set
        q4x_modify_user = user,
        q4x_modify_time = sysdate,
        q4x_status = qu4_constants.status_loaded,
  ...
  *****************************************************************************/
  ...
  begin
    -- Set Key Description
    l_key_desc := 'Batch/Entity ['||qu4_constants.source_id||']['||p_batch_id||']';
    ...
    begin
      ...
      set
        q4x_modify_user = user,
        q4x_modify_time = sysdate,
        q4x_status = qu4_constants.status_processed
      where q4x_batch_id = p_batch_id;
...
-- Synonyms
create or replace public synonym qu4_interface for ods_app.qu4_interface;

-- Grants
grant execute on ods_app.qu4_interface to lics_app;
