...
create or replace package ods_app.qu4_qu4cdw35 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw35
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw35] Entity [ActivityDetailDistribution] Table [qu4_act_dtl_dist][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw35 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw35';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailDistribution';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_of_facings',3);
    lics_inbound_utility.set_csv_definition('is_in_distribution',4);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NoOfFacings,IsInDistribution,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_act_dtl_dist_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_of_facings := qu4_util.get_number('no_of_facings',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_in_distribution := qu4_util.get_boolean_as_number('is_in_distribution',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_act_dtl_dist_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_act_dtl_dist_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_act_dtl_dist_load
    ...
    update ods.qu4_act_dtl_dist_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_act_dtl_dist_hist
    select *
    from ods.qu4_act_dtl_dist_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_act_dtl_dist_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_act_dtl_dist/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw35 for ods_app.qu4_qu4cdw35;

-- Grants
grant execute on ods_app.qu4_qu4cdw35 to lics_app;
