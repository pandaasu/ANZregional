...
create or replace package ods_app.qu4_qu4cdw18 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw18
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw18] Entity [Appointment] Table [qu4_appoint][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw18 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw18';
  g_entity_name constant varchar2(64 char) := 'Appointment';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('call_type_id_desc',3);
    lics_inbound_utility.set_csv_definition('cust_id',4);
    lics_inbound_utility.set_csv_definition('pos_id',5);
    lics_inbound_utility.set_csv_definition('scheduled_date',6);
    lics_inbound_utility.set_csv_definition('sequence',7);
    lics_inbound_utility.set_csv_definition('start_time',8);
    lics_inbound_utility.set_csv_definition('cust_id_desc',9);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,CallTypeId,CallTypeId_Description,Customer_Id,Position_Id,ScheduledDate,Sequence,StartTime,Customer_ID_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_appoint_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.scheduled_date := qu4_util.get_date('scheduled_date',true,g_source_file_error_flag);
      l_entity_load_row.sequence := qu4_util.get_number('sequence',3,0,true,g_source_file_error_flag);
      l_entity_load_row.start_time := qu4_util.get_time('start_time',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_appoint_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_appoint_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu4_appoint_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_appoint_hist
    select *
    from ods.qu4_appoint_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_appoint_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_appoint/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw18 for ods_app.qu4_qu4cdw18;

-- Grants
grant execute on ods_app.qu4_qu4cdw18 to lics_app;
