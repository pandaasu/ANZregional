...
create or replace package ods_app.qu4_qu4cdw12 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw12
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw12] Entity [CustomerContact] Table [qu4_cust_contact][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw12 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw12';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('can_sign',4);
    lics_inbound_utility.set_csv_definition('cust_id',5);
    lics_inbound_utility.set_csv_definition('department_id',6);
    lics_inbound_utility.set_csv_definition('department_id_desc',7);
    ...
    lics_inbound_utility.set_csv_definition('positionid_desc',17);
    lics_inbound_utility.set_csv_definition('preferred_contact_method_id',18);
    lics_inbound_utility.set_csv_definition('preferred_contact_meth_id_desc',19);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_cust_contact_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.can_sign := qu4_util.get_boolean_as_number('can_sign',true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu4_util.get_number('cust_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.department_id := qu4_util.get_number('department_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.department_id_desc := qu4_util.get_string('department_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.email := qu4_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.first_name := qu4_util.get_string('first_name',50,true,g_source_file_error_flag);
      l_entity_load_row.full_name := qu4_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.last_name := qu4_util.get_string('last_name',50,true,g_source_file_error_flag);
      l_entity_load_row.middle_name := qu4_util.get_string('middle_name',50,true,g_source_file_error_flag);
      l_entity_load_row.mobile_no := qu4_util.get_string('mobile_no',30,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := qu4_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.photo_file_name := qu4_util.get_string('photo_file_name',50,true,g_source_file_error_flag);
      l_entity_load_row.pos_id := qu4_util.get_number('pos_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.positionid_desc := qu4_util.get_string('positionid_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.preferred_contact_method_id := qu4_util.get_number('preferred_contact_method_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.preferred_contact_meth_id_desc := qu4_util.get_string('preferred_contact_meth_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_cust_contact_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_cust_contact_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_cust_contact_load
    ...
    update ods.qu4_cust_contact_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_cust_contact_hist
    select *
    from ods.qu4_cust_contact_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_cust_contact_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_cust_contact/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw12 for ods_app.qu4_qu4cdw12;

-- Grants
grant execute on ods_app.qu4_qu4cdw12 to lics_app;
