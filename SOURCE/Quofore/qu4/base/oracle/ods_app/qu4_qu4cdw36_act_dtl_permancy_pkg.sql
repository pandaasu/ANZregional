...
create or replace package ods_app.qu4_qu4cdw36 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw36
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw36] Entity [ActivityDetailPermanency] Table [qu4_act_dtl_permancy][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw36 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw36';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailPermanency';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('permancy_qty',4);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,HierarchyNode_Id,Permanency_Quantity,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_act_dtl_permancy_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.permancy_qty := qu4_util.get_number('permancy_qty',10,0,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu4_act_dtl_permancy_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_act_dtl_permancy_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_act_dtl_permancy_load
    ...
    update ods.qu4_act_dtl_permancy_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_act_dtl_permancy_hist
    select *
    from ods.qu4_act_dtl_permancy_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_act_dtl_permancy_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_act_dtl_permancy/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw36 for ods_app.qu4_qu4cdw36;

-- Grants
grant execute on ods_app.qu4_qu4cdw36 to lics_app;
