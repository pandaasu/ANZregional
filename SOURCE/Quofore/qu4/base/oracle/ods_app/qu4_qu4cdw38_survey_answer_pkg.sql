...
create or replace package ods_app.qu4_qu4cdw38 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu4
    Owner    : ods_app
    Package  : qu4_qu4cdw38
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    Loader Package, Interface [qu4cdw38] Entity [SurveyAnswer] Table [qu4_survey_answer][_load/_hist]
...
create or replace package body ods_app.qu4_qu4cdw38 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu4_qu4cdw38';
  g_entity_name constant varchar2(64 char) := 'SurveyAnswer';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_response_id',2);
    lics_inbound_utility.set_csv_definition('survey_id',3);
    lics_inbound_utility.set_csv_definition('survey_question_id',4);
    lics_inbound_utility.set_csv_definition('response_option_id',5);
    lics_inbound_utility.set_csv_definition('free_form_response',6);
    lics_inbound_utility.set_csv_definition('rep_id',7);
    lics_inbound_utility.set_csv_definition('call_card_id',8);
    lics_inbound_utility.set_csv_definition('survey_response_created_date',9);
    lics_inbound_utility.set_csv_definition('task_created_date',10);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,SurveyResponse_ID,Survey_ID,SurveyQuestion_ID,ResponseOption_ID,FreeFormResponse,Rep_ID,Callcard_ID,SurveyResponseCreatedDate,TaskCreatedDate';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu4_survey_answer_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_response_id := qu4_util.get_number('survey_response_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_id := qu4_util.get_number('survey_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.survey_question_id := qu4_util.get_number('survey_question_id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.response_option_id := qu4_util.get_number('response_option_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.free_form_response := qu4_util.get_string('free_form_response',4000,true,g_source_file_error_flag);
      ...
      l_entity_load_row.survey_response_created_date := qu4_util.get_datetime('survey_response_created_date',true,g_source_file_error_flag);
      l_entity_load_row.task_created_date := qu4_util.get_datetime('task_created_date',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu4_survey_answer_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu4_survey_answer_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu4_survey_answer_load
    ...
    update ods.qu4_survey_answer_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu4_survey_answer_hist
    select *
    from ods.qu4_survey_answer_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu4_survey_answer_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu4_survey_answer/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu4_qu4cdw38 for ods_app.qu4_qu4cdw38;

-- Grants
grant execute on ods_app.qu4_qu4cdw38 to lics_app;
