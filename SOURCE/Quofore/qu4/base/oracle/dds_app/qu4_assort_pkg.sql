...
create or replace package dds_app.qu4_assort_pkg as
  /*****************************************************************************
  ** Table Definition
  ******************************************************************************
    ...
    2014-05-28  Mal Chambeyron        Cleanup source_id
  ...
  *****************************************************************************/
  ...
  type qu4_hier_type is table of qu4_hier_rec;

  -- Public : Type : Assortment Detail -----------------------------------------
  type qu4_assort_dtl_rec is record (
    report_date date,
    --
  ...
  type qu4_assort_dtl_type is table of qu4_assort_dtl_rec;

  -- Public : Type : Customer Assortment ---------------------------------------
  type qu4_cust_assort_rec is record (
    report_date date,
    --
    assort_id number(10,0),
    assort_desc varchar2(50 char),
  ...
  type qu4_cust_assort_type is table of qu4_cust_assort_rec;

  -- Public : Type : Product Assortment ---------------------------------------
  type qu4_prod_assort_rec is record (
    report_date date,
    --
    assort_id number(10,0),
    assort_desc varchar2(50 char),
  ...
  type qu4_prod_assort_type is table of qu4_prod_assort_rec;

-- Public : Type : Customer / Procuct Assortment -------------------------------
  type qu4_cust_prod_assort_rec is record (
    report_date date,
    --
    cust_id number(10,0),
    prod_id number(10,0),
  ...
  type qu4_cust_prod_assort_type is table of qu4_cust_prod_assort_rec;

  -- Public : Type : Master Assortment -----------------------------------------
  type qu4_master_assort_rec is record (
    report_date date,
    --
    cust_id number(10,0),
    prod_id number(10,0),
  ...
  type qu4_master_assort_type is table of qu4_master_assort_rec;

  -- Public : Functions --------------------------------------------------------
  function cust_hier_view_at_date(p_at_date in date) return qu4_hier_type pipelined;
  function prod_hier_view_at_date(p_at_date in date) return qu4_hier_type pipelined;

  function segment_hier_view_at_date(p_at_date in date) return qu4_assort_dtl_type pipelined;
  function segment_up_hier_view_at_date(p_at_date in date) return qu4_assort_dtl_type pipelined;

  function cust_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_assort_type pipelined;
  function prod_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_prod_assort_type pipelined;
  function core_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_prod_assort_type pipelined;
  function cust_prod_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_prod_assort_type pipelined;
  function cust_core_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_prod_assort_type pipelined;

  function master_assort_view_at_date(p_at_date in date) return qu4_master_assort_type pipelined;

end qu4_assort_pkg;
/

create or replace package body qu4_assort_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu4_assort_pkg';
  g_source_id constant number(4) := qu4_constants.source_id;

  /*****************************************************************************
  ** Function : Customer Hierarchy, as at Date
  *****************************************************************************/
  function cust_hier_view_at_date(p_at_date in date) return qu4_hier_type pipelined is
      ...
      from (
        ...
        from (
          ...
          select q4x_batch_id batch_id,
            ...
            cust_name node_desc,
            'Direct:'||channel_hier_id parent_node,
            'Direct' parent_node_type,
            channel_hier_id parent_node_id,
            999 node_level
          from table(qu4_cust_pkg.view_at_date(p_at_date))
          ...
          select q4x_batch_id batch_id,
            ...
            'Customer-Channel:Hierarchy' node_hier_type,
            --
            'Customer:'||id node,
            'Customer' node_type,
            id node_id,
          ...
          union all
          --
          -- Customer-Store-Size > Hierarchy
          select q4x_batch_id batch_id,
            2 node_hier_priority,
            'Customer-Store-Size:Hierarchy' node_hier_type,
            --
            'Customer:'||id node,
            'Customer' node_type,
            id node_id,
            ...
            cust_ref_id node_code,
            cust_name node_desc,
            'Hierarchy:'||store_size_hier_id parent_node,
            'Hierarchy' parent_node_type,
            channel_hier_id parent_node_id,
            998 node_level
          from table(qu4_cust_pkg.view_at_date(p_at_date))
          where is_active = 1 -- active
          and store_size_hier_id is not null
          --
          union all
          --
          -- Hierarchy
          ...
          from table(qu4_hier_pkg.view_at_date(p_at_date))
          where is_active = 1 -- active
  ...
  *****************************************************************************/
  function prod_hier_view_at_date(p_at_date in date) return qu4_hier_type pipelined is
      ...
      from (
        ...
        from (
          ...
          select q4x_batch_id batch_id,
            ...
            999 node_level
          from table(qu4_prod_pkg.view_at_date(p_at_date))
          where is_active = 1 -- active
          --
          union all
          --
  ...
  *****************************************************************************/
  function segment_hier_view_at_date(p_at_date in date) return qu4_assort_dtl_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      from table(qu4_assort_dtl_pkg.view_at_date(p_at_date))
  ...
  *****************************************************************************/
  function segment_up_hier_view_at_date(p_at_date in date) return qu4_assort_dtl_type pipelined is
  ...
  *****************************************************************************/
  function cust_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      select p_at_date report_date,
        ...
        --
      from table(qu4_assort_pkg.cust_hier_view_at_date(p_at_date)) cust_hier,
        table(qu4_cust_assort_dtl_pkg.view_at_date(p_at_date)) cust_assort_dtl,
        table(qu4_assort_pkg.segment_hier_view_at_date(p_at_date)) segment_hier
  ...
  *****************************************************************************/
  function prod_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_prod_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      select p_at_date report_date,
        ...
        segment_hier.node_path segment_hier_path
        --
      from table(qu4_assort_pkg.prod_hier_view_at_date(p_at_date)) prod_hier,
        table(qu4_prod_assort_dtl_pkg.view_at_date(p_at_date)) prod_assort_dtl,
        table(qu4_assort_pkg.segment_hier_view_at_date(p_at_date)) segment_hier
  ...
  *****************************************************************************/
  function core_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_prod_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      from table(qu4_assort_pkg.prod_hier_view_at_date(p_at_date)) prod_hier,
        table(qu4_prod_assort_dtl_pkg.view_at_date(p_at_date)) prod_assort_dtl,
        table(qu4_assort_pkg.segment_up_hier_view_at_date(p_at_date)) core_hier
      where prod_hier.node_type = 'Product'
      and prod_hier.root_node = decode(prod_assort_dtl.prod_id,null,'Hierarchy:'||prod_assort_dtl.prod_hier_node_id,'Product:'||prod_assort_dtl.prod_id)
      and p_at_date between nvl(prod_assort_dtl.effective_from,to_date('00010101','YYYYMMDD')) and nvl(prod_assort_dtl.effective_to,to_date('99991231','YYYYMMDD'))
  ...
  *****************************************************************************/
  function cust_prod_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_prod_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      select p_at_date report_date,
        ...
        cust.assort_id,
        prod.assort_dtl_id
      from table(qu4_assort_pkg.cust_assort_view_at_date(p_at_date,p_filtered)) cust,
        table(qu4_assort_pkg.prod_assort_view_at_date(p_at_date,p_filtered)) prod
  ...
  *****************************************************************************/
  function cust_core_assort_view_at_date(p_at_date in date, p_filtered in number) return qu4_cust_prod_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      select p_at_date report_date,
        ...
        core.assort_dtl_id
      from table(qu4_assort_pkg.cust_assort_view_at_date(p_at_date,p_filtered)) cust,
        table(qu4_assort_pkg.core_assort_view_at_date(p_at_date,p_filtered)) core
  ...
  *****************************************************************************/
  function master_assort_view_at_date(p_at_date in date) return qu4_master_assort_type pipelined is
  ...
  begin
    ...
    for l_entity in (
      ...
      from table(qu4_assort_pkg.cust_prod_assort_view_at_date(p_at_date,l_filtered)) prod,
        table(qu4_assort_pkg.cust_core_assort_view_at_date(p_at_date,l_filtered)) core
...
-- Synonyms
create or replace public synonym qu4_assort_pkg for dds_app.qu4_assort_pkg;

-- Grants
grant execute on dds_app.qu4_assort_pkg to qv_user, bo_user;
