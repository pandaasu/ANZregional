...
create or replace package dds_app.qu4_cust_address_pkg as
  /***************************************************************-****************
  ** Package Definition
  ********************************************************************************
    ...
    System   : qu4
    Owner    : dds_app
    Package  : qu4_cust_address_pkg
    Author   : [Auto-Generate]

    Description
    ------------------------------------------------------------------------------
    [qu4] Quofore - Australia Chocolate
    View Package, Entity [CustomerAddress] Table [qu4_cust_address][_load/_hist]
  ...
  *******************************************************************************/
  ...
  -- Public : Type
  type qu4_cust_address_type is table of ods.qu4_cust_address_hist%rowtype;

  -- Public : Functions
  function view_at_date(p_date in date) return qu4_cust_address_type pipelined;
  function view_history return qu4_cust_address_type pipelined;

end qu4_cust_address_pkg;
/

create or replace package body dds_app.qu4_cust_address_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu4_cust_address_pkg';
  ...
  *****************************************************************************/
  function view_at_date(p_date in date) return qu4_cust_address_type pipelined is

  begin

    for l_entity in (
      select *
      from qu4_cust_address_hist
  ...
  *****************************************************************************/
  function view_history return qu4_cust_address_type pipelined is
...
-- Synonyms
create or replace public synonym qu4_cust_address_pkg for dds_app.qu4_cust_address_pkg;

-- Grants
grant execute on dds_app.qu4_cust_address_pkg to qv_user, bo_user;
