...
create or replace package dds_app.quo_prod_pkg as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : dds_app
  Package : quo_prod_pkg
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Table [quo_prod] Entity [Product] Interface [quocdw06]  
...
*******************************************************************************/
  ...
  -- Public : Type
  type quo_prod_type is table of ods.quo_prod%rowtype;

  -- Public : Functions
  function view_current(p_source_id in number) return quo_prod_type pipelined;
  function view_at_date(p_source_id in number, p_date in date) return quo_prod_type pipelined;
  function view_history(p_source_id in number) return quo_prod_type pipelined;

end quo_prod_pkg;
/

create or replace package body dds_app.quo_prod_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'quo_prod_pkg';
  g_entity_name constant varchar2(32 char) := 'Product';
  
  /*****************************************************************************
  ** Function : Return Entity : Current 
  *****************************************************************************/
  function view_current(p_source_id in number) return quo_prod_type pipelined is
  ...
  *****************************************************************************/
  function view_at_date(p_source_id in number, p_date in date) return quo_prod_type pipelined is
  ...
  *****************************************************************************/
  function view_history(p_source_id in number) return quo_prod_type pipelined is
...
-- Synonyms
create or replace public synonym quo_prod_pkg for dds_app.quo_prod_pkg;

-- Grants
grant execute on dds_app.quo_prod_pkg to qv_user, bo_user;
