...
create or replace package quo_quocdw40 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw40
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw40] Entity [ActivityDetailTraining] Table [quo_act_dtl_training] 
...
create or replace package body quo_quocdw40 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw40';
  g_entity_name constant varchar2(32 char) := 'ActivityDetailTraining';
  ...
  g_entity_load_row ods.quo_act_dtl_training_load%rowtype;
  g_entity_row ods.quo_act_dtl_training%rowtype;
  g_entity_hist_row ods.quo_act_dtl_training_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_participants',5);
    lics_inbound_utility.set_csv_definition('potential_trained',6);
    lics_inbound_utility.set_csv_definition('training_time_training',7);
    lics_inbound_utility.set_csv_definition('training_comments',8);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Activity_ID,Activity_ID_Lookup,NumParticipants,PotentialTrained,TrainingTimeTraining,TrainingComments,ProductHierarchy_Hierarchy_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_dtl_training_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_participants := quo_util.get_number('no_participants',18,4,true,g_source_file_error_flag);
      l_entity_load_row.potential_trained := quo_util.get_number('potential_trained',18,4,true,g_source_file_error_flag);
      l_entity_load_row.training_time_training := quo_util.get_number('training_time_training',18,4,true,g_source_file_error_flag);
      l_entity_load_row.training_comments := quo_util.get_string('training_comments',200,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_act_dtl_training_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_dtl_training_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_act_dtl_training_load
    ...
    delete from ods.quo_act_dtl_training
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_act_dtl_training_load
    ...
    insert into ods.quo_act_dtl_training
    select * 
    from ods.quo_act_dtl_training_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_dtl_training 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_dtl_training_hist
    select * 
    from ods.quo_act_dtl_training_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_dtl_training_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_dtl_training/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw40 for ods_app.quo_quocdw40;

-- Grants
grant execute on ods_app.quo_quocdw40 to lics_app;
