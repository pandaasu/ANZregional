...
create or replace package quo_quocdw03 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw03
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw03] Entity [Position] Table [quo_pos] 
...
create or replace package body quo_quocdw03 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw03';
  g_entity_name constant varchar2(32 char) := 'Position';
  ...
  g_entity_load_row ods.quo_pos_load%rowtype;
  g_entity_row ods.quo_pos%rowtype;
  g_entity_hist_row ods.quo_pos_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('pos_name',3);
    lics_inbound_utility.set_csv_definition('id_lookup',4);
    lics_inbound_utility.set_csv_definition('role_name',5);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,isactive,positionname,id_lookup,rolename';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_pos_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_pos_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_pos_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_pos/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw03 for ods_app.quo_quocdw03;

-- Grants
grant execute on ods_app.quo_quocdw03 to lics_app;
