...
create or replace package quo_quocdw27 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw27
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw27] Entity [ResponseOption] Table [quo_response_opt] 
...
create or replace package body quo_quocdw27 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw27';
  g_entity_name constant varchar2(32 char) := 'ResponseOption';
  ...
  g_entity_load_row ods.quo_response_opt_load%rowtype;
  g_entity_row ods.quo_response_opt%rowtype;
  g_entity_hist_row ods.quo_response_opt_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_question_id',3);
    lics_inbound_utility.set_csv_definition('survey_question_id_lookup',4);
    lics_inbound_utility.set_csv_definition('sort_ord',5);
    lics_inbound_utility.set_csv_definition('enable_set',6);
    lics_inbound_utility.set_csv_definition('response_label',7);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,SurveyQuestion_ID,SurveyQuestion_ID_Lookup,SortOrder,EnableSet,ResponseLabel';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_response_opt_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.sort_ord := quo_util.get_number('sort_ord',3,0,true,g_source_file_error_flag);
      l_entity_load_row.enable_set := quo_util.get_number('enable_set',3,0,true,g_source_file_error_flag);
      l_entity_load_row.response_label := quo_util.get_string('response_label',50,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_response_opt_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_response_opt_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_response_opt_load
    ...
    delete from ods.quo_response_opt
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_response_opt_load
    ...
    insert into ods.quo_response_opt
    select * 
    from ods.quo_response_opt_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_response_opt 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_response_opt_hist
    select * 
    from ods.quo_response_opt_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_response_opt_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_response_opt/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw27 for ods_app.quo_quocdw27;

-- Grants
grant execute on ods_app.quo_quocdw27 to lics_app;
