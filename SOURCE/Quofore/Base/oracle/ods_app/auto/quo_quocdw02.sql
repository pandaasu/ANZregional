...
create or replace package quo_quocdw02 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw02
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw02] Entity [GeneralList] Table [quo_general_list] 
...
create or replace package body quo_quocdw02 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw02';
  g_entity_name constant varchar2(32 char) := 'GeneralList';
  ...
  g_entity_load_row ods.quo_general_list_load%rowtype;
  g_entity_row ods.quo_general_list%rowtype;
  g_entity_hist_row ods.quo_general_list_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('lookup_list_item_is_active',3);
    lics_inbound_utility.set_csv_definition('lookup_list_item_created_date',4);
    lics_inbound_utility.set_csv_definition('lookup_list_item_name',5);
    lics_inbound_utility.set_csv_definition('lookup_list_is_active',6);
    lics_inbound_utility.set_csv_definition('lookup_list_id',7);
    lics_inbound_utility.set_csv_definition('lookup_list_id_lookup',8);
    lics_inbound_utility.set_csv_definition('lookup_list_name',9);
    lics_inbound_utility.set_csv_definition('lookup_list_created_date',10);
    lics_inbound_utility.set_csv_definition('lookup_list_sort_ord',11);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,LookupListItemIsActive,LookupListItemCreatedDate,LookupListItemName,LookupListIsActive,LookupList_ID,LookupList_ID_Lookup,LookupListName,LookupListCreatedDate,LookupListSortOrder';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_general_list_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.lookup_list_item_is_active := quo_util.get_boolean_as_number('lookup_list_item_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_created_date := quo_util.get_datetime('lookup_list_item_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_name := quo_util.get_string('lookup_list_item_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_is_active := quo_util.get_boolean_as_number('lookup_list_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_id := quo_util.get_number('lookup_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_id_lookup := quo_util.get_string('lookup_list_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_name := quo_util.get_string('lookup_list_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_created_date := quo_util.get_datetime('lookup_list_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_sort_ord := quo_util.get_number('lookup_list_sort_ord',5,0,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_general_list_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_general_list_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_general_list_load
    ...
    delete from ods.quo_general_list
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_general_list_load
    ...
    insert into ods.quo_general_list
    select * 
    from ods.quo_general_list_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_general_list 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_general_list_hist
    select * 
    from ods.quo_general_list_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_general_list_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_general_list/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw02 for ods_app.quo_quocdw02;

-- Grants
grant execute on ods_app.quo_quocdw02 to lics_app;
