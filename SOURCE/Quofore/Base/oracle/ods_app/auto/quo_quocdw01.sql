...
create or replace package quo_quocdw01 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw01
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw01] Entity [Hierarchy] Table [quo_hier] 
...
create or replace package body quo_quocdw01 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw01';
  g_entity_name constant varchar2(32 char) := 'Hierarchy';
  ...
  g_entity_load_row ods.quo_hier_load%rowtype;
  g_entity_row ods.quo_hier%rowtype;
  g_entity_hist_row ods.quo_hier_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('id_desc',3);
    lics_inbound_utility.set_csv_definition('is_active',4);
    lics_inbound_utility.set_csv_definition('node',5);
    lics_inbound_utility.set_csv_definition('hier_full_path',6);
    lics_inbound_utility.set_csv_definition('hier_root_is_active',7);
    lics_inbound_utility.set_csv_definition('hier_root_id',8);
    lics_inbound_utility.set_csv_definition('hier_root_id_lookup',9);
    lics_inbound_utility.set_csv_definition('hier_root_id_desc',10);
    lics_inbound_utility.set_csv_definition('hier_root_node',11);
    lics_inbound_utility.set_csv_definition('parent_id',12);
    lics_inbound_utility.set_csv_definition('parent_id_lookup',13);
    lics_inbound_utility.set_csv_definition('parent_id_desc',14);
    lics_inbound_utility.set_csv_definition('parent_node',15);
    lics_inbound_utility.set_csv_definition('parent_is_active',16);
    lics_inbound_utility.set_csv_definition('hier_level',17);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,ID_Description,IsActive,Node,HierarchyFullPath,HierarchyRootIsActive,HierarchyRoot_ID,HierarchyRoot_ID_Lookup,HierarchyRoot_ID_Description,HierarchyRootNode,Parent_ID,Parent_ID_Lookup,Parent_ID_Description,ParentNode,ParentIsactive,Level';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_hier_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.id_desc := quo_util.get_string('id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.is_active := quo_util.get_boolean_as_number('is_active',true,g_source_file_error_flag);
      l_entity_load_row.node := quo_util.get_string('node',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_full_path := quo_util.get_string('hier_full_path',4000,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_is_active := quo_util.get_boolean_as_number('hier_root_is_active',true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id := quo_util.get_number('hier_root_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id_lookup := quo_util.get_string('hier_root_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id_desc := quo_util.get_string('hier_root_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_node := quo_util.get_string('hier_root_node',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id := quo_util.get_number('parent_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_lookup := quo_util.get_string('parent_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_desc := quo_util.get_string('parent_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_node := quo_util.get_string('parent_node',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_is_active := quo_util.get_boolean_as_number('parent_is_active',true,g_source_file_error_flag);
      l_entity_load_row.hier_level := quo_util.get_number('hier_level',10,0,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_hier_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_hier_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_hier/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw01 for ods_app.quo_quocdw01;

-- Grants
grant execute on ods_app.quo_quocdw01 to lics_app;
