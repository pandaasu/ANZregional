...
create or replace package quo_quocdw08 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw08
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw08] Entity [Customer] Table [quo_cust] 
...
create or replace package body quo_quocdw08 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw08';
  g_entity_name constant varchar2(32 char) := 'Customer';
  ...
  g_entity_load_row ods.quo_cust_load%rowtype;
  g_entity_row ods.quo_cust%rowtype;
  g_entity_hist_row ods.quo_cust_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('cust_name',5);
    lics_inbound_utility.set_csv_definition('cust_ref_id',6);
    lics_inbound_utility.set_csv_definition('email',7);
    lics_inbound_utility.set_csv_definition('fax_no',8);
    lics_inbound_utility.set_csv_definition('group_edi_code',9);
    lics_inbound_utility.set_csv_definition('outlet_ref_id',10);
    lics_inbound_utility.set_csv_definition('phone_no',11);
    lics_inbound_utility.set_csv_definition('store_edi_code',12);
    lics_inbound_utility.set_csv_definition('web_addrs',13);
    lics_inbound_utility.set_csv_definition('cust_hier_id',14);
    lics_inbound_utility.set_csv_definition('grade_hier_id',15);
    lics_inbound_utility.set_csv_definition('grd_code',16);
    lics_inbound_utility.set_csv_definition('lead_time',17);
    lics_inbound_utility.set_csv_definition('xml_attach',18);
    lics_inbound_utility.set_csv_definition('is_wholesaler',19);
    lics_inbound_utility.set_csv_definition('call_frequency',20);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'Id,Id_Lookup,IsActive,CreatedDate,CustomerName,CustomerRefID,Email,FaxNumber,GroupEDICode,OutletRefID,PhoneNumber,StoreEDICode,WebAddress,CustomerHierarchy_Hierarchy_ID,GradeHierarchy_Hierarchy_ID,GRDCode,LeadTime,XMLAttachment,IsWholesaler,CallFrequency';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_cust_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.cust_name := quo_util.get_string('cust_name',50,true,g_source_file_error_flag);
      l_entity_load_row.cust_ref_id := quo_util.get_string('cust_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.email := quo_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.fax_no := quo_util.get_string('fax_no',30,true,g_source_file_error_flag);
      l_entity_load_row.group_edi_code := quo_util.get_string('group_edi_code',50,true,g_source_file_error_flag);
      l_entity_load_row.outlet_ref_id := quo_util.get_string('outlet_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := quo_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.store_edi_code := quo_util.get_string('store_edi_code',50,true,g_source_file_error_flag);
      l_entity_load_row.web_addrs := quo_util.get_string('web_addrs',60,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_id := quo_util.get_number('cust_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.grade_hier_id := quo_util.get_number('grade_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.grd_code := quo_util.get_string('grd_code',50,true,g_source_file_error_flag);
      l_entity_load_row.lead_time := quo_util.get_number('lead_time',3,0,true,g_source_file_error_flag);
      l_entity_load_row.xml_attach := quo_util.get_boolean_as_number('xml_attach',true,g_source_file_error_flag);
      l_entity_load_row.is_wholesaler := quo_util.get_boolean_as_number('is_wholesaler',true,g_source_file_error_flag);
      l_entity_load_row.call_frequency := quo_util.get_number('call_frequency',10,0,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_cust_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_cust_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_cust/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw08 for ods_app.quo_quocdw08;

-- Grants
grant execute on ods_app.quo_quocdw08 to lics_app;
