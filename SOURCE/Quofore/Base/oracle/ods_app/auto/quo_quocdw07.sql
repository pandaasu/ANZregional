...
create or replace package quo_quocdw07 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw07
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw07] Entity [ProductBarcode] Table [quo_prod_barcode] 
...
create or replace package body quo_quocdw07 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw07';
  g_entity_name constant varchar2(32 char) := 'ProductBarcode';
  ...
  g_entity_load_row ods.quo_prod_barcode_load%rowtype;
  g_entity_row ods.quo_prod_barcode%rowtype;
  g_entity_hist_row ods.quo_prod_barcode_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('barcode_desc',4);
    lics_inbound_utility.set_csv_definition('barcode_type_id',5);
    lics_inbound_utility.set_csv_definition('barcode_type_id_lookup',6);
    lics_inbound_utility.set_csv_definition('barcode_type_id_desc',7);
    lics_inbound_utility.set_csv_definition('code',8);
    lics_inbound_utility.set_csv_definition('prod_id',9);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',10);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,CreatedDate,BarcodeDescription,BarcodeTypeId,BarcodeTypeId_Lookup,BarcodeTypeId_Description,Code,Product_Id,Product_Id_Lookup,Product_Id_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_prod_barcode_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.barcode_desc := quo_util.get_string('barcode_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.barcode_type_id := quo_util.get_number('barcode_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.barcode_type_id_lookup := quo_util.get_string('barcode_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.barcode_type_id_desc := quo_util.get_string('barcode_type_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.code := quo_util.get_string('code',35,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_prod_barcode_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_prod_barcode_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_prod_barcode_load
    ...
    delete from ods.quo_prod_barcode
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_prod_barcode_load
    ...
    insert into ods.quo_prod_barcode
    select * 
    from ods.quo_prod_barcode_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_prod_barcode 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_prod_barcode_hist
    select * 
    from ods.quo_prod_barcode_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_prod_barcode_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_prod_barcode/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw07 for ods_app.quo_quocdw07;

-- Grants
grant execute on ods_app.quo_quocdw07 to lics_app;
