...
create or replace package quo_quocdw15 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw15
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw15] Entity [ProductAssortmentDetail] Table [quo_prod_assort_dtl] 
...
create or replace package body quo_quocdw15 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw15';
  g_entity_name constant varchar2(32 char) := 'ProductAssortmentDetail';
  ...
  g_entity_load_row ods.quo_prod_assort_dtl_load%rowtype;
  g_entity_row ods.quo_prod_assort_dtl%rowtype;
  g_entity_hist_row ods.quo_prod_assort_dtl_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('assort_id',4);
    lics_inbound_utility.set_csv_definition('assort_id_lookup',5);
    lics_inbound_utility.set_csv_definition('assort_dtl_id',6);
    lics_inbound_utility.set_csv_definition('assort_dtl_id_lookup',7);
    lics_inbound_utility.set_csv_definition('prod_id',8);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',9);
    lics_inbound_utility.set_csv_definition('prod_hier_node_id',10);
    lics_inbound_utility.set_csv_definition('prod_hier_node_id_lookup',11);
    lics_inbound_utility.set_csv_definition('priority_assort_dtl_id',12);
    lics_inbound_utility.set_csv_definition('priority_assort_dtl_id_lookup',13);
    lics_inbound_utility.set_csv_definition('effective_from',14);
    lics_inbound_utility.set_csv_definition('effective_to',15);
    lics_inbound_utility.set_csv_definition('priority_from',16);
    lics_inbound_utility.set_csv_definition('priority_to',17);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,id_lookup,createddate,assortment_id,assortment_id_lookup,assortmentdetail_id,assortmentdetail_id_lookup,product_id,product_id_lookup,product_hierarchynode_id,product_hierarchynode_id_lookup,priorityassortmentdetail_id,priorityassortmentdetail_id_lookup,effectivefrom,effectiveto,priorityfrom,priorityto';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_prod_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_id := quo_util.get_number('assort_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_id_lookup := quo_util.get_string('assort_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_id := quo_util.get_number('assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_id_lookup := quo_util.get_string('assort_dtl_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := quo_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_lookup := quo_util.get_string('prod_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.prod_hier_node_id := quo_util.get_number('prod_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_hier_node_id_lookup := quo_util.get_string('prod_hier_node_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.priority_assort_dtl_id := quo_util.get_number('priority_assort_dtl_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.priority_assort_dtl_id_lookup := quo_util.get_string('priority_assort_dtl_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.effective_from := quo_util.get_datetime('effective_from',true,g_source_file_error_flag);
      l_entity_load_row.effective_to := quo_util.get_datetime('effective_to',true,g_source_file_error_flag);
      l_entity_load_row.priority_from := quo_util.get_datetime('priority_from',true,g_source_file_error_flag);
      l_entity_load_row.priority_to := quo_util.get_datetime('priority_to',true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_prod_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_prod_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_prod_assort_dtl_load
    ...
    delete from ods.quo_prod_assort_dtl
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_prod_assort_dtl_load
    ...
    insert into ods.quo_prod_assort_dtl
    select * 
    from ods.quo_prod_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_prod_assort_dtl 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_prod_assort_dtl_hist
    select * 
    from ods.quo_prod_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_prod_assort_dtl_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_prod_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw15 for ods_app.quo_quocdw15;

-- Grants
grant execute on ods_app.quo_quocdw15 to lics_app;
