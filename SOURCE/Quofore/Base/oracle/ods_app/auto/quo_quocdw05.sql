...
create or replace package quo_quocdw05 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw05
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw05] Entity [RepAddress] Table [quo_rep_addrs] 
...
create or replace package body quo_quocdw05 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw05';
  ...
  g_entity_load_row ods.quo_rep_addrs_load%rowtype;
  g_entity_row ods.quo_rep_addrs%rowtype;
  g_entity_hist_row ods.quo_rep_addrs_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('rep_id',4);
    lics_inbound_utility.set_csv_definition('rep_id_lookup',5);
    lics_inbound_utility.set_csv_definition('addrs_type_id',6);
    lics_inbound_utility.set_csv_definition('addrs_type_id_lookup',7);
    lics_inbound_utility.set_csv_definition('addrs_type_id_desc',8);
    lics_inbound_utility.set_csv_definition('street_1',9);
    lics_inbound_utility.set_csv_definition('street_2',10);
    lics_inbound_utility.set_csv_definition('town',11);
    lics_inbound_utility.set_csv_definition('city',12);
    lics_inbound_utility.set_csv_definition('post_code',13);
    lics_inbound_utility.set_csv_definition('state_id',14);
    lics_inbound_utility.set_csv_definition('state_id_lookup',15);
    lics_inbound_utility.set_csv_definition('state_id_desc',16);
    lics_inbound_utility.set_csv_definition('country_id',17);
    lics_inbound_utility.set_csv_definition('country_id_lookup',18);
    lics_inbound_utility.set_csv_definition('country_id_desc',19);
    lics_inbound_utility.set_csv_definition('latitude',20);
    lics_inbound_utility.set_csv_definition('longitude',21);
    lics_inbound_utility.set_csv_definition('full_name',22);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,IsActive,Rep_ID,Rep_ID_Lookup,AddressTypeID,AddressTypeID_Lookup,AddressTypeID_Description,Street1,Street2,Town,City,PostCode,StateId,StateId_Lookup,StateId_Description,CountryId,CountryId_Lookup,CountryId_Description,Latitude,Longitude,FullName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_rep_addrs_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_rep_addrs_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_rep_addrs_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_rep_addrs 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_rep_addrs_hist
    select * 
    from ods.quo_rep_addrs_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_rep_addrs_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_rep_addrs/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw05 for ods_app.quo_quocdw05;

-- Grants
grant execute on ods_app.quo_quocdw05 to lics_app;
