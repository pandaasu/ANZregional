...
create or replace package quo_quocdw04 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw04
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw04] Entity [Rep] Table [quo_rep] 
...
create or replace package body quo_quocdw04 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw04';
  g_entity_name constant varchar2(32 char) := 'Rep';
  ...
  g_entity_load_row ods.quo_rep_load%rowtype;
  g_entity_row ods.quo_rep%rowtype;
  g_entity_hist_row ods.quo_rep_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('emp_no',5);
    lics_inbound_utility.set_csv_definition('first_name',6);
    lics_inbound_utility.set_csv_definition('last_name',7);
    lics_inbound_utility.set_csv_definition('middle_name',8);
    lics_inbound_utility.set_csv_definition('home_no',9);
    lics_inbound_utility.set_csv_definition('mobile_no',10);
    lics_inbound_utility.set_csv_definition('fax_no',11);
    lics_inbound_utility.set_csv_definition('email',12);
    lics_inbound_utility.set_csv_definition('pos_id',13);
    lics_inbound_utility.set_csv_definition('pos_id_lookup',14);
    lics_inbound_utility.set_csv_definition('full_name',15);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,IsActive,CreatedDate,EmployeeNumber,FirstName,LastName,MiddleName,HomeNumber,MobileNumber,FaxNumber,Email,Position_ID,Position_ID_Lookup,FullName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_rep_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.emp_no := quo_util.get_string('emp_no',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.home_no := quo_util.get_string('home_no',30,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_rep_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_rep_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_rep/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw04 for ods_app.quo_quocdw04;

-- Grants
grant execute on ods_app.quo_quocdw04 to lics_app;
