...
create or replace package quo_quocdw29 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw29
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw29] Entity [TaskAssignment] Table [quo_task_assign] 
...
create or replace package body quo_quocdw29 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw29';
  g_entity_name constant varchar2(32 char) := 'TaskAssignment';
  ...
  g_entity_load_row ods.quo_task_assign_load%rowtype;
  g_entity_row ods.quo_task_assign%rowtype;
  g_entity_hist_row ods.quo_task_assign_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('task_id',4);
    lics_inbound_utility.set_csv_definition('task_id_lookup',5);
    lics_inbound_utility.set_csv_definition('rep_id',6);
    lics_inbound_utility.set_csv_definition('rep_id_lookup',7);
    lics_inbound_utility.set_csv_definition('role_name',8);
    lics_inbound_utility.set_csv_definition('full_name',9);
    lics_inbound_utility.set_csv_definition('task_id_desc',10);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,CreatedDate,Task_ID,Task_ID_Lookup,Rep_ID,Rep_ID_Lookup,RoleName,Fullname,Task_ID_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_task_assign_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.role_name := quo_util.get_string('role_name',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_task_assign_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_task_assign_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    delete from ods.quo_task_assign
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_task_assign_load
    ...
    insert into ods.quo_task_assign
    select * 
    from ods.quo_task_assign_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_task_assign 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_task_assign_hist
    select * 
    from ods.quo_task_assign_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_task_assign_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_task_assign/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw29 for ods_app.quo_quocdw29;

-- Grants
grant execute on ods_app.quo_quocdw29 to lics_app;
