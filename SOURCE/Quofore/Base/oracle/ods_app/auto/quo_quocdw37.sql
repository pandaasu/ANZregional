...
create or replace package quo_quocdw37 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw37
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw37] Entity [ActivityDetailSoSSPC] Table [quo_act_dtl_sos_spc] 
...
create or replace package body quo_quocdw37 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw37';
  g_entity_name constant varchar2(32 char) := 'ActivityDetailSoSSPC';
  ...
  g_entity_load_row ods.quo_act_dtl_sos_spc_load%rowtype;
  g_entity_row ods.quo_act_dtl_sos_spc%rowtype;
  g_entity_hist_row ods.quo_act_dtl_sos_spc_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('act_id',3);
    lics_inbound_utility.set_csv_definition('act_id_lookup',4);
    lics_inbound_utility.set_csv_definition('sos_advance_modules',5);
    lics_inbound_utility.set_csv_definition('sos_eukanuba_modules',6);
    lics_inbound_utility.set_csv_definition('sos_hills_modules',7);
    lics_inbound_utility.set_csv_definition('sos_royalcanine_modules',8);
    lics_inbound_utility.set_csv_definition('sos_proplan_modules',9);
    lics_inbound_utility.set_csv_definition('sos_nutro_modules',10);
    lics_inbound_utility.set_csv_definition('sos_other_natural_modules',11);
    lics_inbound_utility.set_csv_definition('sos_mars_core_modules',12);
    lics_inbound_utility.set_csv_definition('sos_other_modules',13);
    lics_inbound_utility.set_csv_definition('prod_hier_id',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Activity_ID,Activity_ID_Lookup,SOS_AdvanceModules,SOS_EukanubaModules,SOS_HillsModules,SOS_RoyalCanineModules,SOS_ProplanModules,SOS_NutroModules,SOS_OtherNaturalModules,SOS_MarsCoreModules,SOS_OtherModules,ProductHierarchy_Hierarchy_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_dtl_sos_spc_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.act_id := quo_util.get_number('act_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.act_id_lookup := quo_util.get_string('act_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.sos_advance_modules := quo_util.get_number('sos_advance_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_eukanuba_modules := quo_util.get_number('sos_eukanuba_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_hills_modules := quo_util.get_number('sos_hills_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_royalcanine_modules := quo_util.get_number('sos_royalcanine_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_proplan_modules := quo_util.get_number('sos_proplan_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_nutro_modules := quo_util.get_number('sos_nutro_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_other_natural_modules := quo_util.get_number('sos_other_natural_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_mars_core_modules := quo_util.get_number('sos_mars_core_modules',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sos_other_modules := quo_util.get_number('sos_other_modules',18,4,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_act_dtl_sos_spc_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_dtl_sos_spc_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_act_dtl_sos_spc_load
    ...
    delete from ods.quo_act_dtl_sos_spc
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_act_dtl_sos_spc_load
    ...
    insert into ods.quo_act_dtl_sos_spc
    select * 
    from ods.quo_act_dtl_sos_spc_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_dtl_sos_spc 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_dtl_sos_spc_hist
    select * 
    from ods.quo_act_dtl_sos_spc_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_dtl_sos_spc_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_dtl_sos_spc/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw37 for ods_app.quo_quocdw37;

-- Grants
grant execute on ods_app.quo_quocdw37 to lics_app;
