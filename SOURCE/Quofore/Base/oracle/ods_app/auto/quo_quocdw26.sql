...
create or replace package quo_quocdw26 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw26
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw26] Entity [SurveyQuestion] Table [quo_survey_question] 
...
create or replace package body quo_quocdw26 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw26';
  g_entity_name constant varchar2(32 char) := 'SurveyQuestion';
  ...
  g_entity_load_row ods.quo_survey_question_load%rowtype;
  g_entity_row ods.quo_survey_question%rowtype;
  g_entity_hist_row ods.quo_survey_question_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_id',3);
    lics_inbound_utility.set_csv_definition('survey_id_lookup',4);
    lics_inbound_utility.set_csv_definition('question_id',5);
    lics_inbound_utility.set_csv_definition('question_id_lookup',6);
    lics_inbound_utility.set_csv_definition('question',7);
    lics_inbound_utility.set_csv_definition('is_mandatory',8);
    lics_inbound_utility.set_csv_definition('sort_ord',9);
    lics_inbound_utility.set_csv_definition('question_set',10);
    lics_inbound_utility.set_csv_definition('response_type_id',11);
    lics_inbound_utility.set_csv_definition('response_type_id_lookup',12);
    lics_inbound_utility.set_csv_definition('is_single_choice',13);
    lics_inbound_utility.set_csv_definition('freeform_data_type',14);
    lics_inbound_utility.set_csv_definition('question_created_date',15);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Survey_ID,Survey_Id_Lookup,Question_ID,Question_ID_Lookup,Question,IsMandatory,SortOrder,QuestionSet,ResponseType_ID,ResponseType_ID_Lookup,IsSingleChoice,FreeformDataType,QuestionCreatedDate';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_survey_question_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.question_id := quo_util.get_number('question_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.question_id_lookup := quo_util.get_string('question_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.question := quo_util.get_string('question',200,true,g_source_file_error_flag);
      l_entity_load_row.is_mandatory := quo_util.get_boolean_as_number('is_mandatory',true,g_source_file_error_flag);
      l_entity_load_row.sort_ord := quo_util.get_string('sort_ord',50,true,g_source_file_error_flag);
      l_entity_load_row.question_set := quo_util.get_number('question_set',3,0,true,g_source_file_error_flag);
      l_entity_load_row.response_type_id := quo_util.get_number('response_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.response_type_id_lookup := quo_util.get_string('response_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.is_single_choice := quo_util.get_boolean_as_number('is_single_choice',true,g_source_file_error_flag);
      l_entity_load_row.freeform_data_type := quo_util.get_string('freeform_data_type',50,true,g_source_file_error_flag);
      l_entity_load_row.question_created_date := quo_util.get_datetime('question_created_date',true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_survey_question_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_survey_question_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_survey_question_load
    ...
    delete from ods.quo_survey_question
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_survey_question_load
    ...
    insert into ods.quo_survey_question
    select * 
    from ods.quo_survey_question_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_survey_question 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_survey_question_hist
    select * 
    from ods.quo_survey_question_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_survey_question_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_survey_question/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw26 for ods_app.quo_quocdw26;

-- Grants
grant execute on ods_app.quo_quocdw26 to lics_app;
