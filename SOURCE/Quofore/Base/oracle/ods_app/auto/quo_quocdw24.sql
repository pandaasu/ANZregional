...
create or replace package quo_quocdw24 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw24
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw24] Entity [PositionTerritory] Table [quo_pos_terr] 
...
create or replace package body quo_quocdw24 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw24';
  g_entity_name constant varchar2(32 char) := 'PositionTerritory';
  ...
  g_entity_load_row ods.quo_pos_terr_load%rowtype;
  g_entity_row ods.quo_pos_terr%rowtype;
  g_entity_hist_row ods.quo_pos_terr_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('is_active',1);
    lics_inbound_utility.set_csv_definition('created_date',2);
    lics_inbound_utility.set_csv_definition('pos_id',3);
    lics_inbound_utility.set_csv_definition('pos_id_lookup',4);
    lics_inbound_utility.set_csv_definition('terr_id',5);
    lics_inbound_utility.set_csv_definition('terr_id_lookup',6);
    lics_inbound_utility.set_csv_definition('is_primary_terr',7);
    lics_inbound_utility.set_csv_definition('terr_name',8);
    lics_inbound_utility.set_csv_definition('id',9);
    lics_inbound_utility.set_csv_definition('pos_name',10);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'IsActive,CreatedDate,Position_Id,Position_Id_Lookup,Territory_Id,Territory_Id_Lookup,IsPrimaryTerritory,TerritoryName,Id,PositionName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_pos_terr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.terr_id := quo_util.get_number('terr_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.terr_id_lookup := quo_util.get_string('terr_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.is_primary_terr := quo_util.get_boolean_as_number('is_primary_terr',true,g_source_file_error_flag);
      l_entity_load_row.terr_name := quo_util.get_string('terr_name',50,true,g_source_file_error_flag);
      l_entity_load_row.id := quo_util.get_number('id',10,0,false,g_source_file_error_flag);
      l_entity_load_row.pos_name := quo_util.get_string('pos_name',50,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_pos_terr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_pos_terr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_pos_terr 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_pos_terr_hist
    select * 
    from ods.quo_pos_terr_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_pos_terr_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_pos_terr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw24 for ods_app.quo_quocdw24;

-- Grants
grant execute on ods_app.quo_quocdw24 to lics_app;
