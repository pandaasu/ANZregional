...
create or replace package quo_quocdw09 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw09
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw09] Entity [CustomerAddress] Table [quo_cust_addrs] 
...
create or replace package body quo_quocdw09 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw09';
  g_entity_name constant varchar2(32 char) := 'CustomerAddress';
  ...
  g_entity_load_row ods.quo_cust_addrs_load%rowtype;
  g_entity_row ods.quo_cust_addrs%rowtype;
  g_entity_hist_row ods.quo_cust_addrs_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('addrs_type_id_lookup',3);
    lics_inbound_utility.set_csv_definition('addrs_type_id_desc',4);
    lics_inbound_utility.set_csv_definition('city',5);
    lics_inbound_utility.set_csv_definition('country_id',6);
    lics_inbound_utility.set_csv_definition('country_id_lookup',7);
    lics_inbound_utility.set_csv_definition('country_id_desc',8);
    lics_inbound_utility.set_csv_definition('cust_id',9);
    lics_inbound_utility.set_csv_definition('cust_id_lookup',10);
    lics_inbound_utility.set_csv_definition('id_lookup',11);
    lics_inbound_utility.set_csv_definition('is_active',12);
    lics_inbound_utility.set_csv_definition('latitude',13);
    lics_inbound_utility.set_csv_definition('longitude',14);
    lics_inbound_utility.set_csv_definition('post_code',15);
    lics_inbound_utility.set_csv_definition('state_id',16);
    lics_inbound_utility.set_csv_definition('state_id_lookup',17);
    lics_inbound_utility.set_csv_definition('state_id_desc',18);
    lics_inbound_utility.set_csv_definition('street_1',19);
    lics_inbound_utility.set_csv_definition('street_2',20);
    lics_inbound_utility.set_csv_definition('town',21);
    lics_inbound_utility.set_csv_definition('cust_id_desc',22);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,AddressTypeId,AddressTypeID_Lookup,AddressTypeID_Description,City,CountryId,CountryID_Lookup,CountryID_Description,Customer_Id,Customer_Id_Lookup,ID_Lookup,IsActive,Latitude,Longitude,Postcode,StateId,StateID_Lookup,StateID_Description,Street1,Street2,Town,customer_id_description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_cust_addrs_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.addrs_type_id := quo_util.get_number('addrs_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.addrs_type_id_lookup := quo_util.get_string('addrs_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.addrs_type_id_desc := quo_util.get_string('addrs_type_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_cust_addrs_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_cust_addrs_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    delete from ods.quo_cust_addrs
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_cust_addrs_load
    ...
    insert into ods.quo_cust_addrs
    select * 
    from ods.quo_cust_addrs_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_cust_addrs 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_cust_addrs_hist
    select * 
    from ods.quo_cust_addrs_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_cust_addrs_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_cust_addrs/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw09 for ods_app.quo_quocdw09;

-- Grants
grant execute on ods_app.quo_quocdw09 to lics_app;
