...
create or replace package quo_quocdw16 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw16
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw16] Entity [AuthorisedListProduct] Table [quo_auth_list_prod] 
...
create or replace package body quo_quocdw16 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw16';
  g_entity_name constant varchar2(32 char) := 'AuthorisedListProduct';
  ...
  g_entity_load_row ods.quo_auth_list_prod_load%rowtype;
  g_entity_row ods.quo_auth_list_prod%rowtype;
  g_entity_hist_row ods.quo_auth_list_prod_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('auth_list_id',3);
    lics_inbound_utility.set_csv_definition('auth_list_id_lookup',4);
    lics_inbound_utility.set_csv_definition('cust_id',5);
    lics_inbound_utility.set_csv_definition('cust_id_lookup',6);
    lics_inbound_utility.set_csv_definition('cust_hier_id',7);
    lics_inbound_utility.set_csv_definition('is_include',8);
    lics_inbound_utility.set_csv_definition('effective_from',9);
    lics_inbound_utility.set_csv_definition('effective_to',10);
    lics_inbound_utility.set_csv_definition('prod_id',11);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',12);
    lics_inbound_utility.set_csv_definition('prod_hier_id',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,AuthorisedList_ID,AuthorisedList_ID_Lookup,Customer_ID,Customer_ID_Lookup,Customer_Hierarchy_ID,IsInclude,EffectiveFrom,EffectiveTo,Product_ID,Product_ID_Lookup,ProductHierarchy_Hierarchy_ID,product_id_description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_auth_list_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.auth_list_id := quo_util.get_number('auth_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.auth_list_id_lookup := quo_util.get_string('auth_list_id_lookup',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.is_include := quo_util.get_boolean_as_number('is_include',true,g_source_file_error_flag);
      l_entity_load_row.effective_from := quo_util.get_date('effective_from',true,g_source_file_error_flag);
      l_entity_load_row.effective_to := quo_util.get_date('effective_to',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_auth_list_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_auth_list_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_auth_list_prod_load
    ...
    delete from ods.quo_auth_list_prod
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_auth_list_prod_load
    ...
    insert into ods.quo_auth_list_prod
    select * 
    from ods.quo_auth_list_prod_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_auth_list_prod 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_auth_list_prod_hist
    select * 
    from ods.quo_auth_list_prod_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_auth_list_prod_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_auth_list_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw16 for ods_app.quo_quocdw16;

-- Grants
grant execute on ods_app.quo_quocdw16 to lics_app;
