...
create or replace package quo_quocdw39 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw39
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw39] Entity [ActivityDetailSoCSPC] Table [quo_act_dtl_soc_spc] 
...
create or replace package body quo_quocdw39 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw39';
  g_entity_name constant varchar2(32 char) := 'ActivityDetailSoCSPC';
  ...
  g_entity_load_row ods.quo_act_dtl_soc_spc_load%rowtype;
  g_entity_row ods.quo_act_dtl_soc_spc%rowtype;
  g_entity_hist_row ods.quo_act_dtl_soc_spc_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('soc_eukanuba_pods',5);
    lics_inbound_utility.set_csv_definition('soc_hills_pods',6);
    lics_inbound_utility.set_csv_definition('soc_royalcanine_pods',7);
    lics_inbound_utility.set_csv_definition('soc_proplan_pods',8);
    lics_inbound_utility.set_csv_definition('soc_other_natural_pods',9);
    lics_inbound_utility.set_csv_definition('soc_other_pods',10);
    lics_inbound_utility.set_csv_definition('prod_hier_id',11);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Activity_ID,Activity_ID_Lookup,SOC_EukanubaPODs,SOC_HillsPODs,SOC_RoyalCaninePODs,SOC_ProplanPODs,SOC_OtherNaturalPODs,SOC_OtherPODs,ProductHierarchy_Hierarchy_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_dtl_soc_spc_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.soc_eukanuba_pods := quo_util.get_number('soc_eukanuba_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_hills_pods := quo_util.get_number('soc_hills_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_royalcanine_pods := quo_util.get_number('soc_royalcanine_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_proplan_pods := quo_util.get_number('soc_proplan_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_other_natural_pods := quo_util.get_number('soc_other_natural_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_other_pods := quo_util.get_number('soc_other_pods',18,4,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_act_dtl_soc_spc_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_dtl_soc_spc_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_act_dtl_soc_spc_load
    ...
    delete from ods.quo_act_dtl_soc_spc
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_act_dtl_soc_spc_load
    ...
    insert into ods.quo_act_dtl_soc_spc
    select * 
    from ods.quo_act_dtl_soc_spc_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_dtl_soc_spc 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_dtl_soc_spc_hist
    select * 
    from ods.quo_act_dtl_soc_spc_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_dtl_soc_spc_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_dtl_soc_spc/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw39 for ods_app.quo_quocdw39;

-- Grants
grant execute on ods_app.quo_quocdw39 to lics_app;
