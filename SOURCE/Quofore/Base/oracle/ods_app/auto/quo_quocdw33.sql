...
create or replace package quo_quocdw33 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw33
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw33] Entity [ActivityHeader] Table [quo_act_hdr] 
...
create or replace package body quo_quocdw33 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw33';
  g_entity_name constant varchar2(32 char) := 'ActivityHeader';
  ...
  g_entity_load_row ods.quo_act_hdr_load%rowtype;
  g_entity_row ods.quo_act_hdr%rowtype;
  g_entity_hist_row ods.quo_act_hdr_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('task_id_lookup',4);
    lics_inbound_utility.set_csv_definition('rep_id',5);
    lics_inbound_utility.set_csv_definition('rep_id_lookup',6);
    lics_inbound_utility.set_csv_definition('start_date',7);
    lics_inbound_utility.set_csv_definition('is_complete',8);
    lics_inbound_utility.set_csv_definition('end_date',9);
    lics_inbound_utility.set_csv_definition('duration',10);
    lics_inbound_utility.set_csv_definition('callcard_id',11);
    lics_inbound_utility.set_csv_definition('callcard_id_lookup',12);
    lics_inbound_utility.set_csv_definition('incomplete_reason_id',13);
    lics_inbound_utility.set_csv_definition('incomplete_reason_id_lookup',14);
    lics_inbound_utility.set_csv_definition('incomplete_reason_id_desc',15);
    lics_inbound_utility.set_csv_definition('note',16);
    lics_inbound_utility.set_csv_definition('task_name',17);
    lics_inbound_utility.set_csv_definition('due_date',18);
    lics_inbound_utility.set_csv_definition('full_name',19);
    lics_inbound_utility.set_csv_definition('task_id_desc',20);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Task_ID,Task_ID_Lookup,Rep_ID,Rep_ID_Lookup,StartDate,IsComplete,EndDate,Duration,Callcard_ID,Callcard_ID_Lookup,IncompleteReasonID,IncompleteReasonID_Lookup,IncompleteReasonID_Description,Notes,TaskName,DueDate,FullName,Task_ID_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_hdr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.task_id_lookup := quo_util.get_string('task_id_lookup',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.is_complete := quo_util.get_boolean_as_number('is_complete',true,g_source_file_error_flag);
      l_entity_load_row.end_date := quo_util.get_datetime('end_date',true,g_source_file_error_flag);
      ...
      l_entity_load_row.task_name := quo_util.get_string('task_name',50,true,g_source_file_error_flag);
      l_entity_load_row.due_date := quo_util.get_datetime('due_date',true,g_source_file_error_flag);
      l_entity_load_row.full_name := quo_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.task_id_desc := quo_util.get_string('task_id_desc',200,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_act_hdr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_hdr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_hdr 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_hdr_hist
    select * 
    from ods.quo_act_hdr_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_hdr_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_hdr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw33 for ods_app.quo_quocdw33;

-- Grants
grant execute on ods_app.quo_quocdw33 to lics_app;
