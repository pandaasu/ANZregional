...
create or replace package quo_quocdw21 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw21
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw21] Entity [OrderDetail] Table [quo_ord_dtl] 
...
create or replace package body quo_quocdw21 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw21';
  g_entity_name constant varchar2(32 char) := 'OrderDetail';
  ...
  g_entity_load_row ods.quo_ord_dtl_load%rowtype;
  g_entity_row ods.quo_ord_dtl%rowtype;
  g_entity_hist_row ods.quo_ord_dtl_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('bonus_qty',3);
    lics_inbound_utility.set_csv_definition('discount_amount',4);
    lics_inbound_utility.set_csv_definition('ord_hdr_id',5);
    lics_inbound_utility.set_csv_definition('ord_hdr_id_lookup',6);
    lics_inbound_utility.set_csv_definition('ord_qty',7);
    lics_inbound_utility.set_csv_definition('orig_ord_qty',8);
    lics_inbound_utility.set_csv_definition('per_unit_price',9);
    lics_inbound_utility.set_csv_definition('prod_id',10);
    lics_inbound_utility.set_csv_definition('prod_id_lookup',11);
    lics_inbound_utility.set_csv_definition('tax_percent',12);
    lics_inbound_utility.set_csv_definition('total_line_amount',13);
    lics_inbound_utility.set_csv_definition('prod_id_desc',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,BonusQuantity,DiscountAmount,OrderHeader_Id,OrderHeader_Id_Lookup,OrderQuantity,OriginalOrderQuantity,PerUnitPrice,Product_Id,Product_Id_Lookup,TaxPercent,TotalLineAmount,Product_ID_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_ord_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.bonus_qty := quo_util.get_number('bonus_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.discount_amount := quo_util.get_number('discount_amount',18,4,true,g_source_file_error_flag);
      l_entity_load_row.ord_hdr_id := quo_util.get_number('ord_hdr_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.ord_hdr_id_lookup := quo_util.get_string('ord_hdr_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.ord_qty := quo_util.get_number('ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.orig_ord_qty := quo_util.get_number('orig_ord_qty',5,0,true,g_source_file_error_flag);
      l_entity_load_row.per_unit_price := quo_util.get_number('per_unit_price',18,4,true,g_source_file_error_flag);
      ...
      l_entity_load_row.tax_percent := quo_util.get_number('tax_percent',9,6,true,g_source_file_error_flag);
      l_entity_load_row.total_line_amount := quo_util.get_number('total_line_amount',18,4,true,g_source_file_error_flag);
      l_entity_load_row.prod_id_desc := quo_util.get_string('prod_id_desc',50,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_ord_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_ord_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_ord_dtl 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_ord_dtl_hist
    select * 
    from ods.quo_ord_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_ord_dtl_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_ord_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw21 for ods_app.quo_quocdw21;

-- Grants
grant execute on ods_app.quo_quocdw21 to lics_app;
