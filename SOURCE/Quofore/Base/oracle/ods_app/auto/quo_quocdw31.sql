...
create or replace package quo_quocdw31 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw31
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw31] Entity [TaskProduct] Table [quo_task_prod] 
...
create or replace package body quo_quocdw31 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw31';
  g_entity_name constant varchar2(32 char) := 'TaskProduct';
  ...
  g_entity_load_row ods.quo_task_prod_load%rowtype;
  g_entity_row ods.quo_task_prod%rowtype;
  g_entity_hist_row ods.quo_task_prod_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('hier_node_id',8);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,CreatedDate,Task_ID,Task_ID_Lookup,Product_ID,Product_ID_Lookup,HierarchyNode_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_task_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.hier_node_id := quo_util.get_number('hier_node_id',10,0,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_task_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_task_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_task_prod 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_task_prod_hist
    select * 
    from ods.quo_task_prod_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_task_prod_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_task_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw31 for ods_app.quo_quocdw31;

-- Grants
grant execute on ods_app.quo_quocdw31 to lics_app;
