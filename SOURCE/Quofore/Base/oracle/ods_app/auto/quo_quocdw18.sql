...
create or replace package quo_quocdw18 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw18
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw18] Entity [CallCard] Table [quo_callcard] 
...
create or replace package body quo_quocdw18 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw18';
  ...
  g_entity_load_row ods.quo_callcard_load%rowtype;
  g_entity_row ods.quo_callcard%rowtype;
  g_entity_hist_row ods.quo_callcard_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('appoint_id',3);
    lics_inbound_utility.set_csv_definition('appoint_id_lookup',4);
    lics_inbound_utility.set_csv_definition('call_type_id',5);
    lics_inbound_utility.set_csv_definition('call_type_id_lookup',6);
    lics_inbound_utility.set_csv_definition('call_type_id_desc',7);
    lics_inbound_utility.set_csv_definition('cust_id',8);
    lics_inbound_utility.set_csv_definition('cust_id_lookup',9);
    lics_inbound_utility.set_csv_definition('duration',10);
    lics_inbound_utility.set_csv_definition('pos_id',11);
    lics_inbound_utility.set_csv_definition('pos_id_lookup',12);
    lics_inbound_utility.set_csv_definition('rep_id',13);
    lics_inbound_utility.set_csv_definition('rep_id_lookup',14);
    lics_inbound_utility.set_csv_definition('signature_id',15);
    lics_inbound_utility.set_csv_definition('signature_id_lookup',16);
    lics_inbound_utility.set_csv_definition('start_date',17);
    lics_inbound_utility.set_csv_definition('id_lookup',18);
    lics_inbound_utility.set_csv_definition('cust_id_desc',19);
    lics_inbound_utility.set_csv_definition('full_name',20);
    lics_inbound_utility.set_csv_definition('work_with_day_id',21);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,CreatedDate,Appointment_Id,Appointment_Id_Lookup,CallTypeId,CallTypeId_Lookup,CallTypeId_Description,Customer_Id,Customer_Id_Lookup,Duration,Position_Id,Position_Id_Lookup,Rep_Id,Rep_Id_Lookup,Signature_Id,Signature_Id_Lookup,StartDate,ID_Lookup,customer_id_description,fullname,WorkWithDayID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_callcard_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.appoint_id := quo_util.get_number('appoint_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.appoint_id_lookup := quo_util.get_string('appoint_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id := quo_util.get_number('call_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id_lookup := quo_util.get_string('call_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id_desc := quo_util.get_string('call_type_id_desc',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.start_date := quo_util.get_datetime('start_date',true,g_source_file_error_flag);
      l_entity_load_row.id_lookup := quo_util.get_number('id_lookup',10,0,true,g_source_file_error_flag);
      ...
      l_entity_load_row.work_with_day_id := quo_util.get_number('work_with_day_id',10,0,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_callcard_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_callcard_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_callcard 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_callcard_hist
    select * 
    from ods.quo_callcard_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_callcard_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_callcard/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw18 for ods_app.quo_quocdw18;

-- Grants
grant execute on ods_app.quo_quocdw18 to lics_app;
