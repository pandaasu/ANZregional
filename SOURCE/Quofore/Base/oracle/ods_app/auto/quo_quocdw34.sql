...
create or replace package quo_quocdw34 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw34
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw34] Entity [ActivityDetailDistCheck] Table [quo_act_dtl_dist_check] 
...
create or replace package body quo_quocdw34 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw34';
  g_entity_name constant varchar2(32 char) := 'ActivityDetailDistCheck';
  ...
  g_entity_load_row ods.quo_act_dtl_dist_check_load%rowtype;
  g_entity_row ods.quo_act_dtl_dist_check%rowtype;
  g_entity_hist_row ods.quo_act_dtl_dist_check_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('is_ranged',5);
    lics_inbound_utility.set_csv_definition('is_tag_missing',6);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Activity_ID,Activity_ID_Lookup,IsRanged,IsTagMissing,Product_ID,Product_ID_Lookup';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_dtl_dist_check_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.is_ranged := quo_util.get_boolean_as_number('is_ranged',true,g_source_file_error_flag);
      l_entity_load_row.is_tag_missing := quo_util.get_boolean_as_number('is_tag_missing',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_act_dtl_dist_check_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_dtl_dist_check_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_act_dtl_dist_check_load
    ...
    delete from ods.quo_act_dtl_dist_check
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_act_dtl_dist_check_load
    ...
    insert into ods.quo_act_dtl_dist_check
    select * 
    from ods.quo_act_dtl_dist_check_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_dtl_dist_check 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_dtl_dist_check_hist
    select * 
    from ods.quo_act_dtl_dist_check_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_dtl_dist_check_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_dtl_dist_check/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw34 for ods_app.quo_quocdw34;

-- Grants
grant execute on ods_app.quo_quocdw34 to lics_app;
