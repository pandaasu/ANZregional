...
create or replace package quo_quocdw19 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw19
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw19] Entity [CallcardNote] Table [quo_callcard_note] 
...
create or replace package body quo_quocdw19 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw19';
  g_entity_name constant varchar2(32 char) := 'CallcardNote';
  ...
  g_entity_load_row ods.quo_callcard_note_load%rowtype;
  g_entity_row ods.quo_callcard_note%rowtype;
  g_entity_hist_row ods.quo_callcard_note_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('callcard_id',5);
    lics_inbound_utility.set_csv_definition('callcard_id_lookup',6);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,IsActive,CreatedDate,Callcard_Id,Callcard_Id_Lookup,NoteText,Rep_Id,Rep_Id_Lookup,fullname';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_callcard_note_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_callcard_note_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_callcard_note_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_callcard_note_load
    ...
    delete from ods.quo_callcard_note
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_callcard_note_load
    ...
    insert into ods.quo_callcard_note
    select * 
    from ods.quo_callcard_note_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_callcard_note 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_callcard_note_hist
    select * 
    from ods.quo_callcard_note_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_callcard_note_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_callcard_note/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw19 for ods_app.quo_quocdw19;

-- Grants
grant execute on ods_app.quo_quocdw19 to lics_app;
