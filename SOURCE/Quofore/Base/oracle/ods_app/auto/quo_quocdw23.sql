...
create or replace package quo_quocdw23 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw23
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw23] Entity [CustomerTerritory] Table [quo_cust_terr] 
...
create or replace package body quo_quocdw23 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw23';
  g_entity_name constant varchar2(32 char) := 'CustomerTerritory';
  ...
  g_entity_load_row ods.quo_cust_terr_load%rowtype;
  g_entity_row ods.quo_cust_terr%rowtype;
  g_entity_hist_row ods.quo_cust_terr_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('terr_name',7);
    lics_inbound_utility.set_csv_definition('id',8);
    lics_inbound_utility.set_csv_definition('cust_name',9);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'IsActive,CreatedDate,Customer_Id,Customer_Id_Lookup,Territory_Id,Territory_Id_Lookup,TerritoryName,id,customername,customer_id_description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_cust_terr_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_cust_terr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_cust_terr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_cust_terr 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_cust_terr_hist
    select * 
    from ods.quo_cust_terr_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_cust_terr_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_cust_terr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw23 for ods_app.quo_quocdw23;

-- Grants
grant execute on ods_app.quo_quocdw23 to lics_app;
