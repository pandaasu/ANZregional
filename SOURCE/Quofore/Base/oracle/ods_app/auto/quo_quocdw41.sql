...
create or replace package quo_quocdw41 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw41
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw41] Entity [SurveyAnswer] Table [quo_survey_answer] 
...
create or replace package body quo_quocdw41 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw41';
  g_entity_name constant varchar2(32 char) := 'SurveyAnswer';
  ...
  g_entity_load_row ods.quo_survey_answer_load%rowtype;
  g_entity_row ods.quo_survey_answer%rowtype;
  g_entity_hist_row ods.quo_survey_answer_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_response_id',3);
    lics_inbound_utility.set_csv_definition('survey_response_id_lookup',4);
    lics_inbound_utility.set_csv_definition('survey_id',5);
    lics_inbound_utility.set_csv_definition('survey_id_lookup',6);
    lics_inbound_utility.set_csv_definition('survey_question_id',7);
    lics_inbound_utility.set_csv_definition('survey_question_id_lookup',8);
    lics_inbound_utility.set_csv_definition('response_opt_id',9);
    lics_inbound_utility.set_csv_definition('response_opt_id_lookup',10);
    lics_inbound_utility.set_csv_definition('freeform_response',11);
    lics_inbound_utility.set_csv_definition('rep_id',12);
    lics_inbound_utility.set_csv_definition('rep_id_lookup',13);
    lics_inbound_utility.set_csv_definition('callcard_id',14);
    lics_inbound_utility.set_csv_definition('callcard_id_lookup',15);
    lics_inbound_utility.set_csv_definition('task_id',16);
    lics_inbound_utility.set_csv_definition('survey_response_created_date',17);
    lics_inbound_utility.set_csv_definition('task_created_date',18);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,SurveyResponse_ID,SurveyResponse_ID_Lookup,Survey_ID,Survey_ID_Lookup,SurveyQuestion_ID,SurveyQuestion_ID_Lookup,ResponseOption_ID,ResponseOption_ID_Lookup,FreeFormResponse,Rep_ID,Rep_ID_Lookup,Callcard_ID,Callcard_ID_Lookup,Task_ID,SurveyResponseCreatedDate,TaskCreatedDate';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_survey_answer_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_response_id := quo_util.get_number('survey_response_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_response_id_lookup := quo_util.get_string('survey_response_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.survey_id := quo_util.get_number('survey_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_id_lookup := quo_util.get_string('survey_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.survey_question_id := quo_util.get_number('survey_question_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_question_id_lookup := quo_util.get_string('survey_question_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.response_opt_id := quo_util.get_number('response_opt_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.response_opt_id_lookup := quo_util.get_string('response_opt_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.freeform_response := quo_util.get_string('freeform_response',4000,true,g_source_file_error_flag);
      ...
      l_entity_load_row.task_id := quo_util.get_number('task_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_response_created_date := quo_util.get_datetime('survey_response_created_date',true,g_source_file_error_flag);
      l_entity_load_row.task_created_date := quo_util.get_datetime('task_created_date',true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_survey_answer_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_survey_answer_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_survey_answer_load
    ...
    delete from ods.quo_survey_answer
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_survey_answer_load
    ...
    insert into ods.quo_survey_answer
    select * 
    from ods.quo_survey_answer_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_survey_answer 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_survey_answer_hist
    select * 
    from ods.quo_survey_answer_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_survey_answer_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_survey_answer/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw41 for ods_app.quo_quocdw41;

-- Grants
grant execute on ods_app.quo_quocdw41 to lics_app;
