...
create or replace package quo_quocdw14 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw14
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw14] Entity [CustomerAssortmentDetail] Table [quo_cust_assort_dtl] 
...
create or replace package body quo_quocdw14 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw14';
  g_entity_name constant varchar2(32 char) := 'CustomerAssortmentDetail';
  ...
  g_entity_load_row ods.quo_cust_assort_dtl_load%rowtype;
  g_entity_row ods.quo_cust_assort_dtl%rowtype;
  g_entity_hist_row ods.quo_cust_assort_dtl_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('cust_hier_node_id_lookup',11);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,id_lookup,createddate,assortment_id,assortment_id_lookup,assortmentdetail_id,assortmentdetail_id_lookup,customer_id,customer_id_lookup,customer_hierarchynode_id,customer_hierarchynode_id_lookup';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_cust_assort_dtl_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_cust_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_cust_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_cust_assort_dtl_load
    ...
    delete from ods.quo_cust_assort_dtl
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_cust_assort_dtl_load
    ...
    insert into ods.quo_cust_assort_dtl
    select * 
    from ods.quo_cust_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_cust_assort_dtl 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_cust_assort_dtl_hist
    select * 
    from ods.quo_cust_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_cust_assort_dtl_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_cust_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw14 for ods_app.quo_quocdw14;

-- Grants
grant execute on ods_app.quo_quocdw14 to lics_app;
