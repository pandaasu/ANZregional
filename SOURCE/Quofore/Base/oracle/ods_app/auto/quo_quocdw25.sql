...
create or replace package quo_quocdw25 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw25
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw25] Entity [Survey] Table [quo_survey] 
...
create or replace package body quo_quocdw25 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw25';
  g_entity_name constant varchar2(32 char) := 'Survey';
  ...
  g_entity_load_row ods.quo_survey_load%rowtype;
  g_entity_row ods.quo_survey%rowtype;
  g_entity_hist_row ods.quo_survey_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('survey_type_id',5);
    lics_inbound_utility.set_csv_definition('survey_type_id_lookup',6);
    lics_inbound_utility.set_csv_definition('survey_type_id_desc',7);
    lics_inbound_utility.set_csv_definition('name',8);
    lics_inbound_utility.set_csv_definition('note',9);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,IsActive,CreatedDate,SurveyTypeId,SurveyTypeId_Lookup,SurveyTypeId_Description,Name,Notes';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_survey_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_type_id := quo_util.get_number('survey_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_type_id_lookup := quo_util.get_string('survey_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.survey_type_id_desc := quo_util.get_string('survey_type_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_survey_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_survey_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    insert into ods.quo_survey_hist
    select * 
    from ods.quo_survey_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_survey_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_survey/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw25 for ods_app.quo_quocdw25;

-- Grants
grant execute on ods_app.quo_quocdw25 to lics_app;
