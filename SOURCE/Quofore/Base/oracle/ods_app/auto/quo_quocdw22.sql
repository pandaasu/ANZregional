...
create or replace package quo_quocdw22 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw22
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw22] Entity [Territory] Table [quo_terr] 
...
create or replace package body quo_quocdw22 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw22';
  ...
  g_entity_load_row ods.quo_terr_load%rowtype;
  g_entity_row ods.quo_terr%rowtype;
  g_entity_hist_row ods.quo_terr_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('note',5);
    lics_inbound_utility.set_csv_definition('terr_name',6);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,IsActive,CreatedDate,Notes,TerritoryName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_terr_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.quo_terr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_terr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_terr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw22 for ods_app.quo_quocdw22;

-- Grants
grant execute on ods_app.quo_quocdw22 to lics_app;
