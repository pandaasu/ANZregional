...
create or replace package quo_quocdw38 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw38
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw38] Entity [ActivityDetailSoCPSD] Table [quo_act_dtl_soc_psd] 
...
create or replace package body quo_quocdw38 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw38';
  g_entity_name constant varchar2(32 char) := 'ActivityDetailSoCPSD';
  ...
  g_entity_load_row ods.quo_act_dtl_soc_psd_load%rowtype;
  g_entity_row ods.quo_act_dtl_soc_psd%rowtype;
  g_entity_hist_row ods.quo_act_dtl_soc_psd_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('soc_nestle_pods',5);
    lics_inbound_utility.set_csv_definition('soc_private_label_pods',6);
    lics_inbound_utility.set_csv_definition('soc_other_label_pods',7);
    lics_inbound_utility.set_csv_definition('prod_hier_id',8);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Activity_ID,Activity_ID_Lookup,SOC_NestlePODs,SOC_PrivateLabelPODs,SOC_OtherLabelPODs,ProductHierarchy_Hierarchy_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_act_dtl_soc_psd_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.soc_nestle_pods := quo_util.get_number('soc_nestle_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_private_label_pods := quo_util.get_number('soc_private_label_pods',18,4,true,g_source_file_error_flag);
      l_entity_load_row.soc_other_label_pods := quo_util.get_number('soc_other_label_pods',18,4,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_act_dtl_soc_psd_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_act_dtl_soc_psd_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.quo_act_dtl_soc_psd_load
    ...
    delete from ods.quo_act_dtl_soc_psd
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_act_dtl_soc_psd_load
    ...
    insert into ods.quo_act_dtl_soc_psd
    select * 
    from ods.quo_act_dtl_soc_psd_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_act_dtl_soc_psd 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_act_dtl_soc_psd_hist
    select * 
    from ods.quo_act_dtl_soc_psd_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_act_dtl_soc_psd_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_act_dtl_soc_psd/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw38 for ods_app.quo_quocdw38;

-- Grants
grant execute on ods_app.quo_quocdw38 to lics_app;
