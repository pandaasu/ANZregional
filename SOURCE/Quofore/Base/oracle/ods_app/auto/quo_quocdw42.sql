...
create or replace package quo_quocdw42 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw42
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw42] Entity [Graveyard] Table [quo_graveyard] 
...
create or replace package body quo_quocdw42 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw42';
  g_entity_name constant varchar2(32 char) := 'Graveyard';
  ...
  g_entity_load_row ods.quo_graveyard_load%rowtype;
  g_entity_row ods.quo_graveyard%rowtype;
  g_entity_hist_row ods.quo_graveyard_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('entity',3);
    lics_inbound_utility.set_csv_definition('unique_id',4);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Lookup,Entity,UniqueID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_graveyard_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.entity := quo_util.get_string('entity',50,false,g_source_file_error_flag);
      l_entity_load_row.unique_id := quo_util.get_string('unique_id',100,true,g_source_file_error_flag);
    exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.quo_graveyard_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_graveyard_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    delete from ods.quo_graveyard
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    and (q4x_source_id,id,entity) in (
      select q4x_source_id,id,entity 
      from ods.quo_graveyard_load 
      where q4x_source_id = p_source_id
      and q4x_batch_id = p_batch_id
    );
    ...
    insert into ods.quo_graveyard
    select * 
    from ods.quo_graveyard_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id,entity) not in (
      select q4x_source_id,id,entity 
      from ods.quo_graveyard 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_graveyard_hist
    select * 
    from ods.quo_graveyard_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_graveyard_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_graveyard/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw42 for ods_app.quo_quocdw42;

-- Grants
grant execute on ods_app.quo_quocdw42 to lics_app;
