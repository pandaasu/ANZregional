...
create or replace package quo_quocdw17 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw17
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw17] Entity [Appointment] Table [quo_appoint] 
...
create or replace package body quo_quocdw17 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw17';
  g_entity_name constant varchar2(32 char) := 'Appointment';
  ...
  g_entity_load_row ods.quo_appoint_load%rowtype;
  g_entity_row ods.quo_appoint%rowtype;
  g_entity_hist_row ods.quo_appoint_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('call_type_id',2);
    lics_inbound_utility.set_csv_definition('call_type_id_lookup',3);
    lics_inbound_utility.set_csv_definition('call_type_id_desc',4);
    lics_inbound_utility.set_csv_definition('cust_id',5);
    lics_inbound_utility.set_csv_definition('cust_id_lookup',6);
    lics_inbound_utility.set_csv_definition('pos_id',7);
    lics_inbound_utility.set_csv_definition('pos_id_lookup',8);
    lics_inbound_utility.set_csv_definition('scheduled_date',9);
    lics_inbound_utility.set_csv_definition('seq',10);
    lics_inbound_utility.set_csv_definition('start_time',11);
    lics_inbound_utility.set_csv_definition('work_with_day_id',12);
    lics_inbound_utility.set_csv_definition('work_with_day_id_lookup',13);
    lics_inbound_utility.set_csv_definition('work_with_day_id_desc',14);
    lics_inbound_utility.set_csv_definition('id_lookup',15);
    lics_inbound_utility.set_csv_definition('cust_id_desc',16);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,CallTypeId,CallTypeId_Lookup,CallTypeId_Description,Customer_Id,Customer_Id_Lookup,Position_Id,Position_Id_Lookup,ScheduledDate,Sequence,StartTime,WorkWithDayID,WorkWithDayID_Lookup,WorkWithDayID_Description,Id_Lookup,Customer_ID_Description';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_appoint_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.scheduled_date := quo_util.get_date('scheduled_date',true,g_source_file_error_flag);
      l_entity_load_row.seq := quo_util.get_number('seq',3,0,true,g_source_file_error_flag);
      l_entity_load_row.start_time := quo_util.get_time('start_time',true,g_source_file_error_flag);
      l_entity_load_row.work_with_day_id := quo_util.get_number('work_with_day_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.work_with_day_id_lookup := quo_util.get_string('work_with_day_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.work_with_day_id_desc := quo_util.get_string('work_with_day_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_appoint_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_appoint_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_appoint 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_appoint_hist
    select * 
    from ods.quo_appoint_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_appoint_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_appoint/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw17 for ods_app.quo_quocdw17;

-- Grants
grant execute on ods_app.quo_quocdw17 to lics_app;
