...
create or replace package quo_quocdw13 as
/*******************************************************************************
** Package Definition
********************************************************************************
  ...
  System  : quo
  Owner   : ods_app
  Package : quo_quocdw13
  Author  : Mal Chambeyron
  
  Description
  ------------------------------------------------------------------------------
  Quofore Loader Package - Interface [quocdw13] Entity [AssortmentDetail] Table [quo_assort_dtl] 
...
create or replace package body quo_quocdw13 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.quo_quocdw13';
  g_entity_name constant varchar2(32 char) := 'AssortmentDetail';
  ...
  g_entity_load_row ods.quo_assort_dtl_load%rowtype;
  g_entity_row ods.quo_assort_dtl%rowtype;
  g_entity_hist_row ods.quo_assort_dtl_hist%rowtype;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('assort_created_date',6);
    lics_inbound_utility.set_csv_definition('name',7);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id',8);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id_lookup',9);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id_desc',10);
    lics_inbound_utility.set_csv_definition('assort_dtl_name',11);
    lics_inbound_utility.set_csv_definition('hier_level',12);
    lics_inbound_utility.set_csv_definition('cust_hier_node_id',13);
    lics_inbound_utility.set_csv_definition('cust_hier_node_id_lookup',14);
    lics_inbound_utility.set_csv_definition('parent_id',15);
    lics_inbound_utility.set_csv_definition('parent_id_lookup',16);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,id_lookup,createddate,assortment_id,assortment_id_lookup,assortmentcreateddate,name,assortmentdetailtypeid,assortmentdetailtype_id_lookup,assortmentdetailtype_id_description,assortmentdetailname,level,customer_hierarchynode_id,customer_hierarchynode_id_lookup,parent_id,parent_id_lookup';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.quo_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_created_date := quo_util.get_datetime('assort_created_date',true,g_source_file_error_flag);
      l_entity_load_row.name := quo_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id := quo_util.get_number('assort_dtl_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_lookup := quo_util.get_string('assort_dtl_type_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_desc := quo_util.get_string('assort_dtl_type_id_desc',100,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_name := quo_util.get_string('assort_dtl_name',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_level := quo_util.get_number('hier_level',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id := quo_util.get_number('cust_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id_lookup := quo_util.get_string('cust_hier_node_id_lookup',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.quo_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.quo_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    delete from ods.quo_assort_dtl
    where q4x_source_id = p_source_id
    and q4x_batch_id < p_batch_id
    ...
    update ods.quo_assort_dtl_load
    ...
    insert into ods.quo_assort_dtl
    select * 
    from ods.quo_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id
    and (q4x_source_id,id) not in (
      select q4x_source_id,id 
      from ods.quo_assort_dtl 
      where q4x_source_id = p_source_id
    );

    -- Insert ALL of Load Batch into History
    insert into ods.quo_assort_dtl_hist
    select * 
    from ods.quo_assort_dtl_load
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
    ...
    -- Delete Load Batch
    delete from ods.quo_assort_dtl_load 
    where q4x_source_id = p_source_id
    and q4x_batch_id = p_batch_id;
  ...
  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.quo_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym quo_quocdw13 for ods_app.quo_quocdw13;

-- Grants
grant execute on ods_app.quo_quocdw13 to lics_app;
