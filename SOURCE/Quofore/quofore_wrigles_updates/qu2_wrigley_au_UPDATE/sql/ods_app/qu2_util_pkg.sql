...
create or replace package ods_app.qu2_util as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
...
create or replace package body ods_app.qu2_util as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_util';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    select qu2_load_seq.nextval into l_next_seq from dual;
    return l_next_seq; -- Normal Return

  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_next_load_seq] : Failed to Get Next Sequence [ods.qu2_load_seq.nextval] : '||SQLERRM, 1, 4000));
  ...
  *****************************************************************************/
  ...
  begin
    ...
    l_set_group := qu2_constants.setting_group;
  ...
  *****************************************************************************/
  ...
  exception
    when others then
      raise_application_error(-20000, substr('['||g_package_name||'.get_entity_interface_name] : Entity ['||upper(p_entity_name)||'] : Table [ods.qu2_interface_list] : '||SQLERRM, 1, 4000));
  ...
  *****************************************************************************/
  ...
  begin
    ...
    if l_entity_interface_name is null then
      raise_application_error(-20000, 'Entity ['||upper(p_entity_name)||'] Interface Not Configured in Table [ods.qu2_interface_list]');
    end if;
...
-- Synonyms
create or replace public synonym qu2_util for ods_app.qu2_util;

-- Grants
grant execute on ods_app.qu2_util to lics_app, qv_user, bo_user;
