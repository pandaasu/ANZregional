...
create or replace package ods_app.qu2_interface as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System  : qu2
    Owner   : ods_app
    Package : qu2_interface
    Author  : Mal Chambeyron

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Interface Control Package .. Maintains the [qu2_interface_hdr] table
...
create or replace package body ods_app.qu2_interface as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_interface';
  g_package_desc constant varchar2(64 char) := 'Interface Header [ods.qu2_interface_hdr]';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    l_load_seq := qu2_util.get_next_load_seq;

    -- Create Interface Header
    begin

      insert into qu2_interface_hdr (
        q4x_load_seq,
        q4x_create_user,
        q4x_create_time,
        q4x_modify_user,
        ...
        qu2_constants.status_started, -- q4x_status
  ...
  *****************************************************************************/
  ...
  begin
    ...
    if l_status <> qu2_constants.status_started then
        raise_application_error(-20000, g_package_desc||' for '||l_key_desc||' Invalid Status ['||l_status||'] Expected ['||qu2_constants.status_started||']');
    end if;

    -- Update Interface Header
    begin

      update qu2_interface_hdr
      set
        q4x_modify_user = user,
        q4x_modify_time = sysdate,
        q4x_status = qu2_constants.status_loaded,
  ...
  begin
    ...
    begin
      ...
      set
        q4x_modify_user = user,
        q4x_modify_time = sysdate,
        q4x_status = qu2_constants.status_processed
      where q4x_batch_id = p_batch_id;
...
-- Synonyms
create or replace public synonym qu2_interface for ods_app.qu2_interface;

-- Grants
grant execute on ods_app.qu2_interface to lics_app;
