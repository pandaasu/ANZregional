...
create or replace package ods_app.qu2_qu2cdw25 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw25
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw25] Entity [PositionTerritory] Table [qu2_pos_terr][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw25 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw25';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('pos_id',4);
    lics_inbound_utility.set_csv_definition('terr_id',5);
    lics_inbound_utility.set_csv_definition('is_primary_terr',6);
    lics_inbound_utility.set_csv_definition('pos_name',7);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,IsActive,CreatedDate,Position_Id,Territory_Id,IsPrimaryTerritory,PositionName,TerritoryName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_pos_terr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.terr_id := qu2_util.get_number('terr_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.is_primary_terr := qu2_util.get_boolean_as_number('is_primary_terr',true,g_source_file_error_flag);
      l_entity_load_row.pos_name := qu2_util.get_string('pos_name',50,true,g_source_file_error_flag);
      l_entity_load_row.terr_name := qu2_util.get_string('terr_name',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_pos_terr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_pos_terr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_pos_terr_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_pos_terr_hist
    select *
    from ods.qu2_pos_terr_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_pos_terr_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_pos_terr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw25 for ods_app.qu2_qu2cdw25;

-- Grants
grant execute on ods_app.qu2_qu2cdw25 to lics_app;
