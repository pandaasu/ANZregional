...
create or replace package ods_app.qu2_qu2cdw12 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw12
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw12] Entity [CustomerContact] Table [qu2_cust_contact][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw12 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw12';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_cust_contact_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.cansign := qu2_util.get_boolean_as_number('cansign',true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu2_util.get_number('cust_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.dept_id := qu2_util.get_number('dept_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.dept_id_desc := qu2_util.get_string('dept_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.email := qu2_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.first_name := qu2_util.get_string('first_name',50,true,g_source_file_error_flag);
      l_entity_load_row.full_name := qu2_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.last_name := qu2_util.get_string('last_name',50,true,g_source_file_error_flag);
      l_entity_load_row.middle_name := qu2_util.get_string('middle_name',50,true,g_source_file_error_flag);
      l_entity_load_row.mobile_no := qu2_util.get_string('mobile_no',30,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := qu2_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.photo_file_name := qu2_util.get_string('photo_file_name',50,true,g_source_file_error_flag);
      l_entity_load_row.pos_id := qu2_util.get_number('pos_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pos_id_desc := qu2_util.get_string('pos_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.pref_contact_method_id := qu2_util.get_number('pref_contact_method_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pref_contact_method_id_desc := qu2_util.get_string('pref_contact_method_id_desc',50,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu2_cust_contact_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_cust_contact_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_cust_contact_load
    ...
    update ods.qu2_cust_contact_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_cust_contact_hist
    select *
    from ods.qu2_cust_contact_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_cust_contact_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_cust_contact/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw12 for ods_app.qu2_qu2cdw12;

-- Grants
grant execute on ods_app.qu2_qu2cdw12 to lics_app;
