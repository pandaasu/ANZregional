...
create or replace package ods_app.qu2_qu2cdw39 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw39
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw39] Entity [ActivityDetailOffLocation] Table [qu2_act_dtl_off_loc][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw39 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw39';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('promotion_week',3);
    lics_inbound_utility.set_csv_definition('no_gondola',4);
    lics_inbound_utility.set_csv_definition('no_wing',5);
    lics_inbound_utility.set_csv_definition('no_bin',6);
    lics_inbound_utility.set_csv_definition('no_clip_strip',7);
    lics_inbound_utility.set_csv_definition('stock_qty',8);
    lics_inbound_utility.set_csv_definition('prod_id',9);
    lics_inbound_utility.set_csv_definition('promotional_type',10);
    lics_inbound_utility.set_csv_definition('brand',11);
    lics_inbound_utility.set_csv_definition('promotional_impact',12);
    lics_inbound_utility.set_csv_definition('no_pre_pack_units',13);
    lics_inbound_utility.set_csv_definition('tm_influence',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,PromotionWeek,NumGondola,NumWing,NumBin,NumClipStrip,StockQty,Product_ID,PromotionalType,Brand,PromotionalImpact,NumPrepackUnits,TMInfluence';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_act_dtl_off_loc_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.promotion_week := qu2_util.get_number('promotion_week',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_gondola := qu2_util.get_number('no_gondola',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_wing := qu2_util.get_number('no_wing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_bin := qu2_util.get_number('no_bin',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_clip_strip := qu2_util.get_number('no_clip_strip',10,0,true,g_source_file_error_flag);
      l_entity_load_row.stock_qty := qu2_util.get_number('stock_qty',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu2_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.promotional_type := qu2_util.get_number('promotional_type',10,0,true,g_source_file_error_flag);
      l_entity_load_row.brand := qu2_util.get_number('brand',10,0,true,g_source_file_error_flag);
      l_entity_load_row.promotional_impact := qu2_util.get_number('promotional_impact',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_pre_pack_units := qu2_util.get_number('no_pre_pack_units',10,0,true,g_source_file_error_flag);
      l_entity_load_row.tm_influence := qu2_util.get_number('tm_influence',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_act_dtl_off_loc_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_act_dtl_off_loc_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_act_dtl_off_loc_load
    ...
    update ods.qu2_act_dtl_off_loc_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_act_dtl_off_loc_hist
    select *
    from ods.qu2_act_dtl_off_loc_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_act_dtl_off_loc_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_act_dtl_off_loc/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw39 for ods_app.qu2_qu2cdw39;

-- Grants
grant execute on ods_app.qu2_qu2cdw39 to lics_app;
