...
create or replace package ods_app.qu2_qu2cdw07 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw07
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw07] Entity [Product] Table [qu2_prod][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw07 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw07';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.inners_per_case := qu2_util.get_number('inners_per_case',5,0,true,g_source_file_error_flag);
      l_entity_load_row.is_new := qu2_util.get_boolean_as_number('is_new',true,g_source_file_error_flag);
      l_entity_load_row.is_priority := qu2_util.get_boolean_as_number('is_priority',true,g_source_file_error_flag);
      l_entity_load_row.list_price := qu2_util.get_number('list_price',18,4,true,g_source_file_error_flag);
      l_entity_load_row.unit_of_measure_id := qu2_util.get_number('unit_of_measure_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.unit_of_measure_id_desc := qu2_util.get_string('unit_of_measure_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.pack_size := qu2_util.get_number('pack_size',5,0,true,g_source_file_error_flag);
      l_entity_load_row.pack_desc := qu2_util.get_string('pack_desc',200,true,g_source_file_error_flag);
      l_entity_load_row.unit_size := qu2_util.get_number('unit_size',5,0,true,g_source_file_error_flag);
      l_entity_load_row.sku_code := qu2_util.get_string('sku_code',50,true,g_source_file_error_flag);
      l_entity_load_row.name := qu2_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.units_per_case := qu2_util.get_number('units_per_case',5,0,true,g_source_file_error_flag);
      l_entity_load_row.units_per_inner := qu2_util.get_number('units_per_inner',5,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_hier_id := qu2_util.get_number('prod_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.new_start := qu2_util.get_datetime('new_start',true,g_source_file_error_flag);
      l_entity_load_row.new_end := qu2_util.get_datetime('new_end',true,g_source_file_error_flag);
      l_entity_load_row.fivestar := qu2_util.get_boolean_as_number('fivestar',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_prod_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_prod_hist
    select *
    from ods.qu2_prod_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_prod_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw07 for ods_app.qu2_qu2cdw07;

-- Grants
grant execute on ods_app.qu2_qu2cdw07 to lics_app;
