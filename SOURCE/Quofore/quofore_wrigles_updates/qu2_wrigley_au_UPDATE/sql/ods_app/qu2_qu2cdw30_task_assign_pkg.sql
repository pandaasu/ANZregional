...
create or replace package ods_app.qu2_qu2cdw30 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw30
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw30] Entity [TaskAssignment] Table [qu2_task_assign][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw30 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw30';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_task_assign_load%rowtype;
  ...
  begin
    ...
    -- Insert row
    begin
      insert into ods.qu2_task_assign_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_task_assign_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_task_assign_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_task_assign_hist
    select *
    from ods.qu2_task_assign_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_task_assign_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_task_assign/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw30 for ods_app.qu2_qu2cdw30;

-- Grants
grant execute on ods_app.qu2_qu2cdw30 to lics_app;
