...
create or replace package ods_app.qu2_qu2cdw15 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw15
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw15] Entity [CustomerAssortmentDetail] Table [qu2_cust_assort_dtl][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw15 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw15';
  g_entity_name constant varchar2(64 char) := 'CustomerAssortmentDetail';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('assort_dtl_id',4);
    lics_inbound_utility.set_csv_definition('cust_id',5);
    lics_inbound_utility.set_csv_definition('cust_hier_node_id',6);
    lics_inbound_utility.set_csv_definition('assort_dtl_id_lookup',7);
    lics_inbound_utility.set_csv_definition('id_lookup',8);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,createddate,assortment_id,assortmentdetail_id,customer_id,customer_hierarchynode_id,assortmentdetail_id_lookup,id_lookup,customer_id_lookup';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_cust_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.cust_id_lookup := qu2_util.get_number('cust_id_lookup',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_cust_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_cust_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_cust_assort_dtl_load
    ...
    update ods.qu2_cust_assort_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_cust_assort_dtl_hist
    select *
    from ods.qu2_cust_assort_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_cust_assort_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_cust_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw15 for ods_app.qu2_qu2cdw15;

-- Grants
grant execute on ods_app.qu2_qu2cdw15 to lics_app;
