...
create or replace package ods_app.qu2_qu2cdw56 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw56
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw56] Entity [ActivityDetailExecCompliance] Table [qu2_act_dtl_exec_compl][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw56 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw56';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailExecCompliance';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('act_id',2);
    lics_inbound_utility.set_csv_definition('act_name',3);
    lics_inbound_utility.set_csv_definition('objective',4);
    lics_inbound_utility.set_csv_definition('hardware_on_entry',5);
    lics_inbound_utility.set_csv_definition('pos_on_entry',6);
    lics_inbound_utility.set_csv_definition('planogram_on_entry',7);
    lics_inbound_utility.set_csv_definition('allocation_on_entry',8);
    lics_inbound_utility.set_csv_definition('promotion_compliance_on_entry',9);
    lics_inbound_utility.set_csv_definition('over_all_compliance_on_entry',10);
    lics_inbound_utility.set_csv_definition('commentary_on_entry',11);
    lics_inbound_utility.set_csv_definition('hardware_on_exit',12);
    lics_inbound_utility.set_csv_definition('pos_on_exit',13);
    lics_inbound_utility.set_csv_definition('planogram_on_exit',14);
    lics_inbound_utility.set_csv_definition('allocation_on_exit',15);
    lics_inbound_utility.set_csv_definition('promotion_compliance_exit',16);
    lics_inbound_utility.set_csv_definition('over_all_compliance_exit',17);
    lics_inbound_utility.set_csv_definition('commentary_on_exit',18);
    lics_inbound_utility.set_csv_definition('action_plan',19);
    lics_inbound_utility.set_csv_definition('hier_node_id',20);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,ActivityName,Objective,HardwareOnEntry,PositionOnEntry,PlanogramOnEntry,AllocationOnEntry,PromotionComplianceOnEntry,OverallComplianceOnEntry,CommentaryOnEntry,HardwareOnExit,PositionOnExit,PlanogramOnExit,AllocationOnExit,PromotionComplianceExit,OverallComplianceExit,CommentaryOnExit,ActionPlan,HierarchyNode_Id';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_act_dtl_exec_compl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.act_id := qu2_util.get_number('act_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.act_name := qu2_util.get_number('act_name',10,0,true,g_source_file_error_flag);
      l_entity_load_row.objective := qu2_util.get_number('objective',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hardware_on_entry := qu2_util.get_number('hardware_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pos_on_entry := qu2_util.get_number('pos_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.planogram_on_entry := qu2_util.get_number('planogram_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.allocation_on_entry := qu2_util.get_number('allocation_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.promotion_compliance_on_entry := qu2_util.get_number('promotion_compliance_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.over_all_compliance_on_entry := qu2_util.get_number('over_all_compliance_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.commentary_on_entry := qu2_util.get_number('commentary_on_entry',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hardware_on_exit := qu2_util.get_number('hardware_on_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pos_on_exit := qu2_util.get_number('pos_on_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.planogram_on_exit := qu2_util.get_number('planogram_on_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.allocation_on_exit := qu2_util.get_number('allocation_on_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.promotion_compliance_exit := qu2_util.get_number('promotion_compliance_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.over_all_compliance_exit := qu2_util.get_number('over_all_compliance_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.commentary_on_exit := qu2_util.get_number('commentary_on_exit',10,0,true,g_source_file_error_flag);
      l_entity_load_row.action_plan := qu2_util.get_number('action_plan',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hier_node_id := qu2_util.get_number('hier_node_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_act_dtl_exec_compl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_act_dtl_exec_compl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_act_dtl_exec_compl_load
    ...
    update ods.qu2_act_dtl_exec_compl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_act_dtl_exec_compl_hist
    select *
    from ods.qu2_act_dtl_exec_compl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_act_dtl_exec_compl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_act_dtl_exec_compl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw56 for ods_app.qu2_qu2cdw56;

-- Grants
grant execute on ods_app.qu2_qu2cdw56 to lics_app;
