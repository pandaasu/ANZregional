...
create or replace package ods_app.qu2_qu2cdw48 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw48
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw48] Entity [SurveyAnswer] Table [qu2_survey_answer][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw48 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw48';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_survey_answer_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.survey_response_id := qu2_util.get_number('survey_response_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_id := qu2_util.get_number('survey_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.survey_question_id := qu2_util.get_number('survey_question_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.response_opt_id := qu2_util.get_number('response_opt_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.freeform_response := qu2_util.get_string('freeform_response',4000,true,g_source_file_error_flag);
      ...
      l_entity_load_row.survey_response_created_date := qu2_util.get_datetime('survey_response_created_date',true,g_source_file_error_flag);
      l_entity_load_row.task_created_date := qu2_util.get_datetime('task_created_date',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_survey_answer_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_survey_answer_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_survey_answer_load
    ...
    update ods.qu2_survey_answer_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_survey_answer_hist
    select *
    from ods.qu2_survey_answer_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_survey_answer_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_survey_answer/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw48 for ods_app.qu2_qu2cdw48;

-- Grants
grant execute on ods_app.qu2_qu2cdw48 to lics_app;
