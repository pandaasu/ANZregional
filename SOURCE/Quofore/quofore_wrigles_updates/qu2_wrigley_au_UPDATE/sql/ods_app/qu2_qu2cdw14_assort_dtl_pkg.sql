...
create or replace package ods_app.qu2_qu2cdw14 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw14
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw14] Entity [AssortmentDetail] Table [qu2_assort_dtl][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw14 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw14';
  g_entity_name constant varchar2(64 char) := 'AssortmentDetail';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('assort_id',3);
    lics_inbound_utility.set_csv_definition('assort_created_date',4);
    lics_inbound_utility.set_csv_definition('name',5);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id',6);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id_desc',7);
    lics_inbound_utility.set_csv_definition('assort_dtl_name',8);
    lics_inbound_utility.set_csv_definition('hier_level',9);
    lics_inbound_utility.set_csv_definition('cust_hier_node_id',10);
    lics_inbound_utility.set_csv_definition('parent_id',11);
    lics_inbound_utility.set_csv_definition('cust_hier_node_id_lookup',12);
    lics_inbound_utility.set_csv_definition('parent_id_lookup',13);
    lics_inbound_utility.set_csv_definition('assort_dtl_type_id_lookup',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'id,createddate,assortment_id,assortmentcreateddate,name,assortmentdetailtypeid,assortmentdetailtype_id_description,assortmentdetailname,level,customer_hierarchynode_id,parent_id,customer_hierarchynode_id_lookup,parent_id_lookup,assortmentdetailtype_id_lookup';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_assort_dtl_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.assort_id := qu2_util.get_number('assort_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_created_date := qu2_util.get_datetime('assort_created_date',true,g_source_file_error_flag);
      l_entity_load_row.name := qu2_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id := qu2_util.get_number('assort_dtl_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_desc := qu2_util.get_string('assort_dtl_type_id_desc',100,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_name := qu2_util.get_string('assort_dtl_name',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_level := qu2_util.get_number('hier_level',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id := qu2_util.get_number('cust_hier_node_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.parent_id := qu2_util.get_number('parent_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_node_id_lookup := qu2_util.get_string('cust_hier_node_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_lookup := qu2_util.get_string('parent_id_lookup',50,true,g_source_file_error_flag);
      l_entity_load_row.assort_dtl_type_id_lookup := qu2_util.get_string('assort_dtl_type_id_lookup',50,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_assort_dtl_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_assort_dtl_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_assort_dtl_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_assort_dtl_hist
    select *
    from ods.qu2_assort_dtl_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_assort_dtl_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_assort_dtl/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw14 for ods_app.qu2_qu2cdw14;

-- Grants
grant execute on ods_app.qu2_qu2cdw14 to lics_app;
