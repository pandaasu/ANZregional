...
create or replace package ods_app.qu2_qu2cdw21 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw21
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw21] Entity [OrderHeader] Table [qu2_ord_hdr][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw21 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw21';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_ord_hdr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.actual_delivery_date := qu2_util.get_date('actual_delivery_date',true,g_source_file_error_flag);
      l_entity_load_row.callcard_id := qu2_util.get_number('callcard_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.city := qu2_util.get_string('city',50,true,g_source_file_error_flag);
      l_entity_load_row.country_id := qu2_util.get_number('country_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.country_id_desc := qu2_util.get_string('country_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.created_date := qu2_util.get_datetime('created_date',true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu2_util.get_number('cust_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_contact_id := qu2_util.get_number('cust_contact_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.delivery_note := qu2_util.get_string('delivery_note',200,true,g_source_file_error_flag);
      l_entity_load_row.latitude := qu2_util.get_number('latitude',9,6,true,g_source_file_error_flag);
      l_entity_load_row.longitude := qu2_util.get_number('longitude',9,6,true,g_source_file_error_flag);
      l_entity_load_row.note := qu2_util.get_string('note',200,true,g_source_file_error_flag);
      l_entity_load_row.ord_date := qu2_util.get_date('ord_date',true,g_source_file_error_flag);
      l_entity_load_row.ord_no := qu2_util.get_string('ord_no',50,true,g_source_file_error_flag);
      l_entity_load_row.ord_ref := qu2_util.get_string('ord_ref',50,true,g_source_file_error_flag);
      l_entity_load_row.ord_status_id := qu2_util.get_number('ord_status_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.ord_status_id_desc := qu2_util.get_string('ord_status_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.ord_sub_type_id := qu2_util.get_number('ord_sub_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.ord_sub_type_id_desc := qu2_util.get_string('ord_sub_type_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.ord_type_id := qu2_util.get_number('ord_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.ord_type_id_desc := qu2_util.get_string('ord_type_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.orig_delivery_date := qu2_util.get_date('orig_delivery_date',true,g_source_file_error_flag);
      l_entity_load_row.post_code := qu2_util.get_string('post_code',10,true,g_source_file_error_flag);
      l_entity_load_row.rep_id := qu2_util.get_number('rep_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.state_id := qu2_util.get_number('state_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.state_id_desc := qu2_util.get_string('state_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.street_1 := qu2_util.get_string('street_1',50,true,g_source_file_error_flag);
      l_entity_load_row.street_2 := qu2_util.get_string('street_2',50,true,g_source_file_error_flag);
      l_entity_load_row.town := qu2_util.get_string('town',50,true,g_source_file_error_flag);
      l_entity_load_row.cust_id_desc := qu2_util.get_string('cust_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.full_name := qu2_util.get_string('full_name',101,true,g_source_file_error_flag);
      l_entity_load_row.wholesaler_id := qu2_util.get_number('wholesaler_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_copy_email := qu2_util.get_boolean_as_number('cust_copy_email',true,g_source_file_error_flag);
      l_entity_load_row.purchase_ord_no := qu2_util.get_string('purchase_ord_no',31,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_ord_hdr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_ord_hdr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_ord_hdr_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_ord_hdr_hist
    select *
    from ods.qu2_ord_hdr_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_ord_hdr_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_ord_hdr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw21 for ods_app.qu2_qu2cdw21;

-- Grants
grant execute on ods_app.qu2_qu2cdw21 to lics_app;
