...
create or replace package ods_app.qu2_qu2cdw38 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw38
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw38] Entity [ActivityDetailSellIn] Table [qu2_act_dtl_sell_in][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw38 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw38';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailSellIn';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_deals_sold',3);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumDealsSold,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_act_dtl_sell_in_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_deals_sold := qu2_util.get_number('no_deals_sold',10,0,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu2_act_dtl_sell_in_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_act_dtl_sell_in_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_act_dtl_sell_in_load
    ...
    update ods.qu2_act_dtl_sell_in_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_act_dtl_sell_in_hist
    select *
    from ods.qu2_act_dtl_sell_in_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_act_dtl_sell_in_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_act_dtl_sell_in/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw38 for ods_app.qu2_qu2cdw38;

-- Grants
grant execute on ods_app.qu2_qu2cdw38 to lics_app;
