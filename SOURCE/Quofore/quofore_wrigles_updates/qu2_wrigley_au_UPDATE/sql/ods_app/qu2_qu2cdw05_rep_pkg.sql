...
create or replace package ods_app.qu2_qu2cdw05 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw05
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw05] Entity [Rep] Table [qu2_rep][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw05 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw05';
  g_entity_name constant varchar2(64 char) := 'Rep';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('emp_no',4);
    lics_inbound_utility.set_csv_definition('first_name',5);
    lics_inbound_utility.set_csv_definition('last_name',6);
    lics_inbound_utility.set_csv_definition('middle_name',7);
    lics_inbound_utility.set_csv_definition('home_no',8);
    lics_inbound_utility.set_csv_definition('mobile_no',9);
    lics_inbound_utility.set_csv_definition('fax_no',10);
    lics_inbound_utility.set_csv_definition('email',11);
    lics_inbound_utility.set_csv_definition('pos_id',12);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,IsActive,CreatedDate,EmployeeNumber,FirstName,LastName,MiddleName,HomeNumber,MobileNumber,FaxNumber,Email,Position_ID,FullName';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_rep_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.emp_no := qu2_util.get_string('emp_no',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.home_no := qu2_util.get_string('home_no',30,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu2_rep_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_rep_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu2_rep_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_rep_hist
    select *
    from ods.qu2_rep_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_rep_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_rep/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw05 for ods_app.qu2_qu2cdw05;

-- Grants
grant execute on ods_app.qu2_qu2cdw05 to lics_app;
