...
create or replace package ods_app.qu2_qu2cdw02 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw02
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw02] Entity [GeneralList] Table [qu2_general_list][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw02 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw02';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('lookup_list_sort_ord',9);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_general_list_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.lookup_list_item_is_active := qu2_util.get_boolean_as_number('lookup_list_item_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_created_date := qu2_util.get_datetime('lookup_list_item_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_item_name := qu2_util.get_string('lookup_list_item_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_is_active := qu2_util.get_boolean_as_number('lookup_list_is_active',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_id := qu2_util.get_number('lookup_list_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_name := qu2_util.get_string('lookup_list_name',50,true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_created_date := qu2_util.get_datetime('lookup_list_created_date',true,g_source_file_error_flag);
      l_entity_load_row.lookup_list_sort_ord := qu2_util.get_number('lookup_list_sort_ord',5,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_general_list_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_general_list_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_general_list_load
    ...
    update ods.qu2_general_list_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_general_list_hist
    select *
    from ods.qu2_general_list_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_general_list_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_general_list/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw02 for ods_app.qu2_qu2cdw02;

-- Grants
grant execute on ods_app.qu2_qu2cdw02 to lics_app;
