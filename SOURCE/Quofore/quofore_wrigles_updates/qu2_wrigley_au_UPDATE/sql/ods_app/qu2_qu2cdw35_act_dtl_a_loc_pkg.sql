...
create or replace package ods_app.qu2_qu2cdw35 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu2
    Owner    : ods_app
    Package  : qu2_qu2cdw35
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu2] Quofore - Wrigley Australia
    Loader Package, Interface [qu2cdw35] Entity [ActivityDetailALoc] Table [qu2_act_dtl_a_loc][_load/_hist]
...
create or replace package body ods_app.qu2_qu2cdw35 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu2_qu2cdw35';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailALoc';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_total',3);
    lics_inbound_utility.set_csv_definition('no_a_loc',4);
    lics_inbound_utility.set_csv_definition('prod_id',5);
    lics_inbound_utility.set_csv_definition('secondary_loc',6);
    lics_inbound_utility.set_csv_definition('other_loc',7);
    lics_inbound_utility.set_csv_definition('no_ts',8);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumTotal,NumALocation,Product_ID,SecondaryLocation,OtherLocation,NumTS';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu2_act_dtl_a_loc_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_total := qu2_util.get_number('no_total',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_a_loc := qu2_util.get_number('no_a_loc',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu2_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.secondary_loc := qu2_util.get_number('secondary_loc',10,0,true,g_source_file_error_flag);
      l_entity_load_row.other_loc := qu2_util.get_number('other_loc',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_ts := qu2_util.get_number('no_ts',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu2_act_dtl_a_loc_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu2_act_dtl_a_loc_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu2_act_dtl_a_loc_load
    ...
    update ods.qu2_act_dtl_a_loc_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu2_act_dtl_a_loc_hist
    select *
    from ods.qu2_act_dtl_a_loc_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu2_act_dtl_a_loc_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu2_act_dtl_a_loc/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu2_qu2cdw35 for ods_app.qu2_qu2cdw35;

-- Grants
grant execute on ods_app.qu2_qu2cdw35 to lics_app;
