...
create or replace package ods_app.qu3_qu3cdw47 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw47
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw47] Entity [ActivityDetailPackagingChgAudit] Table [qu3_act_dtl_pcking_chg][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw47 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw47';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailPackagingChgAudit';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('percent_stock_in_new_pack',3);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,PctStockInNewPack,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_pcking_chg_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.percent_stock_in_new_pack := qu3_util.get_number('percent_stock_in_new_pack',10,0,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_pcking_chg_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_pcking_chg_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_act_dtl_pcking_chg_load
    ...
    update ods.qu3_act_dtl_pcking_chg_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_pcking_chg_hist
    select *
    from ods.qu3_act_dtl_pcking_chg_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_pcking_chg_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_pcking_chg/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw47 for ods_app.qu3_qu3cdw47;

-- Grants
grant execute on ods_app.qu3_qu3cdw47 to lics_app;
