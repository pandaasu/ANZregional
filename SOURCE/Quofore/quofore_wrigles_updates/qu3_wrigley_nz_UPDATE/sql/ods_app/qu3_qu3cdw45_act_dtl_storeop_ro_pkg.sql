...
create or replace package ods_app.qu3_qu3cdw45 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw45
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw45] Entity [ActivityDetailStoreOppRoute] Table [qu3_act_dtl_storeop_ro][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw45 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw45';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailStoreOppRoute';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_register',3);
    lics_inbound_utility.set_csv_definition('no_chiller_door',4);
    lics_inbound_utility.set_csv_definition('prod_id',5);
    lics_inbound_utility.set_csv_definition('no_parallel_store',6);
    lics_inbound_utility.set_csv_definition('no_paralle_wwy_stand',7);
    lics_inbound_utility.set_csv_definition('no_comp_wwy_stand',8);
    lics_inbound_utility.set_csv_definition('no_oos_wwy_stand',9);
    lics_inbound_utility.set_csv_definition('no_pack_date_issue',10);
    lics_inbound_utility.set_csv_definition('pref_wholesaler',11);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumRegister,NumChillerDoor,Product_ID,NumParallelStore,NumParallelWrigStand,NumCompWrigStand,NumOOSWrigStand,NumPacksDateIssue,PreferredWholesaler';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_storeop_ro_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_register := qu3_util.get_number('no_register',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_chiller_door := qu3_util.get_number('no_chiller_door',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu3_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_parallel_store := qu3_util.get_number('no_parallel_store',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_paralle_wwy_stand := qu3_util.get_number('no_paralle_wwy_stand',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_comp_wwy_stand := qu3_util.get_number('no_comp_wwy_stand',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_oos_wwy_stand := qu3_util.get_number('no_oos_wwy_stand',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_pack_date_issue := qu3_util.get_number('no_pack_date_issue',10,0,true,g_source_file_error_flag);
      l_entity_load_row.pref_wholesaler := qu3_util.get_number('pref_wholesaler',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_storeop_ro_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_storeop_ro_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_act_dtl_storeop_ro_load
    ...
    update ods.qu3_act_dtl_storeop_ro_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_storeop_ro_hist
    select *
    from ods.qu3_act_dtl_storeop_ro_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_storeop_ro_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_storeop_ro/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw45 for ods_app.qu3_qu3cdw45;

-- Grants
grant execute on ods_app.qu3_qu3cdw45 to lics_app;
