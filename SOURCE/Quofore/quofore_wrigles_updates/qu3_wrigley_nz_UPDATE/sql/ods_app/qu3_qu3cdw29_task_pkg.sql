...
create or replace package ods_app.qu3_qu3cdw29 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw29
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw29] Entity [Task] Table [qu3_task][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw29 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw29';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('incomplete_reason_id_desc',5);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_task_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.status_id := qu3_util.get_number('status_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.status_id_desc := qu3_util.get_string('status_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.is_recurring := qu3_util.get_boolean_as_number('is_recurring',true,g_source_file_error_flag);
      l_entity_load_row.is_all_day := qu3_util.get_boolean_as_number('is_all_day',true,g_source_file_error_flag);
      l_entity_load_row.name := qu3_util.get_string('name',50,true,g_source_file_error_flag);
      l_entity_load_row.start_date := qu3_util.get_date('start_date',true,g_source_file_error_flag);
      l_entity_load_row.end_date := qu3_util.get_date('end_date',true,g_source_file_error_flag);
      l_entity_load_row.priority_id := qu3_util.get_number('priority_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.priority_id_desc := qu3_util.get_string('priority_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.due_date := qu3_util.get_date('due_date',true,g_source_file_error_flag);
      l_entity_load_row.time := qu3_util.get_time('time',true,g_source_file_error_flag);
      l_entity_load_row.duration := qu3_util.get_time('duration',true,g_source_file_error_flag);
      l_entity_load_row.note := qu3_util.get_string('note',200,true,g_source_file_error_flag);
      l_entity_load_row.task_type_id := qu3_util.get_number('task_type_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_task_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_task_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu3_task_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_task_hist
    select *
    from ods.qu3_task_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_task_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_task/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw29 for ods_app.qu3_qu3cdw29;

-- Grants
grant execute on ods_app.qu3_qu3cdw29 to lics_app;
