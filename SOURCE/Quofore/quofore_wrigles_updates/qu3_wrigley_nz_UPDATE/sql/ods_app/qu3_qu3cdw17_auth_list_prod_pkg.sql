...
create or replace package ods_app.qu3_qu3cdw17 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw17
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw17] Entity [AuthorisedListProduct] Table [qu3_auth_list_prod][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw17 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw17';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('auth_list_id',2);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_auth_list_prod_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.auth_list_id := qu3_util.get_number('auth_list_id',10,0,true,g_source_file_error_flag);
      ...
      l_entity_load_row.is_include := qu3_util.get_boolean_as_number('is_include',true,g_source_file_error_flag);
      l_entity_load_row.effective_from := qu3_util.get_date('effective_from',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu3_auth_list_prod_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_auth_list_prod_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_auth_list_prod_load
    ...
    update ods.qu3_auth_list_prod_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_auth_list_prod_hist
    select *
    from ods.qu3_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_auth_list_prod_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_auth_list_prod/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw17 for ods_app.qu3_qu3cdw17;

-- Grants
grant execute on ods_app.qu3_qu3cdw17 to lics_app;
