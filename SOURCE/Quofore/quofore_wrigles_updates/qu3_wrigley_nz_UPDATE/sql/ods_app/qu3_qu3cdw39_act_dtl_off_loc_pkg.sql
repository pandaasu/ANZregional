...
create or replace package ods_app.qu3_qu3cdw39 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw39
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw39] Entity [ActivityDetailOffLocation] Table [qu3_act_dtl_off_loc][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw39 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw39';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailOffLocation';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_gondola',3);
    lics_inbound_utility.set_csv_definition('no_wing',4);
    lics_inbound_utility.set_csv_definition('no_flat_pack_tower',5);
    lics_inbound_utility.set_csv_definition('no_pre_pack_tower',6);
    lics_inbound_utility.set_csv_definition('no_flat_pack_cdus',7);
    lics_inbound_utility.set_csv_definition('no_pre_pack_cdus',8);
    lics_inbound_utility.set_csv_definition('no_buckets',9);
    lics_inbound_utility.set_csv_definition('no_clip_strip',10);
    lics_inbound_utility.set_csv_definition('no_other',11);
    lics_inbound_utility.set_csv_definition('stock_qty',12);
    lics_inbound_utility.set_csv_definition('promo_start_date',13);
    lics_inbound_utility.set_csv_definition('promo_end_date',14);
    lics_inbound_utility.set_csv_definition('prod_id',15);
    lics_inbound_utility.set_csv_definition('coop_spend',16);
    lics_inbound_utility.set_csv_definition('sold_in_by',17);
    lics_inbound_utility.set_csv_definition('built_by',18);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumGondola,NumWing,NumFlatPackTower,NumPrePackTower,NumFlatPackCDUs,NumPrePackCDUs,NumBuckets,NumClipStrip,NumOther,StockQty,PromoStartDate,PromoEndDate,Product_ID,Coopspend,SoldInBy,BuiltBy';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_off_loc_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.act_id := qu3_util.get_number('act_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_gondola := qu3_util.get_number('no_gondola',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_wing := qu3_util.get_number('no_wing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_flat_pack_tower := qu3_util.get_number('no_flat_pack_tower',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_pre_pack_tower := qu3_util.get_number('no_pre_pack_tower',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_flat_pack_cdus := qu3_util.get_number('no_flat_pack_cdus',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_pre_pack_cdus := qu3_util.get_number('no_pre_pack_cdus',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_buckets := qu3_util.get_number('no_buckets',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_clip_strip := qu3_util.get_number('no_clip_strip',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_other := qu3_util.get_number('no_other',10,0,true,g_source_file_error_flag);
      l_entity_load_row.stock_qty := qu3_util.get_number('stock_qty',10,0,true,g_source_file_error_flag);
      l_entity_load_row.promo_start_date := qu3_util.get_datetime('promo_start_date',true,g_source_file_error_flag);
      l_entity_load_row.promo_end_date := qu3_util.get_datetime('promo_end_date',true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu3_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.coop_spend := qu3_util.get_number('coop_spend',18,4,true,g_source_file_error_flag);
      l_entity_load_row.sold_in_by := qu3_util.get_number('sold_in_by',10,0,true,g_source_file_error_flag);
      l_entity_load_row.built_by := qu3_util.get_number('built_by',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_off_loc_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_off_loc_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_act_dtl_off_loc_load
    ...
    update ods.qu3_act_dtl_off_loc_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_off_loc_hist
    select *
    from ods.qu3_act_dtl_off_loc_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_off_loc_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_off_loc/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw39 for ods_app.qu3_qu3cdw39;

-- Grants
grant execute on ods_app.qu3_qu3cdw39 to lics_app;
