...
create or replace package ods_app.qu3_qu3cdw44 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw44
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw44] Entity [ActivityDetailStoreOppGrocery] Table [qu3_act_dtl_storeop_gr][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw44 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw44';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailStoreOppGrocery';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_std_chkout_reg',3);
    lics_inbound_utility.set_csv_definition('no_std_chkout_os',4);
    lics_inbound_utility.set_csv_definition('no_std_chkout_fe',5);
    lics_inbound_utility.set_csv_definition('no_confec_free_chkout',6);
    lics_inbound_utility.set_csv_definition('no_confec_free_chkout_top_',7);
    lics_inbound_utility.set_csv_definition('no_express_chkout',8);
    lics_inbound_utility.set_csv_definition('no_express_q',9);
    lics_inbound_utility.set_csv_definition('no_selfscan_chkout',10);
    lics_inbound_utility.set_csv_definition('no_selfscan_q',11);
    lics_inbound_utility.set_csv_definition('no_confec_bay_',12);
    lics_inbound_utility.set_csv_definition('prod_id',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumStdCheckoutReg,NumStdCheckoutOS,NumStdCheckoutFE,NumConfecFreeCheckout,NumConfecFreeCheckoutTopN,NumExpressCheckout,NumExpressQueue,NumSelfScanCheckout,NumSelfscanQueue,NumConfecBays,Product_ID';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_storeop_gr_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_std_chkout_reg := qu3_util.get_number('no_std_chkout_reg',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_std_chkout_os := qu3_util.get_number('no_std_chkout_os',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_std_chkout_fe := qu3_util.get_number('no_std_chkout_fe',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_confec_free_chkout := qu3_util.get_number('no_confec_free_chkout',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_confec_free_chkout_top_ := qu3_util.get_number('no_confec_free_chkout_top_',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_express_chkout := qu3_util.get_number('no_express_chkout',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_express_q := qu3_util.get_number('no_express_q',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_selfscan_chkout := qu3_util.get_number('no_selfscan_chkout',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_selfscan_q := qu3_util.get_number('no_selfscan_q',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_confec_bay_ := qu3_util.get_number('no_confec_bay_',10,0,true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_storeop_gr_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_storeop_gr_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_act_dtl_storeop_gr_load
    ...
    update ods.qu3_act_dtl_storeop_gr_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_storeop_gr_hist
    select *
    from ods.qu3_act_dtl_storeop_gr_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_storeop_gr_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_storeop_gr/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw44 for ods_app.qu3_qu3cdw44;

-- Grants
grant execute on ods_app.qu3_qu3cdw44 to lics_app;
