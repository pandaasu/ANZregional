...
create or replace package ods_app.qu3_qu3cdw01 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw01
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw01] Entity [Hierarchy] Table [qu3_hier][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw01 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw01';
  g_entity_name constant varchar2(64 char) := 'Hierarchy';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('id_desc',2);
    lics_inbound_utility.set_csv_definition('is_active',3);
    lics_inbound_utility.set_csv_definition('node',4);
    lics_inbound_utility.set_csv_definition('hier_full_path',5);
    lics_inbound_utility.set_csv_definition('hier_root_is_active',6);
    lics_inbound_utility.set_csv_definition('hier_root_id',7);
    lics_inbound_utility.set_csv_definition('hier_root_id_desc',8);
    lics_inbound_utility.set_csv_definition('parent_id',9);
    lics_inbound_utility.set_csv_definition('parent_id_desc',10);
    lics_inbound_utility.set_csv_definition('parent_node',11);
    lics_inbound_utility.set_csv_definition('parent_is_active',12);
    lics_inbound_utility.set_csv_definition('hier_level',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,ID_Description,IsActive,Node,HierarchyFullPath,HierarchyRootIsActive,HierarchyRoot_ID,HierarchyRoot_ID_Description,Parent_ID,Parent_ID_Description,ParentNode,ParentIsactive,Level';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_hier_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.id_desc := qu3_util.get_string('id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.is_active := qu3_util.get_boolean_as_number('is_active',true,g_source_file_error_flag);
      l_entity_load_row.node := qu3_util.get_string('node',50,true,g_source_file_error_flag);
      l_entity_load_row.hier_full_path := qu3_util.get_string('hier_full_path',4000,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_is_active := qu3_util.get_boolean_as_number('hier_root_is_active',true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id := qu3_util.get_number('hier_root_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.hier_root_id_desc := qu3_util.get_string('hier_root_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_id := qu3_util.get_number('parent_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.parent_id_desc := qu3_util.get_string('parent_id_desc',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_node := qu3_util.get_string('parent_node',50,true,g_source_file_error_flag);
      l_entity_load_row.parent_is_active := qu3_util.get_boolean_as_number('parent_is_active',true,g_source_file_error_flag);
    ...
    -- Insert row
    begin
      insert into ods.qu3_hier_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_hier_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu3_hier_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_hier_hist
    select *
    from ods.qu3_hier_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_hier_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_hier/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw01 for ods_app.qu3_qu3cdw01;

-- Grants
grant execute on ods_app.qu3_qu3cdw01 to lics_app;
