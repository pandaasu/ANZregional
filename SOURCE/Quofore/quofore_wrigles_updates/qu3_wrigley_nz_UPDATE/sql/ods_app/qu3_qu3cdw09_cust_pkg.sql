...
create or replace package ods_app.qu3_qu3cdw09 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw09
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw09] Entity [Customer] Table [qu3_cust][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw09 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw09';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_cust_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.is_active := qu3_util.get_boolean_as_number('is_active',true,g_source_file_error_flag);
      l_entity_load_row.created_date := qu3_util.get_datetime('created_date',true,g_source_file_error_flag);
      l_entity_load_row.cust_name := qu3_util.get_string('cust_name',50,true,g_source_file_error_flag);
      l_entity_load_row.cust_ref_id := qu3_util.get_string('cust_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.email := qu3_util.get_string('email',60,true,g_source_file_error_flag);
      l_entity_load_row.fax_no := qu3_util.get_string('fax_no',30,true,g_source_file_error_flag);
      l_entity_load_row.group_edi_code := qu3_util.get_string('group_edi_code',50,true,g_source_file_error_flag);
      l_entity_load_row.outlet_ref_id := qu3_util.get_string('outlet_ref_id',10,true,g_source_file_error_flag);
      l_entity_load_row.phone_no := qu3_util.get_string('phone_no',30,true,g_source_file_error_flag);
      l_entity_load_row.store_edi_code := qu3_util.get_string('store_edi_code',50,true,g_source_file_error_flag);
      l_entity_load_row.web_addrs := qu3_util.get_string('web_addrs',60,true,g_source_file_error_flag);
      l_entity_load_row.cust_hier_id := qu3_util.get_number('cust_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.grade_hier_id := qu3_util.get_number('grade_hier_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.structured_id := qu3_util.get_number('structured_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.barons_id := qu3_util.get_number('barons_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.ind_groc_program_id := qu3_util.get_number('ind_groc_program_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.locality_id := qu3_util.get_number('locality_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.abn_no := qu3_util.get_string('abn_no',50,true,g_source_file_error_flag);
      l_entity_load_row.visit_frequency := qu3_util.get_number('visit_frequency',10,0,true,g_source_file_error_flag);
      l_entity_load_row.expo_tier_id := qu3_util.get_string('expo_tier_id',50,true,g_source_file_error_flag);
      l_entity_load_row.is_wholesaler := qu3_util.get_boolean_as_number('is_wholesaler',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_cust_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_cust_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu3_cust_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_cust_hist
    select *
    from ods.qu3_cust_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_cust_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_cust/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw09 for ods_app.qu3_qu3cdw09;

-- Grants
grant execute on ods_app.qu3_qu3cdw09 to lics_app;
