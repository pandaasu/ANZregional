...
create or replace package ods_app.qu3_qu3cdw13 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw13
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw13] Entity [CustomerVisitorDay] Table [qu3_cust_visit_day][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw13 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw13';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('is_donot_call',3);
    ...
    lics_inbound_utility.set_csv_definition('cust_visit_id',7);
    ...
    lics_inbound_utility.set_csv_definition('note',11);
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_cust_visit_day_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.day_of_week := qu3_util.get_number('day_of_week',3,0,true,g_source_file_error_flag);
      l_entity_load_row.is_donot_call := qu3_util.get_boolean_as_number('is_donot_call',true,g_source_file_error_flag);
      l_entity_load_row.is_appoint_reqd := qu3_util.get_boolean_as_number('is_appoint_reqd',true,g_source_file_error_flag);
      l_entity_load_row.visit_from := qu3_util.get_time('visit_from',true,g_source_file_error_flag);
      l_entity_load_row.visit_to := qu3_util.get_time('visit_to',true,g_source_file_error_flag);
      l_entity_load_row.cust_visit_id := qu3_util.get_number('cust_visit_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.cust_id := qu3_util.get_number('cust_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.days_between_visit := qu3_util.get_number('days_between_visit',3,0,true,g_source_file_error_flag);
      l_entity_load_row.visit_duration := qu3_util.get_time('visit_duration',true,g_source_file_error_flag);
      l_entity_load_row.note := qu3_util.get_string('note',200,true,g_source_file_error_flag);
      l_entity_load_row.role_id := qu3_util.get_number('role_id',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_cust_visit_day_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_cust_visit_day_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_cust_visit_day_load
    ...
    update ods.qu3_cust_visit_day_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_cust_visit_day_hist
    select *
    from ods.qu3_cust_visit_day_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_cust_visit_day_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_cust_visit_day/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw13 for ods_app.qu3_qu3cdw13;

-- Grants
grant execute on ods_app.qu3_qu3cdw13 to lics_app;
