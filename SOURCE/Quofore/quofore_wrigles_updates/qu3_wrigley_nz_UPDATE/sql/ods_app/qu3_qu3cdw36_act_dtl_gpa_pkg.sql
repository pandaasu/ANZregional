...
create or replace package ods_app.qu3_qu3cdw36 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw36
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw36] Entity [ActivityDetailGPA] Table [qu3_act_dtl_gpa][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw36 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw36';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailGPA';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_with_wwy_cover',3);
    lics_inbound_utility.set_csv_definition('otc_gum_facings',4);
    lics_inbound_utility.set_csv_definition('no_mp_gum_facing',5);
    lics_inbound_utility.set_csv_definition('no_mint_facing',6);
    lics_inbound_utility.set_csv_definition('no_confec_facing',7);
    lics_inbound_utility.set_csv_definition('no_confec_facing_2',8);
    lics_inbound_utility.set_csv_definition('prod_id',9);
    lics_inbound_utility.set_csv_definition('no_choc_bar_facing',10);
    lics_inbound_utility.set_csv_definition('no_comp_gum_facing',11);
    lics_inbound_utility.set_csv_definition('no_comp_mint_facing',12);
    lics_inbound_utility.set_csv_definition('no_comp_candy_facing',13);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumWithWrigCover,OTCGumFacings,NumMPGumFacing,NumMintFacing,NumConfecFacing,NumConfecFacing2,Product_ID,NumChocBarFacing,NumCompGumFacing,NumCompMintFacing,NumCompCandyFacing';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_gpa_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_with_wwy_cover := qu3_util.get_number('no_with_wwy_cover',10,0,true,g_source_file_error_flag);
      l_entity_load_row.otc_gum_facings := qu3_util.get_number('otc_gum_facings',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_mp_gum_facing := qu3_util.get_number('no_mp_gum_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_mint_facing := qu3_util.get_number('no_mint_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_confec_facing := qu3_util.get_number('no_confec_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_confec_facing_2 := qu3_util.get_number('no_confec_facing_2',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu3_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_choc_bar_facing := qu3_util.get_number('no_choc_bar_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_comp_gum_facing := qu3_util.get_number('no_comp_gum_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_comp_mint_facing := qu3_util.get_number('no_comp_mint_facing',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_comp_candy_facing := qu3_util.get_number('no_comp_candy_facing',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_gpa_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_gpa_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu3_act_dtl_gpa_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_gpa_hist
    select *
    from ods.qu3_act_dtl_gpa_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_gpa_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_gpa/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw36 for ods_app.qu3_qu3cdw36;

-- Grants
grant execute on ods_app.qu3_qu3cdw36 to lics_app;
