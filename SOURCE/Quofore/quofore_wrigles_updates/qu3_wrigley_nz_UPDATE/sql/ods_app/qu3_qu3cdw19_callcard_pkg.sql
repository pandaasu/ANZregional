...
create or replace package ods_app.qu3_qu3cdw19 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw19
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw19] Entity [CallCard] Table [qu3_callcard][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw19 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw19';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('auto_stocker_chk',14);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,CreatedDate,Appointment_Id,CallTypeId,CallTypeId_Description,Customer_Id,Duration,Position_Id,Position_Id_Lookup,Rep_Id,StartDate,Customer_ID_Description,FullName,AutostockerChk';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_callcard_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.appoint_id := qu3_util.get_number('appoint_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id := qu3_util.get_number('call_type_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.call_type_id_desc := qu3_util.get_string('call_type_id_desc',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.pos_id_lookup := qu3_util.get_string('pos_id_lookup',50,true,g_source_file_error_flag);
      ...
      l_entity_load_row.auto_stocker_chk := qu3_util.get_boolean_as_number('auto_stocker_chk',true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_callcard_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_callcard_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    update ods.qu3_callcard_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_callcard_hist
    select *
    from ods.qu3_callcard_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_callcard_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_callcard/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw19 for ods_app.qu3_qu3cdw19;

-- Grants
grant execute on ods_app.qu3_qu3cdw19 to lics_app;
