...
create or replace package ods_app.qu3_qu3cdw46 as
  /*****************************************************************************
  ** Package Definition
  ******************************************************************************
    ...
    System   : qu3
    Owner    : ods_app
    Package  : qu3_qu3cdw46
    Author   : [Auto-Generate]

    Description
    ----------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    Loader Package, Interface [qu3cdw46] Entity [ActivityDetailTopSkuAudit] Table [qu3_act_dtl_top_sku][_load/_hist]
...
create or replace package body ods_app.qu3_qu3cdw46 as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'ods_app.qu3_qu3cdw46';
  g_entity_name constant varchar2(64 char) := 'ActivityDetailTopSkuAudit';
  ...
  *****************************************************************************/
  ...
  begin
    ...
    lics_inbound_utility.set_csv_definition('no_std_chk_belt',3);
    lics_inbound_utility.set_csv_definition('no_std_chk_front',4);
    lics_inbound_utility.set_csv_definition('no_exp_chk_pop',5);
    lics_inbound_utility.set_csv_definition('no_ss_chk_pop',6);
    lics_inbound_utility.set_csv_definition('no_q_zone',7);
    lics_inbound_utility.set_csv_definition('prod_id',8);
    lics_inbound_utility.set_csv_definition('no_std_chk_non_belt',9);
    lics_inbound_utility.set_csv_definition('no_aisle',10);
  ...
  *****************************************************************************/
  procedure process_header_row(p_row in varchar2) is
    ...
    l_expected_row varchar2(4000 char) := 'ID,Activity_ID,NumStdChkBelt,NumStdChkFront,NumExpChkPOP,NumSSChkPOP,NumQueueZone,Product_ID,NumStdChkNonBelt,NumAisle';
  ...
  *****************************************************************************/
  procedure process_data_row(p_row in varchar2) is
    ...
    l_entity_load_row ods.qu3_act_dtl_top_sku_load%rowtype;
  ...
  begin
    ...
    begin
      ...
      l_entity_load_row.no_std_chk_belt := qu3_util.get_number('no_std_chk_belt',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_std_chk_front := qu3_util.get_number('no_std_chk_front',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_exp_chk_pop := qu3_util.get_number('no_exp_chk_pop',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_ss_chk_pop := qu3_util.get_number('no_ss_chk_pop',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_q_zone := qu3_util.get_number('no_q_zone',10,0,true,g_source_file_error_flag);
      l_entity_load_row.prod_id := qu3_util.get_number('prod_id',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_std_chk_non_belt := qu3_util.get_number('no_std_chk_non_belt',10,0,true,g_source_file_error_flag);
      l_entity_load_row.no_aisle := qu3_util.get_number('no_aisle',10,0,true,g_source_file_error_flag);
      exception
      when others then
    ...
    -- Insert row
    begin
      insert into ods.qu3_act_dtl_top_sku_load values l_entity_load_row;
    exception
      when others then
         lics_inbound_utility.add_exception(substr('['||g_package_name||'.process_data_row] Insert Failed [ods.qu3_act_dtl_top_sku_load] : '||SQLERRM, 1, 4000));
         g_source_file_error_flag := true;
    end;
  ...
  *****************************************************************************/
  ...
  begin
    ...
    from ods.qu3_act_dtl_top_sku_load
    ...
    update ods.qu3_act_dtl_top_sku_load
    set q4x_modify_user = user,
      q4x_modify_time = sysdate
    where q4x_batch_id = p_batch_id;

    -- Insert ALL of Load Batch into History
    insert into ods.qu3_act_dtl_top_sku_hist
    select *
    from ods.qu3_act_dtl_top_sku_load
    where q4x_batch_id = p_batch_id;

    -- Delete Load Batch
    delete from ods.qu3_act_dtl_top_sku_load
    where q4x_batch_id = p_batch_id;

  exception
    when others then
      g_abort_processing_flag := true;
      raise_application_error(-20000, substr('['||g_package_name||'.process_batch] Failed [ods.qu3_act_dtl_top_sku/_load/_hist] : '||SQLERRM, 1, 4000));
...
-- Synonyms
create or replace public synonym qu3_qu3cdw46 for ods_app.qu3_qu3cdw46;

-- Grants
grant execute on ods_app.qu3_qu3cdw46 to lics_app;
