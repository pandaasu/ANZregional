...
create or replace package dds_app.qu3_task_cust_pkg as
  /***************************************************************-****************
  ** Package Definition
  ********************************************************************************
    ...
    System   : qu3
    Owner    : dds_app
    Package  : qu3_task_cust_pkg
    Author   : [Auto-Generate]

    Description
    ------------------------------------------------------------------------------
    [qu3] Quofore - Wrigley New Zealand
    View Package, Entity [TaskCustomer] Table [qu3_task_cust][_load/_hist]
  ...
  *******************************************************************************/
  ...
  -- Public : Type
  type qu3_task_cust_type is table of ods.qu3_task_cust_hist%rowtype;

  -- Public : Functions
  function view_at_date(p_date in date) return qu3_task_cust_type pipelined;
  function view_history return qu3_task_cust_type pipelined;

end qu3_task_cust_pkg;
/

create or replace package body dds_app.qu3_task_cust_pkg as
  ...
  -- Private : Constants
  g_package_name constant varchar2(64 char) := 'qu3_task_cust_pkg';
  g_entity_name constant varchar2(64 char) := 'TaskCustomer';

  /*****************************************************************************
  ** Function : Return Entity : As at Date
  *****************************************************************************/
  function view_at_date(p_date in date) return qu3_task_cust_type pipelined is

  begin

    for l_entity in (
      select *
      from qu3_task_cust_hist
  ...
  *****************************************************************************/
  function view_history return qu3_task_cust_type pipelined is
...
-- Synonyms
create or replace public synonym qu3_task_cust_pkg for dds_app.qu3_task_cust_pkg;

-- Grants
grant execute on dds_app.qu3_task_cust_pkg to qv_user, bo_user;
