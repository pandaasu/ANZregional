--
-- BDS_ATLLAD32_FLATTEN  (Package) 
--
CREATE OR REPLACE PACKAGE BDS_APP.bds_atllad32_flatten as
   ...
   /**
    ...
    Package : bds_atllad32_flatten
    Owner   : BDS_APP
    Author  : Ben Halicki

    Description
    -----------
    Business Data Store - ATLLAD32 - Plant Maintenance Functional Location Master
   ...
   *******************************************************************************/
   ...
   /* Public declarations
   /*-*/
   procedure execute(par_action in varchar2, par_tplnr in varchar2);
...
--
CREATE OR REPLACE PACKAGE BODY BDS_APP.bds_atllad32_flatten as
   ...
   /*-*/
   procedure lads_lock(par_tplnr in varchar2);
   procedure bds_flatten(par_tplnr in varchar2);
   ...
   /***********************************************/
   procedure execute(par_action in varchar2, par_tplnr in varchar2) is
   ...
   begin
      ...
      case upper(par_action)
        when '*DOCUMENT' then bds_flatten(par_tplnr);
        when '*DOCUMENT_OVERRIDE' then lads_lock(par_tplnr);
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'bds_atllad32_flatten - EXECUTE ' || par_action || ' - ' || substr(SQLERRM, 1, 1024));
   ...
   /***************************************************/
   procedure bds_flatten(par_tplnr in varchar2) is 
      ...
      /* local cursors
      /*-*/
      cursor csr_lads_flc_hdr is
         select t01.tplnr as tplnr,
                t01.pltxt as pltxt,
                t01.eqfnr as eqfnr,
                t01.swerk as swerk,
           ...
           from lads_flc_hdr t01
          where t01.tplnr = par_tplnr;
        
        rcd_lads_flc_hdr csr_lads_flc_hdr%rowtype;
   ...
   begin
      ...
      if csr_lads_flc_hdr%notfound then
         raise_application_error(-20000, 'LADS Header row not found');
      end if;
      close csr_lads_flc_hdr;

      /*-*/
      /* Set the BDS header values
      /*-*/
      rcd_bds_functnl_locn_hdr.functnl_locn_code := rcd_lads_flc_hdr.tplnr;
      rcd_bds_functnl_locn_hdr.functnl_locn_desc := rcd_lads_flc_hdr.pltxt;
      rcd_bds_functnl_locn_hdr.plant_code := rcd_lads_flc_hdr.swerk;
      rcd_bds_functnl_locn_hdr.sort_field := rcd_lads_flc_hdr.eqfnr;
      rcd_bds_functnl_locn_hdr.sap_idoc_name := rcd_lads_flc_hdr.idoc_name;
      rcd_bds_functnl_locn_hdr.sap_idoc_number := rcd_lads_flc_hdr.idoc_number;
      rcd_bds_functnl_locn_hdr.sap_idoc_timestamp := rcd_lads_flc_hdr.idoc_timestamp;
      rcd_bds_functnl_locn_hdr.bds_lads_date := rcd_lads_flc_hdr.lads_date;
      rcd_bds_functnl_locn_hdr.bds_lads_status := rcd_lads_flc_hdr.lads_status;

      /*-*/
      /* Update the BDS header
      /*-*/
      update bds_functnl_locn_hdr
        set functnl_locn_desc = rcd_bds_functnl_locn_hdr.functnl_locn_desc,
       ...
       if sql%notfound then
          insert into bds_functnl_locn_hdr
            ...
            values(rcd_bds_functnl_locn_hdr.functnl_locn_code,
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'BDS_FLATTEN - ' || 'TPLNR: ' || par_tplnr || ' - ' || substr(SQLERRM, 1, 1024));
   ...
   /*******************************************************************************/
   procedure lads_lock(par_tplnr in varchar2) is 
  ...
  exception
   ...
   begin
      ...
      /*-*/
      bds_table.truncate('bds_functnl_locn_hdr');
