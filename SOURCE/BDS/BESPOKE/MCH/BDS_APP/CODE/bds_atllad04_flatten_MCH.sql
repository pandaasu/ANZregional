create or replace package bds_atllad04_flatten as
   ...
   /**
    ...
    Description
    -----------
    Business Data Store - ATLLAD04 - Material Master (MATMAS) - MCH Version
    ...
    2007/05   Steve Gregan   Included MCH (China) specific logic (BDS_MATERIAL_PLANT_MCH from BDS_MATERIAL_PLANT_MFANZ)
    2007/08   Steve Gregan   Added additional fields to BDS_MATERIAL_PLANT_MCH
...
create or replace package body bds_atllad04_flatten as
   ...
   procedure bds_flatten(par_matnr in varchar2) is
      ...
      rcd_bds_material_text_zh bds_material_text_zh%rowtype;
      ...
      /*-----------------------------------*/
      /* Material/Plant for MCH            */
      /*-----------------------------------*/
      rcd_bds_material_plant_mch bds_material_plant_mch%rowtype;
      ...
      cursor csr_lads_mat_hdr is
         select t01.matnr as matnr,
                ...
                (select max(maktx)
                 from lads_mat_mkt
                 where matnr = t01.matnr
                   and spras_iso = 'ZH') as bds_material_desc_zh,
      ...
      cursor csr_lads_mat_text_zh is
         select nvl(trim(substr(t01.tdname,19,4)),'*NONE') as sales_organisation,
                nvl(trim(substr(t01.tdname,23,2)),'*NONE') as dstrbtn_channel,
                max(t01.txhseq) as txhseq
         from lads_mat_txh t01
         where t01.matnr = rcd_lads_mat_hdr.matnr
           and t01.spras_iso = 'ZH'
           and t01.tdobject = 'MVKE'
           and t01.tdtexttype = 'ASCII'
         group by t01.tdobject, t01.tdname, t01.tdid, t01.tdtexttype, t01.spras_iso;
      rcd_lads_mat_text_zh  csr_lads_mat_text_zh%rowtype;
      ...
      cursor csr_lads_material_plant_mch is	
         select t01.sap_material_code as sap_material_code, 
                ...
                t03.sales_text_135 as sales_text_135_en,
                t03.sales_text_234 as sales_text_234_en,
                t07.sales_text_135 as sales_text_135_zh,
                t07.sales_text_234 as sales_text_234_zh,
         ...
         from bds_material_hdr t01,
              ...
              (select sap_material_code,
                      max(case when sales_organisation = '135' then text end) as sales_text_135,
                      max(case when sales_organisation = '234' then text end) as sales_text_234
         ...
         where t01.sap_material_code = t02.sap_material_code
           ...
           and t01.mars_rprsnttv_item_code = t07.sap_material_code(+)
           ...
           and (t02.plant_code like 'CN%') 
           and t02.deletion_indctr is null
           and t05.vltn_type(+) = '*NONE'
           and t05.deletion_indctr(+) is null;
      rcd_lads_material_plant_mch  csr_lads_material_plant_mch%rowtype;
   ...
   begin
      ...
      /* DELETE BDS TABLES                     */
      /*---------------------------------------*/
      delete bds_material_plant_mch where sap_material_code = par_matnr;
      ...
      delete bds_material_text_zh where sap_material_code = par_matnr;
      ...
      close csr_lads_mat_hdr;
         ...
         rcd_bds_material_hdr.bds_material_desc_zh := rcd_lads_mat_hdr.bds_material_desc_zh;
      ...
      update bds_material_hdr
         set bds_material_desc_en = rcd_bds_material_hdr.bds_material_desc_en,
             bds_material_desc_zh = rcd_bds_material_hdr.bds_material_desc_zh,
         ...
         insert into bds_material_hdr
          ...
          values
            (rcd_bds_material_hdr.sap_material_code,
             ...
             rcd_bds_material_hdr.bds_material_desc_zh,
      ...
      /*-*/
      /* Process Material Text (Chinese)
      /*-*/
      open csr_lads_mat_text_zh;
      loop
         fetch csr_lads_mat_text_zh into rcd_lads_mat_text_zh;
         if (csr_lads_mat_text_zh%notfound) then
            exit;
         end if;

         rcd_bds_material_text_zh.sap_material_code := rcd_bds_material_hdr.sap_material_code;
         rcd_bds_material_text_zh.sales_organisation := rcd_lads_mat_text_zh.sales_organisation;
         rcd_bds_material_text_zh.dstrbtn_channel := rcd_lads_mat_text_zh.dstrbtn_channel;
         rcd_bds_material_text_zh.text := bds_material.join_text_lines(par_matnr, rcd_lads_mat_text_zh.txhseq);

         /*----------------------------*/
         /* UPDATE BDS_MATERIAL_TEXT_ZH*/
         /*----------------------------*/
         insert into bds_material_text_zh
            (sap_material_code,
             sales_organisation,
             dstrbtn_channel,
             text)
          values 
            (rcd_bds_material_text.sap_material_code,
             rcd_bds_material_text_zh.sales_organisation,
             rcd_bds_material_text_zh.dstrbtn_channel,
             rcd_bds_material_text_zh.text);

      end loop;
      close csr_lads_mat_text_zh;
      ...
      /*-*/
      /* Process Material/Plant for MCH
      /*  Note: This is a custom table for MCH Plant use only
      /*        It is intended to be used for Fast Refresh Materialised views to Plant DB's 
      /*-*/
      open csr_lads_material_plant_mch;
      loop
         fetch csr_lads_material_plant_mch into rcd_lads_material_plant_mch;
         if (csr_lads_material_plant_mch%notfound) then
            exit;
         end if;

         rcd_bds_material_plant_mch.sap_material_code := rcd_bds_material_hdr.sap_material_code;
         rcd_bds_material_plant_mch.plant_code := rcd_lads_material_plant_mch.plant_code;
         rcd_bds_material_plant_mch.mars_rprsnttv_item_code := rcd_lads_material_plant_mch.mars_rprsnttv_item_code;
         rcd_bds_material_plant_mch.bds_material_desc_en := rcd_lads_material_plant_mch.bds_material_desc_en;
         rcd_bds_material_plant_mch.bds_material_desc_zh := rcd_lads_material_plant_mch.bds_material_desc_zh;
         rcd_bds_material_plant_mch.material_type := rcd_lads_material_plant_mch.material_type;
         rcd_bds_material_plant_mch.material_grp := rcd_lads_material_plant_mch.material_grp;
         rcd_bds_material_plant_mch.base_uom := rcd_lads_material_plant_mch.base_uom;
         rcd_bds_material_plant_mch.order_unit := rcd_lads_material_plant_mch.order_unit;
         rcd_bds_material_plant_mch.gross_weight := rcd_lads_material_plant_mch.gross_weight;
         rcd_bds_material_plant_mch.net_weight := rcd_lads_material_plant_mch.net_weight;
         rcd_bds_material_plant_mch.gross_weight_unit := rcd_lads_material_plant_mch.gross_weight_unit;
         rcd_bds_material_plant_mch.length := rcd_lads_material_plant_mch.length;
         rcd_bds_material_plant_mch.width := rcd_lads_material_plant_mch.width;
         rcd_bds_material_plant_mch.height := rcd_lads_material_plant_mch.height;
         rcd_bds_material_plant_mch.dimension_uom := rcd_lads_material_plant_mch.dimension_uom;
         rcd_bds_material_plant_mch.interntl_article_no := rcd_lads_material_plant_mch.interntl_article_no;
         rcd_bds_material_plant_mch.total_shelf_life := rcd_lads_material_plant_mch.total_shelf_life;
         rcd_bds_material_plant_mch.mars_intrmdt_prdct_compnt_flag := rcd_lads_material_plant_mch.mars_intrmdt_prdct_compnt_flag;
         rcd_bds_material_plant_mch.mars_merchandising_unit_flag := rcd_lads_material_plant_mch.mars_merchandising_unit_flag;
         rcd_bds_material_plant_mch.mars_prmotional_material_flag := rcd_lads_material_plant_mch.mars_prmotional_material_flag;
         rcd_bds_material_plant_mch.mars_retail_sales_unit_flag := rcd_lads_material_plant_mch.mars_retail_sales_unit_flag;
         rcd_bds_material_plant_mch.mars_semi_finished_prdct_flag := rcd_lads_material_plant_mch.mars_semi_finished_prdct_flag;
         rcd_bds_material_plant_mch.mars_rprsnttv_item_flag := rcd_lads_material_plant_mch.mars_rprsnttv_item_flag;
         rcd_bds_material_plant_mch.mars_traded_unit_flag := rcd_lads_material_plant_mch.mars_traded_unit_flag;
         rcd_bds_material_plant_mch.xplant_status := rcd_lads_material_plant_mch.xplant_status;
         rcd_bds_material_plant_mch.xplant_status_valid := rcd_lads_material_plant_mch.xplant_status_valid;
         rcd_bds_material_plant_mch.batch_mngmnt_reqrmnt_indctr := rcd_lads_material_plant_mch.batch_mngmnt_reqrmnt_indctr;
         rcd_bds_material_plant_mch.mars_plant_material_type := rcd_lads_material_plant_mch.mars_plant_material_type;
         rcd_bds_material_plant_mch.procurement_type := rcd_lads_material_plant_mch.procurement_type;
         rcd_bds_material_plant_mch.special_procurement_type := rcd_lads_material_plant_mch.special_procurement_type;
         rcd_bds_material_plant_mch.issue_storage_location := rcd_lads_material_plant_mch.issue_storage_location;
         rcd_bds_material_plant_mch.mrp_controller := rcd_lads_material_plant_mch.mrp_controller;
         rcd_bds_material_plant_mch.plant_specific_status_valid := rcd_lads_material_plant_mch.plant_specific_status_valid;
         rcd_bds_material_plant_mch.deletion_indctr := rcd_lads_material_plant_mch.deletion_indctr;
         rcd_bds_material_plant_mch.plant_specific_status := rcd_lads_material_plant_mch.plant_specific_status;
         rcd_bds_material_plant_mch.assembly_scrap_percntg := rcd_lads_material_plant_mch.assembly_scrap_percntg;
         rcd_bds_material_plant_mch.component_scrap_percntg := rcd_lads_material_plant_mch.component_scrap_percntg;
         rcd_bds_material_plant_mch.backflush_indctr := rcd_lads_material_plant_mch.backflush_indctr;
         rcd_bds_material_plant_mch.sales_text_135_en := rcd_lads_material_plant_mch.sales_text_135_en;
         rcd_bds_material_plant_mch.sales_text_234_en := rcd_lads_material_plant_mch.sales_text_234_en;
         rcd_bds_material_plant_mch.sales_text_135_zh := rcd_lads_material_plant_mch.sales_text_135_zh;
         rcd_bds_material_plant_mch.sales_text_234_zh := rcd_lads_material_plant_mch.sales_text_234_zh;
         rcd_bds_material_plant_mch.regional_code_8 := rcd_lads_material_plant_mch.regional_code_8;
         rcd_bds_material_plant_mch.future_planned_price_1 := rcd_lads_material_plant_mch.future_planned_price_1;
         rcd_bds_material_plant_mch.vltn_class := rcd_lads_material_plant_mch.vltn_class;
         rcd_bds_material_plant_mch.bds_pce_factor_from_base_uom := rcd_lads_material_plant_mch.bds_pce_factor_from_base_uom;
         rcd_bds_material_plant_mch.mars_pce_item_code := rcd_lads_material_plant_mch.mars_pce_item_code;
         rcd_bds_material_plant_mch.mars_pce_interntl_article_no := rcd_lads_material_plant_mch.mars_pce_interntl_article_no;
         rcd_bds_material_plant_mch.bds_sb_factor_from_base_uom := rcd_lads_material_plant_mch.bds_sb_factor_from_base_uom;
         rcd_bds_material_plant_mch.mars_sb_item_code := rcd_lads_material_plant_mch.mars_sb_item_code;
         rcd_bds_material_plant_mch.effective_out_date := rcd_lads_material_plant_mch.effective_out_date;
         rcd_bds_material_plant_mch.discontinuation_indctr := rcd_lads_material_plant_mch.discontinuation_indctr;
         rcd_bds_material_plant_mch.followup_material := rcd_lads_material_plant_mch.followup_material;
         rcd_bds_material_plant_mch.material_division := rcd_lads_material_plant_mch.material_division;
         rcd_bds_material_plant_mch.mrp_type := rcd_lads_material_plant_mch.mrp_type;
         rcd_bds_material_plant_mch.max_storage_prd := rcd_lads_material_plant_mch.max_storage_prd;
         rcd_bds_material_plant_mch.max_storage_prd_unit := rcd_lads_material_plant_mch.max_storage_prd_unit;

         /*-*/
         /* Calculate Unit Cost
         /*-*/
         case 
            when (rcd_lads_material_plant_mch.stndrd_price is null or
                  rcd_lads_material_plant_mch.stndrd_price = 0) then
               rcd_bds_material_plant_mch.bds_unit_cost := 0;   
            else
               rcd_bds_material_plant_mch.bds_unit_cost := rcd_lads_material_plant_mch.stndrd_price/rcd_lads_material_plant_mch.price_unit;      
         end case;


         /*------------------------------------*/
         /* UPDATE BDS_MATERIAL_PLANT_MCH      */
         /*------------------------------------*/
         insert into bds_material_plant_mch
            (sap_material_code,
             plant_code,
             mars_rprsnttv_item_code,
             bds_material_desc_en,
             ...
             max_storage_prd_unit)
          values 
            (rcd_bds_material_plant_mch.sap_material_code,
             rcd_bds_material_plant_mch.plant_code,
             rcd_bds_material_plant_mch.mars_rprsnttv_item_code,
             rcd_bds_material_plant_mch.bds_material_desc_en,
             rcd_bds_material_plant_mch.bds_material_desc_zh,
             rcd_bds_material_plant_mch.material_type,
             rcd_bds_material_plant_mch.material_grp,
             rcd_bds_material_plant_mch.base_uom,
             rcd_bds_material_plant_mch.order_unit,
             rcd_bds_material_plant_mch.gross_weight,
             rcd_bds_material_plant_mch.net_weight,
             rcd_bds_material_plant_mch.gross_weight_unit,
             rcd_bds_material_plant_mch.length,
             rcd_bds_material_plant_mch.width,
             rcd_bds_material_plant_mch.height,
             rcd_bds_material_plant_mch.dimension_uom,
             rcd_bds_material_plant_mch.interntl_article_no,
             rcd_bds_material_plant_mch.total_shelf_life,
             rcd_bds_material_plant_mch.mars_intrmdt_prdct_compnt_flag,
             rcd_bds_material_plant_mch.mars_merchandising_unit_flag,
             rcd_bds_material_plant_mch.mars_prmotional_material_flag,
             rcd_bds_material_plant_mch.mars_retail_sales_unit_flag,
             rcd_bds_material_plant_mch.mars_semi_finished_prdct_flag,
             rcd_bds_material_plant_mch.mars_rprsnttv_item_flag,
             rcd_bds_material_plant_mch.mars_traded_unit_flag,
             rcd_bds_material_plant_mch.xplant_status,
             rcd_bds_material_plant_mch.xplant_status_valid,
             rcd_bds_material_plant_mch.batch_mngmnt_reqrmnt_indctr,
             rcd_bds_material_plant_mch.mars_plant_material_type,
             rcd_bds_material_plant_mch.procurement_type,
             rcd_bds_material_plant_mch.special_procurement_type,
             rcd_bds_material_plant_mch.issue_storage_location,
             rcd_bds_material_plant_mch.mrp_controller,
             rcd_bds_material_plant_mch.plant_specific_status_valid,
             rcd_bds_material_plant_mch.deletion_indctr,
             rcd_bds_material_plant_mch.plant_specific_status,
             rcd_bds_material_plant_mch.assembly_scrap_percntg,
             rcd_bds_material_plant_mch.component_scrap_percntg,
             rcd_bds_material_plant_mch.backflush_indctr,
             rcd_bds_material_plant_mch.sales_text_135_en,
             rcd_bds_material_plant_mch.sales_text_234_en,
             rcd_bds_material_plant_mch.sales_text_135_zh,
             rcd_bds_material_plant_mch.sales_text_234_zh,
             rcd_bds_material_plant_mch.regional_code_8,
             rcd_bds_material_plant_mch.bds_unit_cost,
             rcd_bds_material_plant_mch.future_planned_price_1,
             rcd_bds_material_plant_mch.vltn_class,
             rcd_bds_material_plant_mch.bds_pce_factor_from_base_uom,
             rcd_bds_material_plant_mch.mars_pce_item_code,
             rcd_bds_material_plant_mch.mars_pce_interntl_article_no,
             rcd_bds_material_plant_mch.bds_sb_factor_from_base_uom,
             rcd_bds_material_plant_mch.mars_sb_item_code,
             rcd_bds_material_plant_mch.effective_out_date,
             rcd_bds_material_plant_mch.discontinuation_indctr,
             rcd_bds_material_plant_mch.followup_material,
             rcd_bds_material_plant_mch.material_division,
             rcd_bds_material_plant_mch.mrp_type,
             rcd_bds_material_plant_mch.max_storage_prd,
             rcd_bds_material_plant_mch.max_storage_prd_unit);


      end loop;
      close csr_lads_material_plant_mch;
   ...
   begin
      ...
      /*-*/
      bds_table.truncate('bds_material_plant_mch');
      ...
      bds_table.truncate('bds_material_text_zh');
