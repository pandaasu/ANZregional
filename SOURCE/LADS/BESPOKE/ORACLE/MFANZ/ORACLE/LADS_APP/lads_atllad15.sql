...
create or replace package body lads_atllad15 as
   ...
   begin
      ...
      if var_trn_ignore = true then
         ...
         begin
            lads_atllad15_monitor.execute_before(rcd_lads_adr_hdr.obj_type, rcd_lads_adr_hdr.obj_id, rcd_lads_adr_hdr.context);
         exception
            when others then
         ...
         begin
            lads_atllad15_monitor.execute_after(rcd_lads_adr_hdr.obj_type, rcd_lads_adr_hdr.obj_id, rcd_lads_adr_hdr.context);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;         
   ...
   begin
      ...
      rcd_lads_adr_hdr.lads_flattened := '0';
      ...
      begin
         ...
         values
            (rcd_lads_adr_hdr.obj_type,
             ...
             rcd_lads_adr_hdr.lads_flattened);
      exception
         when dup_val_on_index then
            ...
            if sql%found and var_idoc_timestamp <= rcd_lads_adr_hdr.idoc_timestamp then
      ...
      update lads_adr_hdr set
         ...
         lads_flattened = rcd_lads_adr_hdr.lads_flattened
      where obj_type = rcd_lads_adr_hdr.obj_type
        and obj_id = rcd_lads_adr_hdr.obj_id
        and context = rcd_lads_adr_hdr.context;
