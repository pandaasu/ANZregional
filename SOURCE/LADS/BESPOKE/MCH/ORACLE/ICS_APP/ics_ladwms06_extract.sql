--
-- ICS_LADWMS06_EXTRACT  (Package) 
--
CREATE OR REPLACE PACKAGE ICS_APP.ics_ladwms06_extract as
...
/**
 System  : LADS
 Package : ics_ladwms06_extract
 Owner   : ICS_APP
 Author  : Steve Gregan

 Description
 -----------
    LADS -> Thailand Warehouse Material Master Extract (DHSK)
...
end ics_ladwms06_extract;
/


--
-- ICS_LADWMS06_EXTRACT  (Synonym) 
--
CREATE PUBLIC SYNONYM ICS_LADWMS06_EXTRACT FOR ICS_APP.ICS_LADWMS06_EXTRACT;


GRANT EXECUTE ON ICS_APP.ICS_LADWMS06_EXTRACT TO LICS_APP;


--
-- ICS_LADWMS06_EXTRACT  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY ICS_APP.ics_ladwms06_extract as
   ...
   /* Constants
   /*-*/
   var_interface constant varchar2(8) := 'LADWMS06';
      ...
      cursor csr_matl_master is
        select 
            ...
            where 
                ...
                and t02.plant_code in ('TH04','TH05')
   ...
   exception
      ...
      when others then
         ...
         /* Raise
         /*-*/
         raise_application_error(-20000,'ICS_LADWMS06_EXTRACT - FORMAT_XML_STR - Error formatting string ['||par_string||'] - ['||SQLERRM||']');
