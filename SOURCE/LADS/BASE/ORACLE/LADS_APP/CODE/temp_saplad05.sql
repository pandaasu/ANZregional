...
/**
 System  : lads
 Package : temp_saplad05
 Owner   : lads_app
 Author  : Steve Gregan

 Description
 -----------
 Local Atlas Data Store - saplad05 - Inbound SAP Factory BOM Interface (TEMPORARY)
...
/* Package Header */
/******************/
create or replace package temp_saplad05 as
...
/* Package Body */
/****************/
create or replace package body temp_saplad05 as
   ...
   procedure load_data is
      ...
      rcd_temp_bom_hdr temp_bom_hdr%rowtype;
      rcd_temp_bom_det temp_bom_det%rowtype;
   ...
   begin
      ...
      loop
         ...
         if rcd_bom_data.bomkey != var_savkey then
            ...
            rcd_temp_bom_hdr.msgfn := '005';
            rcd_temp_bom_hdr.stlnr := trim(rcd_bom_data.stlnr);
            rcd_temp_bom_hdr.stlal := lads_trim_code(trim(rcd_bom_data.stlal));
            rcd_temp_bom_hdr.matnr := lads_trim_code(trim(rcd_bom_data.matnr));
            rcd_temp_bom_hdr.werks := trim(rcd_bom_data.werks);
            rcd_temp_bom_hdr.stlan := trim(rcd_bom_data.stlan);
            rcd_temp_bom_hdr.datuv := trim(rcd_bom_data.hdruv);
            rcd_temp_bom_hdr.datub := '99991231';
            rcd_temp_bom_hdr.bmeng := lads_to_number(replace(trim(rcd_bom_data.bmeng),'.',null))/1000;
            rcd_temp_bom_hdr.bmein := trim(rcd_bom_data.bmein);
            rcd_temp_bom_hdr.stlst := trim(rcd_bom_data.stlst);
            rcd_temp_bom_hdr.idoc_name := rcd_lads_control.idoc_name;
            rcd_temp_bom_hdr.idoc_number := rcd_lads_control.idoc_number;
         ...
         /*-*/
         rcd_temp_bom_det.detseq := rcd_temp_bom_det.detseq + 1;
         rcd_temp_bom_det.msgfn := '005';
         rcd_temp_bom_det.matnr := rcd_temp_bom_hdr.matnr;
         rcd_temp_bom_det.stlal := rcd_temp_bom_hdr.stlal;
         rcd_temp_bom_det.werks := rcd_temp_bom_hdr.werks;
         rcd_temp_bom_det.posnr := trim(rcd_bom_data.posnr);
         rcd_temp_bom_det.postp := trim(rcd_bom_data.postp);
         rcd_temp_bom_det.idnrk := lads_trim_code(trim(rcd_bom_data.idnrk));
         rcd_temp_bom_det.menge := lads_to_number(replace(trim(rcd_bom_data.menge),'.',null))/1000;
         rcd_temp_bom_det.meins := trim(rcd_bom_data.meins);
         rcd_temp_bom_det.datuv := trim(rcd_bom_data.datuv);
         rcd_temp_bom_det.datub := '99991231';
         insert into temp_bom_det values rcd_temp_bom_det;
...
/**************************/
grant execute on lads_app.temp_saplad05 to lics_app;
