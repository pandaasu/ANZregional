...
create or replace package body lads_atllad07 as
   ...
   begin
      ...
      if var_trn_ignore = true then
         ...
         begin
            lads_atllad07_monitor.execute_before(rcd_lads_cla_mas_hdr.klart, rcd_lads_cla_mas_hdr.class);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;
         ...
         begin
            lads_atllad07_monitor.execute_after(rcd_lads_cla_mas_hdr.klart, rcd_lads_cla_mas_hdr.class);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;         
