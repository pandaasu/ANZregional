--
-- LADS_ATLLAD32  (Package) 
--
CREATE OR REPLACE PACKAGE LADS_APP.lads_atllad32 as
   ...
   /**
   System  : lads
   Package : lads_atllad32
   Owner   : lads_app
   Author  : Steve Gregan

   Description
   -----------
   Local Atlas Data Store - atllad32 - Plant Maintenance Functional Location Master
   ...
   **Notes** 1. This package must NOT issue commit/rollback statements.
   ...
   TODO: Implement ATLLAD32_MONITOR
...
--
CREATE OR REPLACE PACKAGE BODY LADS_APP.lads_atllad32 as
   ...
   rcd_lads_flc_hdr lads_flc_hdr%rowtype;
   ...
   procedure complete_transaction is
      ...
      con_ack_code constant varchar2(32)  := 'ATLLAD32';
   ...
   begin
      ...
      if var_trn_ignore = true then
         ...
         begin
            lads_atllad32_monitor.execute_before(rcd_lads_flc_hdr.tplnr); 
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;
         ...
         begin
            lads_atllad32_monitor.execute_after(rcd_lads_flc_hdr.tplnr);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end; 
   ...
   begin
      ...
      /*-*/
      update lads_flc_hdr set lads_status='4';
   ...
   begin
      ...
      /*-*/
      rcd_lads_flc_hdr.tplnr := lics_inbound_utility.get_variable('TPLNR');
      rcd_lads_flc_hdr.pltxt := lics_inbound_utility.get_variable('PLTXT');
      rcd_lads_flc_hdr.eqfnr := lics_inbound_utility.get_variable('EQFNR');
      rcd_lads_flc_hdr.swerk := lics_inbound_utility.get_variable('SWERK');
      rcd_lads_flc_hdr.idoc_name := rcd_lads_control.idoc_name;
      rcd_lads_flc_hdr.idoc_number := rcd_lads_control.idoc_number;
      rcd_lads_flc_hdr.idoc_timestamp := rcd_lads_control.idoc_timestamp;
      rcd_lads_flc_hdr.lads_date := sysdate;
      rcd_lads_flc_hdr.lads_status := '1';
      rcd_lads_flc_hdr.lads_flattened := '0';      
      ...
      /*------------------------------*/
      update lads_flc_hdr set
           tplnr = rcd_lads_flc_hdr.tplnr,
           pltxt = rcd_lads_flc_hdr.pltxt,
           eqfnr = rcd_lads_flc_hdr.eqfnr,
           swerk = rcd_lads_flc_hdr.swerk,
           idoc_name = rcd_lads_flc_hdr.idoc_name,
           idoc_number = rcd_lads_flc_hdr.idoc_number,
           idoc_timestamp = rcd_lads_flc_hdr.idoc_timestamp,
           lads_date = rcd_lads_flc_hdr.lads_date,
           lads_status = rcd_lads_flc_hdr.lads_status,
           lads_flattened = rcd_lads_flc_hdr.lads_flattened
      where
           tplnr = rcd_lads_flc_hdr.tplnr;

      if sql%notfound then
           insert into lads_flc_hdr
           (
                tplnr,
                pltxt,
                eqfnr,
           ...
           values
           (
                rcd_lads_flc_hdr.tplnr,
                rcd_lads_flc_hdr.pltxt,
                rcd_lads_flc_hdr.eqfnr,
                rcd_lads_flc_hdr.swerk,
                rcd_lads_flc_hdr.idoc_name,
                rcd_lads_flc_hdr.idoc_number,
                rcd_lads_flc_hdr.idoc_timestamp,
                rcd_lads_flc_hdr.lads_date,
                rcd_lads_flc_hdr.lads_status,
                rcd_lads_flc_hdr.lads_flattened
           );   
      end if;
