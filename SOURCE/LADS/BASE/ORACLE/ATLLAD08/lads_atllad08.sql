...
create or replace package body lads_atllad08 as
   ...
   begin
      ...
      if var_trn_ignore = true then
         ...
         commit;
         begin
            lads_atllad08_monitor.execute(rcd_lads_mat_bom_hdr.stlnr, rcd_lads_mat_bom_hdr.stlal);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;
   ...
   procedure process_record_hdr(par_record in varchar2) is
      ...
      /* Local cursors
      /*-*/
      cursor csr_lads_mat_bom_hdr_01 is
         select
            t01.stlnr,
            t01.stlal,
            t01.idoc_number,
      ...
      rcd_lads_mat_bom_hdr_01 csr_lads_mat_bom_hdr_01%rowtype;
   ...
   begin
      ...
      /*-*/
      if not(rcd_lads_mat_bom_hdr.stlnr is null)
         and not(rcd_lads_mat_bom_hdr.stlal is null) then
         var_exists := true;
         open csr_lads_mat_bom_hdr_01;
         fetch csr_lads_mat_bom_hdr_01 into rcd_lads_mat_bom_hdr_01;
         if csr_lads_mat_bom_hdr_01%notfound then
            var_exists := false;
         end if;
         close csr_lads_mat_bom_hdr_01;
         if var_exists = true then
            if rcd_lads_mat_bom_hdr.idoc_timestamp > rcd_lads_mat_bom_hdr_01.idoc_timestamp then
      ...
      if sql%notfound then
         ...
         values
            (rcd_lads_mat_bom_hdr.stlnr,
             ...
             rcd_lads_mat_bom_hdr.matnr,
             rcd_lads_mat_bom_hdr.werks,
             rcd_lads_mat_bom_hdr.stlan,
             rcd_lads_mat_bom_hdr.datuv,
             rcd_lads_mat_bom_hdr.stlst,
             rcd_lads_mat_bom_hdr.bmeng_c,
             rcd_lads_mat_bom_hdr.bmein,
