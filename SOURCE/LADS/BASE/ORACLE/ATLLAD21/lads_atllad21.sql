...
create or replace package body lads_atllad21 as
   ...
   begin
      ...
      if var_trn_ignore = true then
         ...
         commit;
         begin
            lads_atllad21_monitor.execute(rcd_lads_chr_mas_hdr.atnam);
         exception
            when others then
               lics_inbound_utility.add_exception(substr(SQLERRM, 1, 512));
         end;
   ...
   procedure process_record_hdr(par_record in varchar2) is
      ...
      /* Local cursors
      /*-*/
      cursor csr_lads_chr_mas_hdr_01 is
         select
            t01.atnam,
            t01.idoc_number,
            t01.idoc_timestamp
      ...
      rcd_lads_chr_mas_hdr_01 csr_lads_chr_mas_hdr_01%rowtype;
   ...
   begin
      ...
      /*-*/
      if not(rcd_lads_chr_mas_hdr.atnam is null) then
         var_exists := true;
         open csr_lads_chr_mas_hdr_01;
         fetch csr_lads_chr_mas_hdr_01 into rcd_lads_chr_mas_hdr_01;
         if csr_lads_chr_mas_hdr_01%notfound then
            var_exists := false;
         end if;
         close csr_lads_chr_mas_hdr_01;
         if var_exists = true then
            if rcd_lads_chr_mas_hdr.idoc_timestamp > rcd_lads_chr_mas_hdr_01.idoc_timestamp then
