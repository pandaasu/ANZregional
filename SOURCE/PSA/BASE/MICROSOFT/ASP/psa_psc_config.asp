...
<!--
   ...
   function loadFunction() {
      ...
      cobjScreens[10] = new clsScreen('dspLShow','hedLShow');
      ...
      cobjScreens[10].hedtxt = 'Line Configuration Selection';
   ...
   function requestWeekList() {
      var strXML;
      if (cstrWeekMore == '0') {
         strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*WEKLST" SRCCDE="*SCH" PSCCDE="'+fixXML(cstrWeekProd)+'" WEKCDE="9999999"/>';
      } else {
         strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*WEKLST" SRCCDE="*SCH" PSCCDE="'+fixXML(cstrWeekProd)+'" WEKCDE="'+fixXML(cstrWeekLast)+'"/>';
      }
      ...
      if (strResponse.substring(0,3) != '*OK') {
         ...
         for (var i=0;i<objElements.length;i++) {
            if (objElements[i].nodeName == 'LSTROW') {
               ...
               objCell.align = 'center';
               objCell.className = 'clsLabelFN';
               if (objElements[i].getAttribute('SLTOPN') == '1') {
                  if (objElements[i].getAttribute('SLTTYP') == '*WEEK') {
   ...
   function doTypeTonnReport() {
      if (!processForm()) {return;}
      doActivityStart(document.body);
      window.setTimeout('requestTonnReport();',10);
   }
   ...
   // Tonnage Functions //
   ///////////////////////
   function requestTonnReport() {
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*RTVTON"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'" WEKCDE="'+fixXML(cstrTypeWeek)+'"/>';
      doPostRequest('<%=strBase%>psa_psc_tonn_report.asp',function(strResponse) {checkTonnReport(strResponse);},false,streamXML(strXML));
   }
   function checkTonnReport(strResponse) {
      doActivityStop();
      if (strResponse.substring(0,3) != '*OK') {
         alert(strResponse);
      } else {
         ...
         for (var i=0;i<objElements.length;i++) {
            if (objElements[i].nodeName == 'WEKTON') {
               alert(objElements[i].getAttribute('TONMSG'));
            }
         }
      }
   }
         ...
         <td class="clsLabelBB" align=left colspan=2 nowrap><nobr>
            <table class="clsTable01" align=center cols=5 cellpadding="0" cellspacing="0">
               <tr>
                  ...
                  <td align=center colspan=1 nowrap><nobr><a class="clsButton" style="font-size:8pt" onClick="doTypeTonnReport();">&nbsp;Tonnage&nbsp;</a></nobr></td>
               </tr>
            </table>
         </nobr></td>
      </tr>
