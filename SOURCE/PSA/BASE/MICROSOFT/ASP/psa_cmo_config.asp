...
'//////////////////////////////////////////////////////////////////
   ...
   '//
   strTarget = "psa_cmo_config.asp"
   strHeading = "Crew Model Maintenance"
...
<!--
   ...
   function loadFunction() {
      ...
      cobjScreens[2].hedtxt = 'Crew Model Selection';
      cobjScreens[3].hedtxt = 'Crew Model Maintenance';
   ...
   function requestTypeList(strAction) {
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="'+strAction+'" CREFLG="1" STRCDE="'+cstrTypeStrCode+'" ENDCDE="'+cstrTypeEndCode+'"/>';
      doPostRequest('<%=strBase%>psa_cmo_config_type.asp',function(strResponse) {checkTypeList(strResponse);},false,streamXML(strXML));
   }
         ...
         for (var i=0;i<objElements.length;i++) {
            if (objElements[i].nodeName == 'LSTROW') {
               if (cstrSelectStrCode == '') {
                  cstrSelectStrCode = objElements[i].getAttribute('CMOCDE');
               }
               cstrSelectEndCode = objElements[i].getAttribute('CMOCDE');
               objRow = objTabBody.insertRow(-1);
               objCell = objRow.insertCell(-1);
               objCell.colSpan = 1;
               objCell.align = 'center';
               objCell.innerHTML = '&nbsp;<a class="clsSelect" onClick="doSelectUpdate(\''+objElements[i].getAttribute('CMOCDE')+'\');">Update</a>&nbsp;/&nbsp;<a class="clsSelect" onClick="doSelectDelete(\''+objElements[i].getAttribute('CMOCDE')+'\');">Delete</a>&nbsp;/&nbsp;<a class="clsSelect" onClick="doSelectCopy(\''+objElements[i].getAttribute('CMOCDE')+'\');">Copy</a>&nbsp;';
               objCell.className = 'clsLabelFN';
               objCell.style.whiteSpace = 'nowrap';
               objCell = objRow.insertCell(-1);
               objCell.colSpan = 1;
               objCell.align = 'left';
               objCell.innerHTML = '&nbsp;'+objElements[i].getAttribute('CMOCDE')+'&nbsp;';
               objCell.className = 'clsLabelFN';
               objCell.style.whiteSpace = 'nowrap';
               objCell = objRow.insertCell(-1);
               objCell.colSpan = 1;
               objCell.align = 'left';
               objCell.innerHTML = '&nbsp;'+objElements[i].getAttribute('CMONAM')+'&nbsp;';
               objCell.className = 'clsLabelFN';
               objCell.style.whiteSpace = 'nowrap';
               objCell = objRow.insertCell(-1);
               objCell.colSpan = 1;
               objCell.align = 'left';
               objCell.innerHTML = '&nbsp;'+objElements[i].getAttribute('CMOSTS')+'&nbsp;';
               objCell.className = 'clsLabelFN';
               objCell.style.whiteSpace = 'nowrap';
            }
         }
   ...
   function requestDelete(strCode) {
      ...
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*DLTDEF" CMOCDE="'+fixXML(strCode)+'"/>';
      doPostRequest('<%=strBase%>psa_cmo_config_delete.asp',function(strResponse) {checkDelete(strResponse);},false,streamXML(strXML));
   }
   ...
   function checkDefineLoad(strResponse) {
      ...
      if (strResponse.substring(0,3) != '*OK') {
         ...
         var strCmoStat = '';
         var objCmoStat = document.getElementById('DEF_CmoStat');
         var objResData = document.getElementById('DEF_ResData');
         ...
         var objInput;
         for (var i=0;i<objElements.length;i++) {
            if (objElements[i].nodeName == 'CMODFN') {
               if (cstrDefineMode == '*UPD') {
                  document.getElementById('DEF_UpdCode').innerHTML = '<p>'+objElements[i].getAttribute('CMOCDE')+'</p>';
               } else {
                  document.getElementById('DEF_CmoCode').value = objElements[i].getAttribute('CMOCDE');
               }
               document.getElementById('DEF_CmoName').value = objElements[i].getAttribute('CMONAM');
               strCmoStat = objElements[i].getAttribute('CMOSTS');
            } else if (objElements[i].nodeName == 'CMORES') {
               objRow = objResData.insertRow(-1);
               objRow.setAttribute('rescod',objElements[i].getAttribute('RESCDE'));
               objCell = objRow.insertCell(-1);
               objCell.colSpan = 1;
               objCell.align = 'right';
               ...
               objInput.type = 'text';
               objInput.value = objElements[i].getAttribute('RESQTY');
               objInput.id = 'DEF_'+objElements[i].getAttribute('RESCDE');
               objInput.size = 9;
         ...
         objCmoStat.selectedIndex = -1;
         for (var i=0;i<objCmoStat.length;i++) {
            if (objCmoStat.options[i].value == strCmoStat) {
               objCmoStat.options[i].selected = true;
               break;
            }
         }
         if (cstrDefineMode == '*UPD') {
            document.getElementById('DEF_CmoName').focus();
         } else {
            document.getElementById('DEF_CmoCode').focus();
         }
      }
   }
   function doDefineAccept() {
      ...
      if (cstrDefineMode == '*UPD') {
         ...
         strXML = strXML+' CMOCDE="'+fixXML(cstrDefineCode)+'"';
      } else {
         strXML = strXML+'<PSA_REQUEST ACTION="*CRTDEF"';
         strXML = strXML+' CMOCDE="'+fixXML(document.getElementById('DEF_CmoCode').value)+'"';
      }
      strXML = strXML+' CMONAM="'+fixXML(document.getElementById('DEF_CmoName').value)+'"';
      if (objCmoStat.selectedIndex == -1) {
         strXML = strXML+' CMOSTS=""';
      } else {
         strXML = strXML+' CMOSTS="'+fixXML(objCmoStat.options[objCmoStat.selectedIndex].value)+'"';
      }
   ...
   function requestDefineAccept(strXML) {
      doPostRequest('<%=strBase%>psa_cmo_config_update.asp',function(strResponse) {checkDefineAccept(strResponse);},false,streamXML(strXML));
   }
...
<body class="clsBody02" scroll="auto" onLoad="parent.setStatus('<%=strStatus%>');parent.setHelp('psa_cmo_config_help.htm');parent.setHeading('<%=strHeading%>');parent.showContent();loadFunction();">
   ...
   <table id="dspSelect" class="clsGrid02" style="display:none;visibility:visible" height=100% width=100% align=center valign=top cols=2 cellpadding=1 cellspacing=0>
      <tr><td align=center colspan=2 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedSelect" class="clsFunction" align=center colspan=2 nowrap><nobr>Crew Model Selection</nobr></td>
      </tr>
      <tr>
   ...
   <table id="dspDefine" class="clsGrid02" style="display:none;visibility:visible" width=100% align=center valign=top cols=2 cellpadding=1 cellspacing=0 onKeyPress="if (event.keyCode == 13) {doDefineAccept();}">
      <tr><td align=center colspan=2 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedDefine" class="clsFunction" align=center valign=center colspan=2 nowrap><nobr>Crew Model Define</nobr></td>
      </tr>
      <tr>
      ...
      <tr id="addDefine" style="display:none;visibility:visible">
         <td class="clsLabelBB" align=right valign=center colspan=1 nowrap><nobr>&nbsp;Crew Model Code:&nbsp;</nobr></td>
         <td class="clsLabelBN" align=left valign=center colspan=1 nowrap><nobr>
            <input class="clsInputNN" style="text-transform:uppercase;" type="text" name="DEF_CmoCode" size="32" maxlength="32" value="" onFocus="setSelect(this);">
         </nobr></td>
      </tr>
      ...
      <tr>
         <td class="clsLabelBB" align=right valign=center colspan=1 nowrap><nobr>&nbsp;Crew Model Name:&nbsp;</nobr></td>
         <td class="clsLabelBN" align=left valign=center colspan=1 nowrap><nobr>
            <input class="clsInputNN" type="text" name="DEF_CmoName" size="80" maxlength="120" value="" onFocus="setSelect(this);">
         </nobr></td>
      </tr>
      <tr>
         <td class="clsLabelBB" align=right valign=center colspan=1 nowrap><nobr>&nbsp;Crew Model Status:&nbsp;</nobr></td>
         <td class="clsLabelBN" align=left valign=center colspan=1 nowrap><nobr>
            <select class="clsInputBN" id="DEF_CmoStat">
