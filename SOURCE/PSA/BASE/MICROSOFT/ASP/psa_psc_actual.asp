...
'// Script  : psa_psc_actual.asp                                 //
'// Author  : Steve Gregan                                       //
'// Date    : December 2009                                      //
'// Text    : This script implements the production actuals      //
...
'//////////////////////////////////////////////////////////////////
   ...
   strHeading = "Production Actuals Maintenance"
   ...
   '//
   strReturn = GetSecurityCheck("PSA_PSC_ACTUAL")
...
<!--
   ...
   function loadFunction() {
      ...
      cobjScreens[3] = new clsScreen('dspLine','hedLine');
      cobjScreens[4] = new clsScreen('dspTime','hedTime');
      cobjScreens[5] = new clsScreen('dspCProd','hedCProd');
      cobjScreens[6] = new clsScreen('dspUProd','hedUProd');
      cobjScreens[7] = new clsScreen('dspLShow','hedLShow');
      cobjScreens[0].hedtxt = '**LOADING**';
      cobjScreens[1].hedtxt = 'Week Selection';
      cobjScreens[2].hedtxt = 'Actual Maintenance';
      cobjScreens[3].hedtxt = 'Line Maintenance';
      cobjScreens[4].hedtxt = 'Actual Time Activity';
      cobjScreens[5].hedtxt = 'Create Actual Production Activity';
      cobjScreens[6].hedtxt = 'Update Actual Production Activity';
      cobjScreens[7].hedtxt = 'Line Configuration Selection';
   ...
   function doTypeWindPaint(intLinIdx) {
      ...
      for (var i=0;i<objShfAry.length;i++) {
         if (objShfAry[i].wintyp == '1') {
            ...
            objDiv.align = 'left';
            objDiv.vAlign = 'top';
            objDiv.style.cssText = 'font-size:10pt;font-weight:bold;background-color:#ffd9ff;color:#000000;border:#c7c7c7 1px solid;padding-left:4px;padding-right:4px;width:1%;white-space:nowrap;cursor:pointer;';
      ...
      for (var i=0;i<objActAry.length;i++) {
         ...
         for (var j=intStrBar;j<=intEndBar;j++) {
            ...
            if (j == intStrBar) {
               ...
               if (objWork.wekflw == '0') {
                  strStyle = strStyle + 'cursor:pointer;';
               }
   ...
   function doTypeUactPaint() {
      ...
      for (var i=0;i<cobjTypeUact.length;i++) {
         ...
         if (objWork.acttyp == 'T') {
            ...
            objCell.appendChild(document.createElement('br'));
            objCell.appendChild(document.createTextNode('Actual Duration ('+objWork.schdur+')'));
         } else {
            ...
            objCell.appendChild(document.createElement('br'));
            if (cstrTypeCode == '*FILL') {
               objCell.appendChild(document.createTextNode('Cases Scheduled('+objWork.reqcas+') Actual('+objWork.schcas+')'));
               objCell.appendChild(document.createElement('br'));
               objCell.appendChild(document.createTextNode('Pouches Scheduled('+objWork.reqpch+') Actual('+objWork.schpch+')'));
               objCell.appendChild(document.createElement('br'));
               objCell.appendChild(document.createTextNode('Mixes Scheduled('+objWork.reqmix+') Actual('+objWork.schmix+')'));
            } else if (cstrTypeCode == '*PACK') {
               objCell.appendChild(document.createTextNode('Cases Scheduled('+objWork.reqcas+') Actual('+objWork.schcas+')'));
               objCell.appendChild(document.createElement('br'));
               objCell.appendChild(document.createTextNode('Pallets Scheduled('+objWork.reqplt+') Actual('+objWork.schplt+')'));
            } else if (cstrTypeCode == '*FORM') {
               objCell.appendChild(document.createTextNode('Pouches Scheduled('+objWork.reqpch+') Actual('+objWork.schpch+')'));
            }
            objCell.appendChild(document.createElement('br'));
   ...
   function requestStckUpdate() {
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*UPDSTK" SRCCDE="*ACT"';
   ...
   function requestLineCreate() {
      cstrLineMode = '*CRT';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*CRTLIN" SRCCDE="*ACT"';
   ...
   function requestLineUpdate() {
      cstrLineMode = '*UPD';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*UPDLIN" SRCCDE="*ACT"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
      strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
      strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
   ...
   function requestLineDelete() {
      cstrLineMode = '*DLT';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*DLTLIN" SRCCDE="*ACT"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
      strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
      strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
      strXML = strXML+' LINCDE="'+fixXML(cstrTypeLcde)+'"';
   ...
   function checkLineLoad(strResponse) {
      if (strResponse.substring(0,3) != '*OK') {
         ...
         if (cstrLineMode == '*UPD') {
            ...
            cobjScreens[3].hedtxt = 'Add Line Configuration';
   ...
   function doLineAccept() {
      ...
      if (cstrLineMode == '*UPD') {
         strXML = strXML+'<PSA_REQUEST ACTION="*UPDLIN" SRCCDE="*ACT"';
         strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
         strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
         strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
         ...
         strXML = strXML+'<PSA_REQUEST ACTION="*CRTLIN" SRCCDE="*ACT"';
         strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
         strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
         strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
   ...
   function requestActvLoad() {
      var strXML = '<?xml version="1.0" encoding="UTF-8"?>';
      strXML = strXML+' <PSA_REQUEST ACTION="'+cstrActvMode+'" SRCCDE="*ACT"';
   ...
   function requestActvAttach() {
      cstrActvMode = '*RTVSCH';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?>';
      strXML = strXML+' <PSA_REQUEST ACTION="*ATTACT" SRCCDE="*ACT"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
      strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
      strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
      strXML = strXML+' LINCDE="'+fixXML(cstrTypeLcde)+'"';
   ...
   function requestActvDetach() {
      cstrActvMode = '*RTVSCH';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?>';
      strXML = strXML+' <PSA_REQUEST ACTION="*DETACT" SRCCDE="*ACT"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
      strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
      strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
      strXML = strXML+' ACTCDE="'+fixXML(cstrTypeAcde)+'"/>';
   ...
   function requestActvDelete() {
      cstrActvMode = '*RTVACT';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?>';
      strXML = strXML+' <PSA_REQUEST ACTION="*DLTACT" SRCCDE="*ACT"';
      strXML = strXML+' PSCCDE="'+fixXML(cstrTypeProd)+'"';
      strXML = strXML+' WEKCDE="'+fixXML(cstrTypeWeek)+'"';
      strXML = strXML+' PTYCDE="'+fixXML(cstrTypeCode)+'"';
      strXML = strXML+' ACTCDE="'+fixXML(cstrTypeAcde)+'"/>';
   ...
   function requestTimeUpdate() {
      cstrTimeMode = '*UPD';
      var strXML = '<?xml version="1.0" encoding="UTF-8"?><PSA_REQUEST ACTION="*UPDACT" SRCCDE="*ACT" ACTCDE="'+fixXML(cstrTypeAcde)+'"/>';
      doPostRequest('<%=strBase%>psa_psc_time_retrieve.asp',function(strResponse) {checkTimeLoad(strResponse);},false,streamXML(strXML));
   }
   ...
   function doTimeAccept() {
      ...
      var strXML = '<?xml version="1.0" encoding="UTF-8"?>';
      if (cstrTimeMode == '*UPD') {
         strXML = strXML+' <PSA_REQUEST ACTION="*UPDACT" SRCCDE="*ACT"';
      } else {
...
<body id="dspBody" class="clsBody02" scroll="auto" onLoad="parent.setStatus('<%=strStatus%>');parent.setHelp('psa_psc_actual_help.htm');parent.setHeading('<%=strHeading%>');parent.showContent();loadFunction();">
   ...
   <table id="dspWeeks" class="clsGrid02" style="display:none;visibility:visible" height=100% width=100% align=center valign=top cols=2 cellpadding=1 cellspacing=0>
      ...
      <tr>
         <td id="hedWeeks" class="clsFunction" align=center colspan=2 nowrap><nobr>Production Actuals Week Selection</nobr></td>
      </tr>
      <tr>
   ...
   <table id="dspType" class="clsGrid02" style="display:none;visibility:visible" height=100% width=100% align=center valign=top cols=2 cellpadding=1 cellspacing=0>
      <tr><td align=center colspan=2 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedType" class="clsFunction" align=center valign=center colspan=2 nowrap><nobr>Production Actuals Maintenance</nobr></td>
      </tr>
      </table></nobr></td></tr>
      <tr>
         ...
         <td class="clsLabelBB" align=left colspan=2 nowrap><nobr>
            <table class="clsTable01" align=center cols=17 cellpadding="0" cellspacing="0">
               <tr>
                  ...
                  <td id="typPulse" class="clsTabB" style="font-size:8pt" align=center colspan=1 nowrap><nobr>&nbsp;Actuals&nbsp;</nobr></td>
   ...
   <table id="dspTime" class="clsGrid02" style="display:none;visibility:visible" width=100% align=center valign=top cols=1 cellspacing=1 cellpadding=0>
      <tr><td align=center colspan=1 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedTime" class="clsFunction" align=center valign=center colspan=2 nowrap><nobr>Create Actual Time Activity</nobr></td>
      </tr>
      <tr>
         <td class="clsLabelBB" align=center colspan=2 nowrap><nobr>&nbsp;</nobr></td>
      </tr>
   ...
   <table id="dspCProd" class="clsGrid02" style="display:none;visibility:visible" width=100% align=center valign=top cols=1 cellspacing=1 cellpadding=0>
      <tr><td align=center colspan=1 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedCProd" class="clsFunction" align=center valign=center colspan=2 nowrap><nobr>Create Actual Production Activity</nobr></td>
      </tr>
      <tr>
         <td class="clsLabelBB" align=center colspan=2 nowrap><nobr>&nbsp;</nobr></td>
      </tr>
   ...
   <table id="dspUProd" class="clsGrid02" style="display:none;visibility:visible" width=100% align=center valign=top cols=1 cellspacing=1 cellpadding=0>
      <tr><td align=center colspan=1 nowrap><nobr><table class="clsPanel" align=center cols=2 cellpadding="0" cellspacing="0">
      <tr>
         <td id="hedUProd" class="clsFunction" align=center valign=center colspan=2 nowrap><nobr>Update Actual Production Activity</nobr></td>
      </tr>
      <tr>
         <td class="clsLabelBB" align=center colspan=2 nowrap><nobr>&nbsp;</nobr></td>
      </tr>
