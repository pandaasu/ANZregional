   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW RRACDE="'||psa_to_xml(tbl_list(idx).rrd_rra_code)||'" RRANAM="'||psa_to_xml(tbl_list(idx).rrd_rra_name)||'" RRASTS="'||psa_to_xml(tbl_list(idx).rrd_rra_status)||'"/>'));
         end loop;
