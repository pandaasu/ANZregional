   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW REQCDE="'||psa_to_xml(tbl_list(idx).rhe_req_code)||'" REQNAM="'||psa_to_xml(tbl_list(idx).rhe_req_name)||'" REQSTS="'||psa_to_xml(tbl_list(idx).rhe_req_status)||'" REQUSR="'||psa_to_xml(tbl_list(idx).rhe_upd_user)||'"/>'));
         end loop;
