   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW CONCDE="'||psa_to_xml(tbl_list(idx).lco_con_code)||'" CONNAM="'||psa_to_xml(tbl_list(idx).lco_con_name)||'" CONSTS="'||psa_to_xml(tbl_list(idx).lco_con_status)||'"/>'));
         end loop;
