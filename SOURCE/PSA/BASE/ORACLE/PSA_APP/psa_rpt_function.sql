...
create or replace package psa_app.psa_rpt_function as
   ...
   *******************************************************************************/
   ...
   function report_shift(par_psc_code in varchar2, par_wek_code in varchar2, par_pty_code in varchar2) return psa_xls_type pipelined;
...
create or replace package body psa_app.psa_rpt_function as
   ...
   /****************************************************/
   /* This procedure performs the report shift routine */
   /****************************************************/
   function report_shift(par_psc_code in varchar2, par_wek_code in varchar2, par_pty_code in varchar2) return psa_xls_type pipelined is
      ...
      cursor csr_pmod is
         select t02.rde_res_code,
           ...
           from psa_cmo_resource t01,
                psa_res_defn t02
          where t01.cmr_res_code = t02.rde_res_code
            and t01.cmr_cmo_code = rcd_week.psw_cmo_code
          order by t01.cmr_res_code asc;
   ...
   begin
      ...
      /*-*/
      /* Retrieve the existing production week
      /*-*/
      var_found := false;
      open csr_week;
      ...
      close csr_week;
      if var_found = false then
         raise_application_error(-20000, 'Production schedule week ('||var_psc_code||' / '||var_wek_code||') does not exist');
      end if;
      ...
      /*-*/
      /* Output the week resources
      /*-*/
      if rcd_week.psw_cmo_code = '*NONE' then
         var_work := '<font style="font-weight:bold;">Plant Resource</font><br>*NONE';
      else
         var_work := '<font style="font-weight:bold;">Plant Resource</font>';
         open csr_pmod;
         loop
            ...
            var_work := var_work||'<br>('||rcd_pmod.cmr_res_qnty||') - '||rcd_pmod.rde_res_name;
         end loop;
         close csr_pmod;
      end if;
      pipe row('<tr><td align=center valign=top rowspan=1 colspan='||to_char(var_cmax)||' style="font-family:Arial;font-size:8pt;font-weight:normal;background-color:#c0ffc0;color:#000000;border:#008000 1pt solid;" nowrap>'||var_work||'</td></tr>');

      /*-*/
      /* Output the schedule headings
      /*-*/
      pipe row('<tr>');
      pipe row('<td align=center colspan=1 style="font-family:Arial;font-size:8pt;font-weight:bold;background-color:#008000;color:#ffffff;border:#c0ffc0 1pt solid;" nowrap>Date</td>');
      pipe row('<td align=center colspan=1 style="font-family:Arial;font-size:8pt;font-weight:bold;background-color:#008000;color:#ffffff;border:#c0ffc0 1pt solid;" nowrap>Time</td>');
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - PSA_RPT_FUNCTION - REPORT_SHIFT - ' || substr(SQLERRM, 1, 1536));
