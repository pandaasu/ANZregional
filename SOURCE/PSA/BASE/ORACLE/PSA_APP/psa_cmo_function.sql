   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW CMOCDE="'||psa_to_xml(tbl_list(idx).cmd_cmo_code)||'" CMONAM="'||psa_to_xml(tbl_list(idx).cmd_cmo_name)||'" CMOSTS="'||psa_to_xml(tbl_list(idx).cmd_cmo_status)||'"/>'));
         end loop;
