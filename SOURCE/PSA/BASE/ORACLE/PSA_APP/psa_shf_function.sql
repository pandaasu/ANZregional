   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW SHFCDE="'||psa_to_xml(tbl_list(idx).sde_shf_code)||'" SHFNAM="'||psa_to_xml(tbl_list(idx).sde_shf_name)||'" SHFSTS="'||psa_to_xml(tbl_list(idx).sde_shf_status)||'"/>'));
         end loop;
