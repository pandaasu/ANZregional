...
create or replace package body psa_app.psa_shf_function as
   ...
   function select_list return psa_xml_type pipelined is
      ...
      cursor csr_slct is
         select t01.*
           from (select t01.shi_shf_code,
                        t01.shi_shf_name,
                        decode(t01.shi_shf_status,'0','Inactive','1','Active','*UNKNOWN') as shi_shf_status
                   from psa_shift t01
                  where (var_str_code is null or t01.shi_shf_code >= var_str_code)
                  order by t01.shi_shf_code asc) t01
      ...
      cursor csr_next is
         select t01.*
           from (select t01.shi_shf_code,
                        ...
                        decode(t01.shi_shf_status,'0','Inactive','1','Active','*UNKNOWN') as shi_shf_status
                   from psa_shift t01
                  where (var_action = '*NXTDEF' and (var_end_code is null or t01.shi_shf_code > var_end_code)) or
                        (var_action = '*PRVDEF')
                  order by t01.shi_shf_code asc) t01
          where rownum <= var_pag_size;
      ...
      cursor csr_prev is
         select t01.*
           from (select t01.shi_shf_code,
                        ...
                        decode(t01.shi_shf_status,'0','Inactive','1','Active','*UNKNOWN') as shi_shf_status
                   from psa_shift t01
                  where (var_action = '*PRVDEF' and (var_str_code is null or t01.shi_shf_code < var_str_code)) or
                        (var_action = '*NXTDEF')
                  order by t01.shi_shf_code desc) t01
          where rownum <= var_pag_size;
   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW SHFCDE="'||to_char(tbl_list(idx).shi_shf_code)||'" SHFNAM="'||psa_to_xml(tbl_list(idx).shi_shf_name)||'" SHFSTS="'||psa_to_xml(tbl_list(idx).shi_shf_status)||'"/>'));
         end loop;
   ...
   function retrieve_data return psa_xml_type pipelined is
      ...
      cursor csr_retrieve is
         select t01.*
           from psa_shift t01
          where t01.shi_shf_code = var_shf_code;
   ...
   begin
      ...
      if var_action = '*UPDDEF' then
         var_output := '<SHIFT SHFCDE="'||psa_to_xml(rcd_retrieve.shi_shf_code||' - (Last updated by '||rcd_retrieve.shi_upd_user||' on '||to_char(rcd_retrieve.shi_upd_date,'yyyy/mm/dd')||')')||'"';
         var_output := var_output||' SHFNAM="'||psa_to_xml(rcd_retrieve.shi_shf_name)||'"';
         var_output := var_output||' SHFSTR="'||to_char(rcd_retrieve.shi_shf_start,'fm0000')||'"';
         var_output := var_output||' SHFDUR="'||to_char(rcd_retrieve.shi_shf_duration)||'"';
         var_output := var_output||' SHFSTS="'||psa_to_xml(rcd_retrieve.shi_shf_status)||'"/>';
         ...
         var_output := var_output||' SHFSTR="0000"';
   ...
   procedure update_data(par_user in varchar2) is
      ...
      var_found boolean;
      rcd_psa_shift psa_shift%rowtype;
      ...
      cursor csr_retrieve is
         select t01.*
           from psa_shift t01
          where t01.shi_shf_code = rcd_psa_shift.shi_shf_code
            for update nowait;
      rcd_retrieve csr_retrieve%rowtype;
   ...
   begin
      ...
      /* Validate the input
      /*-*/
      if rcd_psa_shift.shi_shf_code is null then
         psa_gen_function.add_mesg_data('Shift code must be supplied');
      end if;
      if rcd_psa_shift.shi_shf_name is null then
         psa_gen_function.add_mesg_data('Shift name must be supplied');
      end if;
      if rcd_psa_shift.shi_shf_start is null then
         psa_gen_function.add_mesg_data('Shift start time must be supplied');
      else
         if rcd_psa_shift.shi_shf_start < 0 or rcd_psa_shift.shi_shf_start > 2359 then
            psa_gen_function.add_mesg_data('Shift start time must in the range 00:00 to 23:59');
         end if;
      end if;
      if rcd_psa_shift.shi_shf_duration is null or rcd_psa_shift.shi_shf_duration = 0 then
         psa_gen_function.add_mesg_data('Shift duration minutes must be supplied');
      end if;
      if rcd_psa_shift.shi_shf_status is null or (rcd_psa_shift.shi_shf_status != '0' and rcd_psa_shift.shi_shf_status != '1') then
         psa_gen_function.add_mesg_data('Shift status must be (0)inactive or (1)active');
      end if;
      if rcd_psa_shift.shi_upd_user is null then
      ...
      if var_action = '*UPDDEF' then
         ...
         exception
            when others then
               var_found := true;
               psa_gen_function.add_mesg_data('Shift code ('||rcd_psa_shift.shi_shf_code||') is currently locked');
         end;
         if var_found = false then
            psa_gen_function.add_mesg_data('Shift code ('||rcd_psa_shift.shi_shf_code||') does not exist');
         end if;
         if psa_gen_function.get_mesg_count = 0 then
            update psa_shift
               set shi_shf_name = rcd_psa_shift.shi_shf_name,
                   shi_shf_start = rcd_psa_shift.shi_shf_start,
                   shi_shf_duration = rcd_psa_shift.shi_shf_duration,
                   shi_shf_status = rcd_psa_shift.shi_shf_status,
                   shi_upd_user = rcd_psa_shift.shi_upd_user,
                   shi_upd_date = rcd_psa_shift.shi_upd_date
             where shi_shf_code = rcd_psa_shift.shi_shf_code;
         end if;
      elsif var_action = '*CRTDEF' then
         var_confirm := 'created';
         begin
            insert into psa_shift values rcd_psa_shift;
         exception
            when dup_val_on_index then
               psa_gen_function.add_mesg_data('Shift code ('||rcd_psa_shift.shi_shf_code||') already exists - unable to create');
         end;
      end if;
      if psa_gen_function.get_mesg_count != 0 then
         rollback;
      ...
      /*-*/
      psa_gen_function.set_cfrm_data('Shift ('||to_char(rcd_psa_shift.shi_shf_code)||') successfully '||var_confirm);
