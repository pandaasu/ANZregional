   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW SMOCDE="'||psa_to_xml(tbl_list(idx).smd_smo_code)||'" SMONAM="'||psa_to_xml(tbl_list(idx).smd_smo_name)||'" SMOSTS="'||psa_to_xml(tbl_list(idx).smd_smo_status)||'"/>'));
         end loop;
