...
create or replace package psa_app.psa_lin_function as
   ...
   *******************************************************************************/
   ...
   function select_config return psa_xml_type pipelined;
   function retrieve_config return psa_xml_type pipelined;
   procedure update_config(par_user in varchar2);
...
create or replace package body psa_app.psa_lin_function as
   ...
   begin
      ...
      if var_action = '*UPDDEF' then
         var_output := '<LDEDFN LINCDE="'||psa_to_xml(rcd_retrieve.lde_lin_code||' - (Last updated by '||rcd_retrieve.lde_upd_user||' on '||to_char(rcd_retrieve.lde_upd_date,'yyyy/mm/dd')||')')||'"';
   ...
   begin
      ...
      if csr_prdtype%notfound then
         ...
         if rcd_psa_lin_defn.lde_lin_status = '1' and rcd_prdtype.pty_prd_status != '1' then
            psa_gen_function.add_mesg_data('Production type code ('||rcd_psa_lin_defn.lde_prd_type||') status must be (1)active for an active line');
         end if;
   ...
   /************************************************************/
   /* This procedure performs the select configuration routine */
   /************************************************************/
   function select_config return psa_xml_type pipelined is
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         psa_gen_function.add_mesg_data('FATAL ERROR - PSA_LIN_FUNCTION - SELECT_CONFIG - ' || substr(SQLERRM, 1, 1536));
   ...
   /**************************************************************/
   /* This procedure performs the retrieve configuration routine */
   /**************************************************************/
   function retrieve_config return psa_xml_type pipelined is
   ...
   begin
      ...
      xmlDom.freeDocument(obj_xml_document);
      if var_action != '*UPDCON' and var_action != '*CRTCON' and var_action != '*CPYCON' then
         psa_gen_function.add_mesg_data('Invalid request action');
      end if;
      if psa_gen_function.get_mesg_count != 0 then
         return;
      ...
      /*-*/
      if var_action = '*UPDCON' or var_action = '*CPYCON' then
         var_found := false;
         open csr_retrieve;
         fetch csr_retrieve into rcd_retrieve;
         if csr_retrieve%found then
      ...
      if var_action = '*UPDCON' then
         ...
         pipe row(psa_xml_object(var_output));
      elsif var_action = '*CPYCON' then
         var_output := '<LCODFN CONCDE=""';
         var_output := var_output||' CONNAM="'||psa_to_xml(rcd_retrieve.lco_con_name)||'"';
         var_output := var_output||' CONSTS="'||psa_to_xml(rcd_retrieve.lco_con_status)||'"/>';
         pipe row(psa_xml_object(var_output));
      elsif var_action = '*CRTCON' then
         var_output := '<LCODFN LINCDE=""';
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         psa_gen_function.add_mesg_data('FATAL ERROR - PSA_LIN_FUNCTION - RETRIEVE_CONFIG - ' || substr(SQLERRM, 1, 1536));
   ...
   /************************************************************/
   /* This procedure performs the update configuration routine */
   /************************************************************/
   procedure update_config(par_user in varchar2) is
   ...
   begin
      ...
      obj_psa_request := xslProcessor.selectSingleNode(xmlDom.makeNode(obj_xml_document),'/PSA_REQUEST');
      var_action := upper(xslProcessor.valueOf(obj_psa_request,'@ACTION'));
      if var_action != '*UPDCON' and var_action != '*CRTCON' then
         psa_gen_function.add_mesg_data('Invalid request action');
      end if;
      if psa_gen_function.get_mesg_count != 0 then
         return;
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         psa_gen_function.add_mesg_data('FATAL ERROR - PSA_LIN_FUNCTION - UPDATE_CONFIG - ' || substr(SQLERRM, 1, 1536));
   ...
   /************************************************************/
   /* This procedure performs the delete configuration routine */
   /************************************************************/
   procedure delete_config is
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         psa_gen_function.add_mesg_data('FATAL ERROR - PSA_LIN_FUNCTION - DELETE_CONFIG - ' || substr(SQLERRM, 1, 1536));
