   ...
   begin
      ...
      if var_action = '*SELDEF' then
         ...
         for idx in 1..tbl_list.count loop
            pipe row(psa_xml_object('<LSTROW LINCDE="'||psa_to_xml(tbl_list(idx).lde_lin_code)||'" LINNAM="'||psa_to_xml(tbl_list(idx).lde_lin_name)||'" LINSTS="'||psa_to_xml(tbl_list(idx).lde_lin_status)||'" PTYCDE="'||psa_to_xml(tbl_list(idx).lde_prd_type)||'"/>'));
         end loop;
