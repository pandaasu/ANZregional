...
create or replace package body psa_app.psa_lin_function as
   ...
   begin
      ...
      if var_action = '*UPDDEF' then
         ...
         var_output := var_output||' LINUNT="0"';
         var_output := var_output||' LINEFF="100.00"';
   ...
   begin
      ...
      if csr_prdtype%notfound then
         ...
         if rcd_psa_lin_defn.lde_lin_status = '1' and rcd_prdtype.pty_prd_status != '1' then
            psa_gen_function.add_mesg_data('Production type code ('||rcd_psa_lin_defn.lde_prd_type||') status must be (1)active for and active line');
         end if;
         if rcd_psa_lin_defn.lde_lin_status = '1' and rcd_prdtype.pty_prd_lin_usage != '1' then
