CREATE OR REPLACE package DF_APP.PPLAN_PLAN_ORD_PROCESS as
   ...
   /**
    System  : ips
    Package : DF_APP.PPLAN_PLAN_ORD_PROCESS
    Owner   : df_app
    Author  : David Zhang

    Description
    -----------
    Integrated Planning Demand Financials - PPLAN Planned Order data load and process
...
end pplan_plan_ord_process;
/


CREATE OR REPLACE package body DF_APP.PPLAN_PLAN_ORD_PROCESS as
   ...
   var_line_no  NUMBER;
   rec_prodn_plan_data  prodn_plan_data%ROWTYPE;
   ...
   begin
      ...
      rec_prodn_plan_data.mars_week           := lics_inbound_utility.get_variable('MARS_WEEK');
      rec_prodn_plan_data.plant_code          := lics_inbound_utility.get_variable('PLANT_CODE');
      rec_prodn_plan_data.tdu                 := lics_inbound_utility.get_variable('TDU');
      rec_prodn_plan_data.tdu                 := LTRIM(rec_prodn_plan_data.tdu, '0'); /* Get rid of leading 0s */
      rec_prodn_plan_data.qty_in_base_uom     := lics_inbound_utility.get_variable('QTY_IN_BASE_UOM');
      
      /* Lookup most recent Forecast ID */  
      var_fcst_id := lookupFcstID (var_moe_code) ;
      IF var_fcst_id = -1 THEN
         raise_application_error(-20000, 'DF_APP.PPLAN_PLAN_ORD_PROCESS: Error in getting FCST ID. for MOE ('||var_moe_code||').');
      END IF;
      
      /*Remove duplicate records in the table first */
      ...
      INSERT INTO prodn_plan_data 
            (FCST_ID,       MARS_WEEK, 
             PLANT_CODE,    TDU, 
             QTY_IN_BASE_UOM)
      VALUES 
            (var_fcst_id,                         rec_prodn_plan_data.mars_week,
             rec_prodn_plan_data.plant_code,      rec_prodn_plan_data.tdu,
             rec_prodn_plan_data.qty_in_base_uom);
...
CREATE PUBLIC SYNONYM PPLAN_PLAN_ORD_PROCESS FOR DF_APP.PPLAN_PLAN_ORD_PROCESS;

GRANT EXECUTE ON PPLAN_PLAN_ORD_PROCESS TO APPSUPPORT;
GRANT EXECUTE ON PPLAN_PLAN_ORD_PROCESS TO LICS_APP;