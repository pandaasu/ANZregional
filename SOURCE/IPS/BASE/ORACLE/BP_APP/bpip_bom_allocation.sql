create or replace function bpip_bom_allocation(par_company in varchar2, par_material in varchar2, par_date in varchar2) return bpip_allocation_table pipelined is
   ...
   cursor csr_bom_det is
      select t01.plant,
             ...
             sys_connect_by_path(t01.cmpnt_matl_code,'/') as bom_hierarchy_path
        from bom_det t01,
             bom_hdr t02,
             matl t03,
             matl t04,
...
exception
   ...
   when others then
      ...
      /*-*/
      raise_application_error(-20000, 'BPIP_BOM_ALLOCATION - FATAL ERROR - ' || substr(SQLERRM, 1, 2048));
