...
/* Package Header */
/******************/
create or replace package bp_app.bpip_mvmt_load as

   /******************************************************************************/
   /* Package Definition                                                         */
   /******************************************************************************/
   /**
    Package : bpip_mvmt_load
    Owner   : bp_app

    Description
    -----------
    Integrated Planning Demand Financials - Movement Load 
...
/* Package Body */
/****************/
CREATE OR REPLACE package body BP_APP.bpip_mvmt_load as
   ...
   rcd_load_mvmt_data load_mvmt_data%rowtype;
   ...
   begin
      ...
      lics_inbound_utility.set_csv_definition('CONSUMPTION_QTY',5);
      lics_inbound_utility.set_csv_definition('SALES_QTY',6);
      lics_inbound_utility.set_csv_definition('PRODUCTION_QTY',7);
   ...
   begin
      ...
      if var_trn_start = false then
         ...
         rcd_load_bpip_batch.batch_type_code := 'MOVEMENT';
      ...
      rcd_load_mvmt_data.batch_id := null;
      rcd_load_mvmt_data.line_no := var_line_count;
      rcd_load_mvmt_data.company := lics_inbound_utility.get_variable('COMPANY');
      rcd_load_mvmt_data.period := lics_inbound_utility.get_variable('PERIOD');
      rcd_load_mvmt_data.plant := lics_inbound_utility.get_variable('PLANT');
      rcd_load_mvmt_data.material := lics_inbound_utility.get_variable('MATERIAL');
      rcd_load_mvmt_data.consumption_qty := to_number(translate(upper(lics_inbound_utility.get_variable('CONSUMPTION_QTY')),'# ABCDEFGHIJKLMNOPQRSTUVWXYZ','#'),'999,999,999,990.999');
      rcd_load_mvmt_data.sales_qty := to_number(translate(upper(lics_inbound_utility.get_variable('SALES_QTY')),'# ABCDEFGHIJKLMNOPQRSTUVWXYZ','#'),'999,999,999,990.999');
      rcd_load_mvmt_data.production_qty := to_number(translate(upper(lics_inbound_utility.get_variable('PRODUCTION_QTY')),'# ABCDEFGHIJKLMNOPQRSTUVWXYZ','#'),'999,999,999,990.999');
      rcd_load_mvmt_data.status := 'LOADED';
      rcd_load_mvmt_data.error_msg := null;
      rcd_load_mvmt_data.bus_sgmnt := null;
      ...
      /* Business segment 01
      /*-*/
      rcd_load_mvmt_data.batch_id := var_batch_id_01;
      insert into load_mvmt_data values rcd_load_mvmt_data;

      /*-*/
      /* Business segment 02
      /*-*/
      rcd_load_mvmt_data.batch_id := var_batch_id_02;
      insert into load_mvmt_data values rcd_load_mvmt_data;

      /*-*/
      /* Business segment 05
      /*-*/
      rcd_load_mvmt_data.batch_id := var_batch_id_05;
      insert into load_mvmt_data values rcd_load_mvmt_data;
...
/**************************/
create or replace public synonym bpip_mvmt_load for bp_app.bpip_mvmt_load;
grant execute on bp_app.bpip_mvmt_load to lics_app;