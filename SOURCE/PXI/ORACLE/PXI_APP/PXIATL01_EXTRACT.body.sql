...
*******************************************************************************/
  ...
  begin
    ...
    case i_doc_type 
      ...
      when gc_doc_type_journal then v_doc_type := 'Jrnal';
      when gc_doc_type_claim then v_doc_type := 'Claim';
      else
        v_doc_type := 'Unknn';
    end case;
...
*******************************************************************************/
  ...
  exception
    when others then 
      pxi_common.reraise_promax_exception(pc_package_name,'ADD_CLAIM_RECORD');        
  end add_cliam_record;
   ...
   begin
     ...
     if tv_data.count > 0 then 
       if v_sum_amount <> 0 then 
         pxi_common.raise_promax_error(pc_package_name,c_method_name,'Attempting to send IDOC data, currently it does not balance to zero.');
       else
         perform_send;
       end if;
     end if;
