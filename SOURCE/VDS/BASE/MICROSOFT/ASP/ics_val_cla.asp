...
sub RetrieveRule(strGroup)
   ...
   strQuery = strQuery & " from vds_val_cla_rul t01"
   strQuery = strQuery & " where t01.vcr_class = '" & objForm.Fields("DTA_VacClass").Value & "') t02"
...
sub ProcessInsertLoad()
   ...
   '// Retrieve the rules
   '//
   call RetrieveRule(objForm.Fields("DTA_VacGroup").Value)
   if strMode = "FATAL" then
      exit sub
   end if
...
sub ProcessInsertAccept()
   ...
   call objProcedure.Execute("lics_form.set_value('VAC_CLASS','" & objSecurity.FixString(objForm.Fields("DTA_VacClass").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAC_DESCRIPTION','" & objSecurity.FixString(objForm.Fields("DTA_VacDescription").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAC_GROUP','" & objSecurity.FixString(objForm.Fields("DTA_VacGroup").Value) & "')")
   lngIndex = 1
   do while lngIndex <= len(objForm.Fields("DTA_VacLstQuery").Value)
      call objProcedure.Execute("lics_form.set_value('VAC_LST_QUERY','" & objSecurity.FixString(mid(objForm.Fields("DTA_VacLstQuery").Value,lngIndex,2000)) & "')")
      lngIndex = lngIndex + 2000
   loop
   lngIndex = 1
   do while lngIndex <= len(objForm.Fields("DTA_VacOneQuery").Value)
      call objProcedure.Execute("lics_form.set_value('VAC_ONE_QUERY','" & objSecurity.FixString(mid(objForm.Fields("DTA_VacOneQuery").Value,lngIndex,2000)) & "')")
      lngIndex = lngIndex + 2000
   loop
   call objProcedure.Execute("lics_form.set_value('VAC_EXE_BATCH','" & objSecurity.FixString(objForm.Fields("DTA_VacExeBatch").Value) & "')")
   ...
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VCR_RULE','" & objSecurity.FixString(objForm.Fields("DET_Rule" & i).Value) & "')")
   next

   '//
   '// Insert the class
   '//
   strStatement = "vds_val_configuration.insert_class"
...
sub ProcessUpdateLoad()
   ...
   '// Retrieve the class
   '//
   strStatement = "vds_val_configuration.retrieve_class"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "FATAL"
      exit sub
...
sub ProcessUpdateAccept()
   ...
   '// Update the class
   '//
   strStatement = "vds_val_configuration.update_class"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "UPDATE"
      strError = FormatError(strReturn)
...
sub ProcessDeleteAccept()
   ...
   '// Delete the class
   '//
   strStatement = "vds_val_configuration.delete_class"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
      strError = FormatError(strReturn)
