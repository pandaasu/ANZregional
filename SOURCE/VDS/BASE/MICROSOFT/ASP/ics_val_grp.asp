...
sub ProcessInsertAccept()
   ...
   call objProcedure.Execute("lics_form.set_value('VAG_GROUP','" & objSecurity.FixString(objForm.Fields("DTA_VagGroup").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAG_DESCRIPTION','" & objSecurity.FixString(objForm.Fields("DTA_VagDescription").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAG_COD_LENGTH','" & objSecurity.FixString(objForm.Fields("DTA_VagCodLength").Value) & "')")
   lngIndex = 1
   do while lngIndex <= len(objForm.Fields("DTA_VagCodQuery").Value)
      call objProcedure.Execute("lics_form.set_value('VAG_COD_QUERY','" & objSecurity.FixString(mid(objForm.Fields("DTA_VagCodQuery").Value,lngIndex,2000)) & "')")
   ...
   '// Insert the group
   '//
   strStatement = "vds_val_configuration.insert_group"
...
sub ProcessUpdateLoad()
   ...
   '// Retrieve the group
   '//
   strStatement = "vds_val_configuration.retrieve_group"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "FATAL"
      exit sub
...
sub ProcessUpdateAccept()
   ...
   '// Update the group
   '//
   strStatement = "vds_val_configuration.update_group"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "UPDATE"
      strError = FormatError(strReturn)
...
sub ProcessDeleteAccept()
   ...
   '// Delete the group
   '//
   strStatement = "vds_val_configuration.delete_group"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
      strError = FormatError(strReturn)
