...
sub RetrieveDetail()
   ...
   strQuery = strQuery & " from (select * from vds_val_grp order by vag_group asc)"
   strQuery = strQuery & " union all"
   ...
   strQuery = strQuery & " from (select * from vds_val_cla, vds_val_grp where vac_group = vag_group order by vac_class asc)"
   strQuery = strQuery & " union all"
   ...
   strQuery = strQuery & " from (select * from vds_val_cla_rul, vds_val_cla, vds_val_rul, vds_val_grp where vcr_class = vac_class and vcr_rule = var_rule and var_group = vag_group order by vac_class asc, var_rule asc)"
   strQuery = strQuery & " union all"
   strQuery = strQuery & " select '*TOP' as par_typ,"
   strQuery = strQuery & " '*TOP' as par_id,"
   ...
   strQuery = strQuery & " from (select * from vds_val_typ, vds_val_grp where vat_group = vag_group order by vat_type asc)"
   strQuery = strQuery & " union all"
   ...
   strQuery = strQuery & " from (select * from vds_val_typ_rul, vds_val_typ, vds_val_rul, vds_val_grp where vtr_type = vat_type and vtr_rule = var_rule and var_group = vag_group order by vat_type asc, var_rule asc)"
   strQuery = strQuery & " union all"
   strQuery = strQuery & " select '*TOP' as par_typ,"
   strQuery = strQuery & " '*TOP' as par_id,"
   ...
   strQuery = strQuery & " from (select * from vds_val_fil, vds_val_grp where vaf_group = vag_group order by vaf_filter asc)"
   ...
   strQuery = strQuery & " from vds_val_ema_det t01"
   strQuery = strQuery & " where t01.ved_email = '" & objForm.Fields("DTA_VaeEmail").Value & "') t02"
...
sub ProcessSelect()
   ...
   strQuery = strQuery & " from vds_val_ema t01"
...
sub ProcessInsertAccept()
   ...
   call objProcedure.Execute("lics_form.set_value('VAE_EMAIL','" & objSecurity.FixString(objForm.Fields("DTA_VaeEmail").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAE_DESCRIPTION','" & objSecurity.FixString(objForm.Fields("DTA_VaeDescription").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAE_ADDRESS','" & objSecurity.FixString(objForm.Fields("DTA_VaeAddress").Value) & "')")
   call objProcedure.Execute("lics_form.set_value('VAE_STATUS','" & objSecurity.FixString(objForm.Fields("DTA_VaeStatus").Value) & "')")

   '//
   '// Insert the email detail data
   '//
   lngCount = clng(objForm.Fields("DET_DetailCount").Value)
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_GROUP','" & objSecurity.FixString(trim(mid(objForm.Fields("DET_Key" & i).Value,1,30))) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_CLASS','" & objSecurity.FixString(trim(mid(objForm.Fields("DET_Key" & i).Value,31,30))) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_TYPE','" & objSecurity.FixString(trim(mid(objForm.Fields("DET_Key" & i).Value,61,30))) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_FILTER','" & objSecurity.FixString(trim(mid(objForm.Fields("DET_Key" & i).Value,91,30))) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_RULE','" & objSecurity.FixString(trim(mid(objForm.Fields("DET_Key" & i).Value,121,30))) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH01','" & objSecurity.FixString(objForm.Fields("DET_Search01_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH02','" & objSecurity.FixString(objForm.Fields("DET_Search02_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH03','" & objSecurity.FixString(objForm.Fields("DET_Search03_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH04','" & objSecurity.FixString(objForm.Fields("DET_Search04_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH05','" & objSecurity.FixString(objForm.Fields("DET_Search05_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH06','" & objSecurity.FixString(objForm.Fields("DET_Search06_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH07','" & objSecurity.FixString(objForm.Fields("DET_Search07_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH08','" & objSecurity.FixString(objForm.Fields("DET_Search08_" & i).Value) & "')")
   next
   for i = 1 to lngCount
      call objProcedure.Execute("lics_form.set_value('VED_SEARCH09','" & objSecurity.FixString(objForm.Fields("DET_Search09_" & i).Value) & "')")
   next

   '//
   '// Insert the email
   '//
   strStatement = "vds_val_configuration.insert_email"
...
sub ProcessUpdateLoad()
   ...
   '// Retrieve the email
   '//
   strStatement = "vds_val_configuration.retrieve_email"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "FATAL"
      exit sub
...
sub ProcessUpdateAccept()
   ...
   '// Update the email
   '//
   strStatement = "vds_val_configuration.update_email"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "UPDATE"
      strError = FormatError(strReturn)
...
sub ProcessDeleteAccept()
   ...
   '// Delete the email
   '//
   strStatement = "vds_val_configuration.delete_email"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
      strError = FormatError(strReturn)
