...
sub RetrieveRule(strGroup)
   ...
   strQuery = strQuery & " from vds_val_typ_rul t01"
...
sub ProcessInsertAccept()
   ...
   '// Insert the type
   '//
   strStatement = "vds_val_configuration.insert_type"
...
sub ProcessUpdateLoad()
   ...
   '// Retrieve the type
   '//
   strStatement = "vds_val_configuration.retrieve_type"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "FATAL"
      exit sub
...
sub ProcessUpdateAccept()
   ...
   '// Update the type
   '//
   strStatement = "vds_val_configuration.update_type"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "UPDATE"
      strError = FormatError(strReturn)
...
sub ProcessDeleteAccept()
   ...
   '// Delete the type
   '//
   strStatement = "vds_val_configuration.delete_type"
   strReturn = objFunction.Execute(strStatement)
   if strReturn <> "*OK" then
      strMode = "DELETE"
      strError = FormatError(strReturn)
