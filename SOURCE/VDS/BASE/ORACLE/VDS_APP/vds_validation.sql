...
/**
 System  : vds
 Package : vds_validation
 Owner   : vds_app
 Author  : Steve Gregan

 Description
 -----------
 Validation Data Store - VDS Validation
...
*******************************************************************************/
   ...
   function get_table return vds_validation_table;
...
/* Package Body */
/****************/
create or replace package body vds_validation as
   ...
   var_vir_table vds_validation_table := vds_validation_table();
   ...
   procedure create_message(par_message in vds_val_mes%rowtype);
   ...
   procedure execute(par_group in varchar2) is
      ...
      var_locked boolean;
      var_vds_code varchar2(30 char);
      var_vds_search01 varchar2(256 char);
      var_vds_search02 varchar2(256 char);
      var_vds_search03 varchar2(256 char);
      var_vds_search04 varchar2(256 char);
      var_vds_search05 varchar2(256 char);
      var_vds_search06 varchar2(256 char);
      var_vds_search07 varchar2(256 char);
      var_vds_search08 varchar2(256 char);
      var_vds_search09 varchar2(256 char);
      ...
      csr_test typ_test;
      rcd_vds_val_mes vds_val_mes%rowtype;
      var_sav_code vds_val_mes.vam_code%type;
   ...
   begin
      ...
      /*-*/
      var_log_prefix := 'VDS - VALIDATION';
      var_log_search := 'VDS_VALIDATION-' || upper(par_group);
      var_loc_string := 'VDS_VALIDATION-' || upper(par_group);
      var_locked := false;
      ...
      /* Begin procedure
      /*-*/
      lics_logging.write_log('Begin - VDS Validation - Parameters(' || upper(par_group) || ')');

      /*-*/
      /* Request the lock on the VDS validation
      /*-*/
      begin
      ...
      if var_locked = true then
         ...
         /*-*/
         rcd_vds_val_mes.vam_execution := rcd_group.vag_group||'_'||to_char(sysdate,'yyyymmddhh24miss');
         ...
         /*-*/
         /* Release the lock on the VDS validation
         /*-*/
      ...
      /* End procedure
      /*-*/
      lics_logging.write_log('End - VDS Validation');
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - VDS_VALIDATION - EXECUTE - ' || var_exception);
   ...
   begin
      ...
      /*-*/
      var_vds_code := par_code;
      begin
         var_number := to_number(var_vds_code);
         var_vds_code := to_char(var_number,'fm000000000000000000000000000000');
         var_vds_code := substr(var_vds_code,rcd_classification.vag_cod_length*-1,rcd_classification.vag_cod_length);
      exception
         when others then
            null;
      end;
      ...
      var_single := replace(var_single,'<SAP_CODE>',var_vds_code);
      begin
      ...
      var_vir_table.delete;
      fetch csr_single into var_vds_code;
      if csr_single%notfound then
         var_return := 'SAP code (' || var_vds_code || ') not found for classification';
      else
         var_vir_table.extend;
         var_vir_table(var_vir_table.last) := vds_validation_object(var_vds_code,
                                                                     ltrim(var_vds_code,' 0'),
   ...
   exception
      ...
      when others then
         ...
         /* Raise an exception to the calling application
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - VDS_VALIDATION - EXECUTE_SINGLE - ' || substr(SQLERRM, 1, 1024));
      ...
      cursor csr_type_header is 
         select t02.vaf_type,
           ...
           from vds_val_fil_det t01,
                vds_val_fil t02
          where t01.vfd_filter = t02.vaf_filter
            and t01.vfd_code in (select wrk_trm_code from table(vds_validation.get_table))
   ...
   begin
      ...
      /*-*/
      rcd_vds_val_mes.vam_execution := par_execution;
      rcd_vds_val_mes.vam_class := par_class;
      rcd_vds_val_mes.vam_group := par_group;
      rcd_vds_val_mes.vam_type := '*CLASS';
      rcd_vds_val_mes.vam_filter := '*CLASS';
      rcd_vds_val_mes.vam_version := 0;
      rcd_vds_val_mes.vam_emailed := 0;
      if par_execution = con_single_code then
         rcd_vds_val_mes.vam_version := 1;
      end if;
      ...
      loop
         ...
         /*-*/
         rcd_vds_val_mes.vam_rule := rcd_class_rule.var_rule;
         ...
         var_dynamic := 'select t01.wrk_trm_code,
                                ...
                                t02.val_key,
                                t02.val_message
                           from table(vds_validation.get_table) t01,
      ...
      loop
         ...
         /*-*/
         rcd_vds_val_mes.vam_type := rcd_type_header.vaf_type;
         rcd_vds_val_mes.vam_filter := rcd_type_header.vaf_filter;
         ...
         loop
            ...
            /* Set the message rule
            /*-*/
            rcd_vds_val_mes.vam_rule := rcd_type_rule.var_rule;
            ...
            var_dynamic := 'select t01.wrk_trm_code,
                                   ...
                                   t02.val_message
                              from (select t11.*
                                      from table(vds_validation.get_table) t11,
                                           (select vfd_code from vds_val_fil_det where vfd_filter = ''' || rcd_type_header.vaf_filter || ''') t12
            ...
            if not(var_sav_trm_code) is null then
               ...
               if rcd_type_rule.var_test = '*ANY_ROWS' and var_row_count > 0 then
                  rcd_vds_val_mes.vam_sequence := rcd_vds_val_mes.vam_sequence + 1;
                  rcd_vds_val_mes.vam_text := rcd_type_rule.var_message;
                  create_message(rcd_vds_val_mes);
               end if;
   ...
   /******************************************************/
   procedure create_message(par_message in vds_val_mes%rowtype) is
         ...
         loop
            ...
            if var_class is null or
               ...
               if var_class is null then
                  ...
                  lics_mailer.append_data('VDS Batch Validation ('||par_execution||')');
                  lics_mailer.append_data(rpad('=',length('VDS Batch Validation ('||par_execution||')'),'='));
   ...
   procedure load_statistics(par_group in varchar2) is
      ...
      type typ_test is ref cursor;
      csr_test typ_test;
      rcd_vds_val_sta vds_val_sta%rowtype;
   ...
   begin
      ...
      /*-*/
      rcd_vds_val_sta.vas_group := upper(par_group);

      /*-*/
      /* Delete any existing statistics for the group
      /*-*/
      delete from vds_val_sta
       where vas_group = rcd_vds_val_sta.vas_group;
      ...
      loop
         ...
         if csr_class_statistic%found then
            rcd_vds_val_sta.vas_statistic := '*CLASS';
            rcd_vds_val_sta.vas_identifier := rcd_class_statistic.vac_class;
            rcd_vds_val_sta.vas_description := rcd_class_statistic.vac_description;
            rcd_vds_val_sta.vas_missing := rcd_class_statistic.mis_count;
            rcd_vds_val_sta.vas_error := rcd_class_statistic.err_count;
            rcd_vds_val_sta.vas_valid := var_count - rcd_class_statistic.err_count;
            rcd_vds_val_sta.vas_message := rcd_class_statistic.mes_count;
            insert into vds_val_sta
               (vas_group,
                vas_statistic,
                vas_identifier,
                vas_description,
                ...
                vas_valid,
                vas_message)
               values(rcd_vds_val_sta.vas_group,
                      rcd_vds_val_sta.vas_statistic,
                      rcd_vds_val_sta.vas_identifier,
                      rcd_vds_val_sta.vas_description,
                      rcd_vds_val_sta.vas_missing,
                      rcd_vds_val_sta.vas_error,
                      rcd_vds_val_sta.vas_valid,
                      rcd_vds_val_sta.vas_message);
         end if;
      ...
      loop
         ...
         rcd_vds_val_sta.vas_statistic := '*TYPE';
         rcd_vds_val_sta.vas_identifier := rcd_type_statistic.vat_type;
         rcd_vds_val_sta.vas_description := rcd_type_statistic.vat_description;
         rcd_vds_val_sta.vas_missing := rcd_type_statistic.mis_count;
         rcd_vds_val_sta.vas_error := rcd_type_statistic.err_count;
         rcd_vds_val_sta.vas_valid := rcd_type_statistic.val_count;
         rcd_vds_val_sta.vas_message := rcd_type_statistic.mes_count;
         insert into vds_val_sta
            (vas_group,
             vas_statistic,
             vas_identifier,
      ...
      loop
         ...
         rcd_vds_val_sta.vas_statistic := '*FILTER';
         rcd_vds_val_sta.vas_identifier := rcd_filter_statistic.vaf_filter;
         rcd_vds_val_sta.vas_description := rcd_filter_statistic.vaf_description;
         rcd_vds_val_sta.vas_missing := rcd_filter_statistic.mis_count;
         rcd_vds_val_sta.vas_error := rcd_filter_statistic.err_count;
         rcd_vds_val_sta.vas_valid := rcd_filter_statistic.val_count;
         rcd_vds_val_sta.vas_message := rcd_filter_statistic.mes_count;
         insert into vds_val_sta
            (vas_group,
             vas_statistic,
             vas_identifier,
      ...
      loop
         ...
         rcd_vds_val_sta.vas_statistic := '*RULE';
         rcd_vds_val_sta.vas_identifier := rcd_rule_statistic.var_rule;
         rcd_vds_val_sta.vas_description := rcd_rule_statistic.var_description;
         rcd_vds_val_sta.vas_missing := rcd_rule_statistic.mis_count;
         rcd_vds_val_sta.vas_error := rcd_rule_statistic.err_count;
         rcd_vds_val_sta.vas_valid := rcd_rule_statistic.val_count;
         rcd_vds_val_sta.vas_message := rcd_rule_statistic.mes_count;
         insert into vds_val_sta
            (vas_group,
             vas_statistic,
             vas_identifier,
   ...
   /************************************************/
   function get_table return vds_validation_table is
...
/**************************/
create or replace public synonym vds_validation for vds_app.vds_validation;
grant execute on vds_validation to public;
