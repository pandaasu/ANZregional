#!/bin/ksh
# ---------------------------------------------------------------------
# Script: care_sales_extract.sh
# 04-Sep-2006, ISI.  Modification history at end of script
# ---------------------------------------------------------------------
...
function printUsage {
   ...
   echo .
   echo "Usage: care_sales_extract.sh EXT_PERIOD]"
...
function setEnvironment {
   ...
   QUENAME="QR.SALCAR01"
   DTAPATH="/dwtrnsfr/data"
   ARCPATH="/dwtrnsfr/data/archive" 
   LOGPATH="/dwtrnsfr/log/care"
   SQLPATH="/dwtrnsfr/sql/care"
   ORAUSER="dw_app/isa"
   ORANAME="mca015"
   EXT_SOURCE="0009"
   EXT_ATLAS="N"
...
function checkDatabase {
   ...
   # Check the database availablity - recheck every minute for 4 hours (240 minutes)
   /usr/local/bin/orawait ${ORANAME} 1 240
   if [[ $? -ne 0 ]] then
      errorExit "[ERROR]:[checkDatabase] ORAWAIT timed out waiting for database ${ORANAME}."
   else
      writeLog "[INFO]:[checkDatabase] Database ${ORANAME} available. Continuing ..."
   fi
...
function execProcedure {
   ...
   print "execute care_sales_extract_legacy.extract_sales('${DTAPATH}','${DTAFILE}','${EXT_SOURCE}','${EXT_PERIOD}','${EXT_ATLAS}');" > ${SQLEXEC}
   print "exit;" >> ${SQLEXEC}
   ...
   # Send data file
   if [[ -s ${DTAFULL} ]] then
      writeLog "[INFO]:[execProcedure] Executing mqif [/usr/bin/cat ${DTAFULL} | ${MQIF_EXE} -p -q ${QUENAME} -r 0 -o ${DTAMESG}]"
      /usr/bin/cat ${DTAFULL} | mqif -p -q ${QUENAME} -r 0 -o ${DTAMESG} >> ${LOGFULL} 2<&1
      rc=${?}
      if [[ ${rc} -ne 0 ]] then
         errorExit "[ERROR]:[execProcedure] MQIF failed, return [${rc}], see log file [{LOGFULL}]"
      fi
   else
...
#     -------   -----------     -------------   -----------------------------
#       1.0     04-SEP-2006     S. Gregan       Original
# ---------------------------------------------------------------------------