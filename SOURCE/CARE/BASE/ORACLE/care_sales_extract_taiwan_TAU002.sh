#!/bin/ksh
# ---------------------------------------------------------------------
# Script: care_sales_extract_taiwan.sh
...
function printUsage {
   ...
   echo .
   echo "Usage: care_sales_extract_taiwan.sh EXT_PERIOD]"
...
function setEnvironment {
   ...
   QUENAME="QR.SALCAR01"
   TMPPATH="/tmp"
   DTAPATH="/eftdw/prod/care/data"
   ARCPATH="/eftdw/prod/care/archive" 
   LOGPATH="/eftdw/prod/care/log"
   SQLPATH="/eftdw/prod/care/work"
   ORAUSER="dw_app/swimmer"
   ORANAME="tai005"
   EXT_SOURCE="0201"
   EXT_ATLAS="N"
...
function execProcedure {
   ...
   print "execute care_sales_extract_taiwan.extract_sales('${TMPPATH}','${DTAFILE}','${EXT_SOURCE}','${EXT_PERIOD}','${EXT_ATLAS}');" > ${SQLEXEC}
   print "exit;" >> ${SQLEXEC}
   ...
   # Send data file
   if [[ -s ${DTAFULL} ]] then
      writeLog "[INFO]:[execProcedure] Executing command [/usr/bin/cat ${DTAFULL} | mqif -p -q ${QUENAME} -r 0 -o ${DTAMESG}]"
