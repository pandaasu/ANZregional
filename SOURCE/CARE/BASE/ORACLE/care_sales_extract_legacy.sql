...
/* Package Header */
/******************/
create or replace package care_sales_extract_legacy as

/******************************************************************************/
/* Package Definition                                                         */
/******************************************************************************/
/**
 Package : care_sales_extract_legacy
 Owner   : dw_app

 Description
 -----------
 Care - Sales Extract - Legacy Version
...
end care_sales_extract_legacy;
/

/****************/
/* Package Body */
/****************/
create or replace package body care_sales_extract_legacy as
   ...
   procedure extract_sales(par_file_path in varchar2,
      ...
      cursor csr_sales is
         select t01.prdct_vrnt_code,
                t01.sales_cases,
                t02.num_outers_case,
                t02.num_units_outer
           from (select t01.prdct_vrnt_code as prdct_vrnt_code,
                        sum(t01.sales_cases) as sales_cases
                   from sales_cust_dly t01,
                        cust t02
                  where t01.del_cust_code = t02.cust_code 
                    and t01.sales_prd_date = var_period
                    and trim(t02.fncl_grp_name) = 'MCA - DOMESTIC'
                  group by t01.prdct_vrnt_code) t01,
                prdct_vrnt t02
          where rtrim(t01.prdct_vrnt_code) = t02.prdct_vrnt_code(+)
   ...
   begin
      ...
      loop
         ...
         /*-*/
         var_output := 'DET';
         var_output := var_output || rpad(rcd_sales.prdct_vrnt_code,18,' ');
         var_output := var_output || lpad(to_char(rcd_sales.sales_cases,'9999999999999999990'),20,' ');
         var_output := var_output || lpad(to_char(rcd_sales.num_outers_case*rcd_sales.num_units_outer,'fm9990'),4,' ');
   ...
   exception
      ...
      when others then
         ...
         /*-*/
         raise_application_error(-20000, 'FATAL ERROR - CARE SALES - LEGACY EXTRACT - ' || var_exception);
...
/**************************/
create or replace public synonym care_sales_extract_legacy for dw_app.care_sales_extract_legacy;
grant execute on care_sales_extract_legacy to public;
