prompt :: Compile Package [pxipmx08_extract_v2] :::::::::::::::::::::::::::::::::::::::

create or replace package pxi_app.pxipmx08_extract_v2 as
/*******************************************************************************
** PACKAGE DEFINITION
********************************************************************************

  System  : LADS
  Package : PXIPMX08_EXTRACT_V2
  Owner   : PXI_APP
  Author  : Chris Horn
  ...
  2013-03-12  Mal Chambeyron        Remove DEFAULTS,
                                    ...
                                    Use Suffix
  2014-03-14  Mal Chambeyron        Created EXECUTE_FANOUT to enable "FANOUT" on
                                    Original EXECUTE by Division.
  2014-03-14  Mal Chameyron         Added [pt_px_company_div_for_suffix] to
                                    return the appropriate promax company / division
...
/*******************************************************************************
  NAME:  PT_PX_COMPANY_DIV_FOR_SUFFIX                                     PUBLIC
  PURPOSE:   This function returns the appropriate promax company / division
             combinations for a given interface suffix

  REVISIONS:
  Ver   Date       Author               Description
  ----- ---------- -------------------- ----------------------------------------
  1.1   2014-03-14 Mal Chameyron        Created.
*******************************************************************************/

  type rt_px_company_div_for_suffix is record (
    promax_company varchar2(3 char),
    promax_division varchar2(3 char)
  );

  type tt_px_company_div_for_suffix is table of rt_px_company_div_for_suffix;

  function pt_px_company_div_for_suffix(
    i_interface_suffix in fflu_common.st_interface
  ) return tt_px_company_div_for_suffix pipelined;
...
/*******************************************************************************
  Package Cosntants
*******************************************************************************/
  pc_package_name constant pxi_common.st_package_name := 'PXIPMX08_EXTRACT_V2';
        ...
        from
          ...
          (
            select
              promax_company,
              promax_division
            from table(pt_px_company_div_for_suffix(i_interface_suffix))
          ) t2 -- replace pxi_common:promax_config
        where
...
/*******************************************************************************
  NAME:      EXECUTE_FANOUT                                              PRIVATE
  PURPOSE:   Determines existance of current valid AR claims by division and
             sends them to Promax.
             ***NOTE*** Inserted in Execution PRE the Original EXECUTE to
             Enable "Fanout" by Division

  REVISIONS:
  Ver   Date       Author               Description
  ----- ---------- -------------------- ----------------------------------------
  1.0   2014-06-14 Mal Chambeyron       Created.
*******************************************************************************/
  procedure execute_fanout is

    v_snack_count number;
    v_food_count number;
    v_petcare_count number;
    v_new_zealand_count number;
  ...
  begin
    ...
    select count(1) into v_food_count from table(get_claims) where company_code = pxi_common.fc_australia and div_code = pxi_common.fc_bus_sgmnt_food;
    ...
    */
    ...
    select count(1) into v_new_zealand_count from table(get_claims) where company_code = pxi_common.fc_new_zealand;
    if v_new_zealand_count > 0 then
      execute(pxi_common.fc_interface_nz);
    end if;
  ...
  exception
     when others then
       ...
       pxi_common.reraise_promax_exception(pc_package_name,'EXECUTE_FANOUT');
   end execute_fanout;
...
*******************************************************************************/
  ...
  begin
    ...
    if fflu_data.was_errors = true then
      ...
      execute_fanout; -- Perform the interface extract.
      commit;
...
*******************************************************************************/
  function pt_px_company_div_for_suffix(
    i_interface_suffix in fflu_common.st_interface
  ) return tt_px_company_div_for_suffix pipelined is
  ...
  begin
    ...
    for rv_row in (
      ...
      where (i_interface_suffix = pxi_common.fc_interface_nz and promax_company = pxi_common.fc_new_zealand)
      or (i_interface_suffix <> pxi_common.fc_interface_nz and promax_company = pxi_common.fc_australia
        and promax_division =
          case i_interface_suffix
            when pxi_common.fc_interface_snack then pxi_common.fc_bus_sgmnt_snack
            when pxi_common.fc_interface_food then pxi_common.fc_bus_sgmnt_food
            when pxi_common.fc_interface_pet then pxi_common.fc_bus_sgmnt_petcare
          end
      )
  ...
  exception
    when others then
      fflu_data.log_interface_exception('PT_PX_COMPANY_DIV_FOR_SUFFIX');
  end pt_px_company_div_for_suffix;
